    ...
    public void buildIpv4TcpMatchTest() {
        ...
        for (MatchBuilder matchBuilder : matchBuilds) {
            ...
            if (tcpMatch.getTcpSourcePort() != null) {
                srcTcpMatches.add(tcpMatch.getTcpSourcePort().getValue());
            }
            if (tcpMatch.getTcpDestinationPort() != null) {
                dstTcpMatches.add(tcpMatch.getTcpDestinationPort().getValue());
            }
            // The layer3 match should be null
            assertNull(matchBuilder.getLayer3Match());
        }

        for (int port = TCP_SRC_LOWER_PORT; port <= TCP_SRC_UPPER_PORT; port++) {
            assertTrue(srcTcpMatches.contains(port));
        }

        for (int port = TCP_DST_LOWER_PORT; port <= TCP_DST_UPPER_PORT; port++) {
            assertTrue(dstTcpMatches.contains(port));
        }
    }
