...
public abstract class TestInterfaceManager implements IInterfaceManager {
    ...
    public List<OvsdbTerminationPointAugmentation> getTunnelPortsOnBridge(Uint64 dpnId) {
        if (dpnId.toJava() == GeniusProviderTestParams.DPN_ID_NO_EXIST) {
            // Unfortunately, the getTunnelPortsOnBridge() method may return null
            return null;
        }

        if (dpnId.toJava() == GeniusProviderTestParams.DPN_ID_NO_PORTS) {
        ...
        if (dpnId.toJava() == GeniusProviderTestParams.DPN_ID_NO_VXGPE_PORTS) {
        ...
        if (dpnId.toJava() != GeniusProviderTestParams.DPN_ID_NO_OPTIONS) {
