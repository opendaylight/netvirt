...
public class AclMatches {
    ...
    private void addIpProtocolMatch(AceIp aceIp) {
        ...
        if (srcPort != 0 || dstPort != 0) {
            Map<Integer,Integer> srcPortMaskMap = new HashMap<>();
            Map<Integer,Integer> dstPortMaskMap = new HashMap<>();
            srcPortMaskMap.put(srcPort, srcPortMax);
            dstPortMaskMap.put(dstPort, dstPortMax);
            if (srcPort != 0) {
                srcPortMaskMap = getLayer4MaskForRange(srcPort, srcPortMax);
            }
            if (dstPort != 0) {
                dstPortMaskMap = getLayer4MaskForRange(dstPort, dstPortMax);
            }
