...
public class AclMatches {
    ...
    private Set<GeneralAugMatchNodesNodeTableFlow> tcpMatches;
    ...
    private void addIpProtocolMatch(AceIp aceIp) {
        ...
        if (aceIp.getProtocol() == IPProtocols.TCP.shortValue()) {
            ...
            if (srcPort != 0 && dstPort != 0) {
                ...
                for (Entry<Integer, Integer> srcEntry: srcPortMaskMap.entrySet()) {
                    Integer srcTcpPort = srcEntry.getKey();
                    Integer srcTcpMask = srcEntry.getValue();
                    tcpSrc.setMask(srcTcpMask);
                    tcpSrc.setPort(new PortNumber(srcTcpPort));
                    nxAugMatchTcpSrc = new NxAugMatchNodesNodeTableFlowBuilder().setNxmOfTcpSrc(tcpSrc.build()).build();
                    for (Entry<Integer, Integer> dstEntry : dstPortMaskMap.entrySet()) {
                        Integer dstTcpPort = dstEntry.getKey();
                        Integer dstTcpMask = dstEntry.getValue();
                        tcpDst.setMask(dstTcpMask);
                        tcpDst.setPort(new PortNumber(dstTcpPort));
                        nxAugMatchTcpDst = new NxAugMatchNodesNodeTableFlowBuilder()
                                               .setNxmOfTcpDst(tcpDst.build()).build();
                        genAugMatch = new GeneralAugMatchNodesNodeTableFlowBuilder()
                        .setExtensionList(ImmutableList.of(new ExtensionListBuilder()
                                                      .setExtensionKey(NxmOfTcpSrcKey.class)
                                                      .setExtension(new ExtensionBuilder()
                                                      .addAugmentation(NxAugMatchNodesNodeTableFlow.class,
                                                       nxAugMatchTcpSrc).build()).build(), new ExtensionListBuilder()
                                                      .setExtensionKey(NxmOfTcpDstKey.class)
                                                      .setExtension(new ExtensionBuilder()
                                                      .addAugmentation(NxAugMatchNodesNodeTableFlow.class,
                                                       nxAugMatchTcpDst).build()).build())).build();
                        tcpMatches.add(genAugMatch);
                    }
                }
            } else if (srcPort != 0) {
                NxmOfTcpSrcBuilder tcpSrc = new NxmOfTcpSrcBuilder();
                ...
                for (Entry<Integer, Integer> srcEntry : srcPortMaskMap.entrySet()) {
                    ...
                    .setExtensionList(ImmutableList.of(new ExtensionListBuilder().setExtensionKey(NxmOfTcpSrcKey.class)
                                                       .setExtension(new ExtensionBuilder()
                                                       .addAugmentation(NxAugMatchNodesNodeTableFlow.class,
                                                        nxAugMatchTcpSrc).build()).build())).build();
                    tcpMatches.add(genAugMatch);
                }
            } else if (dstPort != 0) {
                NxmOfTcpDstBuilder tcpDst = new NxmOfTcpDstBuilder();
