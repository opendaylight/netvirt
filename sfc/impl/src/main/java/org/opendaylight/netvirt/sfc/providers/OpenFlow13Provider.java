...
import org.opendaylight.netvirt.sfc.utils.OpenFlow13Utils;
...
public class OpenFlow13Provider {
    ...
    private OpenFlow13Provider(final DataBroker dataBroker) {
    ...
    private List<Flow> createClassifierFlows(NodeId node, String logicalIf, MatchBuilder match, RenderedServicePath rsp) {
        String nodeIpStr = ""; //TODO get the IP from the node
        ...
        flows.add(createIngressClassifierAclFlow(match, node.getValue(), logicalIf, rsp.getPathId(), rsp.getStartingIndex()));
        ...
        flows.add(createEgressClassifierTransportEgressLocalFlow(rsp.getPathId(), nodeIpStr));
    ...
    private Flow createIngressClassifierFilterNshFlow(long nsp) {
        ...
        // TODO change the table to INGRESS_SFC_CLASSIFIER_FILTER_TABLE when 53468 is merged
        return OpenFlow13Utils.createFlowBuilder(NwConstants.SFC_TRANSPORT_CLASSIFIER_TABLE,
                INGRESS_CLASSIFIER_FILTER_NSH_PRIORITY, INGRESS_CLASSIFIER_FILTER_COOKIE,
                INGRESS_CLASSIFIER_FILTER_FLOW_NAME, match, isb).build();
    }
    ...
    private Flow createIngressClassifierFilterNoNshFlow() {
        ...
        // TODO change the table to INGRESS_SFC_CLASSIFIER_ACL_TABLE when 53468 is merged
     ...
     *     Match on inport (corresponds to Neutron NW/tenant), Push NSH, init(nsp, nsi, C1, C2) and
     *     resubmit to Ingress Dispatcher to be sent down the rest of the pipeline
     */
    private Flow createIngressClassifierAclFlow(MatchBuilder match, String nodeName, String logicalIf,
            long nsp, short nsi) {
    ...
    private Flow createEgressClassifierFilterNshFlow(long nsp) {
        ...
        // TODO change the table to EGRESS_SFC_CLASSIFIER_NEXTHOP_TABLE when 53468 is merged
        InstructionsBuilder isb = OpenFlow13Utils.appendGotoTableInstruction(new InstructionsBuilder(),
                NwConstants.SFC_TRANSPORT_CLASSIFIER_TABLE);

        // TODO change the table to EGRESS_SFC_CLASSIFIER_FILTER_TABLE when 53468 is merged
        return OpenFlow13Utils.createFlowBuilder(NwConstants.SFC_TRANSPORT_CLASSIFIER_TABLE,
                EGRESS_CLASSIFIER_FILTER_NSH_PRIORITY, EGRESS_CLASSIFIER_FILTER_COOKIE,
                EGRESS_CLASSIFIER_FILTER_FLOW_NAME, match, isb).build();
    }
     ...
     *     and goto Egress Classifier Transport Egress table on match
     */
    private Flow createEgressClassifierNextHopNoC1C2Flow() {
        ...
        InstructionsBuilder isb = OpenFlow13Utils.wrapActionsIntoApplyActionsInstruction(actionList);
        // TODO change the table to EGRESS_SFC_CLASSIFIER_TRANSPORTEGRESS_TABLE when 53468 is merged
        OpenFlow13Utils.appendGotoTableInstruction(isb, NwConstants.SFC_TRANSPORT_CLASSIFIER_TABLE);

        // TODO change the table to EGRESS_SFC_CLASSIFIER_NEXTHOP_TABLE when 53468 is merged
        return OpenFlow13Utils.createFlowBuilder(NwConstants.SFC_TRANSPORT_CLASSIFIER_TABLE,
                EGRESS_CLASSIFIER_NEXTHOP_NOC1C2_PRIORITY, EGRESS_CLASSIFIER_NEXTHOP_COOKIE,
                EGRESS_CLASSIFIER_NEXTHOP_FLOW_NAME, match, isb).build();
    }
    ...
    private Flow createEgressClassifierTransportEgressLocalFlow(long nsp, String localIpStr) {
        ...
        // TODO change the table to EGRESS_SFC_CLASSIFIER_TPORTEGRESS_TABLE when 53468 is merged
        return OpenFlow13Utils.createFlowBuilder(NwConstants.SFC_TRANSPORT_CLASSIFIER_TABLE,
                EGRESS_CLASSIFIER_EGRESS_LOCAL_PRIORITY, EGRESS_CLASSIFIER_TPORTEGRESS_COOKIE,
                EGRESS_CLASSIFIER_TPORTEGRESS_FLOW_NAME, match, isb).build();
    }
