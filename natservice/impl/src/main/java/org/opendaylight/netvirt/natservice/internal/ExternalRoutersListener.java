...
*/
    ...
    public void advToBgpAndInstallFibAndTsFlows(final BigInteger dpnId, final short tableId, final String vpnName,
        ...
        if (extNwProvType == ProviderTypes.VXLAN) {
            ListenableFutures.addErrorLogging(txRunner.callWithNewWriteOnlyTransactionAndSubmit(
                tx -> evpnSnatFlowProgrammer.evpnAdvToBgpAndInstallFibAndTsFlows(dpnId, tableId, externalIp, vpnName,
                    rd, nextHopIp, tx, routerId, routerName, extNetworkId, writeFlowInvTx)), LOG,
        ...
        coordinator.enqueueJob(NatConstants.NAT_DJC_PREFIX + update.getKey(), () -> {
            ...
            futures.add(NatUtil.waitForTransactionToComplete(
                txRunner.callWithNewWriteOnlyTransactionAndSubmit(
                    removeFlowInvTx -> futures.add(NatUtil.waitForTransactionToComplete(
                        txRunner.callWithNewWriteOnlyTransactionAndSubmit(writeFlowInvTx -> {
                            ...
                            if (originalSNATEnabled != updatedSNATEnabled) {
                                if (originalSNATEnabled) {
                                    ...
                                    Collection<String> externalIps =
                                        NatUtil.getExternalIpsForRouter(dataBroker, routerId);
                            ...
                            if (!Objects.equals(original.getExtGwMacAddress(), update.getExtGwMacAddress())) {
                                NatUtil.installRouterGwFlows(txRunner, vpnManager, original, dpnId,
                            ...
                            List<String> originalExternalIps =
                                NatUtil.getIpsListFromExternalIps(original.getExternalIps());
                            List<String> updatedExternalIps =
                                NatUtil.getIpsListFromExternalIps(update.getExternalIps());
                            ...
                            if (addedExternalIps.size() != 0) {
                                ...
                                for (String addedExternalIp : addedExternalIps) {
                                    /*
                                        ...
                                        2) Initialise the count of the added external IP to 0 in the ExternalCounter
                                           model.
                                     */
                            ...
                            if (removedExternalIps.size() > 0) {
                                ...
                                for (String removedExternalIp : removedExternalIps) {
                                    /*
                                        ...
                                        3) For the corresponding subnet IDs whose external IP mapping was removed,
                                           allocate one of the least loaded external IP.
                                           Store the subnet IP and the reallocated external IP mapping in the
                                           IntExtIp model.
                                        4) Increase the count of the allocated external IP by one.
                                        5) Advertise to the BGP if external IP is allocated for the first time for
                                           the router i.e. the route for the external IP is absent.
                                    ...
                                    LOG.debug(
                                        "update : Remove the mappings in the IntExtIP model which has external IP.");
                                    ...
                                    LOG.debug(
                                        "update : Remove the mappings of the internal IPs from the IntExtIP model.");
                                    ...
                                    // Get the internalIP and internal Port which were associated to the removed
                                    // external IP.
                                    ...
                                    for (Map.Entry protoTypesIntIpPort : protoTypesIntIpPorts) {
                                        ...
                                        List<String> removedInternalIpPorts =
                                            (List<String>) protoTypesIntIpPort.getValue();
                                    ...
                                    LOG.debug("update : Remove the NAPT translation entries from Inbound NAPT tables"
                                        + " for the removed external IP {}", externalIp);
                                    ...
                                    for (Map.Entry<String, List<String>> internalIpPort : internalIpPorts) {
                                        ...
                                        LOG.debug("update : Remove the NAPT translation entries from Outbound NAPT"
                                            + " tables for the removed internal IP {}", internalIp);
                                        ...
                                        for (String internalPort : internalPorts) {
                                            ...
                                            naptPacketInHandler.removeIncomingPacketMap(
                                                routerId + NatConstants.COLON_SEPARATOR
                            ...
                            if (addedSubnetIds.size() != 0) {
                                ...
                                for (Uuid addedSubnetId : addedSubnetIds) {
                                    /*
                                        1) Select the least loaded external IP for the subnet and store the mapping
                                           of the subnet IP and the external IP in the IntExtIp model.
                                ...
                                LOG.debug(
                                    "update : End processing of the Subnet IDs addition during the update operation");
                            }
                            ...
                            if (removedSubnetIds.size() != 0) {
                                ...
                                for (Uuid removedSubnetId : removedSubnetIds) {
                                    ...
                                    if (subnetAddr != null) {
                                        /*
                                            ...
                                            3) Advertise to the BGP for removing the routes of the corresponding
                                               external IP if its not allocated to any other internal IP.
                                        */
                                ...
                                LOG.debug(
                                    "update : End processing of the Subnet IDs removal during the update operation");
                            }
                        }))))));
            return futures;
        }, NatConstants.NAT_DJC_MAX_RETRIES);
    ...
    protected void remove(InstanceIdentifier<Routers> identifier, Routers router) {
        ...
        coordinator.enqueueJob(NatConstants.NAT_DJC_PREFIX + router.getKey(), () -> {
            ...
            return Collections.singletonList(NatUtil.waitForTransactionToComplete(
