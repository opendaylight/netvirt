...
import org.opendaylight.yang.gen.v1.urn.opendaylight.netvirt.l3vpn.rev130911.VpnInterfaceOpDataBuilder;
...
public abstract class AbstractSnatService implements SnatServiceListener {
    ...
    protected AbstractSnatService(final DataBroker dataBroker, final IMdsalApiManager mdsalManager,
                                  ...
                                  final IVpnFootprintService vpnFootprintService, final NatDataUtil natDataUtil) {
    ...
    public boolean handleSnatAllSwitch(Routers routers, BigInteger primarySwitchId,  int addOrRemove) {
        ...
        if (isLastRouterDelete) {
            ...
            removeAdjaceniesFromOpDatastore(routers);
        }
        return true;
    }
    ...
    protected void removeMipAdjacencies(Routers routers) {
        ...
        String externalSubNetId = routers.getExternalIps().get(0).getSubnetId().getValue();
        ...
        try {
            ...
            LOG.warn("Failed to read removeAdjaceniesFromOpDatastore with error {}", e.getMessage());
        } catch (TransactionCommitFailedException e) {
            LOG.warn("Failed to remove removeMipAdjacencies with error {}", e.getMessage());
        }
    }

    protected void removeAdjaceniesFromOpDatastore(Routers routers) {
        LOG.info("removeAdjaceniesFromOpDatastore for router {}", routers.getRouterName());
        String externalSubNetId = routers.getExternalIps().get(0).getSubnetId().getValue();
        InstanceIdentifier<VpnInterfaceOpData> vpnInterfaceOpDataId = InstanceIdentifier
                .builder(VpnInterfaceOpData.class).build();
        try {
            VpnInterfaceOpData vpnInterfaceOpData = SingleTransactionDataBroker.syncRead(dataBroker,
                    LogicalDatastoreType.OPERATIONAL, vpnInterfaceOpDataId);
            List<VpnInterfaceOpDataEntry> updatedVpnOpInterface = new ArrayList<>();
            for (VpnInterfaceOpDataEntry vpnInterfaceOpDataEntry : vpnInterfaceOpData.getVpnInterfaceOpDataEntry()) {
                List<Adjacency> updatedAdjacencies = new ArrayList<>();
                AdjacenciesOp adjacencies = vpnInterfaceOpDataEntry.getAugmentation(AdjacenciesOp.class);
                if (null != adjacencies) {
                    for (Adjacency adjacency : adjacencies.getAdjacency()) {
                        if (!adjacency.getSubnetId().getValue().equals(externalSubNetId)) {
                            updatedAdjacencies.add(adjacency);
                ...
                AdjacenciesOpBuilder adjacenciesOpBuilder = new AdjacenciesOpBuilder();
                adjacenciesOpBuilder.setAdjacency(updatedAdjacencies);
                VpnInterfaceOpDataEntryBuilder vpnInterfaceOpDataEntryBuilder = new VpnInterfaceOpDataEntryBuilder(
                        vpnInterfaceOpDataEntry);
                vpnInterfaceOpDataEntryBuilder.addAugmentation(AdjacenciesOp.class, adjacenciesOpBuilder.build());
                updatedVpnOpInterface.add(vpnInterfaceOpDataEntryBuilder.build());
            }
            VpnInterfaceOpDataBuilder vpnInterfaceOpDataBuilder = new VpnInterfaceOpDataBuilder();
            vpnInterfaceOpDataBuilder.setVpnInterfaceOpDataEntry(updatedVpnOpInterface);
            SingleTransactionDataBroker.syncWrite(dataBroker, LogicalDatastoreType.OPERATIONAL,
                    vpnInterfaceOpDataId, vpnInterfaceOpDataBuilder.build());
        } catch (ReadFailedException e) {
            LOG.warn("Failed to read removeAdjaceniesFromOpDatastore with error {}", e.getMessage());
        }
        catch (TransactionCommitFailedException e) {
            LOG.warn("Failed to remove removeAdjaceniesFromOpDatastore with error {}", e.getMessage());
        }
    }
