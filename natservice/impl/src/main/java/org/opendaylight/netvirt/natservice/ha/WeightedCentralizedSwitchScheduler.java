...
public class WeightedCentralizedSwitchScheduler implements CentralizedSwitchScheduler {
    ...
    public boolean addSwitch(BigInteger dpnId) {
        ...
        if (scheduleRouters) {
            ExtRouters routers = null;
            try {
                routers = txRunner.applyWithNewReadWriteTransactionAndSubmit(CONFIGURATION,
                    tx -> NatUtil.getExternalRouters(tx)).get();
            } catch (InterruptedException | ExecutionException e) {
                LOG.error("Error reading External Router info.");
            }
    ...
    private boolean isPrimarySwitchAllocatedForRouter(String routerName) {
        ...
        try {
            RouterToNaptSwitch rtrToNapt =
                    reader.syncRead(dataBroker, LogicalDatastoreType.CONFIGURATION, routerToNaptSwitch);
