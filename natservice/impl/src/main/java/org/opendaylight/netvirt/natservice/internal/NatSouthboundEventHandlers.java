...
public class NatSouthboundEventHandlers {
    ...
    public void handleAdd(String interfaceName, BigInteger intfDpnId, RouterInterface routerInterface) {
        ...
        NatFlowAddWorker natFlowAddWorker = new NatFlowAddWorker(interfaceName, routerName, intfDpnId);
        coordinator.enqueueJob(NAT_DS + "-" + interfaceName, natFlowAddWorker, NatConstants.NAT_DJC_MAX_RETRIES);
    }

    public void handleRemove(Interface intrf, BigInteger intfDpnId, RouterInterface routerInterface) {
        String routerName = routerInterface.getRouterName();
        NatInterfaceStateRemoveWorker natIfaceStateRemoveWorker = new NatInterfaceStateRemoveWorker(intrf.getName(),
                intfDpnId, routerName);
        ...
        coordinator.enqueueJob(NAT_DS + "-" + intrf.getName(), natFlowRemoveWorker,
                NatConstants.NAT_DJC_MAX_RETRIES);
    }

    public void handleUpdate(Interface original, Interface update, BigInteger intfDpnId, RouterInterface routerInterface) {
        String routerName = routerInterface.getRouterName();
    ...
    private void processInterfaceAdded(String portName, String routerId, BigInteger dpnId,
    ...
    private class NatFlowAddWorker implements Callable<List<ListenableFuture<Void>>> {
        ...
        NatFlowAddWorker(String interfaceName,String routerName, BigInteger dpnId) {
            this.interfaceName = interfaceName;
            this.routerName = routerName;
            this.dpnId = dpnId;
        }
        ...
        public List<ListenableFuture<Void>> call() {
            ...
            try {
                ...
                processInterfaceAdded(interfaceName, routerName, dpnId, futures);
            } catch (Exception ex) {
                LOG.error("call : Exception caught in Interface {} Operational State Up event",
                        interfaceName, ex);
            }
