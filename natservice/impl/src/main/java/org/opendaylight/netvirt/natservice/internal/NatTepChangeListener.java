...
public class NatTepChangeListener extends AbstractAsyncDataTreeChangeListener<TunnelEndPoints> {
    ...
    private boolean hndlTepDelForDnatInEachRtr(RoutersList router, Uint32 routerId,
        ...
        Map<PortsKey, Ports> interfaces = routerPorts.getPorts();
        for (Ports port : interfaces.values()) {
            ...
            Map<InternalToExternalPortMapKey, InternalToExternalPortMap> intExtPortMapList
                    = port.getInternalToExternalPortMap();
            for (InternalToExternalPortMap intExtPortMap : intExtPortMapList.values()) {
