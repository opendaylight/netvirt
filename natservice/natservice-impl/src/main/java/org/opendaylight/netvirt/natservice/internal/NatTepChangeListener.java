...
public class NatTepChangeListener extends
    ...
    protected void remove(InstanceIdentifier<TunnelEndPoints> key,
        ...
        final String srcTepIp = String.valueOf(tep.getIpAddress().getValue());
    ...
    protected void update(InstanceIdentifier<TunnelEndPoints> key,
        ...
        LOG.debug("NO ACTION duing update event : {}", updatedTep.getKey());
    }

    @Override
    protected void add(InstanceIdentifier<TunnelEndPoints> key,
            TunnelEndPoints tep) {
        LOG.debug("NO ACTION duing add event : {}", tep.getKey());
    }
    ...
    private boolean hndlTepDelForDnatInEachRtr(RoutersList router, long routerId, BigInteger tepDeletedDpnId,
    ...
    private void hndlTepDelForSnatInEachRtr(RoutersList router, long routerId, BigInteger dpnId,
            String srcTepIp, Boolean isFipExists, ProviderTypes extNwProvType, WriteTransaction writeFlowInvTx) {
        ...
        if (naptId == null || naptId.equals(BigInteger.ZERO) || !naptId.equals(dpnId)) {
            LOG.error("hndlTepDelForSnatInEachRtr : SNAT -> Ignoring TEP delete for the DPN {} since"
        ...
        if (natMode == NatMode.Conntrack) {
            ...
            if (networkId == null) {
                LOG.error("hndlTepDelForSnatInEachRtr : SNAT -> Ignoring TEP delete for the DPN {} for router {}"
            ...
            if (!isFipExists) {
                ...
                defaultRouteProgrammer.removeDefNATRouteInDPN(dpnId, bgpVpnId, writeFlowInvTx);
            }
            ...
            if (routerData.get().isEnableSnat()) {
                ...
                NatUtil.removeSNATFromDPN(dataBroker, mdsalManager, idManager, naptSwitchHA, dpnId, routerName,
                        routerId, routerVpnId, extNwProvType, writeFlowInvTx);
