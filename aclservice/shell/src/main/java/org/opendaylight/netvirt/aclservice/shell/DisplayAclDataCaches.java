...
public class DisplayAclDataCaches extends OsgiCommandSupport {
    ...
    protected void getRemoteAclIdMap(Class<? extends DirectionBase> direction) {
        if (all == null && uuidStr == null) {
            ...
            if (remoteUuidLst == null || remoteUuidLst.isEmpty()) {
                ...
                remoteUuidLst.forEach(uuid -> printRemoteAcl(uuid));
            }
        } else if (uuidStr == null) {
            if (!validateAll()) {
                printRemoteAclIdMapHelp();
    ...
    private void printRemoteAcl(Uuid uuidRef) {
        boolean first = true;
        if (first) {
            session.getConsole().println(String.format(REM_ID_TAB, uuidRef.getValue()));
            first = false;
        } else {
            session.getConsole().println(String.format(REM_ID_TAB_FOR, "", uuidRef.getValue()));
        }
