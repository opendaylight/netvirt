...
public final class AclServiceOFFlowBuilder {
    ...
    public static Map<String, List<MatchInfoBase>> programIpFlow(Matches matches) {
        if (matches != null) {
            ...
            } else if (acl.getProtocol().toJava() == NwConstants.IP_PROT_TCP) {
                return programTcpFlow(acl);
            } else if (acl.getProtocol().toJava() == NwConstants.IP_PROT_UDP) {
                return programUdpFlow(acl);
            } else if (acl.getProtocol().toJava() == NwConstants.IP_PROT_ICMP) {
                return programIcmpFlow(acl);
            } else if (acl.getProtocol().toJava() != -1) {
