...
public abstract class AbstractAclServiceImpl implements AclServiceListener {
    ...
    protected void programFirstRemoteAclEntryInDispatcherTable(AclInterface port, Integer firstRemoteAclTag,
        ...
        syncFlow(port.getDpId(), port.getInterfaceId(), getAclFilterCumDispatcherTable(), flowId,
                AclConstants.ACE_FIRST_REMOTE_ACL_PRIORITY, "ACL", 0, 0, AclConstants.COOKIE_ACL_BASE, matches,
                instructions, addOrRemove);
    }
    ...
    protected void programLastRemoteAclEntryInDispatcherTable(AclInterface port, Integer lastRemoteAclTag,
        ...
        syncFlow(port.getDpId(), port.getInterfaceId(), getAclFilterCumDispatcherTable(), flowId,
                AclConstants.ACE_LAST_REMOTE_ACL_PRIORITY, "ACL", 0, 0,
                AclServiceUtils.getDropFlowCookie(port.getLPortTag()), matches, instructions, addOrRemove);
    }
    ...
    protected void programForAceNotHavingRemoteAclId(AclInterface port, String aclName, Ace ace,
        ...
        for (Entry<String, List<MatchInfoBase>> entry : flowMap.entrySet()) {
            ...
            syncFlow(port.getDpId(), port.getInterfaceId(), getAclFilterCumDispatcherTable(), flowId, flowPriority,
                    "ACL", 0, 0, AclConstants.COOKIE_ACL_BASE, matches, instructions, operation);
    ...
    protected void programAceSpecificFlows(AclInterface port, String aclName, Ace ace,
        ...
        for (Entry<String, List<MatchInfoBase>> entry : flowMap.entrySet()) {
            ...
            int operation = addOrRemove == NwConstants.MOD_FLOW ? NwConstants.DEL_FLOW : addOrRemove;
            syncFlow(port.getDpId(), port.getInterfaceId(), getAclRuleBasedFilterTable(), flowId, flowPriority, "ACL",
    ...
    private void programAclForExistingTrafficTable(AclInterface port, Ace ace, int addOrRemove, String flowName,
        ...
        syncFlow(port.getDpId(), port.getInterfaceId(), getAclForExistingTrafficTable(), newFlowName, priority, "ACL",
                0, AclServiceUtils.getHardTimoutForApplyStatefulChangeOnExistingTraffic(ace, aclServiceUtils),
    ...
    protected void syncFlow(BigInteger dpId, String interfaceId, short tableId, String flowId, int priority,
        ...
        jobCoordinator.enqueueJob(flowName + interfaceId, () -> {
            if (addOrRemove == NwConstants.DEL_FLOW) {
    ...
    protected void programConntrackRecircRule(BigInteger dpId, int lportTag, String portId,
        ...
        syncFlow(dpId, portId, getAclConntrackSenderTable(), flowName, AclConstants.ACL_DEFAULT_PRIORITY, "ACL", 0, 0,
    ...
    protected void programConntrackInvalidDropRule(BigInteger dpId, int lportTag, String portId, int addOrRemove) {
        ...
        syncFlow(dpId, portId, getAclFilterCumDispatcherTable(), flowId,
                AclConstants.CT_STATE_TRACKED_INVALID_PRIORITY, "ACL", 0, 0,
                AclServiceUtils.getDropFlowCookie(lportTag), matches, instructions, addOrRemove);
    }
    ...
    protected void programAclCommitRuleForConntrack(BigInteger dpId, int lportTag, String portId,
        ...
        syncFlow(dpId, portId, getAclCommitterTable(), flowName, AclConstants.ACL_DEFAULT_PRIORITY, "ACL", 0, 0,
                AclConstants.COOKIE_ACL_BASE, matches, instructions, addOrRemove);
    }
