    ...
    protected void programIcmpv6RARule(AclInterface port, List<SubnetInfo> subnets, int addOrRemove) {
        ...
        if (!AclServiceUtils.isIpv6RaAllowedFromExternalRouters(subnets)) {
            ...
            if (subnetRtrIntMacAddr != null) {
                Ipv6Address subnetRtrllAddr = AclServiceUtils.getIpv6LinkLocalAddressFromMac(subnetRtrIntMacAddr);
                ...
                flowName = "Ingress_ICMPv6" + "_" + port.getDpId() + "_" + port.getLPortTag() + "_"
                        + subnetRtrllAddr.getValue() + "_" + AclConstants.ICMPV6_TYPE_RA + "_LinkLocal_Permit_";
            }
        } else {
        ...
        if (flowName != null) {
            LOG.info("programIcmpv6RARule: Match {} and flowName {}", matches, flowName);
            syncFlow(port.getDpId(), getAclAntiSpoofingTable(), flowName, AclConstants.PROTO_IPV6_ALLOWED_PRIORITY,
                    "ACL", 0, 0, AclConstants.COOKIE_ACL_BASE, matches, instructions,
                    addOrRemove);
        }
    }
