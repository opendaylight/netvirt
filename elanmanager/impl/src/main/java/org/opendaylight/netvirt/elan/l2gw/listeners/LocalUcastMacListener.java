...
public class LocalUcastMacListener extends ChildListener<Node, LocalUcastMacs, String>
    ...
    private void deleteElanMacEntry(InstanceIdentifier<LocalUcastMacs> identifier, LocalUcastMacs macRemoved) {
        ...
        elanClusterUtils.runOnlyInOwnerNode(hwvtepNodeId + ":" + macAddress + HwvtepHAUtil.L2GW_JOB_KEY,
            "remove elan mac entry from config", () -> {
                ListenableFuture<Void> ft = txRunner.callWithNewWriteOnlyTransactionAndSubmit(LogicalDatastoreType.CONFIGURATION,
    ...
    private void updateElanMacInConfigDb(InstanceIdentifier<LocalUcastMacs> identifier, LocalUcastMacs macAdded) {
        ...
        elanClusterUtils.runOnlyInOwnerNode(hwvtepNodeId + ":" + macAddress + HwvtepHAUtil.L2GW_JOB_KEY,
            "update elan mac entry", () -> {
                ...
                ListenableFuture<Void> ft = txRunner.callWithNewWriteOnlyTransactionAndSubmit(LogicalDatastoreType.CONFIGURATION, (tx) -> {
                    tx.mergeParentStructurePut(iid, newElanMac);
                });
                return Lists.newArrayList(ft);
            });
    }
    ...
    protected void onParentAdded(final DataTreeModification<Node> modification) {
        ...
        ListenableFutures.addErrorLogging(txRunner.callWithNewReadWriteTransactionAndSubmit(LogicalDatastoreType.OPERATIONAL, tx -> {
            ...
            localUcastMacDeletedTasks.put(hwvtepNodeId,
                scheduler.getScheduledExecutorService().schedule(() -> {
                    elanClusterUtils.runOnlyInOwnerNode(STALE_LOCAL_UCAST_CLEANUP_JOB + hwvtepNodeId, () -> {
                        txRunner.callWithNewReadOnlyTransactionAndClose(LogicalDatastoreType.CONFIGURATION, configTx -> {
                            ...
                            try {
                                ...
                                txRunner.callWithNewReadOnlyTransactionAndClose(LogicalDatastoreType.OPERATIONAL, operTx -> {
                                    try {
                                        Set<LocalUcastMacsKey> operMacs = macSetToKeySet(getMacs(operTx.read(nodeIid).get()
                                                .orElse(null)));
