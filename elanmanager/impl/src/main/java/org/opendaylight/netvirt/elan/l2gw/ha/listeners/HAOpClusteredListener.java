...
public class HAOpClusteredListener extends HwvtepNodeBaseListener<Operational> implements
    ...
    @Override
    synchronized  void onGlobalNodeDelete(InstanceIdentifier<Node> key, Node added, TypedReadWriteTransaction<Operational> tx)  {
    ...
    public synchronized void onGlobalNodeAdd(InstanceIdentifier<Node> key, Node updated, TypedReadWriteTransaction<Operational> tx) {
        ...
        if (waitingJobs.containsKey(key) && !waitingJobs.get(key).isEmpty()) {
            try {
                ...
                Optional<Node> nodeOptional = tx.read(key).checkedGet();
