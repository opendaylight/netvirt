...
public abstract class HwvtepNodeBaseListener implements DataTreeChangeListener<Node>, AutoCloseable {
    ...
    Labeled<Labeled<Labeled<Labeled<Counter>>>> childModCounter;
    Labeled<Labeled<Labeled<Counter>>> nodeModCounter;
    ...
    Map<Class, Function<DataObject, String>> logicalSwitchExtractor = Collections.unmodifiableMap(
            new HashMap<Class, Function<DataObject, String>>() {
                {
                    ...
                    put(RemoteMcastMacs.class, data -> {
                        return logicalSwitchNameFromIid(((RemoteMcastMacs) data).getKey().getLogicalSwitchRef().getValue());
                    });
                    put(RemoteUcastMacs.class, data -> {
                        return logicalSwitchNameFromIid(((RemoteUcastMacs) data).getKey().getLogicalSwitchRef().getValue());
                    });
                }
            });

    Function<DataObject, String> NO_LOGICAL_SWITCH = (data) -> null;

    String logicalSwitchNameFromChildMod(DataObjectModification<? extends DataObject> childMod) {
        DataObject data = childMod.getDataAfter() != null ? childMod.getDataAfter() : childMod.getDataBefore();
        return logicalSwitchExtractor.getOrDefault(childMod.getModificationType().getClass(), NO_LOGICAL_SWITCH)
                .apply(data);
    }

    String logicalSwitchNameFromIid(InstanceIdentifier input) {
