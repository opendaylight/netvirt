...
public class DpnDmacJob extends DataStoreJob {
    ...
    public List<ListenableFuture<Void>> call() throws Exception {
        ...
        if (added) {
            fts = ucastUtils.installDmacFlowsOnDpn(dpnInterfaces.getDpId().toJava(), device, elan,
            ...
            if (localMacs != null && !localMacs.isEmpty()) {
                for (MacAddress mac : localMacs) {
                    fts.addAll(elanDmacUtils.deleteDmacFlowsToExternalMac(elan.getElanTag().longValue(), dpnInterfaces.getDpId(),
