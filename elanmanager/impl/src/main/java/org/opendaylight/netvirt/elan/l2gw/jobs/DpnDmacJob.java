...
public class DpnDmacJob implements Callable<List<ListenableFuture<Void>>> {
    ...
    public List<ListenableFuture<Void>> call() throws Exception {
        ...
        if (added) {
            elanL2GatewayUtils.installDmacFlowsOnDpn(dpnInterfaces.getDpId(), device, elan,
                    dpnInterfaces.getInterfaces().get(0));
        } else {
            List<MacAddress> localMacs = elanL2GatewayUtils.getL2GwDeviceLocalMacs(elan.getElanInstanceName(), device);
            if (localMacs != null && !localMacs.isEmpty()) {
                for (MacAddress mac : localMacs) {
                    elanDmacUtils.deleteDmacFlowsToExternalMac(elan.getElanTag(), dpnInterfaces.getDpId(),
