 ...
 */
public class VLANProviderUtil implements ConfigInterface {
    private final Logger LOG = LoggerFactory.getLogger(VLANProviderUtil.class);
    ...
    public void providerNetworkFlow(Node envNode, OvsdbTerminationPointAugmentation port, NeutronNetwork network,
        ...
        try {
            ...
            outboundNatProvider.provideNetworkOutput(dpIdInt, ofPort, patchIntPort, macAddress, write);
            outboundNatProvider.provideNetworkPopVlan(dpIdInt, network.getProviderSegmentationID(), ofPort, patchIntPort, write);
            outboundNatProvider.provideNetworkPushVlan(dpIdExt, network.getProviderSegmentationID(), patchExtPort, macAddress, write);
            outboundNatProvider.provideNetworkDrop(dpIdExt, patchExtPort, write);
        } catch(Exception e) {
            LOG.error("providerNetworkFlow:Error while writing a flows. Caused due to, " + e.getMessage());
        }
    }
    ...
    public void setDependencies(final Object impl) {
        if (impl instanceof OutboundNatProvider) {
