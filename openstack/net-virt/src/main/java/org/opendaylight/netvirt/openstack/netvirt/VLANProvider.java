...
public class VLANProvider implements ConfigInterface {
    ...
    private Set<String> lstMacAddress = new HashSet<String>();
    ...
    private void populateVLANProviderCaches() {
        ...
        for (NeutronPort neutronPort : neutronPorts) {
            if (neutronPort != null && neutronPort.getDeviceOwner().equalsIgnoreCase(Constants.OWNER_ROUTER_GATEWAY)) {
                ...
                LOG.debug("In populateVLANProviderCaches macAddress:" + macAddress + "providerSegmentationId:" + providerSegmentationId);
                if (providerSegmentationId == null || providerSegmentationId.isEmpty() || macAddress == null || macAddress.isEmpty()) {
                    return;
                }
                if (vlanProviderCache != null && !vlanProviderCache.isEmpty() && vlanProviderCache.containsKey(providerSegmentationId)) {
