...
public class DistributedArpService implements ConfigInterface {
    ...
    private void handleNeutronPortForArp(NeutronPort neutronPort, Action action) {
        ...
        for (Node node : nodes) {
            ...
            handleArpRule(arpNeeded, neutronPort, existingPort, neutronPortList, providerSegmentationId, dpid, actionForNode);
        }
    }

    private void handleArpRule(boolean arpNeeded, NeutronPort neutronPort, NeutronPort existingPort, List <NeutronPort> neutronPortList,
                String providerSegmentationId, Long dpid, Action actionForNode) {
        if (arpNeeded) {
                    ...
                    for (NeutronPort port : neutronPortCache.getAllPorts()) {
                        ...
                        for (Neutron_IPs neutronIPAddr : port.getFixedIPs()) {
                            ...
                            try {
                                ...
                                LOG.warn("handleArpRule : unable to accept execution.");
                                programStaticRuleStage1(dpid, providerSegmentationId, portMacAddress, portIpAddress, actionForNode);
                            }
                        }
                    }
