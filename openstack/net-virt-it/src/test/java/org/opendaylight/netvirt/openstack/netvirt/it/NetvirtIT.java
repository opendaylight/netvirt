...
public class NetvirtIT extends AbstractMdsalTestBase {
        ...
        try(DockerOvs ovs = new DockerOvs()) {
            ...
            PingableNeutronNetItUtil net =
                    new PingableNeutronNetItUtil(ovs, southboundUtils, UUID.randomUUID().toString(),
                            Boolean.parseBoolean(userSpaceEnabled));
            net.create();
            ...
            logState(ovs, 0, "after ports 2");
            net.preparePortForPing("vm1");
            net.preparePortForPing("vm2");

            LOG.info("wait for stuff to catch up");
            Thread.sleep(30000);
            logState(ovs, 0, "after preparePortForPing");
            net.ping("vm1", "vm2");
            LOG.info("after ping");
            logState(ovs, 0, "after ping");
            net.destroy();
            nodeInfo.disconnect();
        } catch (Exception e) {
    ...
    public void logState(DockerOvs dockerOvs, int dockerInstance, String logText)
