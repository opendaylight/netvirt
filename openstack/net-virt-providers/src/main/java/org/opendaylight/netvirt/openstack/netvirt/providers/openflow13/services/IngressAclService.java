...
public class IngressAclService extends AbstractServiceInstance implements IngressAclProvider, ConfigInterface {
    ...
    public void programPortSecurityRule(Long dpid, String segmentationId, String attachedMac,
        ...
        if (null == portSecurityRule.getSecurityRuleProtocol()
                && securityGrp.getSecurityGroupName().equals("default")) {
            ...
            switch (portSecurityRule.getSecurityRuleProtocol() == null ? ""
                    : portSecurityRule.getSecurityRuleProtocol()) {
                case MatchUtils.TCP:
    ...
    private void ingressOtherProtocolAclHandler(Long dpidLong, String segmentationId, String dstMac,
        ...
        if(null == portSecurityRule.getSecurityRuleProtocol() || portSecurityRule.getSecurityRuleProtocol().equals(MatchUtils.ANY_PROTOCOL)) {
            ...
            if (portSecurityRule.getSecurityRuleProtocol().equals(MatchUtils.TCP_PROTOCOL)) {
                ...
                matchBuilder = MatchUtils.createIpProtocolAndEthMatch(matchBuilder, proto,
                        isIpv6 ? MatchUtils.ETHERTYPE_IPV6 : MatchUtils.ETHERTYPE_IPV4, null, dstMac);
                if (null != srcAddress) {
    ...
    public void programFixedSecurityGroup(Long dpid, String segmentationId, String dhcpMacAddress,
        ...
        programNeighborDiscoveryRule(dpid, segmentationId, attachMac, write);
    }
    ...
    private void addTcpSynFlagMatchIpv6Drop(Long dpidLong, String segmentationId, String dstMac,
        ...
        MatchBuilder matchBuilder = new MatchBuilder();
        matchBuilder = MatchUtils.createV6EtherMatch(matchBuilder, null, dstMac);
        matchBuilder = MatchUtils.addTcpSynMatch(matchBuilder);
        FlowBuilder flowBuilder = FlowUtils.createFlowBuilder(flowId, priority, matchBuilder, getTable());
        addPipelineInstruction(flowBuilder, null, true);
        NodeBuilder nodeBuilder = FlowUtils.createNodeBuilder(dpidLong);
    ...
    private void programNeighborDiscoveryRule(Long dpid, String segmentationId, String attachedMac, boolean write) {
        for (Integer icmpNDType : Constants.ICMPv6_INGRESS_NEIGHBOR_DISCOVERY_TYPE) {
    ...
    private void neighborDiscoveryRule(Long dpid, String segmentationId, String attachedMac,
        ...
        MatchBuilder matchBuilder = new MatchBuilder();
        String flowId = "Ingress_NS_" + segmentationId + "_" + attachedMac + "_" + type + "_" + code + "_";
        MatchUtils.createV6EtherMatch(matchBuilder, null, attachedMac);
    ...
    private void ingressAclIcmpV4(Long dpidLong, String segmentationId, String dstMac,
        ...
        if(isIcmpAll)
        {
            ...
            addIcmpFlow(nodeBuilder, portSecurityRule, segmentationId, dstMac, protoPortMatchPriority - 1,
                    srcAddress, write);
        } else {
            flowId = flowId + "_Permit";
            addConntrackMatch(matchBuilder, MatchUtils.TRACKED_NEW_CT_STATE, MatchUtils.TRACKED_NEW_CT_STATE_MASK);
    ...
    private void addIcmpFlow(NodeBuilder nodeBuilder, NeutronSecurityRule portSecurityRule, String segmentationId,
                             String dstMac, Integer protoPortMatchPriority, String srcAddress, boolean write){
        MatchBuilder matchBuilder = new MatchBuilder();
        String flowId = "Ingress_ICMP_" + segmentationId + "_" + dstMac + "_";
        matchBuilder = MatchUtils.createV4EtherMatchWithType(matchBuilder,null,dstMac,MatchUtils.ETHERTYPE_IPV4);
        if (null != srcAddress) {
    ...
    private void addIcmpV6Flow(NodeBuilder nodeBuilder, NeutronSecurityRule portSecurityRule, String segmentationId,
                               Integer protoPortMatchPriority, String srcAddress, String dstMac, boolean write){
        MatchBuilder matchBuilder = new MatchBuilder();
        ...
        if (null != srcAddress) {
            ...
            flowId = flowId + portSecurityRule.getSecurityRuleRemoteIpPrefix();
            matchBuilder = MatchUtils.addRemoteIpv6Prefix(matchBuilder,
                    new Ipv6Prefix(portSecurityRule.getSecurityRuleRemoteIpPrefix()),null);
        }
        flowId = flowId + "all" + "_";
        ...
        FlowBuilder flowBuilder = FlowUtils.createFlowBuilder(
                rangeflowId, protoPortMatchPriority, matchBuilder, getTable());
        addPipelineInstruction(flowBuilder, null, false);
        syncFlow(flowBuilder ,nodeBuilder, write);
    }
    ...
    private void ingressAclIcmpV6(Long dpidLong, String segmentationId, String dstMac,
        ...
        if(isIcmpAll)
        {
            ...
            for(Map.Entry<Integer, String> entry : map.entrySet()) {
                ...
                String rangeflowId = flowId + "_" + entry.getKey() + "_" + entry.getValue();
                addConntrackMatch(matchBuilder, MatchUtils.TRACKED_NEW_CT_STATE, MatchUtils.TRACKED_NEW_CT_STATE_MASK);
                FlowBuilder flowBuilder = FlowUtils.createFlowBuilder(rangeflowId, protoPortMatchPriority, matchBuilder,
                        getTable());
                addInstructionWithLearnConntrackCommit(portSecurityRule, flowBuilder, entry.getValue(), "0");
                syncFlow(flowBuilder ,nodeBuilder, write);
            }
            addIcmpV6Flow(nodeBuilder, portSecurityRule, segmentationId, protoPortMatchPriority - 1,
                    srcAddress, dstMac, write);
        } else {
            addConntrackMatch(matchBuilder, MatchUtils.TRACKED_NEW_CT_STATE, MatchUtils.TRACKED_NEW_CT_STATE_MASK);
            flowId = flowId + "_Permit";
            FlowBuilder flowBuilder = FlowUtils.createFlowBuilder(flowId, protoPortMatchPriority, matchBuilder,
                    getTable());
    ...
    private FlowBuilder addInstructionWithLearnConntrackCommit(NeutronSecurityRule portSecurityRule,
                                                               FlowBuilder flowBuilder,
        ...
        if (isIpv6) {
            if (portSecurityRule.getSecurityRuleProtocol().equalsIgnoreCase(MatchUtils.TCP)
                ...
                return IngressAclLearnServiceUtil.programIngressAclLearnRuleForIpv6Icmp(
                        flowBuilder, instructionBuilder, icmpType, icmpCode);
            }
        } else {
            if (portSecurityRule.getSecurityRuleProtocol().equalsIgnoreCase(MatchUtils.TCP)
                    || portSecurityRule.getSecurityRuleProtocol().equalsIgnoreCase(MatchUtils.TCP_PROTOCOL)) {
                ...
                return IngressAclLearnServiceUtil.programIngressAclLearnRuleForUdp(flowBuilder, instructionBuilder);
                    ...
                    || portSecurityRule.getSecurityRuleProtocol().equalsIgnoreCase(MatchUtils.ICMP_PROTOCOL)) {
                return IngressAclLearnServiceUtil.programIngressAclLearnRuleForIcmp(flowBuilder, instructionBuilder,
                        icmpType, icmpCode);
            }
        }
        return flowBuilder;
