...
public class ResubmitService extends AbstractServiceInstance implements ConfigInterface, ResubmitProvider {
    ...
    public ResubmitService() {
        super(Service.RESUBMIT_SERVICE);
    }

    public ResubmitService(Service service) {
        super(service);
    }
    ...
    public Status programResubmit(Long dpid) {
        ...
        ActionBuilder aba = new ActionBuilder();
        aba.setAction(new NxActionResubmitRpcAddGroupCaseBuilder().setNxResubmit(nxarsb.build()).build());
        aba.setKey(new ActionKey(0));
        listAction.add(aba.build());

        nxarsb = new NxResubmitBuilder();
        nxarsb.setTable(Short.parseShort("40"));
        aba = new ActionBuilder();
        aba.setAction(new NxActionResubmitRpcAddGroupCaseBuilder().setNxResubmit(nxarsb.build()).build());
        aba.setKey(new ActionKey(1));
        listAction.add(aba.build());
    ...
    public void setDependencies(BundleContext bundleContext, ServiceReference serviceReference) {
        super.setDependencies(bundleContext.getServiceReference(ResubmitProvider.class.getName()), this);
    }
