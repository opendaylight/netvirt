...
public class PingableNeutronNetItUtil extends NeutronNetItUtil {
    ...
    public void createPort(Node bridge, String portName, String owner, NeutronSecurityGroup... secGroups)
        ...
        dockerOvs.runInContainer(5000, 0, "ip", "tuntap", "add", portInfo.name, "mode", "tap");
        dockerOvs.runInContainer(5000, 0, "ip", "link", "set", "dev", portInfo.name, "address", portInfo.mac);
    ...
    public void preparePortForPing(String portName) throws IOException, InterruptedException {
        ...
        dockerOvs.runInContainer(5000, 0, "ip", "netns", "add", nsName);
        dockerOvs.runInContainer(5000, 0, "ip", "link", "set", portName, "netns", nsName);
        dockerOvs.runInContainer(5000, 0, "ip", "netns", "exec", nsName, "ip", "addr", "add", "dev", portName, portInfo.ip + "/24");
        dockerOvs.runInContainer(5000, 0, "ip", "netns", "exec", nsName, "ip", "link", "set", "dev", portName, "up");
        dockerOvs.runInContainer(5000, 0, "ip", "netns", "exec", nsName, "ip", "route", "add", "default", "via", portInfo.ip);
    }
    ...
    public void pingIp(String fromPort, String ip) throws IOException, InterruptedException {
        ...
        dockerOvs.runInContainer(5000, 0, "ip", "netns", "exec", fromNs, "ping", "-c", "4", ip);
    }
}
