...
public class ShowVpnIpToPort extends OsgiCommandSupport {
    ...
    private void startArpMonitorTaskForMips(VpnInterface vpnInterface, String vpnName) {
        ...
        VpnPortipToPort vpnVipToPort = VpnUtil.getVpnPortipToPort(dataBroker, vpnName, learntIpAddr);
        if (vpnVipToPort != null && vpnVipToPort.isLearntIp()) {
            synchronized ((vpnName + learntIpAddr).intern()) {
                VpnUtil.createLearntVpnVipToPortEvent(dataBroker, vpnName, learntIpAddr, learntIpAddr, interfaceName,
