    ...
    private void handleRouterInterfaceAdded(Port routerPort) {
        if (routerPort.getDeviceId() != null) {
            ...
            if (existingVpnId == null) {
                ...
                boolean isV4Subnet = false;
                boolean isV6Subnet = false;
                ...
                if (neutronvpnUtils.shouldVpnHandleIpVersionChoiceChange(
                        isV4Subnet ? IpVersionChoice.IPV4 : IpVersionChoice.IPV6, routerId, true)) {
                    LOG.debug("vpnInstanceOpDataEntry is getting update with ip address family {} for VPN {}",
                            isV4Subnet && isV6Subnet == true ? "IPv4 and IPv6" : isV4Subnet == true
                                    ? "IPv4" : "IPv6", vpnId.getValue());
                    neutronvpnUtils.updateVpnInstanceWithIpFamily(vpnId.getValue(), isV4Subnet, isV6Subnet,
                            true);
                }
        ...
        if (routerPort.getDeviceId() != null) {
            ...
            ListenableFutures.addErrorLogging(txRunner.callWithNewWriteOnlyTransactionAndSubmit(CONFIGURATION,
                confTx -> {
                    ...
                    if (neutronvpnUtils.shouldVpnHandleIpVersionChoiceChange(
                            isV4Subnet ? IpVersionChoice.IPV4 : IpVersionChoice.IPV6, routerId, false)) {
                        LOG.debug("vpnInstanceOpDataEntry is getting update with ip address family {} for VPN {}",
                                isV4Subnet && isV6Subnet == true ? "IPv4 and IPv6"
                                        : isV4Subnet == true ? "IPv4" : "IPv6", vpnId.getValue());
                        neutronvpnUtils.updateVpnInstanceWithIpFamily(vpnId.getValue(), isV4Subnet,
                                isV6Subnet, false);
                    }
