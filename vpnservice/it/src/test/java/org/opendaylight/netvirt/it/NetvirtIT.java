    ...
    public void testNeutronNet() throws InterruptedException {
        ...
        try (DockerOvs ovs = new DockerOvs()) {
            ...
            if (ovs.usingExternalDocker()) {
                netOvs = new RealNetOvsImpl(ovs, Boolean.parseBoolean(userSpaceEnabled), mdsalUtils,
                        neutron, southboundUtils);
            } else {
                netOvs = new DockerNetOvsImpl(ovs, Boolean.parseBoolean(userSpaceEnabled), mdsalUtils,
                        neutron, southboundUtils);
            }
