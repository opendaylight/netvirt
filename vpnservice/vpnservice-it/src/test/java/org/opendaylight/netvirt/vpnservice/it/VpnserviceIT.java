...
public class VpnserviceIT extends AbstractMdsalTestBase {

    private static final Logger LOG = LoggerFactory.getLogger(VpnserviceIT.class);
    ...
    public Option getLoggingOption() {
        return composite(
                editConfigurationFilePut(ORG_OPS4J_PAX_LOGGING_CFG,
                        logConfiguration(VpnserviceIT.class),
                        LogLevel.INFO.name()),
                editConfigurationFilePut(ORG_OPS4J_PAX_LOGGING_CFG,
...
//                        LogLevel.TRACE.name()),
    ...
    public void setup() throws InterruptedException {
        ...
        assertTrue("Did not find " + NETVIRT_TOPOLOGY_ID, this.getNetvirtTopology().booleanValue());
...
//      TODO: need to implement new pipelineOrchestrator for the vpnservice pipeline
    ...
    private void getProperties() {
        ...
        addressStr = props.getProperty(VpnserviceITConstants.SERVER_IPADDRESS);
        portStr = props.getProperty(VpnserviceITConstants.SERVER_PORT, VpnserviceITConstants.DEFAULT_SERVER_PORT);
        connectionType = props.getProperty(VpnserviceITConstants.CONNECTION_TYPE, "active");
        controllerStr = props.getProperty(VpnserviceITConstants.CONTROLLER_IPADDRESS, "0.0.0.0");
        String userSpaceEnabled = props.getProperty(VpnserviceITConstants.USERSPACE_ENABLED, "no");
    ...
    private Boolean getNetvirtTopology() {
        LOG.info("getNetvirtTopology: looking for {}...", "netvirt:1");
        Boolean found = Boolean.valueOf(false);
        TopologyId topologyId = new TopologyId(new Uri(NETVIRT_TOPOLOGY_ID));
        InstanceIdentifier<Topology> path =
                InstanceIdentifier.create(NetworkTopology.class).child(Topology.class, new TopologyKey(topologyId));
        for(int i = 0; i < 60; ++i) {
            Topology topology = mdsalUtils.read(LogicalDatastoreType.OPERATIONAL, path);
            if(topology != null) {
                LOG.info("getNetvirtTopology: found {}...", "netvirt:1");
                found = Boolean.valueOf(true);
                break;
            }

            LOG.info("getNetvirtTopology: still looking ({})...", Integer.valueOf(i));
    ...
    public void testNetVirt() throws InterruptedException {
        try(DockerOvs ovs = new DockerOvs()) {
            ...
            OvsdbTerminationPointAugmentation terminationPointOfBridge = southboundUtils.
                    getTerminationPointOfBridge(nodeInfo.bridgeNode, VpnserviceITConstants.PORT_NAME);
            assertNotNull("Did not find " + VpnserviceITConstants.PORT_NAME, terminationPointOfBridge);
    ...
    public void testNeutronNet() throws InterruptedException {
        ...
        try(DockerOvs ovs = new DockerOvs()) {
            ...
            VpnserviceNeutronITUtil net = new VpnserviceNeutronITUtil(ovs, southboundUtils, mdsalUtils);
