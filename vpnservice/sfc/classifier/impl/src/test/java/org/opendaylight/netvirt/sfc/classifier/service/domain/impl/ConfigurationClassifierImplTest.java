...
public class ConfigurationClassifierImplTest {
    ...
    private static final DpnIdType NODE_DPN = new DpnIdType(BigInteger.valueOf(1234));
    private static final NodeId NODE_ID = new NodeId("openflow:" + NODE_DPN.getValue().intValue());
    private static final String NODE_IP = "127.0.0.1";
    private static final String OUTPUT_INTERFACE = "output";
    private static final String SFF_IP = "127.0.0.101";
    private static final String INPUT_CONNECTOR = NODE_ID.getValue() + ":0";
    ...
    public void setup() throws ReadFailedException {
        ...
        when(geniusProvider.getIpFromDpnId(NODE_DPN))
                .thenReturn(Collections.singletonList(new IpAddress(new Ipv4Address(NODE_IP))));
        when(geniusProvider.getIpFromInterfaceName(SF_INTERFACE)).thenReturn(Optional.of(SFF_IP));
    ...
    public void getAllEntries() throws Exception {
        ...
        assertThat(entries, Matchers.containsInAnyOrder(
                ...
                ClassifierEntry.buildMatchEntry(NODE_ID, INPUT_CONNECTOR, MATCHES, PATH_ID, PATH_INDEX, SFF_IP),
                ClassifierEntry.buildPathEntry(NODE_ID, PATH_ID, NODE_IP),
    ...
    public void getAllEntriesMultipleNodes() throws Exception {
        ...
        final DpnIdType otherNodeDpn = new DpnIdType(BigInteger.valueOf(4321));
        final NodeId otherNodeId = new NodeId("openflow:" + otherNodeDpn.getValue().intValue());
        final String otherNodeIp = "127.0.0.2";
        final String otherConnector = otherNodeId.getValue() + ":9";
        final String otherOutput = "other_output";
        when(netvirtProvider.getLogicalInterfacesFromNeutronNetwork(NEUTRON_NETWORK))
                .thenReturn(Arrays.asList(INPUT_INTERFACE, otherInput));
        when(geniusProvider.getNodeIdFromLogicalInterface(otherInput)).thenReturn(Optional.of(otherNodeId));
        when(geniusProvider.getNodeConnectorIdFromInterfaceName(otherInput)).thenReturn(Optional.of(otherConnector));
        when(geniusProvider.getInterfacesFromNode(otherNodeId)).thenReturn(Collections.singletonList(otherOutput));
        when(geniusProvider.getIpFromDpnId(otherNodeDpn))
                .thenReturn(Collections.singletonList(new IpAddress(new Ipv4Address(otherNodeIp))));
        ...
        assertThat(entries, Matchers.containsInAnyOrder(
                ...
                ClassifierEntry.buildMatchEntry(NODE_ID, INPUT_CONNECTOR, MATCHES, PATH_ID, PATH_INDEX, SFF_IP),
                ClassifierEntry.buildMatchEntry(otherNodeId, otherConnector, MATCHES, PATH_ID, PATH_INDEX, SFF_IP),
                ClassifierEntry.buildPathEntry(NODE_ID, 100L, NODE_IP),
                ClassifierEntry.buildPathEntry(otherNodeId, 100L, otherNodeIp),
                ClassifierEntry.buildNodeEntry(NODE_ID),
                ClassifierEntry.buildNodeEntry(otherNodeId),
                ClassifierEntry.buildEgressEntry(new InterfaceKey(OUTPUT_INTERFACE)),
    ...
    public void getAllEntriesMultipleInputs() throws Exception {
        ...
        final String otherInput = "other_input";
        final String otherConnector = NODE_ID.getValue() + ":9";
        when(netvirtProvider.getLogicalInterfacesFromNeutronNetwork(NEUTRON_NETWORK))
        ...
        assertThat(entries, Matchers.containsInAnyOrder(
                ...
                ClassifierEntry.buildIngressEntry(new InterfaceKey(otherInput)),
                ClassifierEntry.buildMatchEntry(NODE_ID, INPUT_CONNECTOR, MATCHES, PATH_ID, PATH_INDEX, SFF_IP),
                ClassifierEntry.buildMatchEntry(NODE_ID, otherConnector, MATCHES, PATH_ID, PATH_INDEX, SFF_IP),
                ClassifierEntry.buildPathEntry(NODE_ID, PATH_ID, NODE_IP),
                ClassifierEntry.buildNodeEntry(NODE_ID),
                ClassifierEntry.buildEgressEntry(new InterfaceKey(OUTPUT_INTERFACE))
        ));
    ...
    public void getAllEntriesMultipleNetworks() throws Exception {
        ...
        final String otherInput = "other_input";
        final String otherConnector = NODE_ID + ":9";
        ...
        assertThat(entries, Matchers.containsInAnyOrder(
                ...
                ClassifierEntry.buildIngressEntry(new InterfaceKey(otherInput)),
                ClassifierEntry.buildMatchEntry(NODE_ID, INPUT_CONNECTOR, MATCHES, PATH_ID, PATH_INDEX, SFF_IP),
                ClassifierEntry.buildMatchEntry(NODE_ID, otherConnector, otherMatches, PATH_ID, PATH_INDEX, SFF_IP),
                ClassifierEntry.buildPathEntry(NODE_ID, PATH_ID, NODE_IP),
                ClassifierEntry.buildNodeEntry(NODE_ID),
                ClassifierEntry.buildEgressEntry(new InterfaceKey(OUTPUT_INTERFACE))
        ));
    ...
    public void getAllEntriesMultipleRedirects() throws Exception {
        ...
        final String otherSfIp = "127.0.0.102";
        ...
        assertThat(entries, Matchers.containsInAnyOrder(
                ...
                ClassifierEntry.buildPathEntry(NODE_ID, PATH_ID, NODE_IP),
                ClassifierEntry.buildPathEntry(NODE_ID, otherPathId, NODE_IP),
                ClassifierEntry.buildNodeEntry(NODE_ID),
                ClassifierEntry.buildEgressEntry(new InterfaceKey(OUTPUT_INTERFACE))
        ));
    }
