...
public class CounterRetriever {
    ...
    public CounterRetriever(final RpcProviderRegistry rpcProviderRegistry,
            final NotificationService notificationService, OpendaylightDirectStatisticsService odlDirectStatsService) {
    ...
    public CounterResultDataStructure getNodeCountersDirect(Node node) {
        ...
        for (NodeConnector nodeConnector : node.getNodeConnector()) {
            CounterResultDataStructure currentResult =
                    getNodeConnectorCountersDirect(node.getId(), nodeConnector.getId());
            if (currentResult == null) {
                CounterRetrieverCounters.failed_getting_node_counters.inc();
                LOG.warn("Failed getting statistics for node connector: {}", nodeConnector.getId().getValue());
                return null;
            } else {
                countersResults.add(currentResult);
            }
        }
