...
public class CountersServiceUtils {
    ...
    public static MatchInfoBase buildLPortTagMatch(int lportTag, ElementCountersDirection direction) {
        if (ElementCountersDirection.INGRESS.equals(direction)) {
            ...
            return (new NxMatchInfo(NxMatchFieldType.nxm_reg_6, new long[] {
                    MetaDataUtil.getReg6ValueForLPortDispatcher(lportTag, NwConstants.EGRESS_SFC_CLASSIFIER_SERVICE_INDEX) }));
        }
        return null;
    }
    ...
    public static List<MatchInfoBase> getCounterFlowMatch(ElementCountersRequest ecr, int lportTag,
        ...
        tcpFilterExist = buildTcpMatchIfExists(ecr, matches, tcpFilterExist);

        if (!tcpFilterExist) {
            buildUdpMatchIfExists(ecr, matches, udpFilterExist);
        }
    ...
    private static void buildUdpMatchIfExists(ElementCountersRequest ecr, List<MatchInfoBase> matches,
            boolean udpFilterExist) {
