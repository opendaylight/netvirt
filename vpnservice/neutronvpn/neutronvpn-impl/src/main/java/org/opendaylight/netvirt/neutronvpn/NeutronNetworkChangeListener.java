...
public class NeutronNetworkChangeListener extends AsyncDataTreeChangeListenerBase<Network, NeutronNetworkChangeListener>
    ...
    protected void add(InstanceIdentifier<Network> identifier, Network input) {
        ...
        if ((NeutronvpnUtils.isVxlanNetwork(input) || NeutronvpnUtils.isVlanNetwork(input)) && NeutronUtils
                .getSegmentationIdFromNeutronNetwork(input, NetworkTypeVxlan.class) == null) {
            LOG.error("Segmentation ID is null for configured provider network {}. Abandoning any further "
                    + "processing for the network", input.getUuid().getValue());
            return;
        }
