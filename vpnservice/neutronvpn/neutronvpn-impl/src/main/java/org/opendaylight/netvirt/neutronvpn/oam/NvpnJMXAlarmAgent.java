...
public class NvpnJMXAlarmAgent {
    ...
    public void invokeFMclearmethod(String alarmId, String text, String src) {
        try {
            mbs.invoke(alarmName, OP_CLEARALARM, new Object[] {alarmId, text, src},
    ...
    public void invokeFMraisemethod(String alarmId, String text, String src) {
        try {
            mbs.invoke(alarmName, OP_RAISEALARM, new Object[] {alarmId, text, src},
                    new String[] {String.class.getName(), String.class.getName(), String.class.getName()});
            Log.debug("Invoked raiseAlarm function for Mbean " + BEANNAME + " with source " + src);
        } catch (InstanceNotFoundException | MBeanException | ReflectionException e) {
