...
public class NeutronvpnManager implements NeutronvpnService, AutoCloseable, EventListener {
    ...
    private void clearFromVpnMaps(Uuid vpnId, Uuid routerId, List<Uuid> networkIds) {
        ...
        if (optionalVpnMap.isPresent()) {
            ...
            if (routerId != null) {
                ...
                vpnMapBuilder.setRouterIds(null);
            }
            if (networkIds != null) {
    ...
    protected Adjacencies createPortIpAdjacencies(Uuid vpnId, Port port, Boolean isRouterInterface,
        ...
        for (FixedIps ip : port.getFixedIps()) {
            ...
            if (routerId != null && isRouterInterface) {
                ...
                if (rtr != null && rtr.getRoutes() != null) {
                    ...
                    // bacause router can be dualstack, or can have
                    // more that one IPv4 subnet ports or more than one IPv6 subnet ports
    ...
    protected void withdrawPortIpFromVpnIface(Uuid vpnId, Port port, Subnetmap sn,
                                              WriteTransaction wrtConfigTxn) {
        boolean isLockAcquired = false;
        String infName = port.getUuid().getValue();
        ...
        for (FixedIps ip : portIps) {
            String ipValue = String.valueOf(ip.getIpAddress().getValue());
            if (FibHelper.isBelongingPrefix(ipValue, sn.getSubnetIp())) {
                // remove VpnPortipToPort entry
                ipToRemove = ipValue;
                NeutronvpnUtils.removeVpnPortFixedIpToPort(dataBroker, vpnId.getValue(), ipToRemove, wrtConfigTxn);
            } else {
        ...
        for (Adjacency adj : vpnAdjsList) {
            if (adj.getIpAddress().equals(ipToRemove)) {
        ...
        if (!isIpFromAnotherSubnet) {
            if (port.getDeviceOwner().equals(NeutronConstants.DEVICE_OWNER_ROUTER_INF)) {
                // Router port can have more than one IPv6 address, thats why we should be sure
                // (isIpFromAnotherSubnet provides this info), that given router port doesn't have
                // addresses from another subnets.
        ...
        if (portList != null) {
            for (final Uuid portId : portList) {
                ...
                if (isRouterInterface) {
                    addToNeutronRouterInterfacesMap(sn.getRouterId(), vpnInfName);
                }
            ...
            if (portList != null) {
                for (final Uuid portId : portList) {
                    ...
                    portDataStoreCoordinator.enqueueJob("PORT-" + portId.getValue(), () -> {
                        ...
                        if (port != null) {
                            // remove extra route Adjacencies from VpnInterface
                            Router rtr = NeutronvpnUtils.getNeutronRouter(dataBroker, sn.getRouterId());
                            if (rtr != null && rtr.getRoutes() != null) {
                                List<Routes> routeList = rtr.getRoutes();
                                removeAdjacencyforExtraRoute(vpnId, routeList);
                            }
                            // remove VM Adjacencies from VpnInterface and remove VpnPortipToPort entry
        ...
        try {
            ...
            if (vpnMap != null) {
                if (rtr != null) {
                    ...
                    if (!vpnMap.getRouterIds().isEmpty()) {
