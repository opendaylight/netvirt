...
public class NeutronvpnManager implements NeutronvpnService, AutoCloseable, EventListener {
    ...
    protected void withdrawIpAdjacenciesFromVpnIface(Uuid vpnId, Port port, String ipValue, WriteTransaction wrtConfigTxn) {
        boolean isLockAcquired = false;
        String infName = port.getUuid().getValue();
        ...
        try {
            ...
            List<Adjacency> vpnAdjsList = vpnIfBuilder.getAugmentation(Adjacencies.class).getAdjacency();
            List<Adjacency> updatedAdjsList = vpnAdjsList;
            for (Adjacency adj : vpnAdjsList) {
                if (adj.getIpAddress().equals(ipValue)){
            ...
            LOG.error("withdrawIpAdjacenciesFromVpnIface: failed to update vpninterface {}", infName, ex);
        } finally {
            if (isLockAcquired) {
    ...
    protected void withdrawPortIpFromVpnIface(Uuid vpnId, Port port, Subnetmap sn, WriteTransaction wrtConfigTxn) {
        ...
        for (FixedIps ip : portIps) {
            ...
            if (FibHelper.isBelongingPrefix(ipValue, sn.getSubnetIp())) {
                ...
                withdrawIpAdjacenciesFromVpnIface(vpnId, port, ipValue, wrtConfigTxn);
            } else {
