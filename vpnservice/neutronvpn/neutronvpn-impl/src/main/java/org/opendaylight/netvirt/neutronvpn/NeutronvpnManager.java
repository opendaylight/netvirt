...
public class NeutronvpnManager implements NeutronvpnService, AutoCloseable, EventListener {
    ...
    public NeutronvpnManager(
        ...
        nvpnNatManager = new NeutronvpnNatManager(this.dataBroker, this);
    ...
    private void updateVpnMaps(Uuid vpnId, String name, Uuid router, Uuid tenantId, List<Uuid> networks) {
        ...
        try {
            ...
            if (router != null) {
                List<org.opendaylight.yang.gen.v1.urn.opendaylight.netvirt
                    .neutronvpn.rev150602.vpnmaps.vpnmap.RouterIds> routerIdsList = builder.getRouterIds();
                if (routerIdsList == null || routerIdsList.isEmpty()) {
                    org.opendaylight.yang.gen.v1.urn.opendaylight.netvirt
                        .neutronvpn.rev150602.vpnmaps.vpnmap.RouterIds
                            vpnRouterId = NeutronvpnUtils.getvpnMapRouterIds(router);
                    builder.setRouterIds(Collections.singletonList(vpnRouterId));
                } else if (!NeutronvpnUtils.vpnMapRouterIdsContainsRouterId(router, routerIdsList)) {
                    org.opendaylight.yang.gen.v1.urn.opendaylight.netvirt
                        .neutronvpn.rev150602.vpnmaps.vpnmap.RouterIds
                            vpnRouterId = NeutronvpnUtils.getvpnMapRouterIds(router);
                    routerIdsList.add(vpnRouterId);
    ...
    private void clearFromVpnMaps(Uuid vpnId, Uuid routerId, List<Uuid> networkIds) {
        ...
        if (optionalVpnMap.isPresent()) {
            ...
            if (routerId != null) {
                ...
                List<org.opendaylight.yang.gen.v1.urn.opendaylight.netvirt
                    .neutronvpn.rev150602.vpnmaps.vpnmap.RouterIds> routerIds = vpnMap.getRouterIds();
                if (routerIds != null && routerIds.size() > 0) {
                    //we have to delete the routerId to this list
                    NeutronvpnUtils.removeVpnMapRouterIdsFromList(routerId, routerIds);
                    vpnMapBuilder.setRouterIds(routerIds);
                }
            }
            if (networkIds != null) {
    ...
    protected void withdrawPortIpFromVpnIface(Uuid vpnId, Uuid internetVpnId,
            ...
            if (adj.getAdjacencyType() == AdjacencyType.PrimaryAdjacency) {
                ...
                if (port.getDeviceOwner()
                    ...
                    if (rtr == null && rtr.getRoutes() != null) {
                        ...
                        if (internetVpnId != null) {
                            LOG.error("withdrawPortIpFromVpnIface: suppressing extraRoute {} for vpnId {}",
                                  extraRoutesToRemove, internetVpnId);
                            removeAdjacencyforExtraRoute(vpnId, extraRoutesToRemove);
                        }
                    }
                }
        ...
        if ((sn.getVpnId() != null && !sn.getVpnId().equals(vpnId))
            || (internetVpnId != sn.getInternetVpnId())
            || (sn.getInternetVpnId() != null && internetVpnId != null
                && !sn.getInternetVpnId().equals(internetVpnId))) {
            isIpFromAnotherSubnet = true; // still other vpn present
        }
    ...
    protected boolean deleteVpnInterface(String infName, String vpnId, WriteTransaction wrtConfigTxn) {
        ...
        if (optionalVpnInterface != null && vpnId != null && optionalVpnInterface.isPresent()) {
            ...
            if (vpnList != null
                && VpnHelper.vpnInterfaceVpnInstanceNamesContainsVpnName(vpnId, vpnList)) {
        ...
        try {
            ...
            if (optionalVpnInterface.isPresent()) {
                ...
                if (listVpn != null && vpnId != null
                    && VpnHelper.vpnInterfaceVpnInstanceNamesContainsVpnName(vpnId.getValue(), listVpn)) {
    ...
    protected void updateVpnInterface(Uuid vpnId, Uuid oldVpnId, Port port, boolean isBeingAssociated,
        ...
        try {
            ...
            if (optionalVpnInterface.isPresent()) {
                ...
                List<VpnInstanceNames> listVpn = optionalVpnInterface.get().getVpnInstanceNames();
                if (listVpn != null && oldVpnId.getValue() != null
                    && VpnHelper.vpnInterfaceVpnInstanceNamesContainsVpnName(oldVpnId.getValue(), listVpn)) {
                    VpnHelper.removeVpnInterfaceVpnInstanceNamesFromList(oldVpnId.getValue(), listVpn);
                }
                if (listVpn != null && vpnId.getValue() != null
                    && !VpnHelper.vpnInterfaceVpnInstanceNamesContainsVpnName(vpnId.getValue(), listVpn)) {
                    listVpn.add(vpnInstance);
                }
                if (listVpn == null || listVpn.isEmpty()) {
                    listVpn = new ArrayList<>();
                    listVpn.add(vpnInstance);
                }
                VpnInterfaceBuilder vpnIfBuilder = new VpnInterfaceBuilder(optionalVpnInterface.get())
                         .setVpnInstanceNames(listVpn);
    ...
    public Future<RpcResult<CreateL3VPNOutput>> createL3VPN(CreateL3VPNInput input) {
        ...
        for (L3vpn vpn : vpns) {
            ...
            if (vpn.getRouterIds() != null && !vpn.getRouterIds().isEmpty()) {
                List<Uuid> routerIdsList = NeutronvpnUtils.getVpnInstanceRouterIdsListUuid(vpn.getRouterIds());
                for (Uuid routerId : routerIdsList) {
            ...
            try {
                createVpn(vpn.getId(), vpn.getName(), vpn.getTenantId(), vpn.getRouteDistinguisher(),
                        vpn.getImportRT(), vpn.getExportRT(),
                        NeutronvpnUtils.getVpnInstanceRouterIdsListUuid(vpn.getRouterIds()),
                        vpn.getNetworkIds(), vpnInstanceType, l3vni);
            } catch (Exception ex) {
    ...
    public Future<RpcResult<GetL3VPNOutput>> getL3VPN(GetL3VPNInput input) {
        ...
        try {
            ...
            for (VpnInstance vpnInstance : vpns) {
                ...
                if (optionalVpnMap.isPresent()) {
                    VpnMap vpnMap = optionalVpnMap.get();
                    l3vpn.setRouterIds(NeutronvpnUtils.getVpnInstanceRouterIdsList(
                             NeutronUtils.getVpnMapRouterIdsListUuid(vpnMap.getRouterIds())))
                            .setNetworkIds(vpnMap.getNetworkIds())
    ...
    public void removeVpn(Uuid id) {
        ...
        List<Uuid> routerIdsList = vpnMap != null
              ? NeutronUtils.getVpnMapRouterIdsListUuid(vpnMap.getRouterIds()) : null;
        // dissociate router
        if (routerIdsList != null && !routerIdsList.isEmpty()) {
            for (Uuid routerId : routerIdsList) {
    ...
    public Future<RpcResult<Void>> associateRouter(AssociateRouterInput input) {
        ...
        StringBuilder returnMsg = new StringBuilder();
        Uuid vpnId = input.getVpnId();
        List<Uuid> routerIds = NeutronvpnUtils.getAssociateRouterInputRouterIdsListUuid(input.getRouterIds());
        String routerIdsString = "";
        try {
            if (routerIds == null || routerIds.isEmpty() || vpnId == null) {
                String error = "Error in try to associate with a null vpnId or routerId. "
                                + "vpnId : " + vpnId + " ,routerId : " + routerIds;
    ...
    public Future<RpcResult<Void>> dissociateRouter(DissociateRouterInput input) {
        ...
        StringBuilder returnMsg = new StringBuilder();
        Uuid vpnId = input.getVpnId();
        List<Uuid> routerIdList = NeutronvpnUtils.getDissociateRouterInputRouterIdsListUuid(input.getRouterIds());
        String routerIdsString = "";
        try {
            if (routerIdList == null || routerIdList.isEmpty() || vpnId == null) {
                String error = "Error in try to dissociate with a null vpnId or routerId. "
                                   + "vpnId : " + vpnId + " ,routerId : " + routerIdList;
                throw new NullPointerException(error);
            }
            if (NeutronvpnUtils.getVpnMap(dataBroker, vpnId) != null) {
            ...
            if (optionalVpnInterface.isPresent()) {
                ...
                if (optionalVpnInterface.get().getVpnInstanceNames() != null) {
                    if (optionalVpnInterface.get().getVpnInstanceNames().isEmpty()
                        || !VpnHelper.vpnInterfaceVpnInstanceNamesContainsVpnName(
                            vpnId.getValue(),optionalVpnInterface.get().getVpnInstanceNames())) {
                        List<VpnInstanceNames> listVpnInstances = optionalVpnInterface.get().getVpnInstanceNames();
                        VpnInstanceNames vpnInstance = VpnHelper
                             .getVpnInterfaceVpnInstanceNames(vpnId.getValue(), AssociatedSubnetType.V4AndV6Subnets);
