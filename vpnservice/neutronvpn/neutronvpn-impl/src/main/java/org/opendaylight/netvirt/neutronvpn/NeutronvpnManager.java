...
public class NeutronvpnManager implements NeutronvpnService, AutoCloseable, EventListener {
    ...
    public NeutronvpnManager(
        ...
        configureFeatures();
        nvpnNatManager.setNeutronvpnManager(this);
    }
    ...
    protected void createSubnetmapNode(Uuid subnetId, String subnetIp, Uuid tenantId, Uuid networkId,
        ...
        try {
            ...
            // Uuid internetVpnUuid = netIsExternal ? null
            //         :  NeutronvpnUtils.getInternetvpnUuidBoundToSubnetRouter(dataBroker, subnetId);
            Uuid internetVpnUuid = null;
            ...
            synchronized (subnetId.getValue().intern()) {
                ...
                SubnetmapBuilder subnetmapBuilder = new SubnetmapBuilder().setKey(new SubnetmapKey(subnetId))
                        ...
                        .setInternetVpnId(internetVpnUuid);
    ...
    protected Subnetmap removeFromSubnetNode(Uuid subnetId, Uuid networkId, Uuid routerId,
                                            Uuid vpnId, Uuid portId, Uuid internetVpnId) {
        Subnetmap subnetmap = null;
        InstanceIdentifier<Subnetmap> id = InstanceIdentifier.builder(Subnetmaps.class)
                .child(Subnetmap.class, new SubnetmapKey(subnetId))
                .build();
    ...
    private void updateVpnInstanceNode(String vpnName, List<String> rd, List<String> irt, List<String> ert,
        ...
        try {
            ...
            builder.setVpnConfig(vpnconfigBuilder.build());
    ...
    private void updateVpnMaps(Uuid vpnId, String name, Uuid router, Uuid tenantId, List<Uuid> networks) {
        ...
        try {
            ...
            if (router != null) {
                List<Uuid> routerIdsList = builder.getRouterIds();
                if (routerIdsList == null || routerIdsList.isEmpty()) {
                    builder.setRouterIds(Collections.singletonList(router));
                } else if (routerIdsList != null && !routerIdsList.contains(router)) {
                    routerIdsList.add(router);
                    builder.setRouterIds(routerIdsList);
                }
            }
            if (networks != null) {
    ...
    private void clearFromVpnMaps(Uuid vpnId, Uuid routerId, List<Uuid> networkIds) {
        ...
        if (optionalVpnMap.isPresent()) {
            ...
            if (routerId != null) {
                ...
                List<Uuid> ri = vpnMap.getRouterIds();
                if (ri != null && ri.size() > 0) {
                    //we have to delete the routerId to this list
                    if (ri.contains(routerId)) {
                        ri.remove(routerId);
                        vpnMapBuilder.setRouterIds(ri);
                    }
                }
            }
            if (networkIds != null) {
    ...
    protected Adjacencies createPortIpAdjacencies(Port port, Boolean isRouterInterface,
        ...
        if (port == null || port.getFixedIps() == null) {
            return new AdjacenciesBuilder().build();
        }
        ...
        for (FixedIps ip : port.getFixedIps()) {
            ...
            Subnetmap sn = NeutronvpnUtils.getSubnetmap(dataBroker, ip.getSubnetId());
            if (sn == null) {
                LOG.error("createPortIpAdjacencies: subnetmap for {} not found", ip.getSubnetId());
                continue;
            }
            Uuid routerId = sn.getRouterId();
            Uuid vpnId = sn.getVpnId();
            if (vpnId != null) {
            ...
            if (sn.getInternetVpnId() != null) {
                NeutronvpnUtils.createVpnPortFixedIpToPort(dataBroker, sn.getInternetVpnId().getValue(),
    ...
    protected void createVpnInterface(List<Uuid> vpnIds, Port port, WriteTransaction wrtConfigTxn) {
        ...
        Adjacencies adjs = createPortIpAdjacencies(port, isRouterInterface, wrtConfigTxn);
        String infName = port.getUuid().getValue();
    ...
    protected void withdrawPortIpFromVpnIface(Uuid vpnId, Uuid internetVpnId,
        ...
        LOG.info("withdrawPortIpFromVpnIface vpn {} internetVpn {} Port {}",
        ...
        for (FixedIps ip : portIps) {
            if (FibHelper.isBelongingPrefix(ip.getIpAddress().toString(), sn.getSubnetIp())) {
                ...
                if (internetVpnId != null) {
                    NeutronvpnUtils.removeVpnPortFixedIpToPort(dataBroker,
                            internetVpnId.getValue(),
                            ipToRemove.getIpAddress().toString(), wrtConfigTxn);
                }
            } else {
        ...
        // adjacencies from config DS are common for both vpnId and internetVpnId
        ...
        if (port.getDeviceOwner().equals(NeutronConstants.DEVICE_OWNER_ROUTER_INF)) {
            ...
            if (routerId != null) {
                ...
                if (rtr == null && rtr.getRoutes() != null) {
                    ...
                    // adjacency context between vpnId and internetvpnId are shared
                    if (vpnId != null) {
                        removeAdjacencyforExtraRoute(vpnId, extraRoutesToRemove);
                    } else if (internetVpnId != null) {
                        removeAdjacencyforExtraRoute(internetVpnId, extraRoutesToRemove);
                    }
                }
            }
        }
        if (!isIpFromAnotherSubnet) {
            ...
            boolean ret = true;
            // all vpn instance are either vpnId or internetVpnId
            ret = deleteVpnInterface(infName, null, wrtConfigTxn);
            if (sn.getRouterId() != null && ret) {
    ...
    // TODO Clean up the exception handling
    @SuppressWarnings("checkstyle:IllegalCatch")
    protected boolean deleteVpnInterface(String infName, String vpnId, WriteTransaction wrtConfigTxn) {
        ...
        try {
            ...
            LOG.error("Deletion of vpninterface {} already done", infName, ex);
            return true;
        }
        if (optionalVpnInterface != null && vpnId != null && optionalVpnInterface.isPresent()) {
            VpnInterface vpnInterface = optionalVpnInterface.get();
            if (vpnInterface.getVpnInstanceNames() != null &&  vpnInterface.getVpnInstanceNames().contains(vpnId)) {
                List<String> vpnList = vpnInterface.getVpnInstanceNames();
                vpnList.remove(vpnId);
                VpnInterfaceBuilder vpnIfBuilder = new VpnInterfaceBuilder(optionalVpnInterface.get())
                        .setVpnInstanceNames(vpnList);
                wrtConfigTxn.merge(LogicalDatastoreType.CONFIGURATION, vpnIfIdentifier, vpnIfBuilder
                        .build());
                if (!vpnList.isEmpty()) {
            ...
            LOG.debug("Deletion of vpn interface {} already done", infName);
            return true;
        }
        try {
    ...
    protected void removeVpnFromVpnInterface(Uuid vpnId, Port port, WriteTransaction writeConfigTxn) {
        ...
        try {
            ...
            if (optionalVpnInterface.isPresent()) {
                ...
                if (listVpn != null && vpnId != null
                    && listVpn.contains(vpnId.getValue())) {
                    listVpn.remove(vpnId.getValue());
                }
                VpnInterfaceBuilder vpnIfBuilder = new VpnInterfaceBuilder(optionalVpnInterface.get())
                ...
                while (adjacencyIter.hasNext()) {
                    ...
                    if (optionalVpnVipToPort.isPresent()) {
                        ...
                        NeutronvpnUtils.removeLearntVpnVipToPort(dataBroker, vpnId.getValue(), mipToQuery);
    ...
    protected void updateVpnInterface(Uuid vpnId, Uuid oldVpnId, Port port, boolean isBeingAssociated,
        ...
        try {
            ...
            if (optionalVpnInterface.isPresent()) {
                List<String> listVpn = optionalVpnInterface.get().getVpnInstanceNames();
                if (listVpn != null && oldVpnId != null
                    && listVpn.contains(oldVpnId.getValue())) {
                    listVpn.remove(oldVpnId.getValue());
                }
                if (listVpn != null && !listVpn.contains(vpnId.getValue())) {
                    listVpn.add(vpnId.getValue());
                }
                if (listVpn == null || listVpn.isEmpty()) {
                    listVpn = new ArrayList<String>();
                    listVpn.add(vpnId.getValue());
                }
                VpnInterfaceBuilder vpnIfBuilder = new VpnInterfaceBuilder(optionalVpnInterface.get())
                         .setVpnInstanceNames(listVpn);
     ...
     * @param routerIdsList list of neutron router Id to associate with created VPN
    ...
    public void createVpn(Uuid vpn, String name, Uuid tenant, List<String> rd, List<String> irt, List<String> ert,
                            List<Uuid> routerIdsList, List<Uuid> networks, VpnInstance.Type type, long l3vni)
                                    throws Exception {
        ...
        if (routerIdsList != null) {
            for (Uuid routerId : routerIdsList) {
                IpVersionChoice vers = NeutronvpnUtils.getIpVersionChoicesFromRouterUuid(dataBroker, routerId);
        ...
        if (routerIdsList != null) {
            for (Uuid routerId : routerIdsList) {
                associateRouterToVpn(vpn, routerId);
            }
        }
        if (networks != null) {
    ...
    public Future<RpcResult<CreateL3VPNOutput>> createL3VPN(CreateL3VPNInput input) {
        ...
        for (L3vpn vpn : vpns) {
            ...
            if (vpn.getRouterIds() != null && !vpn.getRouterIds().isEmpty()) {
                List<Uuid> routerIdsList = vpn.getRouterIds();
                for (Uuid routerId : routerIdsList) {
                    if (NeutronvpnUtils.getNeutronRouter(dataBroker, routerId) == null) {
                        msg = String.format("Creation of L3VPN failed for VPN %s due to router not found %s",
                                vpn.getId().getValue(), routerId.getValue());
                        LOG.warn(msg);
                        error = RpcResultBuilder.newWarning(ErrorType.PROTOCOL, "invalid-input", msg);
                        errorList.add(error);
                        warningcount++;
                    ...
                    if (vpnId != null) {
                        msg = String.format("Creation of L3VPN failed for VPN %s due to router %s already "
                                + "associated to " + "another VPN %s", vpn.getId().getValue(), routerId.getValue(),
                                vpnId.getValue());
                        LOG.warn(msg);
                        error = RpcResultBuilder.newWarning(ErrorType.PROTOCOL, "invalid-input", msg);
                        errorList.add(error);
            ...
            try {
                createVpn(vpn.getId(), vpn.getName(), vpn.getTenantId(), vpn.getRouteDistinguisher(),
                        vpn.getImportRT(), vpn.getExportRT(), vpn.getRouterIds(), vpn.getNetworkIds(),
    ...
    public Future<RpcResult<GetL3VPNOutput>> getL3VPN(GetL3VPNInput input) {
        ...
        try {
            ...
            for (VpnInstance vpnInstance : vpns) {
                ...
                if (vpnInstance.getVpnConfig().getVpnTargets() != null) {
                ...
                if (optionalVpnMap.isPresent()) {
                    VpnMap vpnMap = optionalVpnMap.get();
                    l3vpn.setRouterIds(vpnMap.getRouterIds()).setNetworkIds(vpnMap.getNetworkIds())
    ...
    protected void addSubnetToVpn(final Uuid vpnId, Uuid subnet, final Uuid internetVpnId) {
        LOG.debug("addSubnetToVpn: Adding subnet {} to vpn {}", subnet.getValue(),
                  vpnId != null ? vpnId.getValue() : internetVpnId.getValue());
        Uuid networkUuid = NeutronvpnUtils.getSubnetmap(dataBroker, subnet).getNetworkId();
        Network network = NeutronvpnUtils.getNeutronNetwork(dataBroker, networkUuid);
        boolean netIsExternal = NeutronvpnUtils.getIsExternal(network);
        ...
        if (internetVpnId != null) {
            VpnMap vpnMap = NeutronvpnUtils.getVpnMap(dataBroker, internetVpnId);
            if (vpnMap == null) {
            ...
            for (final Uuid portId : portList) {
                ...
                portDataStoreCoordinator.enqueueJob("PORT-" + portId.getValue(), () -> {
                    ...
                    if (vpnIface == null) {
                        ...
                        List<Uuid> listVpn = new ArrayList<Uuid>();
                        if (vpnId != null) {
                            listVpn.add(vpnId);
                        }
                        if (internetId != null) {
    ...
    protected void removeSubnetFromVpn(final Uuid vpnId, Uuid subnet, Uuid internetVpnId) {
        ...
        if (vpnId != null) {
            vpnMap = NeutronvpnUtils.getVpnMap(dataBroker, vpnId);
            if (vpnMap == null) {
        ...
        if (internetVpnId != null) {
            vpnMap = NeutronvpnUtils.getVpnMap(dataBroker, internetVpnId);
            if (vpnMap == null) {
        ...
        if (vpnId != null && sn.getVpnId() != null
            && sn.getVpnId().getValue().matches(vpnId.getValue())) {
            ...
            if (portList != null) {
                for (final Uuid portId : portList) {
                    ...
                    portDataStoreCoordinator.enqueueJob("PORT-" + portId.getValue(), () -> {
                        ...
                        if (port != null) {
                            if (vpnId != null || internetId != null) {
                                withdrawPortIpFromVpnIface(vpnId, internetId,
                                          port, sn, wrtConfigTxn);
                            }
                        } else {
                    ...
                    removeFromSubnetNode(subnet, sn.getNetworkId(), sn.getRouterId(), vpnId, portId, internetId);
                }
            }
        } else {
    ...
    public void updateVpnInternetForSubnet(Subnetmap sm, Uuid vpn, boolean isBeingAssociated) {
        LOG.debug("updateVpnInternetForSubnet : {} subnet {} with BGPVPN Internet {} ",
             isBeingAssociated ? "associating" : "dissociating", sm.getSubnetIp().toString(),
             vpn.getValue());
        if (isBeingAssociated) {
        ...
        try {
            WriteTransaction wrtConfigTxn = dataBroker.newWriteOnlyTransaction();
            if (isBeingAssociated) {
                updateVpnInterface(vpn, null,
                            NeutronvpnUtils.getNeutronPort(dataBroker, sm.getRouterInterfacePortId()),
                            true, true, wrtConfigTxn);
            } else {
                removeVpnFromVpnInterface(vpn,
                            NeutronvpnUtils.getNeutronPort(dataBroker,
                                       sm.getRouterInterfacePortId()),
                                       wrtConfigTxn);
            }
            wrtConfigTxn.submit().checkedGet();
        } catch (TransactionCommitFailedException e) {
            LOG.error("Failed to update router interface {} in subnet {} "
                     + "while handling vpn {}, returning",
                     sm.getRouterInterfacePortId().getValue(),
                     sm.getSubnetIp().toString(), vpn);
        }
        ...
        if (portList != null) {
            for (Uuid port : portList) {
                ...
                portDataStoreCoordinator.enqueueJob("PORT-" + port.getValue(), () -> {
                    ...
                    List<ListenableFuture<Void>> futures = new ArrayList<>();
                    if (isBeingAssociated) {
                        updateVpnInterface(vpn, null, NeutronvpnUtils.getNeutronPort(dataBroker, port),
                                 isBeingAssociated, false, wrtConfigTxn);
                    } else {
                        removeVpnFromVpnInterface(vpn,
                               NeutronvpnUtils.getNeutronPort(dataBroker, port), wrtConfigTxn);
                    }
                    futures.add(wrtConfigTxn.submit());
                    return futures;
                });
    ...
    private void updateVpnForSubnet(Uuid oldVpnId, Uuid newVpnId, Uuid subnet, boolean isBeingAssociated) {
        ...
        Uuid vpnExtUuid = netIsExternal ? null
                : NeutronvpnUtils.getInternetvpnUuidBoundToSubnetRouter(dataBroker, subnet);
    ...
    public void removeVpn(Uuid id) {
        ...
        List<Uuid> routerIdsList = vpnMap != null ? vpnMap.getRouterIds() : null;
        // dissociate router
        if (routerIdsList != null && !routerIdsList.isEmpty()) {
            for (Uuid routerId : routerIdsList) {
                dissociateRouterFromVpn(id, routerId);
            }
            if (routerIdsList.contains(id)) {
                // dissociate networks
        ...
        if (routerSubnets != null) {
            for (Uuid subnetId : routerSubnets) {
                ...
                NeutronvpnUtils.updateVpnInstanceWithIpFamily(dataBroker, vpnId.getValue(),
                              NeutronvpnUtils.getIpVersionFromSubnet(dataBroker, subnetId), true);
            }
        }
        try {
    ...
    protected void associateRouterToInternalVpn(Uuid vpnId, Uuid routerId) {
        List<Uuid> routerSubnets = NeutronvpnUtils.getNeutronRouterSubnetIds(dataBroker, routerId);
        Uuid internetVpnId = NeutronvpnUtils.getInternetvpnUuidBoundToRouterId(dataBroker, routerId);
        LOG.debug("Adding subnets to internal vpn {}", vpnId.getValue());
        for (Uuid subnet : routerSubnets) {
            IpVersionChoice version = NeutronvpnUtils.getIpVersionFromSubnet(dataBroker, subnet);
    ...
    protected void dissociateRouterFromVpn(Uuid vpnId, Uuid routerId) {
        ...
        if (routerSubnets != null) {
            for (Uuid subnetId : routerSubnets) {
                if (NeutronvpnUtils.isSubnetmapIpVersionChangesVpnInstance(dataBroker, subnetId, vpnId)) {
                    vpnInstanceIpVersionsToRemove.addVersion(NeutronvpnUtils
                                  .getIpVersionFromSubnet(dataBroker, subnetId));
                ...
                try {
                    ...
                    if (network == null) {
                        ...
                        if (NeutronvpnUtils.getIsExternal(network)) {
                            VpnMap vpnMap = NeutronvpnUtils.getVpnMap(dataBroker, vpn);
                            if (vpnMap == null) {
                ...
                if (network == null) {
                    ...
                    if (vpn.equals(vpnId)) {
                        ...
                        if (networkSubnets != null) {
                            ...
                            for (Uuid subnet : networkSubnets) {
                                if (NeutronvpnUtils.isSubnetmapIpVersionChangesVpnInstance(dataBroker, subnet, vpn)) {
                                    vpnInstanceIpVersionsToRemove.addVersion(NeutronvpnUtils
                                               .getIpVersionFromSubnet(dataBroker, subnet));
                                    vpnInstanceIpVersionsRemoved = true;
                                }
                                Subnetmap sn = NeutronvpnUtils.getSubnetmap(dataBroker, subnet);
                                if (sn != null && sn.getInternetVpnId() != null
                                    && NeutronvpnUtils.isSubnetmapIpVersionChangesVpnInstance(dataBroker,
                                                            subnet, sn.getInternetVpnId())) {
                            ...
                            if (vpnInstanceInternetIpVersionRemoved) {
                                NeutronvpnUtils.updateVpnInstanceWithIpFamily(dataBroker, vpnIdInternet.getValue(),
                        ...
                        if (NeutronvpnUtils.getIsExternal(network)) {
                            ...
                            if (smList != null && !smList.isEmpty()) {
                                for (Subnetmap sm : smList) {
                                    if (NeutronvpnUtils.isSubnetmapIpVersionChangesVpnInstance(dataBroker,
                                                   sm.getId(), vpn)) {
                                        vpnInstanceInternetIpVersionRemoved = true;
                                    }
    ...
    public Future<RpcResult<Void>> associateRouter(AssociateRouterInput input) {
        ...
        StringBuilder returnMsg = new StringBuilder();
        Uuid vpnId = input.getVpnId();
        List<Uuid> routerIds = input.getRouterIds();
        String routerIdsString = "";
        try {
            if (routerIds == null || routerIds.isEmpty() || vpnId == null) {
                throw new Exception("Error in try to associate any router with a vpnId : " + vpnId);
            }
            for (Uuid routerId : routerIds) {
                routerIdsString += routerId.getValue() + ", ";
                VpnMap vpnMap = NeutronvpnUtils.getVpnMap(dataBroker, vpnId);
                Router rtr = NeutronvpnUtils.getNeutronRouter(dataBroker, routerId);
                if (vpnMap != null) {
                    if (rtr != null) {
                        ...
                        if (vpnMap.getRouterIds() != null && vpnMap.getRouterIds().size() > 1) {
                            returnMsg.append("vpn ").append(vpnId.getValue())
                                    .append(" already associated to two routers ")
                                    .append(routerId.getValue());
            ...
            String message = String.format("associate routers %s to vpn %s failed due to %s", routerIdsString,
                    vpnId, ex.getMessage());
            LOG.error(message, ex);
    ...
    public Future<RpcResult<Void>> dissociateRouter(DissociateRouterInput input) {
        ...
        StringBuilder returnMsg = new StringBuilder();
        Uuid vpnId = input.getVpnId();
        List<Uuid> routerIdList = input.getRouterIds();
        String routerIdsString = "";
        try {
            if (routerIdList == null || routerIdList.isEmpty() || vpnId == null) {
                throw new Exception("Error in try to dissociate any router with a vpnId : " + vpnId);
            }
            if (NeutronvpnUtils.getVpnMap(dataBroker, vpnId) != null) {
                for (Uuid routerId : routerIdList) {
                    if (routerId != null) {
                        routerIdsString += routerId.getValue() + ", ";
                        Router rtr = NeutronvpnUtils.getNeutronRouter(dataBroker, routerId);
                        if (rtr != null) {
            ...
            if (returnMsg.length() != 0) {
                String message = String.format("dissociate router %s to vpn %s failed due to %s", routerIdsString,
                        vpnId.getValue(), returnMsg);
                LOG.error(message);
                String errorResponse = String.format("ErrorType: PROTOCOL, ErrorTag: invalid-value, ErrorMessage: "
                        + message);
            ...
            String message = String.format("disssociate router %s to vpn %s failed due to %s", routerIdsString,
                    vpnId, ex.getMessage());
            LOG.error(message, ex);
            result.set(RpcResultBuilder.<Void>failed().withError(ErrorType.APPLICATION, message).build());
        }
    ...
    protected void handleNeutronRouterDeleted(Uuid routerId, List<Uuid> routerSubnetIds) {
        ...
        Uuid vpnId = NeutronvpnUtils.getVpnForRouter(dataBroker, routerId, true);
        Uuid internetVpnId = NeutronvpnUtils.getInternetvpnUuidBoundToRouterId(
                          dataBroker, routerId);
        if (vpnId != null) {
    ...
    private void createExternalVpnInterface(Uuid vpnId, String infName, WriteTransaction wrtConfigTxn) {
        List listVpn = new ArrayList<Uuid>();
        listVpn.add(vpnId);
        writeVpnInterfaceToDs(listVpn, infName, null,
                false /* not a router iface */, wrtConfigTxn);
    }

    // TODO Clean up the exception handling
    @SuppressWarnings("checkstyle:IllegalCatch")
    private void writeVpnInterfaceToDs(List<Uuid> vpnIdList, String infName, Adjacencies adjacencies,
            Boolean isRouterInterface, WriteTransaction wrtConfigTxn) {
        List<String> vpnIdListString = new ArrayList();
        if ((vpnIdList == null || vpnIdList.isEmpty()) || infName == null) {
            LOG.error("vpn id or interface is null");
            return;
        }
        for (Uuid vpnId: vpnIdList) {
            vpnIdListString.add(vpnId.getValue());
        }
        Boolean wrtConfigTxnPresent = true;
        if (wrtConfigTxn == null) {
            wrtConfigTxnPresent = false;
        ...
        VpnInterfaceBuilder vpnb = new VpnInterfaceBuilder().setKey(new VpnInterfaceKey(infName))
                .setName(infName)
                .setVpnInstanceNames(vpnIdListString)
        ...
        try {
            ...
            if (optionalVpnInterface.isPresent()) {
                ...
                if (optionalVpnInterface.get().getVpnInstanceNames() != null) {
                    if (optionalVpnInterface.get().getVpnInstanceNames().isEmpty()
                        || !optionalVpnInterface.get().getVpnInstanceNames().contains(vpnId.getValue())) {
                        List<String> listVpnInstances = optionalVpnInterface.get().getVpnInstanceNames();
                        listVpnInstances.add(vpnId.getValue());
                        vpnIfBuilder.setVpnInstanceNames(listVpnInstances);
                    }
                } else {
                    List<String> listVpnInstances = new ArrayList<String>();
                    listVpnInstances.add(vpnId.getValue());
                    vpnIfBuilder.setVpnInstanceNames(listVpnInstances);
                }
