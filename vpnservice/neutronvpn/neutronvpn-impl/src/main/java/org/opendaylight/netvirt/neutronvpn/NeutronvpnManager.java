...
public class NeutronvpnManager implements NeutronvpnService, AutoCloseable, EventListener {
    ...
    protected void createSubnetmapNode(Uuid subnetId, String subnetIp, Uuid tenantId, Uuid networkId,
        ...
        try {
            Uuid externalVpnUuid = getExternalvpnUuidInAnotherSubnetmap(subnetId);
            ...
            synchronized (subnetId.getValue().intern()) {
                ...
                SubnetmapBuilder subnetmapBuilder = new SubnetmapBuilder().setKey(new SubnetmapKey(subnetId))
                        ...
                        .setNetworkType(networkType).setSegmentationId(segmentationId).setExternalVpnId(externalVpnUuid);
    ...
    protected void addSubnetToExternalVpn(final Uuid vpnId, Uuid subnet) {// Noel remarque : Methode cree par Philippe elle est a modifier et pour l'instant elle n'est jamais appellée;
    ...
    protected void addSubnetToVpn(final Uuid vpnId, Uuid subnet) {
        ...
        Uuid vpnExtUuid = getExternalvpnUuidInAnotherSubnetmap(subnet);
    ...
    protected void removeSubnetFromExternalVpn(final Uuid vpnId, Uuid subnet) {//Noel remarque : Methode cree par Philippe elle est a modifier et pour l'instant elle n'est jamais appellée;
        ...
        if (sn != null) {
            ...
            removeFromSubnetNode(subnet, null, null, null, vpnId);
        } else {
    ...
    /** This method get Uuid of external vpn if existing one attached at the same router of the subnetUuid arg. 
     * <br><br><u>Explanation:</u><br>If the subnet (of arg subnetUuid) have a router attached and this router have an
     ...
     */
    public Uuid getExternalvpnUuidInAnotherSubnetmap(Uuid subnetUuid) {
        Subnetmap subnetmap = NeutronvpnUtils.getSubnetmap(dataBroker, subnetUuid);
        Uuid routerUuid = subnetmap.getRouterId();
        List<Uuid> subnetUuids = NeutronvpnUtils.getNeutronRouterSubnetIds(dataBroker, routerUuid);
        List<Network> externalNetwork = Collections.emptyList();
        for (Uuid subUuid : subnetUuids) {
            Uuid netUuid = NeutronvpnUtils.getNetworkForSubnet(dataBroker, subUuid);
            Network net = NeutronvpnUtils.getNeutronNetwork(dataBroker, netUuid);
            NetworkL3Extension augmnt = net.getAugmentation(NetworkL3Extension.class);
            if ( augmnt !=null && augmnt.isExternal()) {
                externalNetwork.add(net);
            }
        }
        if (externalNetwork.size()>0) {
            for (Network n : externalNetwork) {
                Uuid vpnExtUuid = NeutronvpnUtils.getVpnForNetwork(dataBroker, n.getUuid());
                if (vpnExtUuid != null) {
                    return vpnExtUuid;
                }
            }
