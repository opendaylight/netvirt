...
public class NeutronvpnManager implements NeutronvpnService, AutoCloseable, EventListener {
    ...
    // TODO Clean up the exception handling
    @SuppressWarnings("checkstyle:IllegalCatch")
    protected Subnetmap removeFromSubnetNode(Uuid subnetId, Uuid networkId, Uuid routerId, Uuid vpnId, Uuid vpnExternalId, Uuid portId) {
        Subnetmap subnetmap = null;
        InstanceIdentifier<Subnetmap> id = InstanceIdentifier.builder(Subnetmaps.class)
                .child(Subnetmap.class, new SubnetmapKey(subnetId))
                .build();
    ...
    /** Removethe vpn external Uuid to the subnetmap
     * @param vpnExternalUuid Uuid for the external vpn
     * @param subnet Uuid of subnet
     */
    protected void removeSubnetFromExternalVpn(final Uuid vpnExternalUuid, Uuid subnet) {//Noel remarque : Methode cree par Philippe elle est a modifier et pour l'instant elle n'est jamais appellée;
        LOG.debug("Removing subnet {} from external vpn {}", subnet.getValue(), vpnExternalUuid.getValue());
        VpnMap vpnMap = NeutronvpnUtils.getVpnMap(dataBroker, vpnExternalUuid);
        if (vpnMap == null) {
            LOG.error("No vpnMap for vpnId {}, cannot remove subnet {} from VPN",
                    vpnExternalUuid.getValue(), subnet.getValue());
            return;
        }
        ...
        if (sn != null) {
            ...
            removeFromSubnetNode(subnet, null, null, null, vpnExternalUuid, null);
        } else {
            ...
            for (Uuid nw : networks) {
                ...
                if (network == null) {
                    ...
                    if (NeutronvpnUtils.getIsExternal(network)) {
                        ...
                        //addSubnetToExternalVpn(vpn, null, subnet);
