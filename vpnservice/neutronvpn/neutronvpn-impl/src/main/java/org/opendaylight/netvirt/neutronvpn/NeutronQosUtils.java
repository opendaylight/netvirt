...
public class NeutronQosUtils {
    ...
    public static void handleNeutronPortQosAdd(DataBroker db, OdlInterfaceRpcService odlInterfaceRpcService,
                                               IMdsalApiManager mdsalApiManager, Port port, Uuid qosUuid) {
        ...
        if (qosPolicy != null && qosPolicy.getDscpmarkingRules() != null
            ...
            setPortDscpMarking(db, odlInterfaceRpcService, mdsalApiManager,
    ...
    public static void handleNeutronPortQosUpdate(DataBroker db, OdlInterfaceRpcService odlInterfaceRpcService,
                                                  IMdsalApiManager mdsalApiManager, Port port, Uuid qosUuidNew,
                                                  Uuid qosUuidOld) {
        ...
        if (qosPolicyNew != null && qosPolicyNew.getDscpmarkingRules() != null
            ...
            if (qosPolicyOld != null && qosPolicyOld.getDscpmarkingRules() != null
                ...
                unsetPortDscpMark(db, odlInterfaceRpcService, mdsalApiManager, port);
            }
        }
    }
    ...
    public static void handleNeutronPortQosRemove(DataBroker db, OdlInterfaceRpcService odlInterfaceRpcService,
        ...
        if (network != null && network.getAugmentation(QosNetworkExtension.class) != null) {
            ...
            if (networkQosUuid != null) {
                handleNeutronPortQosUpdate(db, odlInterfaceRpcService, mdsalApiManager, port, networkQosUuid, qosUuid);
            }
        } else {
    ...
    public static void handleNeutronNetworkQosUpdate(DataBroker db, OdlInterfaceRpcService odlInterfaceRpcService,
                                                     IMdsalApiManager mdsalApiManager, Network network, Uuid qosUuid) {
    ...
    public static void handleNeutronNetworkQosBwRuleRemove(DataBroker dataBroker, OdlInterfaceRpcService odlInterfaceRpcService,
                                                           Network network,
    ...
    public static void handleNeutronNetworkQoskDscpRuleRemove(DataBroker dataBroker, OdlInterfaceRpcService odlInterfaceRpcService,
        ...
        for (Uuid subnetId: subnetIds) {
            ...
            for (Uuid portId : portIds) {
                ...
                if (port != null && (port.getAugmentation(QosPortExtension.class) == null ||
                        port.getAugmentation(QosPortExtension.class).getQosPolicyId() == null)) {
                    unsetPortDscpMark(dataBroker, odlInterfaceRpcService, mdsalApiManager, port);
                }
            }
        }
    }
    ...
    public static void setPortDscpMarking(DataBroker db, OdlInterfaceRpcService odlInterfaceRpcService,
        ...
        //1. OF rules
        syncFlow(db, dpnId, NwConstants.ADD_FLOW, mdsalApiManager, dscpValue, ifName, ipAddress);
    ...
    public static void unsetPortDscpMark(DataBroker dataBroker, OdlInterfaceRpcService odlInterfaceRpcService,
        ...
        // 1. OF
        syncFlow(dataBroker, dpnId, NwConstants.DEL_FLOW, mdsalApiManager, (short) 0, ifName, ipAddress);
        // 2. OVSDB
        OvsdbBridgeRef bridgeRefEntry = getBridgeRefEntryFromOperDS(dpnId, dataBroker);
        Optional<Node> bridgeNode = MDSALUtil.read(LogicalDatastoreType.OPERATIONAL,
                bridgeRefEntry.getValue().firstIdentifierOf(Node.class), dataBroker);

        TerminationPoint tp = getTerminationPoint(bridgeNode.get(), port.getUuid().getValue());

        MDSALUtil.syncDelete(dataBroker, LogicalDatastoreType.CONFIGURATION, InstanceIdentifier
                .create(NetworkTopology.class)
                .child(Topology.class, new TopologyKey(SouthboundUtils.OVSDB_TOPOLOGY_ID))
                .child(Node.class, bridgeNode.get().getKey())
    ...
    private static void syncFlow(DataBroker db, BigInteger dpnId, int addOrRemove,
                          IMdsalApiManager mdsalApiManager, Short dscpValue,
        ...
        if (addOrRemove == NwConstants.ADD_FLOW) {
            actionsInfos.add(new ActionInfo(ActionType.set_field_dscp, new String[]{String.valueOf(dscpValue)}));
            ...
            FlowEntity flowEntity = MDSALUtil.buildFlowEntity(dpnId, NwConstants.QOS_DSCP_TABLE,
                    ...
                    10, "QoSConfigFlow", 0, 0, NwConstants.COOKIE_QOS_TABLE, matches, instructions);
            mdsalApiManager.installFlow(flowEntity);
            LOG.trace("Installing QoS DpnId"); //to be removed

        } else {
            FlowEntity flowEntity = MDSALUtil.buildFlowEntity(dpnId, NwConstants.QOS_DSCP_TABLE,
                    getQosFlowId(NwConstants.QOS_DSCP_TABLE, dpnId, ifIndex),
                    10, "QoSRemoveFlow", 0, 0, NwConstants.COOKIE_QOS_TABLE, matches, null);
            LOG.trace("Removing QoS Flow DpnId"); // to be removed
            mdsalApiManager.removeFlow(flowEntity);
        }
    }
