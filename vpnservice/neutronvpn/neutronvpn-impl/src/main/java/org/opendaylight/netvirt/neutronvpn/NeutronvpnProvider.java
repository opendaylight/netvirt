...
public class NeutronvpnProvider implements INeutronVpnManager, AutoCloseable {
    ...
    private static final Logger LOG = LoggerFactory
            .getLogger(NeutronvpnProvider.class);
    ...
    public NeutronvpnProvider(final DataBroker dataBroker,
        ...
        this.lockManager = rpcProviderRegistry
                .getRpcService(LockManagerService.class);
    }
    ...
    public void start() {
        try {
            ...
            rpcRegistration = rpcProviderRegistry
                    .addRpcImplementation(NeutronvpnService.class, nvManager);
            ...
            portListener = new NeutronPortChangeListener(dbx, nvManager,
                    nvNatManager, notificationPublishService,
                    notificationService, floatingIpMapListener);
            portListener.setLockManager(lockManager);
            portListener.setLockManager(lockManager);
            floatingIpMapListener = new NeutronFloatingToFixedIpMappingChangeListener(
                    dbx);
            nvManager.setLockManager(lockManager);
            portListener.setLockManager(lockManager);
            floatingIpMapListener.setLockManager(lockManager);
            l2GatewayProvider = new L2GatewayProvider(dbx, rpcProviderRegistry,
    ...
    public void close() throws Exception {
        ...
        if (securityRuleListener != null) {
            securityRuleListener.close();
        }
        if (floatingIpMapListener != null) {
