...
public class NeutronPortChangeListener extends AsyncDataTreeChangeListenerBase<Port, NeutronPortChangeListener> {
    ...
    private static final String VNIC_TYPE_DIRECT = "direct";
    private static final String BINDING_PROFILE_CAPABILITIES = "capabilities";
    private static final String SWITCHDEV = "switchdev";
    ...
    private Hostconfig getHostConfig(final Port port, final DataBroker dataBroker) {
        String hostId = getPortHostId(port);
        if (hostId == null) {
            return null;
        }
        Hostconfig hostConfig = this.hostConfigCache.get(hostId);
        if (hostConfig != null) {
            return hostConfig;
        }
    ...
    private InstanceIdentifier<Hostconfig> hostConfigIid(String hostId) {
    ...
    private boolean isSupportedVnicTypeByHost(final Port port,
    ...
    private boolean isPortTypeSwitchdev(final Port port) {
        ...
        NeutronDictJsonAdapter dictJson = new NeutronDictJsonAdapter();
    ...
    private void handleNeutronPortCreated(final Port port) {
        ...
        jobCoordinator.enqueueJob("PORT- " + portName, () -> {
            ...
            if (!(NeutronUtils.isPortVnicTypeNormal(port)
                ...
                && isSupportedVnicTypeByHost(port, VNIC_TYPE_DIRECT, dataBroker)))) {
