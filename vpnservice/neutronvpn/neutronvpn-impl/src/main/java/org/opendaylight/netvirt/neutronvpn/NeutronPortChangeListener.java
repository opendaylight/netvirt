...
public class NeutronPortChangeListener extends AsyncDataTreeChangeListenerBase<Port, NeutronPortChangeListener>
    ...
    private Interface createInterface(Port port) {
        ...
        if (tp != null) {
            ...
            parentRefName = ovsdbTp.getName();
            LOG.info("Creating new interface for neutron port {}, using parentRefName {} acquired by external ID",
                    interfaceName, parentRefName);
        } else {
            parentRefName = NeutronvpnUtils.getVifPortName(port);
            LOG.info("Creating new interface for neutron port {}, using parentRefName {} deducted by VIF type",
                    interfaceName, parentRefName);
        }
