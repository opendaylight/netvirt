        ...
        portDataStoreCoordinator.enqueueJob("PORT- " + portupdate.getUuid().getValue(), () -> {
            ...
            for (FixedIps ip: ipsList) {
                ...
                if (vpnIdNew != null) {
                    ...
                    nvpnManager.deleteVpnInterface(vpnIdNew, Arrays.asList(subnetMapNew), portupdate, wrtConfigTxn);
                    // create vpn-interface on this neutron port
                    LOG.debug("Adding VPN Interface for port {}", portupdate.getUuid().getValue());
                    if (subnetMapNew != null) {
                        nvpnManager.createVpnInterface(vpnIdNew, Arrays.asList(subnetMapNew), portupdate, wrtConfigTxn);
                    }
                }
                futures.add(wrtConfigTxn.submit());
            }
