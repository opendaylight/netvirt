    ...
    private void handleRouterInterfaceAdded(Port routerPort) {
        if (routerPort.getDeviceId() != null) {
            ...
            if (existingVpnId == null) {
                ...
                //update RouterInterfaces map with new port
                ...
                for (FixedIps portIP : portIps) {
                    // NOTE:  Please donot change the order of calls updateSubnetNodeWithFixedIP
                    // and ddSubnetToVpn here
                    String ipValue = String.valueOf(portIP.getIpAddress().getValue());
                    Uuid subnetId = portIP.getSubnetId();
                    nvpnManager.updateSubnetNodeWithFixedIp(subnetId, routerId, routerPort.getUuid(), ipValue,
                        routerPort.getMacAddress().getValue());
