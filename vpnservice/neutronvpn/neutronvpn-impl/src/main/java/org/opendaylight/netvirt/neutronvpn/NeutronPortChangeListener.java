            ...
            () -> Collections.singletonList(txRunner.callWithNewWriteOnlyTransactionAndSubmit(tx -> {
                ...
                for (Uuid snId: originalSnMapsIds) {
                    if (!updateSnMapsIds.remove(snId)) {
                        ...
                        Subnetmap subnetMapOld = nvpnManager.removePortsFromSubnetmapNode(snId,
                                                            portoriginal.getUuid(), null);
                ...
                if (oldVpnIds.size() > 0) {
                    ...
                    nvpnManager.deleteVpnInterface(portoriginal.getUuid().getValue(), tx);
                }
                if (newVpnIds.size() > 0) {
                    LOG.info("Adding VPN Interface for port {}", portupdate.getUuid().getValue());
                    nvpnManager.createVpnInterface(newVpnIds, portupdate, tx);
