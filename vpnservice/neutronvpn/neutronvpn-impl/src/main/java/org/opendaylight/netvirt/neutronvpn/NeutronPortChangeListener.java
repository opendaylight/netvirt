...
public class NeutronPortChangeListener extends AsyncDataTreeChangeListenerBase<Port, NeutronPortChangeListener>
    ...
    private void handleNeutronPortUpdated(final Port portoriginal, final Port portupdate) {
        List<FixedIps> portoriginalIps = portoriginal.getFixedIps();
        List<FixedIps> portupdateIps = portupdate.getFixedIps();
        LOG.trace("Port original fixed IPs: {}", portoriginalIps.toString());
        LOG.trace("Port update fixed IPs: {}", portupdateIps.toString());
        ...
        portDataStoreCoordinator.enqueueJob("PORT- " + portupdate.getUuid().getValue(), () -> {
            WriteTransaction wrtConfigTxn = dataBroker.newWriteOnlyTransaction();
            List<ListenableFuture<Void>> futures = new ArrayList<>();
            List<Uuid> originalSnMaps = portoriginalIps.stream().map(ip -> ip.getSubnetId())
                    .collect(Collectors.toList());
            List<Uuid> updateSnMaps = portupdateIps.stream().map(ip -> ip.getSubnetId()).collect(Collectors.toList());
            List<Uuid> snIds = Stream.concat(originalSnMaps.stream(), updateSnMaps.stream()).distinct()
                    .collect(Collectors.toList());
            for (Uuid snId: snIds) {
                if (originalSnMaps.contains(snId) && updateSnMaps.contains(snId)) {
                    continue;
                }
                if (originalSnMaps.contains(snId) && !updateSnMaps.contains(snId)) {
            ...
            Uuid oldVpnId = NeutronvpnUtils.getVpnForNetwork(dataBroker, portoriginal.getNetworkId());
            if (oldVpnId != null) {
            ...
            Uuid newVpnId = NeutronvpnUtils.getVpnForNetwork(dataBroker, portupdate.getNetworkId());
            if (newVpnId != null) {
