...
public class NeutronPortChangeListener extends AsyncDataTreeChangeListenerBase<Port, NeutronPortChangeListener> {
    ...
    private void handleRouterInterfaceRemoved(Port routerPort) {
        if (routerPort.getDeviceId() != null) {
            ...
            for (FixedIps portIP : routerPort.getFixedIps()) {
                ...
                nvpnNatManager.handleSubnetsForExternalRouter(routerId);
                String ipValue = String.valueOf(portIP.getIpAddress().getValue());
                neutronvpnUtils.removeVpnPortFixedIpToPort(vpnId.getValue(), ipValue, null /*writeTransaction*/);
            }
        }
    }
    ...
    private void handleNeutronPortUpdated(final Port portoriginal, final Port portupdate) {
        ...
        LOG.trace("PORT ORIGINAL: {} from network {} with fixed IPs: {}; "
                    ...
                    portoriginal.getName(), portoriginal.getNetworkId(), portoriginalIps,
                    portupdate.getName(), portupdate.getNetworkId(), portupdateIps);
        ...
        jobCoordinator.enqueueJob("PORT- " + portupdate.getUuid().getValue(), () -> {
            ...
            final Uuid oldVpnId = neutronvpnUtils.getVpnForNetwork(portoriginal.getNetworkId());
            if (oldVpnId != null) {
            ...
            final Uuid newVpnId = neutronvpnUtils.getVpnForNetwork(portupdate.getNetworkId());
            if (newVpnId != null) {
