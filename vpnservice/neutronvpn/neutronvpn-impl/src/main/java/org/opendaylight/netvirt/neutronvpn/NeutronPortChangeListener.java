...
public class NeutronPortChangeListener extends AsyncDataTreeChangeListenerBase<Port, NeutronPortChangeListener>
    ...
    private Map<String, ConcurrentHashMap<String, HashSet<String>>> vbdToVppNodesToInterfaces = new ConcurrentHashMap<>();
            ...
            if ((oldPortBindingHost == null) || (oldPortBindingHost.isEmpty())) {
                ...
                synchronized(vbdToVppNodesToInterfaces) {
                    if (vbdToVppNodesToInterfaces.get(update.getNetworkId().getValue()) != null) {
                        HashSet<String> vppInterfacesInHost = vbdToVppNodesToInterfaces.get(update.getNetworkId().getValue()).get(newPortBindingHost);
                        if ((vppInterfacesInHost != null) && (vppInterfacesInHost.contains(update.getUuid().getValue()))) {
                            newPort = false;
                        }
                    }
                }
                if (newPort) {
                    LOG.debug("handleNeutronPortUpdate: perform VPP interface creation operations for port: {}", update);
                    handleNeutronPortCreated(update);
                }
            }
            ...
            public List<ListenableFuture<Void>> call() throws Exception {
                ...
                if (isValidVPPVhostUser(port)) {
                    ...
                    synchronized(vbdToVppNodesToInterfaces) {
                        ...
                        //Check if portBindingHost is already part of bridge domain. If not, clone the bridge domain on that host
                        ...
                        if (vppNodesInVbd.get(portBindingHost).contains(port.getUuid().getValue())) {
                            LOG.debug("VPP port {} is already part of a VPP bridge domain on host {}", port, portBindingHost);
                            futures.add(wrtConfigTxn.submit());
                            return futures;
                        }
                    }
    ...
    private boolean createVPPBridgeDomainOnNode(Port port, Node host, DataBroker dataBroker) {
       ...
       VxlanBuilder tunneltypeBuilder = new VxlanBuilder().setVni(new VxlanVniType(Long.parseLong(providerAug.getSegmentationId())));
       CreateVirtualBridgeDomainOnNodesInputBuilder vbdInputBuilder = new CreateVirtualBridgeDomainOnNodesInputBuilder()
    ...
    private boolean createVPPInterfaceOnNode(Port port, Node host) {
       ...
       if (port.getDeviceOwner().contains(COMPUTE_OWNER)) {
           String vhostuserSocket = port.getAugmentation(PortBindingExtension.class).getVifDetails()
                                        .stream()
    ...
    private boolean deleteVPPBridgeDomainFromNode(String bridgeDomainId, Node host) {
       DeleteVirtualBridgeDomainFromNodesInputBuilder vbdInputBuilder = new DeleteVirtualBridgeDomainFromNodesInputBuilder()
                                                                            .setBridgeDomainId(bridgeDomainId)
                                                                            .setBridgeDomainNode(Arrays.asList(host.getNodeId()));
            ...
            public List<ListenableFuture<Void>> call() throws Exception {
                ...
                if (isValidVPPVhostUser(port)) {
                    ...
                    synchronized(vbdToVppNodesToInterfaces) {
                        ...
                        String portBindingHost = port.getAugmentation(PortBindingExtension.class).getHostId();
                        if (portBindingHost == null) {
                            LOG.warn("Cannot delete VPP bridge domain. binding:host_id not specified in neutron port: {}", port);
                            futures.add(wrtConfigTxn.submit());
                            return futures;
                        }
                        Node node = getVPPNode(portBindingHost, dataBroker);
                        if (node == null) {
                            LOG.warn("Cannot delete VPP bridge domain. Unable to find binding:host_id in topology: {}", port);
                            futures.add(wrtConfigTxn.submit());
                            return futures;
                        }
                        HashSet<String> vppInterfacesInHost = vbdToVppNodesToInterfaces.get(networkId).get(portBindingHost);
                        if ((vppInterfacesInHost == null) || (!vppInterfacesInHost.contains(port.getUuid().getValue()))) {
                            LOG.warn("Cannot delete VPP port. Unable to find corresponding interface {} on vpp node: {}", port, portBindingHost);
                            futures.add(wrtConfigTxn.submit());
                            return futures;
                        }
                        ...
                        if (vppInterfacesInHost.isEmpty()) {
                            //No more VPP interfaces on the node for this bridge domain. Delete the bridge domain from nodes
