        ...
        portDataStoreCoordinator.enqueueJob("PORT- " + portName, () -> {
            ...
            for (FixedIps ip: portIpAddrsList) {
                ...
                if (subnetMap != null) {
                    ...
                    LOG.info("VPN ID {}", vpnId.toString());
                    if (vpnId == null) {
                        continue;
                    }
                    LOG.info("Subnetmap ID {}",  subnetMap.getId().toString());
                    subnetMapList.add(subnetMap);
                }
            }
            if (vpnId != null) {
                // create new vpn-interface object on this neutron port
                LOG.info("VPN ID: {}, SubnetMapList: {}", vpnId.toString(), subnetMapList.toString());
                LOG.info("Adding VPN Interface for port {}", portName);
