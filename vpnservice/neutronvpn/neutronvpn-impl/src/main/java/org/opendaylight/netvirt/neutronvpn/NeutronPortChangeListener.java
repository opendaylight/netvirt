...
public class NeutronPortChangeListener extends AsyncDataTreeChangeListenerBase<Port, NeutronPortChangeListener>
    ...
    private void handleFloatingIpPortUpdated(Port original, Port update) {
        if (((original == null) || (original.getDeviceId().equals(NeutronConstants.FLOATING_IP_DEVICE_ID_PENDING)))
            ...
            Uuid subnetId = null;
            for (FixedIps portIp : update.getFixedIps()) {
                if (portIp.getIpAddress().getIpv4Address() != null) {
                    subnetId = portIp.getSubnetId();
                    break;
                }
            }
            addToFloatingIpPortInfo(new Uuid(update.getDeviceId()), update.getUuid(), subnetId,
                        update.getMacAddress().getValue());
        ...
        portDataStoreCoordinator.enqueueJob("PORT- " + portupdate.getUuid().getValue(), () -> {
            ...
            for (Uuid snId: originalSnMapsIds) {
                if (!updateSnMapsIds.remove(snId)) {
                    ...
                    oldRouterId = (subnetMapOld != null) ? subnetMapOld.getRouterId() : null;
                }
            }
            Uuid newVpnId = null;
            Uuid newRouterId = null;
            for (Uuid snId: updateSnMapsIds) {
                ...
                newRouterId = (subnetMapNew != null) ? subnetMapNew.getRouterId() : null;
            }
            WriteTransaction wrtConfigTxn = dataBroker.newWriteOnlyTransaction();
            List<ListenableFuture<Void>> futures = new ArrayList<>();
            if (oldVpnId != null) {
            ...
            if (newVpnId != null) {
                ...
                if (newRouterId != null) {
                    nvpnManager.addToNeutronRouterInterfacesMap(newRouterId, portupdate.getUuid().getValue());
                }
            }
            futures.add(wrtConfigTxn.submit());
            return futures;
