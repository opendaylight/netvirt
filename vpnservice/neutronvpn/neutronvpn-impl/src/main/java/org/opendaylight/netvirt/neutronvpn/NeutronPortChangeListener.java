...
/* TODO: DELETE ALL COMMENTED IMPORTS ONCE ELAN VPP RENDERER IS WORKING
...
public class NeutronPortChangeListener extends AsyncDataTreeChangeListenerBase<Port, NeutronPortChangeListener>
    ...
    public static final TopologyId VPP_TOPOLOGY_ID = new TopologyId("topology-netconf");
    ...
    protected void update(InstanceIdentifier<Port> identifier, Port original, Port update) {
        ...
        /* TODO: DELETE THE BELOW LOGIC ONCE ELAN VPP RENDERER IS WORKING
    ...
    InstanceIdentifier<Topology> getVPPTopologyIId() {
        return InstanceIdentifier.builder(NetworkTopology.class)
                .child(Topology.class, new TopologyKey(VPP_TOPOLOGY_ID))
                .build();
    }
    ...
    private boolean createVPPInterfaceOnNode(Port port, Node host) {
        ...
        try {
            ...
            LOG.error("failed to create vpp interface for port {} on host {}",
                   port, host, e);
            return false;
        }
    ...
    private boolean addVPPInterfaceToBridgeDomain(Port port, Node host, String bridgeDomainId) {
        ...
        try {
            ...
            LOG.error("failed to add vpp interface for port {} to bridge domain {}",
                    port, bridgeDomainId, e);
            return false;
        }
    ...
    private boolean deleteVPPInterfaceFromBridgeDomain(Port port, Node host, String bridgeDomainId) {
        ...
        try {
            ...
            LOG.error("failed to delete vpp interface for port {} from bridge domain {}",
                          port, bridgeDomainId, e);
            return false;
        }
    ...
    private boolean deleteVPPInterfaceFromNode(Port port, Node host) {
        ...
        try {
            ...
            LOG.error("failed to delete vpp interface for port {} on node  {}",
                          port, host, e);
            return false;
        }
    ...
    private Interface createInterface(Port port) {
        ...
        PortBindingExtension portBindingExt = port.getAugmentation(PortBindingExtension.class);
        if (portBindingExt != null) {
            if (portBindingExt.getHostId() != null) {
                //Find the topology subtree for the host
                //TODO: Currently port-binding info in neutron port is used only for VPP interfaces
                //Extend the below check when the support for port-binding for other types of
                //interfaces is added
                if (getVPPNode(portBindingExt.getHostId(), dataBroker) != null) {
                    NodeIdentifierBuilder nodeIdBuilder = new NodeIdentifierBuilder()
                            .setTopologyId(VPP_TOPOLOGY_ID.getValue())
            ...
            parentrefBuilder.setVifDetails(Arrays.asList(vifdetailsBuilder.build()));
        }
