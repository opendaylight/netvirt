...
public class NeutronPortChangeListener extends AsyncDataTreeChangeListenerBase<Port, NeutronPortChangeListener>
    ...
    private void handleNeutronPortCreated(final Port port) {
        ...
        portDataStoreCoordinator.enqueueJob("PORT- " + portName, () -> {
            ...
            if (vpnId != null) {
                ...
                LOG.debug("handleNeutronPortCreated: VPN ID: {}, SubnetMapList: {}", vpnId.toString(),
                         subnetMapList.toString());
                LOG.debug("handleNeutronPortCreated: Adding VPN Interface for port {}", portName);
    ...
    private void handleNeutronPortUpdated(final Port portoriginal, final Port portupdate) {
        LOG.trace("Port original fixed IPs: {}", portoriginal.getFixedIps().toString());
        LOG.trace("Port update fixed IPs: {}", portupdate.getFixedIps().toString());
        ...
        portDataStoreCoordinator.enqueueJob("PORT- " + portupdate.getUuid().getValue(), () -> {
            ...
            List<Subnetmap> oldSubnetMapList = NeutronvpnUtils.getSubnetMapListForPort(dataBroker, portoriginal,
                    portoriginalNetId);
            for (Subnetmap oldSn: oldSubnetMapList) {
                Subnetmap subnetMapOld = nvpnManager.removePortsFromSubnetmapNode(oldSn.getId(), portoriginal.getUuid(),
                        null);
            }
            ...
            List<Subnetmap> newSubnetMapList = NeutronvpnUtils.getSubnetMapListForPort(dataBroker, portupdate,
                    portupdateNetId);
            for (Subnetmap newSn: newSubnetMapList) {
                Subnetmap subnetMapNew = nvpnManager.updateSubnetmapNodeWithPorts(newSn.getId(), portupdate.getUuid(),
                        null);
            }
