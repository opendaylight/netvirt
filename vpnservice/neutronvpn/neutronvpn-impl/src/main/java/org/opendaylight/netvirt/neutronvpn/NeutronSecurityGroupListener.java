...
public class NeutronSecurityGroupListener
    ...
    @Inject
    public NeutronSecurityGroupListener(final DataBroker dataBroker, final JobCoordinator jobCoordinator) {
    ...
    protected void add(InstanceIdentifier<SecurityGroup> instanceIdentifier, SecurityGroup securityGroup) {
        LOG.trace("added securityGroup: {}", securityGroup);

        jobCoordinator.enqueueJob("SG-" + securityGroup.getUuid().getValue(), () -> {
            Acl acl = buildAcl(securityGroup);
            InstanceIdentifier<Acl> identifier = getAclInstanceIdentifier(securityGroup);
            SingleTransactionDataBroker.syncWrite(dataBroker, LogicalDatastoreType.CONFIGURATION, identifier, acl);
    ...
    private Acl buildAcl(SecurityGroup sg) {
        return new AclBuilder().setAclName(sg.getUuid().getValue()).setAclType(NeutronSecurityRuleConstants.ACLTYPE)
                .build();
    }

    @Override
    protected void remove(InstanceIdentifier<SecurityGroup> instanceIdentifier, SecurityGroup securityGroup) {
        LOG.trace("removed securityGroup: {}", securityGroup);
        jobCoordinator.enqueueJob("SG-" + securityGroup.getUuid().getValue(), () -> {
            InstanceIdentifier<Acl> identifier = getAclInstanceIdentifier(securityGroup);
    ...
    protected void update(InstanceIdentifier<SecurityGroup> instanceIdentifier, SecurityGroup oldSecurityGroup,
        ...
        // Nothing to handle.
        LOG.trace("updates on security groups not supported.");
    }
