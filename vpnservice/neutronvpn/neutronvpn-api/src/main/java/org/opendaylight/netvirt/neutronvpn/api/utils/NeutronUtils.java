...
//import org.opendaylight.netvirt.neutronvpn.api.utils.DictJsonAdapter;
    ...
    */
    ...
    public static boolean isPortTypeSwitchdev(final Port port) {
        ...
        if (profile == null || profile.isEmpty()) {
            ...
            LOG.info("Port {} hase no binding:profile values", port.getUuid());
            return false;
        }

        DictJsonAdapter dictJson = new DictJsonAdapter();
        Map<String, JsonElement> mapProfile = null;
        try {
            mapProfile = dictJson.unmarshal(profile);
        } catch (Exception e) {
            // LOG.error("Failed to unmarshal profile {}", profile);
            LOG.info("Failed to unmarshal profile {}", profile);
            return false;
        }
        JsonElement capabilities = mapProfile.get("capabilities");
