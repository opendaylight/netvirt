    ...
    protected Object doExecute() {
        ...
        if (detail == null) {
            ...
            for (VpnInterface vpnInterface : vpnInterfaceConfigList) {
                for (String s : vpnInterface.getVpnInstanceName()) {
                    if (s != null) {
                        routerIds.add(s);
                    }
                }
            ...
            for (VpnInterfaceOpDataEntry vpnInterfaceOp : vpnInterfaceOperListOpDataEntries) {
                ifPresent = instanceNameToOperInterfaceMap.get(vpnInterfaceOp.getVpnName());
                if (ifPresent == null) {
                    instanceNameToOperInterfaceMap.put(vpnInterfaceOp.getVpnName(), 1);
                } else {
                    instanceNameToOperInterfaceMap.put(vpnInterfaceOp.getVpnName(),
                        instanceNameToOperInterfaceMap.get(vpnInterfaceOp.getVpnName()) + 1);
                }
            }
