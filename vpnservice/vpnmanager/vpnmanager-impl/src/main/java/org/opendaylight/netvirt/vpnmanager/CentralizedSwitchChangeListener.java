 ...
 * beyond NAT use cases, the associated models may need to be renamed and moved
 * to either vpnmanager or new bundle as part of Carbon model changes
 *
 */
public class CentralizedSwitchChangeListener
    ...
    protected void remove(InstanceIdentifier<RouterToNaptSwitch> key, RouterToNaptSwitch routerToNaptSwitch) {
        ...
        vpnManager.setupRouterGwMacFlow(router.getRouterName(), router.getExtGwMacAddress(), router.getNetworkId(),
                routerToNaptSwitch.getPrimarySwitchId(), NwConstants.DEL_FLOW);
    }
    ...
    protected void update(InstanceIdentifier<RouterToNaptSwitch> key, RouterToNaptSwitch origRouterToNaptSwitch,
        ...
        Routers router = VpnUtil.getExternalRouter(dataBroker, updatedRouterToNaptSwitch.getRouterName());
        if (router == null) {
        ...
        vpnManager.setupRouterGwMacFlow(router.getRouterName(), router.getExtGwMacAddress(), router.getNetworkId(),
                origRouterToNaptSwitch.getPrimarySwitchId(), NwConstants.DEL_FLOW);
        vpnManager.setupRouterGwMacFlow(router.getRouterName(), router.getExtGwMacAddress(), router.getNetworkId(),
                updatedRouterToNaptSwitch.getPrimarySwitchId(), NwConstants.ADD_FLOW);
    }
    ...
    protected void add(InstanceIdentifier<RouterToNaptSwitch> key, RouterToNaptSwitch routerToNaptSwitch) {
        ...
        vpnManager.setupRouterGwMacFlow(router.getRouterName(), router.getExtGwMacAddress(), router.getNetworkId(),
                routerToNaptSwitch.getPrimarySwitchId(), NwConstants.ADD_FLOW);
    }
