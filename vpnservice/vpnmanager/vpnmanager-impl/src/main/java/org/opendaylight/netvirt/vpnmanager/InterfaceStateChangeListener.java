        ...
        try {
            if (L2vlan.class.equals(intrf.getType())) {
                ...
                jobCoordinator.enqueueJob("VPNINTERFACE-" + ifName, () -> {
                    List<ListenableFuture<Void>> futures = new ArrayList<>(3);
                    ListenableFuture<Void> futr = txRunner.callWithNewWriteOnlyTransactionAndSubmit(writeConfigTxn -> {
                    ...
                    futures.add(futr);
                    Futures.addCallback(futr, new PostVpnInterfaceThreadWorker(intrf.getName(), false,
                            "Operational"));
                    return futures;
                });
            }
