...
public class TunnelInterfaceStateListener extends AsyncDataTreeChangeListenerBase<StateTunnelList,
    ...
    protected void remove(InstanceIdentifier<StateTunnelList> identifier, StateTunnelList del) {
        ...
        if (isGreTunnel(del)) {
            fibManager.removeOrUpdateDcGwLoadBalancingGroup(getDcGwIps(), getSrcDpnId(del), getDestIp(del));
        }
    ...
    protected void update(InstanceIdentifier<StateTunnelList> identifier, StateTunnelList original,
        ...
        if (isGreTunnel(update)) {
            fibManager.updateDcGwLoadBalancingGroup(getDcGwIps(), getSrcDpnId(update), getDestIp(update), isTunnelUp);
        }
    }
    ...
    protected void add(InstanceIdentifier<StateTunnelList> identifier, StateTunnelList add) {
        ...
        if (isGreTunnel(add)) {
            fibManager.createDcGwLoadBalancingGroup(getDcGwIps(), getSrcDpnId(add), getDestIp(add));
        }
    ...
    private String getDestIp(StateTunnelList tunnelState) {
        IpAddress dcGwIp = tunnelState.getDstInfo().getTepIp();
        if (dcGwIp == null) {
            return null;
        }
        return String.valueOf(dcGwIp.getValue());
    }

    private BigInteger getSrcDpnId(StateTunnelList tunnelState) {
