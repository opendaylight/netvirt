        ...
        try {
                    ...
                    if (vpnIdRdMap.containsKey(vpnId)) {
                        ...
                        for (Adjacency adj : adjList) {
                            ...
                            if ((tunnelAction == TunnelAction.TUNNEL_EP_ADD)
                                ...
                                fibManager.manageRemoteRouteOnDPN(true, srcDpnId, vpnId,
                                        rd, prefix, destTepIp, label, stateTunnelList.getTunnelInterfaceName());
                            }

                            if ((tunnelAction == TunnelAction.TUNNEL_EP_DELETE)
                                && (tunTypeVal == VpnTunnelLocType.ITMTunnelLocType.Internal)) {
                                fibManager.manageRemoteRouteOnDPN(false, srcDpnId, vpnId,
                                        rd, prefix, destTepIp, label, stateTunnelList.getTunnelInterfaceName());
                            }
                        }
                    }
            ...
            for (Map.Entry<Long, String> entry : vpnIdRdMap.entrySet()) {
                ...
                if ((tunnelAction == TunnelAction.TUNNEL_EP_ADD)
                    ...
                    fibManager.populateExternalRoutesOnDpn(srcDpnId, vpnId, rd, srcTepIp,
                            destTepIp, stateTunnelList.getTunnelInterfaceName());
                } else if ((tunnelAction == TunnelAction.TUNNEL_EP_DELETE)
                    && (tunTypeVal == VpnTunnelLocType.ITMTunnelLocType.External)) {
