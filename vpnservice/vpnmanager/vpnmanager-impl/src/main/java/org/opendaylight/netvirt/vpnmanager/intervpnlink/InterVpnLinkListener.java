...
public class InterVpnLinkListener extends AsyncDataTreeChangeListenerBase<InterVpnLink, InterVpnLinkListener>
    ...
    protected void add(InstanceIdentifier<InterVpnLink> identifier, InterVpnLink add) {
        ...
        try {
            ...
            Preconditions.checkState(!VpnUtil.isVpnPendingDelete(dataBroker, vpn1PrimaryRd),
                    "Vpn with RD " + vpn1PrimaryRd + "is pending delete");
            String vpn2PrimaryRd = VpnUtil.getPrimaryRd(dataBroker, vpn1Name);
            Preconditions.checkState(!VpnUtil.isVpnPendingDelete(dataBroker, vpn2PrimaryRd),
            ...
            LOG.error("add: Unable to process intervpn-link for vpns {} and {} due to {}", vpn1Name, vpn2Name,
                    e.getMessage());
        }
    }
