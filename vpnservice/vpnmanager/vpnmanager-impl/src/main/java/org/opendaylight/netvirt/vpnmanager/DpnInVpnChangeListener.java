...
public class DpnInVpnChangeListener implements OdlL3vpnListener {
    ...
    @Override
    public void onAddInterfaceOnDpnEvent(AddInterfaceOnDpnEvent notification) {
        AddInterfaceEventData data = notification.getAddInterfaceEventData();
        String interfaceName = data.getInterfaceName();
        String vpnName = data.getVpnName();
        BigInteger dpnId = data.getDpnId();
        LOG.trace("Processing vpn interface {} addition", interfaceName);
        aclServiceUtil.updateBoundServicesFlow(interfaceName, vpnName);
        aclServiceUtil.updateRemoteAclFilterTable(interfaceName, vpnName, dpnId, 0/*ADD*/);

    }

    @Override
    public void onRemoveInterfaceOnDpnEvent(RemoveInterfaceOnDpnEvent notification) {
        ...
        aclServiceUtil.updateRemoteAclFilterTable(interfaceName, vpnName, dpnId, 1/*DELETE*/);
    }
}
