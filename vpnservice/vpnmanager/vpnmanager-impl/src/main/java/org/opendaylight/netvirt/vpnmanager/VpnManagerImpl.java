...
public class VpnManagerImpl implements IVpnManager {
    ...
    public void setupRouterGwArpResponderFlows(String routerName, Collection<String> routerGwIps, String routerGwMac,
        ...
        LOG.info("{} router GW ARP responder flows for router-id {} fixed-ips {} on switch {}",
    ...
    private void installRouterGwArpResponderFlow(String routerGwMac, BigInteger dpnId, String extInterfaceName,
        ...
        List<Instruction> instructions = getRouterGwArpResponderInstructions(routerGwMac, extInterfaceName, routerGwIp);
    ...
    private List<Instruction> getRouterGwArpResponderInstructions(String routerGwMac, String extInterfaceName,
            String routerGwIp) {
        List<Instruction> instructions = new ArrayList<>();
        // reset the split-horizon bit to allow traffic to be sent back to the
        // provider port
        instructions.add(0, new InstructionInfo(InstructionType.write_metadata,
        ...
        List<Action> actions = ArpResponderUtil.getActions(ifaceMgrRpcService, extInterfaceName, routerGwIp,
                routerGwMac);
        // replace resubmit action with goto so it can co-exist with write-metadata
        for (Iterator<Action> iterator = actions.iterator(); iterator.hasNext();) {
            ...
            if (actionClass instanceof NxActionResubmitRpcAddGroupCase) {
                ...
                if (tableId > NwConstants.ARP_RESPONDER_TABLE) {
                    iterator.remove();
                    instructions.add(2,
                            new InstructionInfo(InstructionType.goto_table, new long[] { tableId.longValue() })
                                    .buildInstruction(0));
                }
            }
        }

        instructions.add(1, MDSALUtil.buildApplyActionsInstruction(actions, 1));
        return instructions;
    }
