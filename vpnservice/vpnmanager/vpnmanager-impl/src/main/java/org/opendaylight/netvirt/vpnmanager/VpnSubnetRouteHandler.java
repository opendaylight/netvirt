...
public class VpnSubnetRouteHandler {
    ...
    private static final String loggingPrefix = "SUBNETROUTE:";
    ...
    public void onSubnetAddedToVpn(Subnetmap subnetmap, boolean isBgpVpn, Long elanTag) {
        ...
        Preconditions.checkNotNull(subnetId, loggingPrefix + " onSubnetAddedToVpn: SubnetId cannot be null or empty!");
        Preconditions.checkNotNull(subnetIp,
                loggingPrefix + " onSubnetAddedToVpn: SubnetPrefix cannot be null or empty!");
        Preconditions.checkNotNull(elanTag, loggingPrefix + " onSubnetAddedToVpn: ElanTag cannot be null or empty!");
            ...
            if (vpnId == VpnConstants.INVALID_ID) {
                ...
                if (vpnId == VpnConstants.INVALID_ID) {
                    LOG.error("{} onSubnetAddedToVpn: VpnInstance to VPNId mapping not yet available for VpnName {} "
                              + "processing subnet {} with IP {}, bailing out now.", loggingPrefix, vpnName, subnetId,
                            subnetIp);
                    return;
                }
            }
        ...
        LOG.info("{} onSubnetAddedToVpn: Subnet {} with IP {}being added to vpn {}", loggingPrefix,
        ...
        try {
            ...
            try {
                ...
                if (!sm.isPresent()) {
                    LOG.error("{} onSubnetAddedToVpn: Unable to retrieve subnetmap entry for subnet {} IP {}"
                            + " vpnName {}",  loggingPrefix, subnetId, subnetIp, vpnName);
                    return;
                }
                subMap = sm.get();
                ...
                if (isBgpVpn) {
                    ...
                    if (optionalNets.isPresent()) {
                        LOG.info("{} onSubnetAddedToVpn: subnet {} with IP {} is an external subnet on external "
                                + "network {}, so ignoring this for SubnetRoute on vpn {}", loggingPrefix,
                ...
                if (optionalSubs.isPresent()) {
                    LOG.error("{} onSubnetAddedToVpn: SubnetOpDataEntry for subnet {} with ip {} and vpn {} already"
                            + " detected to be present", loggingPrefix, subnetId.getValue(), subnetIp, vpnName);
                    return;
                }
                LOG.trace("{} onSubnetAddedToVpn: Creating new SubnetOpDataEntry node for subnet {} subnetIp {}"
                        + " vpn {}", loggingPrefix, subnetId.getValue(), subnetIp, vpnName);
                ...
                if (isBgpVpn && !VpnUtil.isBgpVpn(vpnName, primaryRd)) {
                    LOG.error("{} onSubnetAddedToVpn: The VPN Instance name {} does not have RD. Bailing out for"
                            + " subnet {} subnetIp {} ", loggingPrefix, vpnName, subnetId.getValue(), subnetIp);
                    return;
                }
                ...
                LOG.info("{} onSubnetAddedToVpn: Added subnetopdataentry to OP Datastore for subnet {} subnetIp {} "
                        + "vpn {} rd {} TaskState {}", loggingPrefix, subnetId.getValue(), subnetIp, vpnName,
                        primaryRd, subOpEntry.getRouteAdvState());
            } catch (Exception ex) {
                LOG.error("{} onSubnetAddedToVpn: Creation of SubnetOpDataEntry for subnet {} subnetIp {} vpn {}"
                        + " failed {}", loggingPrefix, subnetId.getValue(), subnetIp, vpnName, ex);
            } finally {
            ...
            try {
                        ...
                        if (intfState != null) {
                            try {
                                ...
                                LOG.error("{} onSubnetAddedToVpn: Unable to obtain dpnId for interface {},"
                                        ...
                                        + " failed with exception {}", loggingPrefix, port.getValue(),
                                        subnetId.getValue(), subnetIp, vpnName, e);
                                continue;
                            }
                            if (dpnId.equals(BigInteger.ZERO)) {
                                LOG.error("{} onSubnetAddedToVpn: Port {} is not assigned DPN yet,"
                                        + " ignoring subnet {} subnetIP {} vpn {}", loggingPrefix, port.getValue(),
                                        subnetId.getValue(), subnetIp, vpnName);
                                continue;
                            }
                            ...
                            if (intfState.getOperStatus() != OperStatus.Up) {
                                LOG.error("{} onSubnetAddedToVpn: Port {} is not UP yet, ignoring subnet {}"
                                        + " subnetIp {} vpn {}", loggingPrefix, port.getValue(), subnetId.getValue(),
                                        subnetIp, vpnName);
                                continue;
                            }
                ...
                LOG.info("{} onSubnetAddedToVpn: Added PortOpDataEntry and VpnInterfaces to SubnetOpData"
                        + " for subnet {} subnetIp {} vpn {} TaskState {} lastTaskState {}", loggingPrefix,
            ...
            LOG.error("{} onSubnetAddedToVpn: Unable to handle subnet {} with ip {} added to vpn {} {}", loggingPrefix,
                    subnetId.getValue(), subnetIp, vpnName, e);
        }
    }
    ...
    public void onSubnetDeletedFromVpn(Subnetmap subnetmap, boolean isBgpVpn) {
        Uuid subnetId = subnetmap.getId();
        LOG.info("{} onSubnetDeletedFromVpn: Subnet {} with ip {} being removed from vpnId {}", loggingPrefix,
        ...
        try {
            ...
            try {
                ...
                if (!optionalSubs.isPresent()) {
                    LOG.error("{} onSubnetDeletedFromVpn: SubnetOpDataEntry for subnet {} subnetIp {} vpn {}"
                            + " not available in datastore", loggingPrefix, subnetId.getValue(),
                ...
                LOG.trace("{} onSubnetDeletedFromVpn: Removing the SubnetOpDataEntry node for subnet {} subnetIp {}"
                        + " vpnName {} rd {} TaskState {}", loggingPrefix, subnetId.getValue(),
                ...
                if (!sm.isPresent()) {
                    LOG.error("{} onSubnetDeletedFromVpn: Stale ports removal: Unable to retrieve subnetmap entry"
                            + " for subnet {} subnetIp {} vpnName {}", loggingPrefix, subnetId.getValue(),
                            optionalSubs.get().getSubnetCidr(), optionalSubs.get().getVpnName());
                } else {
                    ...
                    if (portList != null) {
                        for (Uuid port : portList) {
                            ...
                            LOG.trace("{} onSubnetDeletedFromVpn: Deleting portOpData entry for port {}"
                                    ...
                                    loggingPrefix, port.getValue(), subnetId.getValue(),
                                    optionalSubs.get().getSubnetCidr(), optionalSubs.get().getVpnName(),
                ...
                LOG.info("{} onSubnetDeletedFromVpn: Removed subnetopdataentry successfully from Datastore"
                        + " for subnet {} subnetIp {} vpnName {} rd {}", loggingPrefix, subnetId.getValue(), subnetIp,
                        vpnName, rd);
            } catch (Exception ex) {
                LOG.error("{} onSubnetDeletedFromVpn: Removal of SubnetOpDataEntry for subnet {} subnetIp {}"
                        + " vpnId {} failed {}", loggingPrefix, subnetId.getValue(), subnetmap.getSubnetIp(),
            ...
            LOG.error("{} onSubnetDeletedFromVpn: Unable to handle subnet {} with Ip {} removed from vpn {} {}",
                    loggingPrefix, subnetId.getValue(), subnetmap.getSubnetIp(), subnetmap.getVpnId(), e);
        }
    }
    ...
    public void onSubnetUpdatedInVpn(Subnetmap subnetmap, Long elanTag) {
        ...
        Preconditions.checkNotNull(subnetId,
                loggingPrefix + " onSubnetUpdatedInVpn: SubnetId cannot be null or empty!");
        Preconditions.checkNotNull(subnetIp,
                loggingPrefix + " onSubnetUpdatedInVpn: SubnetPrefix cannot be null or empty!");
        Preconditions.checkNotNull(vpnName, loggingPrefix + " onSubnetUpdatedInVpn: VpnName cannot be null or empty!");
        Preconditions.checkNotNull(elanTag, loggingPrefix + " onSubnetUpdatedInVpn: ElanTag cannot be null or empty!");
        ...
        LOG.info("{} onSubnetUpdatedInVpn: subnet {} with Ip {} updated successfully for vpn {}", loggingPrefix,
                subnetId.getValue(), subnetIp, vpnName);
    }
            ...
            try {
                ...
                if (!optionalSubs.isPresent()) {
                    LOG.error("{} onPortAddedToSubnet: Port {} is part of a subnet {} that is not in VPN, ignoring",
                            loggingPrefix, portId.getValue(), subnetId.getValue());
                    return;
                }
                LOG.info("{} onPortAddedToSubnet: Port {} being added to subnet {} subnetIp {} vpnName {} rd {}"
                        + " TaskState {}", loggingPrefix, portId.getValue(), subnetId.getValue(),
                        optionalSubs.get().getSubnetCidr(), optionalSubs.get().getVpnName(),
                        optionalSubs.get().getVrfId(), optionalSubs.get().getRouteAdvState());
                ...
                try {
                    ...
                    LOG.error("{} onPortAddedToSubnet: Unable to obtain dpnId for interface {}. subnetroute inclusion"
                            + " for this interface failed for subnet {} subnetIp {} vpn {} rd {} with exception {}",
                            loggingPrefix, portId.getValue(), subnetId.getValue(), optionalSubs.get().getSubnetCidr(),
                            optionalSubs.get().getVpnName(), optionalSubs.get().getVrfId(), e);
                    return;
                }
                if (dpnId.equals(BigInteger.ZERO)) {
                    LOG.error("{} onPortAddedToSubnet: Port {} is not assigned DPN yet, ignoring subnetRoute inclusion"
                            + " for the interface into subnet {} subnetIp {} vpnName {} rd {}", loggingPrefix,
                ...
                if (intfState.getOperStatus() != OperStatus.Up) {
                    LOG.error("{} onPortAddedToSubnet: Port {} is not UP yet, ignoring subnetRoute inclusion for the"
                            + " interface into subnet {} subnetIp {} vpnName {} rd {}", loggingPrefix,
                            portId.getValue(), subnetId.getValue(), optionalSubs.get().getSubnetCidr(),
                            optionalSubs.get().getVpnName(), optionalSubs.get().getVrfId());
                    return;
                }
                LOG.debug("{} onPortAddedToSubnet: Port {} added. Updating the SubnetOpDataEntry node for subnet {}"
                        + " subnetIp {} vpnName {} rd {} TaskState {}", loggingPrefix, portId.getValue(),
                        subnetId.getValue(), optionalSubs.get().getSubnetCidr(), optionalSubs.get().getVpnName(),
                        optionalSubs.get().getVrfId(), optionalSubs.get().getRouteAdvState());
                ...
                LOG.info("{} onPortAddedToSubnet: Updated subnetopdataentry to OP Datastore for port {} subnet {}"
                        + " subnetIp {} vpnName {} rd {} TaskState {} lastTaskState {}", loggingPrefix,
                ...
                LOG.error("{} onPortAddedToSubnet: Updation of subnetOpEntry for port {} subnet {} falied {}",
                        loggingPrefix, portId.getValue(), subnetId.getValue(), ex);
            } finally {
                VpnUtil.unlockSubnet(lockManager, subnetId.getValue());
            }
        } catch (Exception e) {
            LOG.error("{} onPortAddedToSubnet: Unable to handle port {} added to subnet {} {}", loggingPrefix,
    ...
    public void onPortRemovedFromSubnet(Subnetmap subnetmap, Uuid portId) {
        ...
        try {
            ...
            try {
                ...
                if (dpnId == null) {
                    LOG.error("{} onPortRemovedFromSubnet:  Port {} does not have a DPNId associated," +
                            " ignoring removal from subnet {}", loggingPrefix, portId.getValue(), subnetId.getValue());
                    return;
                }
                ...
                if (!optionalSubs.isPresent()) {
                    LOG.info("{} onPortRemovedFromSubnet: Port {} is part of a subnet {} that is not in VPN,"
                            + " ignoring", loggingPrefix, portId.getValue(), subnetId.getValue());
                    return;
                }
                LOG.info("{} onPortRemovedFromSubnet: Port {} being removed. Updating the SubnetOpDataEntry"
                        + " for subnet {} subnetIp {} vpnName {} rd {} TaskState {} lastTaskState {}", loggingPrefix,
                        portId.getValue(), subnetId.getValue(), optionalSubs.get().getSubnetCidr(),
                        optionalSubs.get().getVpnName(), optionalSubs.get().getVrfId(),
                ...
                if ((nhDpnId != null) && (nhDpnId.equals(dpnId))) {
                    ...
                    if (last) {
                        LOG.debug("{} onPortRemovedFromSubnet: Last port {} being removed from subnet {} subnetIp {}"
                                + " vpnName {} rd {}", loggingPrefix, portId.getValue(), subnetId.getValue(),
                        ...
                        LOG.info("{} onPortRemovedFromSubnet: Updated subnetopdataentry to OP Datastore"
                                + " removing port {} from subnet {} subnetIp {} vpnName {} rd {}", loggingPrefix,
            ...
            LOG.error("{} onPortRemovedFromSubnet: Unable to handle port {} removed from subnet {} {}",loggingPrefix,
                    portId.getValue(), subnetId.getValue(), e);
        }
    }
    ...
    public void onInterfaceUp(BigInteger dpnId, String intfName, Uuid subnetId) {
        ...
        if ((dpnId == null) || Objects.equals(dpnId, BigInteger.ZERO)) {
            LOG.error("{} onInterfaceUp: Unable to determine the DPNID for port {} on subnet {}", loggingPrefix,
            ...
            try {
                ...
                if (!optionalSubs.isPresent()) {
                    LOG.error("{} onInterfaceUp: SubnetOpDataEntry for subnet {} is not available."
                            + " Ignoring interfaceUp for port{}", loggingPrefix, subnetId.getValue(), intfName);
                    return;
                }
                ...
                LOG.info("{} onInterfaceUp: Updating the SubnetOpDataEntry node for subnet {} subnetIp {} vpn {}"
                        + " rd {} TaskState {} lastTaskState {}" , loggingPrefix, subnetId.getValue(),
                        subOpBuilder.getSubnetCidr(), subOpBuilder.getVpnName(), subOpBuilder.getVrfId(),
                ...
                LOG.info("{} onInterfaceUp: Updated subnetopdataentry to OP Datastore port {} up for subnet {}"
                        + " subnetIp {} vpnName {} rd {} TaskState {} lastTaskState {} ", loggingPrefix, intfName,
                ...
                LOG.error("{} onInterfaceUp: Updation of SubnetOpDataEntry for subnet {} on port {} up failed {}",
                        loggingPrefix, subnetId.getValue(), intfName, ex);
            } finally {
                VpnUtil.unlockSubnet(lockManager, subnetId.getValue());
            }
        } catch (Exception e) {
            LOG.error("{} onInterfaceUp: Unable to handle interface up event for port {} in subnet {} {}",
                    loggingPrefix, intfName, subnetId.getValue(), e);
        }
    }
    ...
    public void onInterfaceDown(final BigInteger dpnId, final String interfaceName, Uuid subnetId) {
        if ((dpnId == null) || (Objects.equals(dpnId, BigInteger.ZERO))) {
            LOG.error("{} onInterfaceDown: Unable to determine the DPNID for port {} on subnet {}", loggingPrefix,
            ...
            try {
                ...
                if (!optionalSubs.isPresent()) {
                    LOG.error("{} onInterfaceDown: SubnetOpDataEntry for subnet {} is not available."
                            + " Ignoring port {} down event.", loggingPrefix, subnetId.getValue(), interfaceName);
                    return;
                }
                SubnetOpDataEntry subnetOpDataEntry = optionalSubs.get();
                SubnetOpDataEntryBuilder subOpBuilder = new SubnetOpDataEntryBuilder(subnetOpDataEntry);
                LOG.info("{} onInterfaceDown: Updating the SubnetOpDataEntry node for subnet {} subnetIp {}"
                        + " vpnName {} rd {} TaskState {} lastTaskState {} on port {} down", loggingPrefix,
                ...
                if ((nhDpnId != null) && (nhDpnId.equals(dpnId))) {
                    ...
                    if (last) {
                        LOG.debug("{} onInterfaceDown: Last active port {} on the subnet {} subnetIp {} vpn {}"
                                + " rd {}", loggingPrefix, interfaceName, subnetId.getValue(),
                                subOpBuilder.getSubnetCidr(), subOpBuilder.getVpnName(), subOpBuilder.getVrfId());
                        // last port on this DPN, so we need to elect the new NHDpnId
                        ...
                        LOG.info("{} onInterfaceDown: Updated subnetopdataentry for subnet {} subnetIp {} vpnName {}"
                                + " rd {} to OP Datastore on port {} down ", loggingPrefix, subnetId.getValue(),
                                subOpBuilder.getSubnetCidr(), subOpBuilder.getVpnName(), subOpBuilder.getVrfId(),
                                interfaceName);
                    }
                }
                ...
                LOG.error("{} onInterfaceDown: SubnetOpDataEntry update on interface {} down event for subnet {}"
                        + " falied {}", loggingPrefix, interfaceName, subnetId.getValue(), ex);
            } finally {
                VpnUtil.unlockSubnet(lockManager, subnetId.getValue());
            }
        } catch (Exception e) {
            LOG.error("{} onInterfaceDown: Unable to handle interface down event for port {} in subnet {} {}",
                    loggingPrefix, interfaceName, subnetId.getValue(), e);
        }
    }
            ...
            try {
                ...
                if (!optionalSubs.isPresent()) {
                    LOG.error("{} updateSubnetRouteOnTunnelUpEvent: SubnetOpDataEntry for subnet {} is not available",
                            loggingPrefix, subnetId.getValue());
                    return;
                }
                LOG.info("{} updateSubnetRouteOnTunnelUpEvent: Subnet {} subnetIp {} vpnName {} rd {} TaskState {}"
                        + " lastTaskState {} Dpn {}", loggingPrefix, subnetId.getValue(),
                        optionalSubs.get().getSubnetCidr(), optionalSubs.get().getVpnName(),
                        optionalSubs.get().getVrfId(), optionalSubs.get().getRouteAdvState(),
                ...
                LOG.info("{} updateSubnetRouteOnTunnelUpEvent: Updated subnetopdataentry to OP Datastore tunnel up"
                        ...
                        loggingPrefix, dpnId.toString(), subnetId.getValue(), subOpEntry.getSubnetCidr(),
                ...
                LOG.error("{} updateSubnetRouteOnTunnelUpEvent: updating subnetRoute for subnet {} on dpn {}",
                        loggingPrefix, subnetId.getValue(), dpnId.toString(), ex);
            } finally {
                VpnUtil.unlockSubnet(lockManager, subnetId.getValue());
            }
        } catch (Exception e) {
            LOG.error("{} updateSubnetRouteOnTunnelUpEvent: Unable to handle tunnel up event for subnetId {} dpnId {}"
                    + " with exception {}", loggingPrefix, subnetId.getValue(), dpnId.toString(), e);
        }
    }
            ...
            try {
                ...
                if (!optionalSubs.isPresent()) {
                    LOG.error("{} updateSubnetRouteOnTunnelDownEvent: SubnetOpDataEntry for subnet {}"
                            + " is not available", loggingPrefix, subnetId.getValue());
                    return;
                }
                LOG.debug("{} updateSubnetRouteOnTunnelDownEvent: Dpn {} Subnet {} subnetIp {} vpnName {} rd {}"
                        + " TaskState {} lastTaskState {}", loggingPrefix, dpnId.toString(), subnetId.getValue(),
                        optionalSubs.get().getSubnetCidr(), optionalSubs.get().getVpnName(),
                        optionalSubs.get().getVrfId(), optionalSubs.get().getRouteAdvState(),
                ...
                if ((nhDpnId != null) && (nhDpnId.equals(dpnId))) {
                    ...
                    LOG.info("{} updateSubnetRouteOnTunnelDownEvent: Subnet {} Dpn {} subnetIp {} vpnName {} rd {}"
                            + " TaskState {} lastTaskState {}", loggingPrefix, subnetId.getValue(), dpnId.toString(),
                            optionalSubs.get().getSubnetCidr(), optionalSubs.get().getVpnName(),
                            optionalSubs.get().getVrfId(), optionalSubs.get().getRouteAdvState(),
                            optionalSubs.get().getLastAdvState());
                }
                ...
                LOG.error("{} updateSubnetRouteOnTunnelDownEvent: Updation of SubnetOpDataEntry for subnet {}"
                        + " on dpn {} failed {}", loggingPrefix, subnetId.getValue(), dpnId, ex);
            } finally {
                VpnUtil.unlockSubnet(lockManager, subnetId.getValue());
            }
        } catch (Exception e) {
            LOG.error("{} updateSubnetRouteOnTunnelDownEvent: Unable to handle tunnel down event for subnetId {}"
                    + " dpnId {} with exception {}", loggingPrefix, subnetId.getValue(), dpnId.toString(), e);
        }
    }
    ...
    private void publishSubnetRouteToBgp(SubnetOpDataEntryBuilder subOpBuilder, String nextHopIp) {
        try {
            ...
            LOG.error("{} spublishSubnetRouteToBgp: Subnet route not advertised for subnet {} subnetIp {} vpn {} rd {}"
                    + " with dpnid {}", loggingPrefix, subOpBuilder.getSubnetId().getValue(),
    ...
    private boolean addSubnetRouteToFib(String rd, String subnetIp, BigInteger nhDpnId, String nextHopIp,
        ...
        Preconditions.checkNotNull(rd,
                loggingPrefix + " addSubnetRouteToFib: RouteDistinguisher cannot be null or empty!");
        Preconditions.checkNotNull(subnetIp,
                loggingPrefix + " addSubnetRouteToFib: SubnetRouteIp cannot be null or empty!");
        Preconditions.checkNotNull(vpnName, loggingPrefix + " addSubnetRouteToFib: vpnName cannot be null or empty!");
        Preconditions.checkNotNull(elanTag, loggingPrefix + " addSubnetRouteToFib: elanTag cannot be null or empty!");
        Preconditions.checkNotNull(nextHopIp,
                loggingPrefix + " addSubnetRouteToFib: NextHopIp cannot be null or empty!");
        Preconditions.checkNotNull(label, loggingPrefix + " addSubnetRouteToFib: label cannot be null or empty!");
        VrfEntry.EncapType encapType = VpnUtil.getEncapType(VpnUtil.isL3VpnOverVxLan(l3vni));
        ...
        LOG.info("{} addSubnetRouteToFib: Adding SubnetRoute fib entry for vpnName {}, subnetIP {}, elanTag {}",
                loggingPrefix, vpnName, subnetIp, elanTag);
        ...
        Preconditions.checkNotNull(nextHopIp, loggingPrefix + "NextHopIp cannot be null or empty!");
        ...
        try {
            ...
            LOG.error("{} addSubnetRouteToFib: Subnet route not advertised for subnet {} subnetIp {} vpnName {} rd {} "
                    + "with dpnid {}", loggingPrefix, subnetId.getValue(), subnetIp, vpnName, rd, nhDpnId, e);
            return false;
        }
        return true;
    }
    ...
    private int getLabel(String rd, String subnetIp) {
        ...
        LOG.trace("{} getLabel: Allocated subnetroute label {} for rd {} prefix {}", loggingPrefix, label, rd,
                subnetIp);
        return label;
    }
    ...
    private boolean deleteSubnetRouteFromFib(String rd, String subnetIp, String vpnName, boolean isBgpVpn) {
        Preconditions.checkNotNull(rd,
                loggingPrefix + " deleteSubnetRouteFromFib: RouteDistinguisher cannot be null or empty!");
        Preconditions.checkNotNull(subnetIp,
                loggingPrefix +  " deleteSubnetRouteFromFib: SubnetRouteIp cannot be null or empty!");
        ...
        if (isBgpVpn) {
            try {
                ...
                LOG.error("{} deleteSubnetRouteFromFib: Subnet route not withdrawn for subnetIp {} vpn {} rd {}"
                        + "  due to exception {}", loggingPrefix, subnetIp, vpnName, rd, e);
                return false;
            }
        }
        return true;
    ...
    private void electNewDpnForSubnetRoute(SubnetOpDataEntryBuilder subOpBuilder, BigInteger oldDpnId, Uuid subnetId,
        ...
        LOG.info("{} electNewDpnForSubnetRoute: Handling subnet {} subnetIp {} vpn {} rd {} TaskState {}"
                + " lastTaskState {}", loggingPrefix, subnetId.getValue(), subnetIp, subOpBuilder.getVpnName(),
                subOpBuilder.getVrfId(), subOpBuilder.getRouteAdvState(), subOpBuilder.getLastAdvState());
        if (!isBgpVpn) {
            ...
            if (isRouteAdvertised) {
                ...
                LOG.error("{} electNewDpnForSubnetRoute: Unable to find TepIp for subnet {} subnetip {} vpnName {}"
                        + " rd {} for dpnid {}, attempt next dpn", loggingPrefix, subnetId.getValue(), subnetIp,
                        vpnName, rd, nhDpnId.toString());subOpBuilder.setRouteAdvState(TaskState.PendingAdvertise);
            }
            return;
        }
            ...
            if (vpnNodeListener.isConnectedNode(nhDpnId)) {
                ...
                try {
                    ...
                    LOG.warn("{} electNewDpnForSubnetRoute: Unable to find TepIp for rd {} subnetroute subnetip {}"
                            + " for dpnid {}, attempt next", loggingPrefix, rd, subnetIp, nhDpnId.toString(), e);
                    continue;
                }
            }
        }
        if (!isAlternateDpnSelected) {
            ...
            if (isRouteAdvertised(subOpBuilder) && (oldDpnId != null)) {
                LOG.info("{} electNewDpnForSubnetRoute: No alternate DPN available for subnet {} subnetIp {} vpn {}"
                        + " rd {} Prefix withdrawn from BGP", loggingPrefix, subnetId.getValue(), subnetIp, vpnName,
                        rd);
                ...
                if (routeWithdrawn) {
                    ...
                    LOG.error("{} electNewDpnForSubnetRoute: Withdrawing NextHopDPN {} for subnet {} subnetIp {}"
                            + " vpn {} rd {} from BGP failed", loggingPrefix, oldDpnId.toString(), subnetId.getValue(),
                            subnetIp, vpnName, rd);subOpBuilder.setRouteAdvState(TaskState.PendingWithdraw);
                }
            }
        } else {
            ...
            if (isRouteAdvertised) {
                ...
                LOG.error("{} electNewDpnForSubnetRoute: Swapping to add new NextHopDpn {} for subnet {} subnetIp {}"
                        + " vpn {} rd {} failed", loggingPrefix, nhDpnId, subnetId.getValue(), subnetIp, vpnName, rd);
