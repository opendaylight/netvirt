...
public class VpnSubnetRouteHandler {
    ...
    public void onSubnetAddedToVpn(Subnetmap subnetmap, boolean isBgpVpn, Long elanTag) {
        ...
        try {
            ...
            VpnTargets vpnTargets = VpnUtil.getVpnTargets(dataBroker, vpnName);
    ...
    private VpnInstanceBuilder withdrawIpFamilFromVpnInstance(VpnInstanceBuilder vpnInstanceBuilder, IpVersionChoice ipVersion) {
        if (ipVersion.isIpVersionChosen(IpVersionChoice.IPV4)) {
        ...
        try {
            ...
            try {
                InstanceIdentifier<SubnetOpDataEntry> subOpIdentifier =
                    InstanceIdentifier.builder(SubnetOpData.class).child(SubnetOpDataEntry.class,
                        new SubnetOpDataEntryKey(subnetId)).build();
                Optional<SubnetOpDataEntry> allOptionalSubs =
                ...
                List<SubnetOpDataEntry> vpnSubnetOpDataEntries = new ArrayList<>();
                SubnetOpDataEntry subnetOpDataEntry = null;
                for (SubnetOpDataEntry opData : allOptionalSubs.get()) {
                    if (opData.getSubnetId() == subnetId) {
                        subnetOpDataEntry = opData;
                    } else if (opData.getVpnName() == vpnName) {
                        vpnSubnetOpDataEntries.add(opData);
                    }
                }
                ...
                InstanceIdentifier<Subnetmap> subMapid =
                    InstanceIdentifier.builder(Subnetmaps.class).child(Subnetmap.class,
                        new SubnetmapKey(subnetId, vpnName)).build();
                Optional<Subnetmap> sm = VpnUtil.read(dataBroker, LogicalDatastoreType.CONFIGURATION, subMapid);
                if (!sm.isPresent()) {
            ...
            VpnInstanceBuilder vpnInstanceBuilder = new VpnInstanceBuilder(vpnInstance);
            IpVersionChoice ipVersion = NeutronUtils.getIpVersion(subnetIp);
            if (vpnSubnetOpDataEntries.isEmpty()) {
                VpnInstanceBuilder vpnInstanceBuilder = withdrawIpFamilyFromVpnInstance(vpnInstanceBuilder, ipVersion);
            }
            List<IpVersionChoice> ipChoices = vpnSubnetOpDataEntries.stream().map(NeutronUtils.getIpVersion(opData -> opData.getSubnetCidr())).collect(Collectors.toList());
