...
public class VpnSubnetRouteHandler {
    ...
    public void onSubnetAddedToVpn(Subnetmap subnetmap, boolean isBgpVpn, Long elanTag) {
            ...
            if (vpnId == VpnConstants.INVALID_ID) {
                ...
                if (vpnId == VpnConstants.INVALID_ID) {
                    LOG.error("{} onSubnetAddedToVpn: VpnInstance to VPNId mapping not yet available for VpnName {} "
                                  + "processing subnet {} with IP {}, bailing out now.", LOGGING_PREFIX, vpnName,
                                  subnetId, subnetIp);
                    return;
                }
            }
        } else {
            LOG.error("{} onSubnetAddedToVpn: VpnId {} for subnet {} not found, bailing out", LOGGING_PREFIX,
                          subnetmap.getVpnId(), subnetId);
            return;
        }
        ...
        if (isBgpVpn && !VpnUtil.isBgpVpn(vpnName, primaryRd)) {
            LOG.error("{} onSubnetAddedToVpn: The VPN Instance name {} does not have RD", LOGGING_PREFIX, vpnName);
            return;
        }
        ...
        try {
            ...
            try {
                ...
                LOG.error("{} onSubnetAddedToVpn: Creation of SubnetOpDataEntry for subnet {} failed ", LOGGING_PREFIX,
    ...
    public void onSubnetDeletedFromVpn(Subnetmap subnetmap, boolean isBgpVpn) {
        ...
        if (vpnName == null) {
            LOG.error("{} onSubnetDeletedFromVpn: VpnName for subnet {} is not found", LOGGING_PREFIX,
                        subnetId.getValue());
            return;
        }
        LOG.info("{} onSubnetDeletedFromVpn: Subnet {} being removed from vpn {}", LOGGING_PREFIX, subnetId.getValue(),
                    vpnName);
        //TODO(vivek): Change this to use more granularized lock at subnetId level
        ...
        try {
            ...
            try {
                ...
                // get all SubnetOpDataEntries from OPERATIONAL DS
                ...
                LOG.trace("{} onSubnetDeletedFromVpn: Removing the SubnetOpDataEntry node for subnet: {}",
                            LOGGING_PREFIX, subnetId.getValue());
                ...
                LOG.info("{} onSubnetDeletedFromVpn: Removed subnetopdataentry successfully from Datastore"
                            + " for subnet {} subnetIp {} vpnName {}", LOGGING_PREFIX, subnetId.getValue(), subnetIp,
                            vpnName);
            } catch (Exception ex) {
