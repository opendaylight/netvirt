...
import static org.opendaylight.netvirt.vpnmanager.LdhDataTreeChangeListenerBase.ListenerDependencyHelperCounters.*;
...
public abstract class LdhDataTreeChangeListenerBase<T extends DataObject, K extends DataTreeChangeListener>
     ...
     * to avoid looping of events between listener and defer-infra, delay of DEFER_DELAY_TIME_RESOLVED_EVENT is introduced
     */
    ...
    public enum ListenerDependencyHelperCounters {
        events_dequeued,
        ...
        ListenerDependencyHelperCounters() {
    ...
    static private Runnable listenerDependencyHandlerThread = new Runnable() {
        public void run() {
     ...
     * @return int :: returns success or failure based on the registration.
     **/
        ...
        while(dependentIIDListIt.hasNext()) {
            ...
            synchronized (dependentIID.wildCardPath) {
                ...
                if (wildCradListener == null)  {
                    ...
                    LOG.trace("LDH: registered listener for deferEvent with key: {} \t, wildcard path: {}\t, DS Type: {}\t" +
                                    "number of IID's waiting for wildCardListener are:{} ", dependentIID.iid,
                            dependentIID.wildCardPath, dependentIID.getDsType(), wildCradListener.getNumOfDependentIIDs());
                } else {
                    /* WHY BELOW MD-SAL read is required (?)
                      ...
                      At time T+x, LDH thread wokeUP and saw DeferEvent and noticied WildCardPath-w is already registered.
                      ...
                      Assume, between T and T+x: the requested event was satisfied.
                         In this scenario, as we Re-Use existing Listener and timeGap of 'x' between application and infra
            ...
            synchronized (dependentIID.wildCardPath) {
                ...
                if (expiredEventListener != null) {
                    ...
                    if (expiredEventListener.getNumOfDependentIIDs() == 0) {
                        //unRegister Listener
                        try {
                            LOG.trace("LDH: Unregister Listener for wildcardPath:{}, listener: {}",
    ...
    protected void runPendingTasks(LdhDataTreeChangeListenerBase listener) {
        ...
        while (listenerDependencyHelperQueue.size() > 0) {
            ...
            try {
                ...
                LOG.error("LDH: dequeuing element from listenerDependencyHelperQueue failed");
            ...
            if (((event.getDependentIIDs() != null) || (event.getDependentIIDs().size() > 0)) ||
                    ...
                    ((event.getDependentIIDs().size() == 0) && (event.getDb() == null) && (event.isDeferTimerBased()))) {
    ...
    /* check whether delayResolvedDeferedEventQueue has elements which are waiting beyond DEFER_DELAY_TIME_RESOLVED_EVENT
     * if wait time beyond, DEFER_DELAY_TIME_RESOLVED_EVENT -> execute call back for those events.
     */
        if (enableDelayInResolvingEvent) {
            if (!delayResolvedDeferedEventQueue.isEmpty()) {
                ...
                if ((System.currentTimeMillis() - timedResolvedDeferEvent.queuedTimestamp) > DEFER_DELAY_TIME_RESOLVED_EVENT) {
     ...
     */
        ...
        while (waitingActualIidMapIt.hasNext()) {
            ...
            if (currentTime > pendingActualIIDEvent.getExpiryTime()) {
                LOG.trace("LDH: Expired event key: {} type: {}, event Queued Time: {}, currentTime: {}, listener: {}",
                    ...
                    while (pendingAcutalIIDQueueIt.hasNext()) {
                        ...
                        if (currentTime > expiredActualIIDEvent.getExpiryTime()) {
                            ...
                            LOG.error("LDH: Expired event, removing key: {} type: {}, event Queued Time: {}, " +
                                    ...
                                    expiredActualIIDEvent.getEventType(), new Date(expiredActualIIDEvent.getExpiryTime()),
                            ...
                            if ((expiredActualIIDEvent.isDeferTimerBased())) {
                                ...
                                if (expiredActualIIDEvent.getDependentIIDs() != null) {
                                    ...
                                    LOG.error("LDH: expiredActualIIDEvent has null dependency list, key:{}, listener: {}",
                                            expiredActualIIDEvent.key, liName);
                                }
                            }
                        } else {
            ...
            switch (mod.getModificationType()) {
                case DELETE:
                    if (isEventToBeProcessed(key, null, null,
                            DeferedEvent.EventType.REMOVE, DEFER_EVENT_IID_REMOVE_WAIT_TIME, DEFER_EVENT_DS_RETRY_COUNT)) {
                ...
                case WRITE:
                    if (mod.getDataBefore() == null) {
                        ...
                        if (isEventToBeProcessed(key, mod.getDataBefore(), mod.getDataAfter(),
                            ...
                            LOG.error("LDH: Event Update either Queued/Suppressed, key:{}, eventType:{}", key, "UPDATE");
                        }
                    }
                    break;
                default:
     ...
     * @param retry_count: number of times want to retry
     *
     */
    protected boolean isEventToBeProcessed(InstanceIdentifier<T> key, T oldData,
        ...
        if (whatToDoWithEvent == DEFER_EVENT_HAS_TO_BE_QUEUED) {
            ...
            deferEvent(key, oldData, newData, eventType, retryTimeInterval, null, retry_count, true/*timer*/, null);
     ...
     **/
    public static void ldhDebug(String detail){
        System.out.println("-----------------------------------------------------------------------");
        System.out.println(String.format("                      %s   %14s  ", "Counter Name", "Value"));
        System.out.println("-----------------------------------------------------------------------");
        System.out.println(String.format("events dequeued from LDH              : %s", events_dequeued.get()));
        System.out.println(String.format("events enqueued to IID Q-Key MAP      : %s", events_enqueued_waitingKeyQ.get()));
        System.out.println(String.format("events dequeued from IID Q-Key MAP    : %s", events_dequeued_waitingKeyQ.get()));
        System.out.println(String.format("Listener: resolved during transistion : %s", event_dependent_iids_resolved_transistion.get()));
        System.out.println(String.format("Listener: based events                : %s", events_listenerbased.get()));
        System.out.println(String.format("Listener: registered Listener         : %s", events_registerd_listener.get()));
        System.out.println(String.format("Listener: reUsed Listener             : %s", events_reuse_listener.get()));
        System.out.println(String.format("Listener: freed Listener              : %s", events_freed_listener.get()));
        System.out.println(String.format("Listener: Unregister Listener         : %s", events_unregisterd_listener.get()));
        System.out.println(String.format("Listener: waiting for Add call back   : %s", events_listener_add.get()));
        System.out.println(String.format("Listener: waiting for Delete call back: %s", events_listener_delete.get()));
        System.out.println(String.format("Listener: Add call back received      : %s", wildcard_listener_add.get()));
        System.out.println(String.format("Listener: Delete call back received   : %s", events_listener_delete.get()));
        System.out.println(String.format("Timer Expired events                  : %s", events_expired_timer.get()));
        System.out.println(String.format("Timer Dependency Resolved events      : %s", timer_dependency_resolved.get()));
        System.out.println(String.format("Listener Dependency Resolved events   : %s", listener_dependency_resolved.get()));
        System.out.println(String.format("Infra queued (ordering) Events        : %s", nonApplication_queued_events.get()));
        System.out.println(String.format("Infra resolved (ordering) Events      : %s", nonApplication_resolved_events.get()));
        System.out.println(String.format("Dependent IIDs-ADD resolved transition: %s", dependent_iid_add_resolved_transision.get()));
        System.out.println(String.format("Dependent IIDs-DEL resolved transition: %s", dependent_iid_del_resolved_transision.get()));

        System.out.println("-----------------------------------------------------------------------");

        if (detail != null){
            // print detailed contents
            System.out.println("------------------Listener Dependency Helper Current State-------------");
            for (LdhDataTreeChangeListenerBase listener : listeners) {
                ConcurrentHashMap <InstanceIdentifier, ConcurrentLinkedQueue<DeferedEvent>> waitingActualIIDMap =
                        listener.getWaitingActualIIDMap();
                System.out.println(String.format("------------------%s-------------", listener.clazz.getSimpleName()));
                /* IID-List of DeferedEvents */
                System.out.println(String.format("         %s   %14s  ", "LDH Read Queue size",
                        listener.getListenerDependencyHelperQueue().size()));

                System.out.println(String.format("         %s   %14s  ", "Waiting Events size",
                        waitingActualIIDMap.size()));
                System.out.println("Events waiting for depenency Resolution (both listener/timer)");
                if (waitingActualIIDMap.size() > 0) {
                    for(InstanceIdentifier iid: waitingActualIIDMap.keySet()){
                        ...
                        while (keyWaitEventIt.hasNext()) {
                            ...
                            System.out.println(String.format("Waiting Event: %s, queuedTime: %s", iid,
                            ...
                            for(DependencyData keyWaitEventDependentIid: keyWaitEventDeferDependentIids) {
                                System.out.println(String.format("\tDepedent IID : %s", keyWaitEventDependentIid.getIid()));
                            }
                        }
                    }

                } else {
                    System.out.println("There are NO Events waiting for depenency Resolution (both listener/timer)");
                }

                /* IID-Listener */
                ConcurrentHashMap <InstanceIdentifier, WildCardListener> wildCardListenerMap
                        = listener.getWildCardListenerMap();
                System.out.println("Events waiting for depenency Resolution in WildCard Listener)");
                if (wildCardListenerMap.size() > 0) {
                    for (InstanceIdentifier wildCardPath : wildCardListenerMap.keySet()) {
                        ...
                        System.out.println(String.format("wildCardPath: %s, listener: %s", wildCardPath,
                                keyWildListener.getClass().getSimpleName()));
                        System.out.println(String.format("wildCard Path: %s, listener: %s", keyWildListener.wildCardPath,
                                keyWildListener.getNumOfDependentIIDs()));
                        /* Waiting Add IIDs in WildCardListener (waitingForAddIids)*/
                        HashMap<InstanceIdentifier, List<DeferedEvent>> waitingForAddIids =  keyWildListener.waitingForAddIids;
                        for(InstanceIdentifier iidWaitingAddListener: waitingForAddIids.keySet()){
                            ...
                            for(DeferedEvent deferedEvent: keyWaitingAddIidList ) {
                                System.out.println(String.format("Waiting add Events: %s, queuedTime: %s", deferedEvent.key,
                                        deferedEvent.getQueuedTime()));
                            }
                        }

                        /* Waiting Delete IIDs in WildCardListener (waitingForDeleteIids)*/
                        HashMap<InstanceIdentifier, List<DeferedEvent>> waitingForDeleteIids =  keyWildListener.waitingForDeleteIids;
                        for(InstanceIdentifier iidWaitingDeleteListener: waitingForDeleteIids.keySet()){
                            List<DeferedEvent> keyWaitingDeleteIidList = waitingForDeleteIids.get(iidWaitingDeleteListener);
                            for(DeferedEvent deferedEvent: keyWaitingDeleteIidList) {
                                System.out.println(String.format("Waiting delete Events: %s, queuedTime: %s", deferedEvent.key,
                                        deferedEvent.getQueuedTime()));
                            }
                        }
                    }
                    ...
                    System.out.println("There are NO Listeners registered with IID");
                }
