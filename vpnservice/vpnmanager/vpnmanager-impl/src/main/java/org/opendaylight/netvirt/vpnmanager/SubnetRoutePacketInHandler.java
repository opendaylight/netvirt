...
public class SubnetRoutePacketInHandler implements PacketProcessingListener {
                ...
                if (pkt instanceof IPv4) {
                    ...
                    if (!vpnIdsOptional.isPresent()) {
                        ...
                        SubnetRoutePacketInHandler.LOG.debug(
                                "Ignoring IPv4 packet with destination Ip {} and source Ip {} as it came on "
                                + "unknown VPN with ID {}",
                                new Object[] { dstIpStr, srcIpStr, vpnId });
                        return;
                    }
                    ...
                    if (VpnUtil.getNeutronPortFromVpnPortFixedIp(this.dataBroker, vpnIdVpnInstanceName,
                        ...
                        VpnManagerCounters.subnet_route_packet_ignored.inc();
                        SubnetRoutePacketInHandler.LOG.debug(
                                "IPv4 Packet received with Target IP {} is a valid Neutron port, "
                                + "ignoring subnet route processing",
                                (Object) dstIpStr);
                        return;
                    }
                    ...
                    if (VpnUtil.getLearntVpnVipToPort(this.dataBroker, vpnIdVpnInstanceName, dstIpStr) != null) {
                        VpnManagerCounters.subnet_route_packet_ignored.inc();
                        SubnetRoutePacketInHandler.LOG.debug(
                                "IPv4 Packet received with Target IP {} is an already discovered IPAddress, "
                                + "ignoring subnet route processing",
                                (Object) dstIpStr);
                        return;
                    }
                    ...
                    if (!((VpnIds) vpnIdsOptional.get()).isExternalVpn()) {
                        this.handleInternalVpnSubnetRoutePacket(metadata, dstIp, dstIpStr, vpnIdVpnInstanceName,
                                elanTag);
                        return;
                    }
    ...
    private void handleInternalVpnSubnetRoutePacket(BigInteger metadata, byte[] dstIp,
            String dstIpStr, String vpnIdVpnInstanceName, long elanTag)
        ...
        if (vpnInterface.getVpnInstanceName().equals(vpnIdVpnInstanceName)) {
            this.handleInternalNetwork(vpnInterfaceName, vpnInstanceNameUuid, dstIp, elanTag);
        } else {
            this.handleExternalNetwork(vpnInstanceNameUuid, vpnInterface, dstIp, elanTag);
        }
    }

    private void transmitArpPacket(BigInteger dpnId, String sourceIpAddress, String sourceMac,
            byte[] dstIp, long elanTag) throws UnknownHostException {
    ...
    private void handleInternalNetwork(String vpnInterfaceName, Uuid vpnInstanceNameUuid,
            byte[] dstIp, long elanTag) throws UnknownHostException {
        Optional<Uuid> subnetId = this.getSubnetId(vpnInterfaceName);
        if (!subnetId.isPresent()) {
            SubnetRoutePacketInHandler.LOG.debug("Couldn't find subnet id for vpn interface {}",
                    (Object) vpnInterfaceName);
            VpnManagerCounters.subnet_route_packet_arp_failed.inc();
            return;
        }
        InstanceIdentifier<SubnetOpDataEntry> subOpIdentifier =
                (InstanceIdentifier<SubnetOpDataEntry>) InstanceIdentifier.builder((Class) SubnetOpData.class)
                        .child((Class) SubnetOpDataEntry.class, new SubnetOpDataEntryKey((Uuid) subnetId.get()))
                        .build();
        Optional<SubnetOpDataEntry> optionalSubs =
                VpnUtil.read(this.dataBroker, LogicalDatastoreType.OPERATIONAL, subOpIdentifier);
        if (!optionalSubs.isPresent()) {
            SubnetRoutePacketInHandler.LOG.debug("Couldn't find subnet {}", subnetId.get());
            VpnManagerCounters.subnet_route_packet_arp_failed.inc();
            return;
        }
        BigInteger dpnId = ((SubnetOpDataEntry) optionalSubs.get()).getNhDpnId();
        if (Objects.equals(dpnId, BigInteger.ZERO)) {
            VpnManagerCounters.subnet_route_packet_arp_failed.inc();
            SubnetRoutePacketInHandler.LOG.debug("No dpn id for subnet {}.", subnetId.get());
            return;
        }
        InstanceIdentifier<Subnetmap> id =
                (InstanceIdentifier<Subnetmap>) InstanceIdentifier.builder((Class) Subnetmaps.class)
                        .child((Class) Subnetmap.class, new SubnetmapKey((Uuid) subnetId.get())).build();
        Optional<Subnetmap> subnetMap = VpnUtil.read(this.dataBroker, LogicalDatastoreType.CONFIGURATION, id);
        if (!subnetMap.isPresent()) {
            SubnetRoutePacketInHandler.LOG.debug("Couldn't find subnet map for subnet {}", subnetId.get());
            VpnManagerCounters.subnet_route_packet_arp_failed.inc();
            return;
        }
        ...
        this.transmitArpPacket(dpnId, sourceIp, sourceMac, dstIp, elanTag);
    }

    private void handleExternalNetwork(Uuid vpnInstanceNameUuid, VpnInterface vpnInterface,
            byte[] dstIp, long elanTag) throws UnknownHostException {
        ...
        java.util.Optional<ExternalIps> externalIp =
                (java.util.Optional<ExternalIps>) externalRouter.getExternalIps().stream()
                        .filter(eip -> vpnInstanceNameUuid.equals((Object) eip.getSubnetId())).findFirst();
        ...
        BigInteger dpnId =
                this.centralizedSwitchProvider.getPrimarySwitchForRouter(externalRouter.getRouterName());
        if (Objects.equals(dpnId, BigInteger.ZERO)) {
            VpnManagerCounters.subnet_route_packet_arp_failed.inc();
