...
public class SubnetRoutePacketInHandler implements PacketProcessingListener {
    ...
    public void onPacketReceived(PacketReceived notification) {
        ...
        if (tableId == NwConstants.L3_SUBNET_ROUTE_TABLE) {
            LOG.trace("Some packet received as {}", notification);
            try {
            ...
            try {
                ...
                if (pkt instanceof IPv4) {
                    ...
                    LOG.info("Processing IPv4 Packet received with Source IP {} " + "and Target IP {} and vpnId {}",
                    ...
                    LOG.info("Processing IPv4 Packet received with Source IP {} " + "and Target IP {} and elan Tag {}",
                            srcIpStr, dstIpStr, elanTag);
                    ...
                    if (targetSubnetForPacketOut != null) {
                        ...
                        transmitArpPacket(targetSubnetForPacketOut.getNhDpnId(), srcIpStr,
                                NWUtil.toStringMacAddress(srcMac), dstIp, elanTag);
                    } else {
                        VpnManagerCounters.subnet_route_packet_arp_failed.inc();
                        LOG.debug("Could not find target subnet for packet out {}", dstIpStr);
                    }
                }
            } catch (Exception ex) {
    ...
    private static SubnetOpDataEntry getTargetSubnetForPacketOut(DataBroker broker, long elanTag, int ipAddress) {
        ...
        if (elanInfo == null) {
            LOG.trace("Unable to retrieve ElanInfo for elanTag {}", elanTag);
            return null;
        }
