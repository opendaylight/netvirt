...
public class SubnetRoutePacketInHandler implements PacketProcessingListener {
    ...
    public void onPacketReceived(PacketReceived notification) {
        ...
        LOG.trace("{} onPacketReceived: Packet punted from table {}", loggingPrefix, tableId);
        ...
        if (tableId == NwConstants.L3_SUBNET_ROUTE_TABLE) {
            LOG.trace("{} onPacketReceived: Some packet received as {}", loggingPrefix, notification);
            try {
                res.deserialize(data, 0, data.length * NetUtils.NumBitsInAByte);
            } catch (PacketException e) {
                LOG.error("{} onPacketReceived: Failed to decode Packet ", loggingPrefix, e);
            ...
            try {
                ...
                if (pkt instanceof IPv4) {
                    ...
                    LOG.info("{} onPacketReceived: Processing IPv4 Packet received with Source IP {} and Target IP {}"
                            + " and vpnId {}", loggingPrefix, srcIpStr, dstIpStr, vpnId);
                    ...
                    if (!vpnIdsOptional.isPresent()) {
                        ...
                        LOG.info("{} onPacketReceived: Ignoring IPv4 packet with destination Ip {} and source Ip {}"
                                + " as it came on unknown VPN with ID {}", loggingPrefix, dstIpStr, srcIpStr, vpnId);
                        return;
                    }
                    ...
                    if (VpnUtil.getNeutronPortFromVpnPortFixedIp(dataBroker, vpnIdVpnInstanceName, dstIpStr) != null) {
                        ...
                        LOG.info("{} onPacketReceived: IPv4 Packet received with Target IP {} source IP {} vpnId {} "
                                + "is a valid Neutron port,ignoring subnet route processing", loggingPrefix, dstIpStr,
                                srcIp, vpnId);
                        return;
                    }
                    ...
                    if (VpnUtil.getLearntVpnVipToPort(dataBroker, vpnIdVpnInstanceName, dstIpStr) != null) {
                        ...
                        LOG.info("{} onPacketReceived: IPv4 Packet received with Target IP {} source Ip {} vpnId {}"
                                ...
                                loggingPrefix, dstIpStr, srcIp, vpnId);
                        return;
                    }
                    ...
                    if (elanTag == 0L) {
                        ...
                        LOG.error("{} onPacketReceived: elanTag value from metadata found to be 0, for IPv4 "
                                + " Packet received with Target IP {} src Ip vpnId", loggingPrefix, dstIpStr, srcIp,
                                vpnId);
                        return;
                    }
                ...
                VpnManagerCounters.subnet_route_packet_failed.inc();
                LOG.error("{} onPacketReceived: Failed to handle subnetroute packet.", loggingPrefix, ex);
            }
            return;
        }
    ...
    private void handleBgpVpnSubnetRoute(IPv4 ipv4, byte[] srcMac, byte[] dstIp, String dstIpStr, String srcIpStr,
        ...
        LOG.info("{} handleBgpVpnSubnetRoute: Processing IPv4 Packet received with Source IP {} and Target IP {}"
                + " and elan Tag {}", loggingPrefix, srcIpStr, dstIpStr, elanTag);
        ...
        if (targetSubnetForPacketOut != null) {
            ...
            VpnManagerCounters.subnet_route_packet_failed.inc();
            LOG.debug("{} handleBgpVpnSubnetRoute: Could not find target subnet for packet out {}", loggingPrefix,
                    dstIpStr);
        }
    }
    ...
    private void handlePacketToExternalNetwork(Uuid vpnInstanceNameUuid, VpnInterface vmVpnInterface, byte[] dstIp,
        ...
        if (externalRouter == null) {
            VpnManagerCounters.subnet_route_packet_failed.inc();
            LOG.debug("{} handlePacketToExternalNetwork: Can't find external router with id {}", loggingPrefix,
                    routerId);
            return;
        }
        ...
        if (externalIps == null || externalIps.isEmpty()) {
            ...
            LOG.debug("{} handlePacketToExternalNetwork: Router {} doesn't have any external ips.",
                    loggingPrefix, externalRouter.getRouterName());
            return;
        }
        ...
        if (!externalIp.isPresent()) {
            ...
            LOG.debug("{} handlePacketToExternalNetwork: Router {} doesn't have an external ip for subnet id {}.",
                    loggingPrefix, externalRouter.getRouterName(), vpnInstanceNameUuid);
            return;
        }
    ...
    private static SubnetOpDataEntry getTargetSubnetForPacketOut(DataBroker broker, long elanTag, int ipAddress) {
        ...
        if (elanInfo == null) {
            LOG.error("{} getTargetDpnForPacketOut: Unable to retrieve ElanInfo for elanTag {}", loggingPrefix,
                    elanTag);
            return null;
        }
        ...
        LOG.debug("{} getTargetDpnForPacketOut: Obtained subnetList as {} for network {}", loggingPrefix, subnetList,
                elanInfo.getName());
        for (Uuid subnetId : subnetList) {
            ...
            if (subOpEntry.getNhDpnId() != null) {
                LOG.trace("{} getTargetDpnForPacketOut: Viewing Subnet {}", loggingPrefix, subnetId.getValue());
                boolean match = NWUtil.isIpInSubnet(ipAddress, subOpEntry.getSubnetCidr());
                LOG.trace("{} getTargetDpnForPacketOut: Viewing Subnet {} matching {}", loggingPrefix,
