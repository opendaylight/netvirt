...
public class VpnFootprintService {
    ...
    private void removeOrUpdateVpnToDpnList(long vpnId, BigInteger dpnId, String intfName, String vpnName) {
        ...
        synchronized (vpnName.intern()) {
            ...
            if (dpnInVpn == null) {
                LOG.warn("removeOrUpdateVpnToDpnList: Could not find DpnToVpn map for VPN=[name={} rd={} id={}].",
                         vpnName, rd, id);
                return;
            }
            List<VpnInterfaces> vpnInterfaces = dpnInVpn.getVpnInterfaces();
            if (vpnInterfaces == null) {
                LOG.warn("Could not find vpnInterfaces for DpnInVpn map for VPN=[name={} rd={} id={}]",
                         vpnName, rd, id);
                return;
            }
            ...
            if (vpnInterfaces.remove(currVpnInterface)) {
                WriteTransaction writeTxn = dataBroker.newWriteOnlyTransaction();
                boolean submitTx = true; // SubmitTx only if needed
                if (vpnInterfaces.isEmpty()) {
                    List<IpAddresses> ipAddresses = dpnInVpn.getIpAddresses();
                    if (ipAddresses == null || ipAddresses.isEmpty()) {
                        VpnToDpnListBuilder dpnInVpnBuilder =
                            new VpnToDpnListBuilder(dpnInVpn).setDpnState(VpnToDpnList.DpnState.Inactive)
