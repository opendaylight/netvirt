...
public class ArpNotificationHandler implements OdlArputilListener {
    ...
    // temp IPs cache where Key is VPNInstance+IP and value is timestamp
    private Cache<Pair<String, String>, BigInteger> migrArpReqCache =
            CacheBuilder.newBuilder().maximumSize(10000).expireAfterWrite(60, TimeUnit.SECONDS).build();
    private Cache<Pair<String, String>, BigInteger> migrArpRespCache =
            CacheBuilder.newBuilder().maximumSize(10000).expireAfterWrite(60, TimeUnit.SECONDS).build();
                    ...
                    if (!oldMac.equalsIgnoreCase(srcMac.getValue())) {
                        ...
                        if (!vpnPortipToPort.isConfig()) {
                            synchronized ((vpnName + ipToQuery).intern()) {
                                ...
                                LOG.debug("ARP_MIGR_CACHE: add to dirty cache IP {} with MAC {}", ipToQuery, srcMac);
                                migrArpReqCache.put(new ImmutablePair<>(vpnName, ipToQuery),
...
//                            }
                ...
                } else if (isLearnMacFromArpPackets(vpnName, ipToQuery, migrArpReqCache)) {
                    ...
                    if (!oldMac.equalsIgnoreCase(srcMac.getValue())) {
                        ...
                        if (!vpnPortipToPort.isConfig()) {
                            synchronized ((vpnName + ipToQuery).intern()) {
                                ...
                                LOG.debug("ARP_MIGR_CACHE: add to dirty cache IP {} with MAC {}", ipToQuery, srcMac);
                                migrArpRespCache.put(new ImmutablePair<>(vpnName, ipToQuery),
                                        new BigInteger(String.valueOf(System.currentTimeMillis())));
                            }
//                            try {
//                                Thread.sleep(2000);
...
//                            }
                ...
                } else if (isLearnMacFromArpPackets(vpnName, ipToQuery, migrArpRespCache)) {
                    learnMacFromArpPackets(vpnName, srcInterface, srcIP, srcMac);
                }
            }
        }
    ...
    private boolean isLearnMacFromArpPackets(String vpnName, String ipToQuery, Cache<Pair<String, String>, BigInteger> migrArpCache) {
        boolean needToLearn = true;
        if (migrArpCache == null || migrArpCache.size() == 0)
            return needToLearn;
        long sizeCached = migrArpCache.size();
        Pair<String, String> keyPair = new ImmutablePair<>(vpnName, ipToQuery);
        BigInteger prevTimeStampCached = migrArpCache.getIfPresent(keyPair);
        if (prevTimeStampCached != null) {
            if (System.currentTimeMillis() < prevTimeStampCached.longValue() + 2000) {
                LOG.debug("ARP_MIGR_CACHE: younger than 2 sec - ignore learning IP {}, size of cache {}", ipToQuery, sizeCached);
                needToLearn = false;
            } else {
                LOG.debug("ARP_MIGR_CACHE: older than 2sec - remove from dirty cache IP {}, size of cache {}", ipToQuery, sizeCached);
                migrArpCache.invalidate(keyPair);
            }
        } else {
            LOG.debug("ARP_MIGR_CACHE: there is no IP {} in dirty cache, so learn it", ipToQuery);
        }
        return needToLearn;
    }
}
