...
public class ArpNotificationHandler implements OdlArputilListener {
    ...
    // dirty IPs hashmap where Key is VPNInstance+IP and value is Timestamp
    private Map<Pair<String, String>, BigInteger> migratedArpReqTimeMap  = Maps.newHashMap();
    private Map<Pair<String, String>, BigInteger> migratedArpRespTimeMap = Maps.newHashMap();
                    ...
                    if (!oldMac.equalsIgnoreCase(srcMac.getValue())) {
                        ...
                        if (!vpnPortipToPort.isConfig()) {
                            synchronized ((vpnName + ipToQuery).intern()) {
                                ...
                                LOG.debug("ARP_MIGR: add to dirty map IP {} with MAC {}", ipToQuery, srcMac);
                                migratedArpReqTimeMap.put(new ImmutablePair<>(vpnName, ipToQuery),
...
//                            }
                ...
                } else if (isLearnMacFromArpPackets(vpnName, ipToQuery, migratedArpReqTimeMap)) {
                    ...
                    if (!oldMac.equalsIgnoreCase(srcMac.getValue())) {
                        ...
                        if (!vpnPortipToPort.isConfig()) {
                            synchronized ((vpnName + ipToQuery).intern()) {
                                ...
                                LOG.debug("ARP_MIGR: add to dirty map IP {} with MAC {}", ipToQuery, srcMac);
                                migratedArpRespTimeMap.put(new ImmutablePair<>(vpnName, ipToQuery),
                                        new BigInteger(String.valueOf(System.currentTimeMillis())));
                            }
//                            try {
//                                Thread.sleep(2000);
...
//                            }
                ...
                } else if (isLearnMacFromArpPackets(vpnName, ipToQuery, migratedArpRespTimeMap)) {
                    learnMacFromArpPackets(vpnName, srcInterface, srcIP, srcMac);
                }
            }
        }
    ...
    private boolean isLearnMacFromArpPackets(String vpnName, String ipToQuery, Map<Pair<String, String>, BigInteger> migratedArpTimeMap) {
        boolean needToLearn = true;
        Pair<String, String> keyPair = new ImmutablePair<>(vpnName, ipToQuery);
        if (migratedArpTimeMap != null && migratedArpTimeMap.containsKey(keyPair)) {
            BigInteger prevTimeStamp = migratedArpTimeMap.get(keyPair);
            if (System.currentTimeMillis() < prevTimeStamp.longValue() + 2000) {
                LOG.debug("ARP_MIGR: younger than 2 sec - ignore learning IP {}", ipToQuery);
                needToLearn = false;
            } else {
                LOG.debug("ARP_MIGR: older than 2sec - remove from dirty map IP {}", ipToQuery);
                migratedArpTimeMap.remove(keyPair);
            }
        }
        return needToLearn;
    }
