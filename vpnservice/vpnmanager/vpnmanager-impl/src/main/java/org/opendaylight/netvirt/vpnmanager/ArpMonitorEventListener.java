...
public class ArpMonitorEventListener implements AlivenessMonitorListener {
    ...
    public void onMonitorEvent(MonitorEvent notification) {
        ...
        if(macEntry == null)
        {
            LOG.debug("No MacEntry found assosciated with the monitor Id{}",monitorId);
            return;
        }
        ...
        if(livenessState.equals(LivenessState.Down)) {
            ...
            coordinator.enqueueJob(ArpMonitoringHandler.buildJobKey(macEntry.getIpAddress().getHostAddress(),macEntry.getVpnName()),
