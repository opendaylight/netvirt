    ...
    protected void add(InstanceIdentifier<Interface> identifier, Interface intrf) {
        ...
        try {
            if (intrf != null && intrf.getType() != null && (!intrf.getType().equals(Tunnel.class))) {
                ...
                if (portOpEntry == null) {
                    LOG.info("SubnetRouteInterfaceListener add: Port " + intrf.getName()
                            + " is part of a subnet not in VPN, ignoring");
                    return;
                }

                DataStoreJobCoordinator dataStoreCoordinator = DataStoreJobCoordinator.getInstance();
                dataStoreCoordinator.enqueueJob("SUBNETROUTEINTF-" + portOpEntry.getSubnetId().getValue(),
                                ...
                                if (configInterface != null && (configInterface.getType() != null)) {
                                    if (!configInterface.getType().equals(Tunnel.class)) {
                                        ...
                                        if (vpnInterface != null) {
                                            ...
                                            vpnSubnetRouteHandler.onInterfaceUp(dpnId, intrf.getName(), writeOperTxn);
    ...
    protected void remove(InstanceIdentifier<Interface> identifier, Interface intrf) {
        ...
        try {
            if (intrf != null && intrf.getType() != null && (!intrf.getType().equals(Tunnel.class))) {
                ...
                PortOpDataEntry portOpEntry = subOpDpnManager.getPortOpDataEntry(intrf.getName());
                if (portOpEntry == null) {
                    LOG.info("SubnetRouteInterfaceListener remove: Port " + intrf.getName()
                            + " is part of a subnet not in VPN, ignoring");
                    return;
                }
                        ...
                        new Callable<List<ListenableFuture<Void>>>() {
                            ...
                            public List<ListenableFuture<Void>> call() throws Exception {
                                ...
                                if (optVpnInterface.isPresent()) {
                                    ...
                                    vpnSubnetRouteHandler.onInterfaceDown(dpnId, intrf.getName(), writeOperTxn);
                                    futures.add(writeOperTxn.submit());
                                } else {
    ...
    protected void update(InstanceIdentifier<Interface> identifier,
        ...
        if (update != null && (update.getType() != null)) {
            if (!update.getType().equals(Tunnel.class)) {
                ...
                if (portOpEntry == null) {
                    LOG.info("SubnetRouteInterfaceListener update: Port " + interfaceName
                            + " is part of a subnet not in VPN, ignoring");
                    return;
                }
                            ...
                            public List<ListenableFuture<Void>> call() throws Exception {
                                ...
                                if (vpnInterface != null) {
                                    ...
                                    if (update.getOperStatus().equals(Interface.OperStatus.Up)) {
                                        vpnSubnetRouteHandler.onInterfaceUp(dpnId, update.getName(), writeOperTxn);
                                    } else if (update.getOperStatus().equals(Interface.OperStatus.Down)) {
                                        vpnSubnetRouteHandler.onInterfaceDown(dpnId, update.getName(), writeOperTxn);
                                    }
                                    futures.add(writeOperTxn.submit());
                                } else {
