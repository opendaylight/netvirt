...
public class SubnetRouteInterfaceStateChangeListener extends AsyncDataTreeChangeListenerBase<Interface,
    ...
    protected void add(InstanceIdentifier<Interface> identifier, Interface intrf) {
        LOG.trace("{} add: Received interface {} up event", loggingPrefix, intrf);
        final Uuid subnetId;
        try {
                    ...
                    dataStoreCoordinator.enqueueJob("SUBNETROUTE-" + subnetId,
                        () -> {
                            ...
                            LOG.info("{} add: Received port UP event for interface {} subnetId {}",
                                    loggingPrefix, interfaceName, subnetId.getValue());
                            try {
            ...
            LOG.info("{} add: Processed interface {} up event", loggingPrefix, intrf.getName());
        } catch (Exception e) {
            LOG.error("{} add: Exception observed in handling addition for VPN Interface {}.", loggingPrefix,
                intrf.getName(), e);
        }
    }
    ...
    protected void remove(InstanceIdentifier<Interface> identifier, Interface intrf) {
        ...
        try {
            if (L2vlan.class.equals(intrf.getType())) {
                LOG.trace("{} remove: Received interface {} down event", loggingPrefix, intrf);
                subnetId = getSubnetId(intrf);
                if (subnetId == null) {
                    LOG.error("{} remove: Port {} doesnt exist in configDS", loggingPrefix, intrf.getName());
                    return;
                }
                DataStoreJobCoordinator dataStoreCoordinator = DataStoreJobCoordinator.getInstance();
                dataStoreCoordinator.enqueueJob("SUBNETROUTE-" + subnetId,
                    () -> {
                        ...
                        try {
                            ...
                            LOG.error("{} remove: Unable to retrieve dpnId for interface {} in subnet {}. "
                                    ...
                                    loggingPrefix, intrf.getName(), subnetId.getValue(), e);
            ...
            LOG.info("{} remove: Processed interface {} down event in ", loggingPrefix, intrf.getName());
        } catch (Exception e) {
            LOG.error("{} remove: Exception observed in handling deletion of VPN Interface {}.", loggingPrefix,
                intrf.getName(), e);
        }
    }
    ...
    protected void update(InstanceIdentifier<Interface> identifier,
        ...
        try {
            ...
            if (L2vlan.class.equals(update.getType())) {
                LOG.trace("{} update: Operation Interface update event - Old: {}, New: {}", loggingPrefix,
                    original, update);
                subnetId = getSubnetId(update);
                if (subnetId == null) {
                    LOG.error("{} update: Port {} doesnt exist in configDS", loggingPrefix, update.getName());
                    return;
                }
                DataStoreJobCoordinator dataStoreCoordinator = DataStoreJobCoordinator.getInstance();
                dataStoreCoordinator.enqueueJob("SUBNETROUTE-" + subnetId,
                    () -> {
                        ...
                        try {
                            ...
                            LOG.error("{} remove: Unable to retrieve dpnId for interface {} in subnet  {}. "
                                    + "Fetching from vpn interface itself due to exception {}", loggingPrefix,
                        ...
                        if (!dpnId.equals(BigInteger.ZERO)) {
                            if (update.getOperStatus().equals(Interface.OperStatus.Up)) {
                                LOG.info("{} update: Received port UP event for interface {} in subnet {}",
                                        loggingPrefix, update.getName(), subnetId.getValue());
                                ...
                                LOG.info("{} update: Received port {} event for interface {} in subnet {} ",
                                        loggingPrefix, update.getOperStatus().equals(Interface.OperStatus.Unknown)
            ...
            LOG.info("{} update: Processed Interface {} update event", loggingPrefix, update.getName());
        } catch (Exception e) {
            LOG.error("{} update: Exception observed in handling deletion of VPNInterface {}", loggingPrefix,
                    update.getName(), e);
        }
    }
    ...
    protected Uuid getSubnetId(Interface intrf) {
        ...
        LOG.trace("{} getSubnetId : Received Port {} event for {} that is not part of subnetRoute", loggingPrefix,
