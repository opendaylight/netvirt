...
public class ArpResponderUtil {
     ...
     * <P>
     * <b>NOTE:</b>A temporary fix until in_port is overridden in table=0
     * 
     * @param actionKey
     *            action key
     * @param inPortVal
     *            in port value to be set
     * @return load in_port action
     */
    public static org.opendaylight.yang.gen.v1.urn.opendaylight.action.types.rev131112.action.list.Action createNxOfInPortAction(
            final int actionKey, final int inPortVal) {

        NxRegLoad r = new NxRegLoadBuilder()
                .setDst(new DstBuilder()
                        .setDstChoice(new DstNxOfInPortCaseBuilder()
                                .setOfInPort(Boolean.TRUE).build())
                        .setStart(Integer.valueOf(0))
                        .setEnd(Integer.valueOf(15)).build())
