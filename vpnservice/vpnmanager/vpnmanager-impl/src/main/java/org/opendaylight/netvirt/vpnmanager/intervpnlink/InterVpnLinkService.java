...
public class InterVpnLinkService {
    ...
    public List<BigInteger> selectSuitableDpns(InterVpnLink interVpnLink) {
        ...
        List<BigInteger> result =
            dpnIdPool.stream().filter(dpId -> excludingDPNs == null || !excludingDPNs.contains(dpId))
                              .limit(numberOfDpns).collect(Collectors.toList());
        ...
        if (currentNbrOfItems < numberOfDpns) {
            ...
            result.addAll(dpnIdPool.subList(0, numberOfDpns - currentNbrOfItems));
        }
        return result;
    }
