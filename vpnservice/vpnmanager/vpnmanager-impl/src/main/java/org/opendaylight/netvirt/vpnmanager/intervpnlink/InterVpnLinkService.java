...
public class InterVpnLinkService {
    ...
    public InterVpnLinkService(final DataBroker dataBroker/*, final VpnOpDataSyncer vpnOpDataSyncer*/) {
    ...
    private List<BigInteger> findDpnsToAvoid(InterVpnLink interVpnLink) {
        ...
        for ( InterVpnLinkDataComposite ivl : sameGroupInterVpnLinks ) {
            resultDpns.addAll(ivl.getFirstEndpointDpns().or(Collections.emptyList()));
            resultDpns.addAll(ivl.getSecondEndpointDpns().or(Collections.emptyList()));
        }
    ...
    public List<String> getIRTsByVpnName(String vpnName) {
