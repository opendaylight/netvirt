...
public class SubnetMacHandler extends AsyncDataTreeChangeListenerBase<VpnPortipToPort, SubnetMacHandler> {
    ...
    protected void add(InstanceIdentifier<VpnPortipToPort> key, VpnPortipToPort value) {
        if (value.isSubnetIp()) {
            ...
            String networkInstanceName = getNetworkInstanceName(value.getPortName());
            if (networkInstanceName != null) {
                VpnUtil.setupSubnetMacIntoVpnInstanceOnDpns(dataBroker, mdsalManager, networkInstanceName,
