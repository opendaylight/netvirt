...
public class ArpResponderHandler {
     ...
     *
     * @see {@link #CONNECTED_MAC} {@link #ROUTER_MAC}
     */
    public static enum MAC_TYPE {
        /**
     ...
     *            mac address
     * @param macType
     *            Mac Address type {@link MAC_TYPE}
     */
    public void addArpResponderFlow(final BigInteger dpnId, final int lportTag, final String vpnName, final long vpnId,
            ...
            final MAC_TYPE macType) {

        if (MAC_TYPE.ROUTER_MAC == macType) {
    ...
    public void removeArpResponderFlow(final BigInteger dpId, final int lportTag, final String ifName,
        ...
        if (gwIp.isPresent()) {
            ...
            final VpnPortipToPort gwPort = VpnUtil.getNeutronPortFromVpnPortFixedIp(dataBroker, vpnName, gwIp.get());
            LOG.trace("Removing ARP Responder for VPNInterface {} with adjacency Gsteway IP {}", ifName, gwIp.get());
    ...
    /**
     * Get Mac address from given gateway port and inteface name
     *
     * @param gwPort
     *            gateway port
     * @param ifName
