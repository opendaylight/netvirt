...
public class ArpResponderHandler {
    ...
    private boolean anyOtherVpnIfaceOnDpnOfSubnet(BigInteger dpnId, String ifaceName, Uuid subnetId, long vpnId) {
        InstanceIdentifier<VpnIds> vpnIdfr = VpnUtil.getPrefixToInterfaceIdentifier(vpnId);
        Optional<VpnIds> vpnIds = VpnUtil.read(dataBroker, LogicalDatastoreType.OPERATIONAL, vpnIdfr);
     ...
     */
    // Retaining this method for future use where flow is add/removed per subnet
    // for router case
    // public void addArpResponderFlow(final BigInteger dpnId, final int
    // lportTag, final String vpnName,
    // final long vpnId, final String interfaceName, final Uuid subnetId, final
    // String gatewayIp,
    // final String mac,final MacType macType) {
    //
    // if (MacType.ROUTER_MAC == macType) {
    // if (!anyOtherVpnIfaceOnDpnOfSubnet(dpnId, interfaceName, subnetId,
    // vpnId)) {
    // LOG.trace("Creating the ARP Responder flow for VPN Interface {}",
    // interfaceName);
    // elanService.addArpResponderFlow(dpnId, interfaceName, gatewayIp, mac,
    ...
    // Retaining this method for future use where flow is add/removed per subnet
    // for router case
    // public void removeArpResponderFlow(final BigInteger dpId, final int
    // lportTag, final String ifName,
    // final String vpnName, final long vpnId, final Uuid subnetUuid) {
    // final Optional<String> gwIp = VpnUtil.getVpnSubnetGatewayIp(dataBroker,
    // subnetUuid);
    // if (gwIp.isPresent()) {
    //
    // if (!anyOtherVpnIfaceOnDpnOfSubnet(dpId, ifName, subnetUuid, vpnId)) {
    // LOG.trace("VPN Interface {} is the last interface on the DPN, removing
    // the ARP Responder flow",
    // ifName, dpId);
    // elanService.removeArpResponderFlow(dpId, ifName, gwIp.get(),
    // java.util.Optional.empty());
    // }
    // elanService.removeArpResponderFlow(dpId, ifName, gwIp.get(),
    // java.util.Optional.of(lportTag));
     ...
     *            VPN Interface
     */
    public void removeArpResponderFlowOnRouterIfRemove(VpnInterface vpnInterface) {
        ...
        if (adjacencies != null && adjacencies.getAdjacency() != null) {
            adjacencies.getAdjacency().stream().filter(Adjacency::isPrimaryAdjacency).findFirst().ifPresent(adj -> {
                java.util.Optional<VpnIds> vpnIds = java.util.Optional
                    ...
                    if (vpn.getPrefixes() != null && adj.getIpAddress() != null) {
                        vpn.getPrefixes().stream()
                                ...
                                .map(Prefixes::getDpnId).distinct().forEach(dpnId -> {
                                    ...
                                    String gwIp = adj.getIpAddress().substring(0, adj.getIpAddress().indexOf('/'));
