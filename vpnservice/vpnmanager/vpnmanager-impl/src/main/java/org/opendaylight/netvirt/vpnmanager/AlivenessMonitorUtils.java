...
public class AlivenessMonitorUtils {
    ...
    public static void startArpMonitoring(MacEntry macEntry,Long profileId, AlivenessMonitorService alivenessMonitorService, DataBroker dataBroker, OdlInterfaceRpcService interfaceRpc) {
        Optional<IpAddress> gatewayIpOptional = VpnUtil.getGatewayIpAddressFromInterface(macEntry.getInterfaceName(), dataBroker);
        IpAddress gatewayIp;
        ...
        Optional<String> gatewayMacOptional = VpnUtil.getGWMacAddressFromInterface(macEntry, gatewayIp, dataBroker, interfaceRpc);
        ...
        {
            Optional<Long> profileIdOptional = allocateProfile(alivenessMonitorService, ArpConstants.FAILURE_THRESHOLD, ArpConstants.ARP_CACHE_TIMEOUT_MILLIS, ArpConstants.MONITORING_WINDOW,
                    EtherTypes.Arp);
    ...
    private static Interface getSourceEndPointType(String interfaceName, IpAddress ipAddress, PhysAddress gwMac) {
        return new InterfaceBuilder().setInterfaceIp(ipAddress).setInterfaceName(interfaceName).setMacAddress(gwMac).build();
    }

    public static Optional<Long> allocateProfile(AlivenessMonitorService alivenessMonitor, long FAILURE_THRESHOLD, long MONITORING_INTERVAL,
    ...
    private static org.opendaylight.yang.gen.v1.urn.opendaylight.genius.alivenessmonitor.rev160411.endpoint.endpoint.type.IpAddress getEndPointIpAddress(IpAddress ip) {
