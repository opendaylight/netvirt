...
public class VpnUtil {
    ...
    static Routes getVpnToExtraroute(String vrfId, String ipPrefix, List<String> nextHopList) {
        return new RoutesBuilder().setKey(new RoutesKey(ipPrefix)).setNexthopIpList(nextHopList).build();
    }
        ...
        if (vrfTablesOpc.isPresent()) {
            ...
            for (VrfEntry vrfEntry : vrfTables.getVrfEntry()) {
                for (RoutePaths routePaths : vrfEntry.getRoutePaths()) {
                    for (NexthopAddresses nexthopAddresses : routePaths.getNexthopAddresses()) {
                        if (nexthopAddresses.getIpAddress() != null && nexthop.equals(nexthopAddresses.getIpAddress())) {
