...
public class VpnUtil {
    ...
    public static VpnInterfaceToGwSrcMacList populateAndGwSrcMac(final DataBroker dataBroker, final String vpnName, final String interfaceName, final String gwIp){
        ...
        if(gwPort !=null && gwPort.isSubnetIp()){
           ...
           gwSrcType = GwSrcType.ROUTER;
        }else{
           macaddress = InterfaceUtils.getMacAddressForInterface(dataBroker, interfaceName).get();
           gwSrcType = GwSrcType.CONNECTED;
        }
        final InstanceIdentifier<VpnInterfaceToGwSrcMacList> path = InstanceIdentifier.create(VpnInterfaceToGwSrcMac.class).child(VpnInterfaceToGwSrcMacList.class, new VpnInterfaceToGwSrcMacListKey(interfaceName));
        VpnInterfaceToGwSrcMacList srcGwMac = new VpnInterfaceToGwSrcMacListBuilder().setGwSrcMac(macaddress).setGwSrcType(gwSrcType).setVpnInterface(interfaceName).setKey(new VpnInterfaceToGwSrcMacListKey(interfaceName)).build();
        MDSALUtil.syncWrite(dataBroker, LogicalDatastoreType.OPERATIONAL, path, srcGwMac);
        return srcGwMac;
    }

    public static void removeGwSrcMac(final DataBroker dataBroker, final String interfaceName){
