...
public final class VpnUtil {
    ...
    public static IpVersionChoice getVpnType(DataBroker dataBroker, String vpnName) {
        ...
        Optional<org.opendaylight.yang.gen.v1.urn.huawei.params.xml.ns
                    ...
                    .VpnInstance> opVpnInstance = read(dataBroker, LogicalDatastoreType.CONFIGURATION, id);
    ...
    public static boolean isBgpVpnInternet(DataBroker dataBroker, String vpnName) {
        ...
        if (!optionalSubnetmaps.isPresent()) {
            LOG.error("isBgpVpnInternet: no subnetmaps");
            return false;
        }
        List<Subnetmap> subnetmapList = optionalSubnetmaps.get().getSubnetmap();
        Uuid routerId = null;
        for (Subnetmap sm : subnetmapList) {
            if (sm.getInternetVpnId() != null && sm.getInternetVpnId().getValue().equals(vpnName)) {
                return true;
            }
            if (sm.getNetworkId() != null
                && (getNeutronNetwork(dataBroker, sm.getNetworkId()) != null)
                && getIsExternal(getNeutronNetwork(dataBroker, sm.getNetworkId()))
                && sm.getVpnId().getValue().equals(vpnName)) {
                return true;
            }
        }
        return false;
