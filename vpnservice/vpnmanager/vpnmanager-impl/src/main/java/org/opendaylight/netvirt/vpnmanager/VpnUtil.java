...
public class VpnUtil {
    ...
    public static void updateVpnInterfacesForUnProcessAdjancencies(DataBroker dataBroker, VpnInterfaceManager vpnInterfaceManager,
                                                       String vpnName) {
        InstanceIdentifier<VpnInterfaces> vpnInterfacesId = InstanceIdentifier.builder(VpnInterfaces.class).build();
        Optional<VpnInterfaces> existingConfigVpnInterfaces = read(dataBroker, LogicalDatastoreType.CONFIGURATION, vpnInterfacesId);
        ...
        vpnInterfaces.stream().filter(vpnInterface -> {return vpnInterface.getVpnInstanceName().equals(vpnName);})
            .forEach(vpnInterface -> {
                InstanceIdentifier<VpnInterface> existingVpnInterfaceId = getVpnInterfaceIdentifier(vpnInterface.getName());
            Optional<VpnInterface> vpnInterfaceOperDS = read(dataBroker, LogicalDatastoreType.OPERATIONAL, existingVpnInterfaceId);
            ...
            if(vpnInterfaceOperDS.isPresent()) {
                ...
                List<Adjacency> operationVPNadjacencies = vpnInterfaceOperationDS.getAugmentation(Adjacencies.class).getAdjacency();
