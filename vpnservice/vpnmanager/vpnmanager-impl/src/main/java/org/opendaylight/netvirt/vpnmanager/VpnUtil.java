...
import org.opendaylight.yang.gen.v1.urn.opendaylight.netvirt.l3vpn.rev130911.vpn.vip.to.port.data.VpnVipToPortBuilder;
import org.opendaylight.yang.gen.v1.urn.opendaylight.netvirt.l3vpn.rev130911.vpn.vip.to.port.data.VpnVipToPortKey;
...
public class VpnUtil {
    ...
    protected static void createVpnPortFixedIpToPort(DataBroker broker, String vpnName, String fixedIp, String
            portName, String macAddress, boolean isSubnetIp) {
        synchronized ((vpnName + fixedIp).intern()) {
            ...
            VpnPortipToPortBuilder builder = new VpnPortipToPortBuilder().setKey(
                    ...
                    (portName).setMacAddress(macAddress.toLowerCase()).setSubnetIp(isSubnetIp);
    ...
    protected static void createVpnVipFixedIpToPort(DataBroker broker, String vpnName, String fixedIp, String
        ...
        synchronized ((vpnName + fixedIp).intern()) {
            InstanceIdentifier<VpnVipToPort> id = buildVpnMipipToPortIdentifier(vpnName, fixedIp);
            VpnVipToPortBuilder builder = new VpnVipToPortBuilder().setKey(
                    new VpnVipToPortKey(fixedIp, vpnName)).setVpnName(vpnName).setPortFixedip(fixedIp).setPortName
    ...
    private static InstanceIdentifier<VpnVipToPort> buildVpnMipipToPortIdentifier(String vpnName, String fixedIp) {
        InstanceIdentifier<VpnVipToPort> id = InstanceIdentifier.builder(VpnVipToPortData.class).child
                (VpnVipToPort.class, new VpnVipToPortKey(fixedIp, vpnName)).build();
        return id;
    }
    ...
    static VpnVipToPort getNeutronPortFromVpnVipFixedIp(DataBroker broker, String vpnName, String fixedIp) {
        InstanceIdentifier id = buildVpnMipipToPortIdentifier(vpnName, fixedIp);
        Optional<VpnVipToPort> vpnVipipToPortData = read(broker, LogicalDatastoreType.OPERATIONAL, id);
