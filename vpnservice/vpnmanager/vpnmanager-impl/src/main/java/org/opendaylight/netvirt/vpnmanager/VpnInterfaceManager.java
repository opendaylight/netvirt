    ...
    protected void processVpnInterfaceUp(final BigInteger dpId, VpnInterface vpnInterface,
        ...
        if (!isInterfaceUp) {
            ...
            if (!waitForVpnInterfaceOpRemoval) {
                ...
                if (vpnInstanceOpDataEntry.getType().equals(VpnInstanceOpDataEntry.Type.L2)) {
                    setupGwMacForEvpn(dpId, vpnId, getGatewayMac(interfaceName), writeInvTxn, NwConstants.ADD_FLOW);
                }
...
//    }
    ...
    private void bindServiceForDcGwTunnel(BigInteger dpId, final String vpnInstanceName, final String vpnInterfaceName,
                             int lPortTag, WriteTransaction writeConfigTxn, WriteTransaction writeInvTxn) {
    ...
    private void setupGwMacForEvpn(BigInteger dpnId, long vpnId, String gatewayMac, WriteTransaction writeInvTxn, int addOrRemove) {
                ...
                if (vpnInstanceOpData.getType().equals(VpnInstanceOpDataEntry.Type.L2)) {
                    ...
                    if (rd != null) {
                        ...
                        addPrefixToBGP(rd, null /*macAddress*/, nextHop.getIpAddress(), nextHopIp, encapType, label,
                                0 /*evi*/, null /*gatewayMacAddress*/, writeConfigTxn);
                        ...
                        for (VpnInstanceOpDataEntry vpn : vpnsToImportRoute) {
                            ...
                            if (vpnRd != null) {
                                ...
                                fibManager.addOrUpdateFibEntry(dataBroker, vpnRd, null /*macAddress*/, nextHop.getIpAddress(), Arrays.asList(nextHopIp), encapType,
                                        (int) label, 0 /*evi*/, null /*gatewayMacAddress*/, RouteOrigin.SELF_IMPORTED, writeConfigTxn);
                            }
                        }
                    } else {
                        // ### add FIB route directly
                        fibManager.addOrUpdateFibEntry(dataBroker, vpnName, null /*macAddress*/, nextHop.getIpAddress(), Arrays.asList(nextHopIp), encapType,
                                (int) label, 0 /*evi*/, null /*gatewayMacAddress*/, RouteOrigin.STATIC, writeConfigTxn);
                    }
                }
            }
        }
    ...
    private void addPrefixToBGP(String rd, String macAddress, String prefix, String nextHopIp,
                                VrfEntry.EncapType encapType, long label, long evi, String gatewayMac,
                                WriteTransaction writeConfigTxn) {
        try {
            ...
            fibManager.addOrUpdateFibEntry(dataBroker, rd, macAddress, prefix, Arrays.asList(nextHopIp), encapType, (int)label,
                                            evi, gatewayMac, RouteOrigin.STATIC, writeConfigTxn);
    ...
    private void unbindService(BigInteger dpId, String vpnInstanceName, final String vpnInterfaceName,
        ...
        VpnInstanceOpDataEntry vpnInstanceOpDataEntry = VpnUtil.getVpnInstanceOpData(dataBroker, vpnRd);
        if (vpnInstanceOpDataEntry.getType().equals(VpnInstanceOpDataEntry.Type.L2)) {
            setupGwMacForEvpn(dpId, vpnId, getGatewayMac(vpnInterfaceName), writeInvTxn, NwConstants.DEL_FLOW);
        }
    }
    ...
    protected void addExtraRoute(String destination, String nextHop, String rd, String routerID, int label,
                                 String intfName, WriteTransaction writeOperTxn, WriteTransaction writeConfigTxn) {
        ...
        if ( optInterVpnLink.isPresent() ) {
            ...
            if (rd != null) {
                addPrefixToBGP(rd, null /*macAddress*/, destination, nextHop, VrfEntry.EncapType.Mplsgre, label,
                        0 /*evi*/, null /*gatewayMacAddress*/, null /*writeTxn*/);
            } else {
                // ### add FIB route directly
                fibManager.addOrUpdateFibEntry(dataBroker, routerID, null /*macAddress*/, destination, Arrays.asList(nextHop),
                        VrfEntry.EncapType.Mplsgre, label, 0 /*evi*/, null /*gatewayMacAddress*/, RouteOrigin.STATIC, null /*writeTxn*/);
            }
        }
