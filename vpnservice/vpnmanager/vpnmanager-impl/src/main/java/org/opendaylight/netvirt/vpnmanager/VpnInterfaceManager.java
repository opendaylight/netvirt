...
public class VpnInterfaceManager extends AsyncDataTreeChangeListenerBase<VpnInterface, VpnInterfaceManager> {
    ...
    private void updateBindServices(String newVpnName, String interfaceName, VpnInterface vpnInterface) {
        ...
        if (nbVpnInterfacesAvailable == 1 || nbVpnInterfacesAvailable == 0) {
            ...
            LOG.debug("VPN Interface.updateBindServices: interface {} adding bindRule with {} service {}",
                      interfaceName, newVpnName, ipVersion == IpVersionChoice.IPV4
                      ? NwConstants.L3VPN_SERVICE_INDEX : NwConstants.L3_VPNV6_SERVICE_INDEX);
            if (ipVersion == IpVersionChoice.IPV4) {
                VpnUtil.bindService(newVpnName, interfaceName, dataBroker,
                     false /*isTunnelInterface*/, jobCoordinator, NwConstants.L3VPN_SERVICE_INDEX);
            } else {
                VpnUtil.bindService(newVpnName, interfaceName, dataBroker,
    ...
    private void addBindServices(final InstanceIdentifier<VpnInterface> identifier,
        ...
        if (nbVpnInterfacesAvailable > 1) {
            if (vpnIpv4Name != null && vpnIpv6Name != null) {
                LOG.debug("VPN Interface.updateBindServices: interface {} adding bindRule with {} service {}",
                          interfaceName, vpnIpv4Name, NwConstants.L3VPN_SERVICE_INDEX);
                LOG.debug("VPN Interface.updateBindServices: interface {} adding bindRule with {} service {}",
                          interfaceName, vpnIpv6Name, NwConstants.L3_VPNV6_SERVICE_INDEX);
                VpnUtil.bindService(vpnIpv4Name, interfaceName, dataBroker,
                        false /*isTunnelInterface*/, jobCoordinator, NwConstants.L3VPN_SERVICE_INDEX);
                VpnUtil.bindService(vpnIpv6Name, interfaceName, dataBroker,
                        false /*isTunnelInterface*/, jobCoordinator, NwConstants.L3_VPNV6_SERVICE_INDEX);
            } else if (arbitrateProvision == true) {
                if (firstVpnName != null && secondVpnName != null) {
                    LOG.debug("VPN Interface.updateBindServices: interface {} adding bindRule with {} service {}",
                              interfaceName, firstVpnName, NwConstants.L3VPN_SERVICE_INDEX);
                    LOG.debug("VPN Interface.updateBindServices: interface {} adding bindRule with {} service {}",
                              interfaceName, secondVpnName, NwConstants.L3_VPNV6_SERVICE_INDEX);
    ...
    private void addVpnInterfaceCall(final InstanceIdentifier<VpnInterface> identifier, final VpnInterface vpnInterface,
        ...
        VpnInterfaceState vpnInterfaceState = null;
        ...
        if (vpnInterfaceOpIdentifier != null) {
            ...
            if (optVpnInterface.isPresent()) {
                ...
                vpnInterfaceState = optVpnInterface.get().getVpnInterfaceState();
                isRouter = optVpnInterface.get().isRouterInterface();
            }
        }
        WriteTransaction writeOperTxn = dataBroker.newWriteOnlyTransaction();
        if (!isRouter && vpnInterfaceState != null) {
            addVpnInterfaceToOperational(vpnName, interfaceName, dpnId, aug,
                                writeOperTxn, false, vpnInterfaceState);
        } else {
            addVpnInterfaceToOperational(vpnName, interfaceName, dpnId, aug,
    ...
    protected void processVpnInterfaceAdjacencies(BigInteger dpnId, final int lportTag, String vpnName,
        ...
        Boolean isScheduledForRemove = Boolean.FALSE;
        if (!adjacencies.isPresent()) {
            addVpnInterfaceToOperational(vpnName, interfaceName, dpnId, null, writeOperTxn, isScheduledForRemove,
        ...
        Optional<VpnInterface> vpnInterfaceOp = VpnUtil.read(dataBroker, LogicalDatastoreType.CONFIGURATION,
                identifier);
        VpnInterfaceState stateIf = VpnInterfaceState.Active;
        if (vpnInterfaceOp.isPresent() && vpnInterfaceOp.get().isRouterInterface()) {
            stateIf = VpnInterfaceState.Inactive;
        } else {
            stateIf = VpnInterfaceState.Active;
        }
        addVpnInterfaceToOperational(vpnName, interfaceName, dpnId, aug, writeOperTxn, isScheduledForRemove, stateIf);
    ...
    private void addVpnInterfaceToOperational(String vpnName, String interfaceName, BigInteger dpnId, AdjacenciesOp aug,
            WriteTransaction writeOperTxn, boolean isScheduledForRemove, VpnInterfaceState state) {
        VpnInterfaceOpDataEntry opInterface = VpnUtil.getVpnInterfaceOpDataEntry(interfaceName, vpnName, aug, dpnId,
                  Boolean.valueOf(isScheduledForRemove), state);
    ...
    private void removeAdjacenciesFromVpn(final BigInteger dpnId, final int lportTag, final String interfaceName,
        ...
        if (adjacencies.isPresent()) {
            ...
            Optional<VpnInterfaceOpDataEntry> optVpnInterface =
                  VpnUtil.read(dataBroker, LogicalDatastoreType.OPERATIONAL, identifier);
            if (optVpnInterface.isPresent() && optVpnInterface.get().isRouterInterface()) {
                isScheduledForRemove = true;
            }
            List<Adjacency> nextHops = adjacencies.get().getAdjacency();
            addVpnInterfaceToOperational(vpnName, interfaceName, dpnId, adjacencies.get(), writeOperTxn,
                    isScheduledForRemove, VpnInterfaceState.Inactive);
            if (!nextHops.isEmpty()) {
            ...
            if (newVpnName != null && (original.getVpnInstanceNames() == null
                ...
                if (!VpnUtil.isVpnPendingDelete(dataBroker, primaryRd)) {
                    ...
                    if (canHandleNewVpnInterface(identifier, update, newVpnName)) {
                        AdjacenciesOp aug = null;
                        boolean isrouterIface = false;
                        VpnInterfaceState ifaceState = VpnInterfaceState.Inactive;
                        if (vpnInterfaceOpIdentifier != null) {
                            ...
                            if (optVpnInterface.isPresent()) {
                                aug = optVpnInterface.get().getAugmentation(AdjacenciesOp.class);
                                isrouterIface = optVpnInterface.get().isRouterInterface();
                                ifaceState = optVpnInterface.get().getVpnInterfaceState();
                            }
                        }
                        WriteTransaction writeOperTxn = dataBroker.newWriteOnlyTransaction();
                        if (!isrouterIface && isVpnInstanceReady(newVpnName)) {
                            addVpnInterfaceToOperational(newVpnName, vpnInterfaceName, dpnId, aug,
                                    writeOperTxn, false, VpnInterfaceState.Active);
                        } else {
                            addVpnInterfaceToOperational(newVpnName, vpnInterfaceName, dpnId, aug,
                                    writeOperTxn, false, ifaceState);
                        }
                        ListenableFuture<Void> operFuture = writeOperTxn.submit();
                        try {
                            operFuture.get();
            ...
            */
            ...
            for (UpdateData updData : processQueue) {
                ...
                for (VpnInstanceNames vpnInterfaceVpnInstance : updData.getUpdate().getVpnInstanceNames()) {
                    ...
                    jobCoordinator.enqueueJob("VPNINTERFACE-" + interfaceName + newVpnName, () -> {
                        LOG.debug("VPN Interface.addBindServices: interface {} vpn {} bindService",
                                   updData.getUpdate().getName(), newVpnName);
                        updateBindServices(newVpnName, interfaceName, updData.getUpdate());
                        return Collections.emptyList();
                    });
