...
public class VpnInstanceListener extends AsyncDataTreeChangeListenerBase<VpnInstance, VpnInstanceListener>
    ...
    protected void remove(InstanceIdentifier<VpnInstance> identifier, VpnInstance del) {
        ...
        dataStoreCoordinator.enqueueJob("VPN-" + vpnName,
            new DeleteVpnInstanceWorker(dataBroker, primaryRd, vpnName));
    }

    private class DeleteVpnInstanceWorker implements Callable<List<ListenableFuture<Void>>> {
        private final Logger logger = LoggerFactory.getLogger(DeleteVpnInstanceWorker.class);
        DataBroker broker;
        String vpnInstanceName;
        String primaryRd;

        DeleteVpnInstanceWorker(DataBroker broker, String primaryRd, String vpnInstanceName) {
            this.broker = broker;
        ...
        public List<ListenableFuture<Void>> call() {
            ...
            logger.info("call: Operational status set to PENDING_DELETE for vpn {} with rd {}", vpnInstanceName,
                    primaryRd);
            return futures;
        }
    }
