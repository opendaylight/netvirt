...
public class VpnInstanceListener extends AsyncDataTreeChangeListenerBase<VpnInstance, VpnInstanceListener>
    ...
    static final short VPN_VNI_DEMUX_TABLE = 23;
    private static final int DEFAULT_FIB_FLOW_PRIORITY = 10;
    private static final BigInteger COOKIE_VM_FIB_TABLE = new BigInteger("8000003", 16);
    private static final String L3VPN_SERVICE_NAME = "L3VPN_SERVICE";
        ...
        public List<ListenableFuture<Void>> call() throws Exception {
            ...
            if (VpnUtil.isL3VpnOverVxLan(vpnInstance.getL3vni())) { //Handled for L3VPN Over VxLAN
                ...
                List<InstructionInfo> instructions =
                        Arrays.asList(new InstructionWriteMetadata(MetaDataUtil.getVpnIdMetadata(vpnId),
                                MetaDataUtil.METADATA_MASK_VRFID), new InstructionGotoTable(NwConstants.L3_FIB_TABLE));

                String flowRef = getFibFlowRef( vpnInstance.getVpnInstanceName(), vpnId);

                for ( BigInteger dpnId: NWUtil.getOperativeDPNs( dataBroker ) ) {
                    FlowEntity flowEntity = MDSALUtil.buildFlowEntity(dpnId, NwConstants.L3_GW_MAC_TABLE,
                            flowRef, DEFAULT_FIB_FLOW_PRIORITY, "VxLAN VPN Tunnel Bind Service", 0, 0,
        ...
        dataStoreCoordinator.enqueueJob(tunnelInterfaceName,
                new Callable<List<ListenableFuture<Void>>>() {
                    ...
                    public List<ListenableFuture<Void>> call() throws Exception {
                        ...
                        instructions.add(MDSALUtil.buildAndGetGotoTableInstruction(VPN_VNI_DEMUX_TABLE,
                                ++instructionKey));

                        BoundServices
                                serviceInfo =
                                InterfaceUtils.getBoundServices(String.format("%s.%s.%s", "vpn",vpnInstanceName,
                                        tunnelInterfaceName), ServiceIndex.getIndex(L3VPN_SERVICE_NAME,
                                        VPN_VNI_DEMUX_TABLE), priority, NwConstants.COOKIE_VM_INGRESS_TABLE,
                                        instructions);
                        writeTxn.put(LogicalDatastoreType.CONFIGURATION,
                                InterfaceUtils.buildServiceId(tunnelInterfaceName, ServiceIndex.getIndex(
                                        L3VPN_SERVICE_NAME, NwConstants.L3VPN_SERVICE_INDEX)), serviceInfo, true);
