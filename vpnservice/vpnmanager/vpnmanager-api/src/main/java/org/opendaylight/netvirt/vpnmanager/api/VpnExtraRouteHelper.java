...
public class VpnExtraRouteHelper {
    ...
    public static  InstanceIdentifier<Vpn> getVpnToExtrarouteIdentifier(String vpnName) {
        return InstanceIdentifier.builder(VpnToExtraroutes.class)
        ...
        for (String rd : usedRds) {
            Optional<Routes> extraRouteInfo =
                    MDSALUtil.read(dataBroker, LogicalDatastoreType.OPERATIONAL,
                            VpnExtraRouteHelper.getVpnToExtrarouteIdentifier(vpnName, rd, destPrefix));
    ...
    public static  List<String> getUsedRds(DataBroker broker, long vpnId, String destPrefix) {
        ...
        Optional<DestPrefixes> usedRds = MDSALUtil.read(broker, LogicalDatastoreType.OPERATIONAL, usedRdsId);
