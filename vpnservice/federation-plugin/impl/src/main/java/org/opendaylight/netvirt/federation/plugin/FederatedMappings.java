...
public class FederatedMappings {
    ...
    private final Map<Uuid, Uuid> producerToConsumerSecurityGroupsMap = Maps.newConcurrentMap();

    public FederatedMappings(List<FederatedNetworkPair> federatedNetworkPairs,
        List<FederatedSecurityGroupPair> secGroupsPairs) {
        ...
        if (secGroupsPairs != null) {
            secGroupsPairs.stream().forEach(
                (pair) -> producerToConsumerSecurityGroupsMap.put(pair.producerSecGroupId, pair.consumerSecGroupId));
        }
    }

    public Uuid getConsumerSecurityGroupId(Uuid producerSecGroupdId) {
        Uuid consumerId = producerToConsumerSecurityGroupsMap.get(producerSecGroupdId);
        if (consumerId != null) {
            return consumerId;
        } else {
            return producerSecGroupdId;
        }
    }
    ...
    public String toString() {
        return "FederatedMappings [producerToConsumerNetworkMap=" + producerToConsumerNetworkMap
            ...
            + producerToConsumerSecurityGroupsMap + "]";
    }
}
