...
public class FederationL2GatewayTransformer implements FederationPluginTransformer<L2gateway, Neutron> {
    ...
    public Neutron applyEgressTransformation(L2gateway l2Gateway, FederatedMappings federatedMappings,
        ...
        return new NeutronBuilder().setL2gateways(new L2gatewaysBuilder()
                .setL2gateway(Collections.singletonList(l2gwBuilder.build())).build()).build();
    }

    @Override
    public Pair<InstanceIdentifier<L2gateway>, L2gateway> applyIngressTransformation(
            Neutron neutron, ModificationType modificationType, int generationNumber, String remoteIp) {
        ...
        l2gwBuilder.addAugmentation(L2gwShadowProperties.class,
                new L2gwShadowPropertiesBuilder(l2gwBuilder.getAugmentation(L2gwShadowProperties.class))
