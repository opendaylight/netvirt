...
public class AbstractEnd2EndTest {
    ...
    protected <T extends DataObject> void addFederationEntity(String listenerKey,
            InstanceIdentifier<T> instanceIdentifier, T newObject) throws TransactionCommitFailedException {
        LogicalDatastoreType datastoreType = FederationPluginUtils.getListenerDatastoreType(listenerKey);
        WriteTransaction tx = dataBroker.newWriteOnlyTransaction();
        tx.merge(datastoreType, instanceIdentifier, newObject, true);
        try {
    ...
    protected void assertSentMessages(int noMsg) throws InterruptedException {
        Thread.sleep(200);
        assertEquals(noMsg, sentMessages.size());
    }

    protected void assertNoSentMessages() throws InterruptedException {
