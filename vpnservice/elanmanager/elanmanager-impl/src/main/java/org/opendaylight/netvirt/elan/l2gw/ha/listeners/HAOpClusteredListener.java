...
public class HAOpClusteredListener extends HwvtepNodeBaseListener implements ClusteredDataTreeChangeListener<Node> {
    ...
    private final Set<InstanceIdentifier<Node>> connectedNodes = Sets.newConcurrentHashSet();
    ...
    /*
        HwvtepPhysicalSwitchListner on add calls this function submitting the job to be run .
        If connected nodes contain the iid (i.e Global node already present at the time ps node
        add ) it execute the consumer .
        Consumer does the following thing .
           in case the PS node added is a HA parent node , it gets l2gw and connection associalted
           and provision it.
        In case connected Nodes are not present , it enques the job in waitingJob map which is looked
        upon by this(HAOpClusterListner) and executed if global node is connected .
        check onGlobalNodeAdd() for that
     */
