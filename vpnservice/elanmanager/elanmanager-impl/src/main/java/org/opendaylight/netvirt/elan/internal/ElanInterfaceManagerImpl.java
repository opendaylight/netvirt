 ...
 */
@Singleton
public class ElanInterfaceManagerImpl extends AsyncDataTreeChangeListenerBase<ElanInterface, ElanInterfaceManager>
        implements ElanInterfaceManager, AutoCloseable {
    ...
    @Inject
    public ElanInterfaceManagerImpl(final DataBroker dataBroker, final IdManagerService managerService,
    ...
    @Override
    public void removeEntriesForElanInterface(List<ListenableFuture<Void>> futures, ElanInstance elanInfo, InterfaceInfo
    ...
    * */
    ...
    @Override
    public void addElanInterface(List<ListenableFuture<Void>> futures, ElanInterface elanInterface,
    ...
    @Override
    public void setupEntriesForElanInterface(List<ListenableFuture<Void>> futures, ElanInstance elanInstance,
    ...
    private int addInterfaceIfRootInterface(int bucketId, String ifName, List<Bucket> listBucket,
        ...
        if (etreeInterface != null
                && etreeInterface.getEtreeInterfaceType() == EtreeInterface.EtreeInterfaceType.Root) {
            listBucket.add(MDSALUtil.buildBucket(getInterfacePortActions(ifInfo), MDSALUtil.GROUP_WEIGHT, bucketId,
                    MDSALUtil.WATCH_PORT, MDSALUtil.WATCH_GROUP));
            bucketId++;
        }
    ...
    private void bindEtreeService(ElanInstance elanInfo, ElanInterface elanInterface, int lportTag,
            WriteTransaction tx) {
        if (elanInterface.getAugmentation(EtreeInterface.class).getEtreeInterfaceType()
    ...
    @Override
    public void unbindService(String interfaceName, WriteTransaction tx) {
        short elanServiceIndex = ServiceIndex.getIndex(NwConstants.ELAN_SERVICE_NAME, NwConstants.ELAN_SERVICE_INDEX);
        InstanceIdentifier<BoundServices> bindServiceId = ElanUtils.buildServiceId(interfaceName, elanServiceIndex);
        Optional<BoundServices> existingElanService = ElanUtils.read(broker, LogicalDatastoreType.CONFIGURATION,
                bindServiceId);
