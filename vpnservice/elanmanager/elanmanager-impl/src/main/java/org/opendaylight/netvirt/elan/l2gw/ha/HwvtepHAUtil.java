...
import org.opendaylight.netvirt.elan.l2gw.utils.HwvtepHACache;
...
public class HwvtepHAUtil {
    ...
    public static Node readNode(ReadWriteTransaction tx, LogicalDatastoreType storeType, InstanceIdentifier<Node> nodeId)
    ...
    public static InstanceIdentifier<Node> getGlobalNodePathFromPSNode(Node psNode) {
        ...
        return (InstanceIdentifier<Node>)psNode.getAugmentation(PhysicalSwitchAugmentation.class).getManagedBy().getValue();
    }
    ...
    public static void buildGlobalConfigForHANode(ReadWriteTransaction tx,
        ...
        tx.merge(CONFIGURATION, haNodePath, configHANode);
    }
    ...
    public static void deleteNodeIfPresent(ReadWriteTransaction tx,
        ...
        if (tx.read(logicalDatastoreType, iid).checkedGet().isPresent()) {
            LOG.error("deleting child node {}", getNodeIdVal(iid));
            tx.delete(logicalDatastoreType, iid);
        } else {
            LOG.error("could not delete {} ", getNodeIdVal(iid));
        }
    }
    ...
    public static void deletePSNodesOfNode(InstanceIdentifier<Node> key,
        ...
        //also read from managed by attribute of switches and cleanup them
        //as a back up if the above cleanup fails
        Optional<Topology> topologyOptional = tx.read(CONFIGURATION, (InstanceIdentifier<Topology>)key.firstIdentifierOf(Topology.class)).checkedGet();
