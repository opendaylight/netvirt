...
public class HAOpNodeListener extends HwvtepNodeBaseListener {
    ...
    void clearNodeAvailability(InstanceIdentifier<Node> key) {
        ...
        int index = id.indexOf(HwvtepHAUtil.PHYSICALSWITCH);
        if (index > 0) {
            availablePsNodes.remove(id.substring(0, index));
        } else {
    ...
    void updateNodeAvailability(InstanceIdentifier<Node> key) {
        ...
        int index = id.indexOf(HwvtepHAUtil.PHYSICALSWITCH);
        if (index > 0) {
            availablePsNodes.put(id.substring(0, index), Boolean.TRUE);
        } else {
