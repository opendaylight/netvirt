...
public class ElanNodeListener extends AbstractDataChangeListener<Node> implements AutoCloseable {
    ...
    private void createL2ControlFlows(BigInteger dpId) {
        createLLDPFlows(dpId);
        createPauseFrameFlow(dpId);
    }

    private void createPauseFrameFlow(BigInteger dpId) {
        List<MatchInfo> mkMatches = new ArrayList<>();
        mkMatches.add(new MatchInfo(MatchFieldType.eth_type, new long[] { 0x8808 }));
        mkMatches.add(new MatchInfo(MatchFieldType.eth_dst, new String[] { "01:80:C2:00:00:01" } ));
        ...
        FlowEntity pauseFlow = MDSALUtil.buildFlowEntity(dpId, NwConstants.ELAN_DMAC_TABLE,
                NwConstants.ELAN_DMAC_TABLE + "pauseFrame", 15, "Pause frame dMac Table Flow", 0, 0,
                ElanConstants.COOKIE_ELAN_KNOWN_DMAC, mkMatches, mkInstructions);

        mdsalManager.installFlow(pauseFlow);    }

    private void createLLDPFlows(BigInteger dpId) {
        createLLDPFlow(dpId, "01:80:C2:00:00:00");
        createLLDPFlow(dpId, "01:80:C2:00:00:03");
        createLLDPFlow(dpId, "01:80:C2:00:00:0E");
    }

    private void createLLDPFlow(BigInteger dpId, String dstMac) {
        List<MatchInfo> mkMatches = new ArrayList<>();
        mkMatches.add(new MatchInfo(MatchFieldType.eth_type, new long[] { 0x88CC }));
        mkMatches.add(new MatchInfo(MatchFieldType.eth_dst, new String[] { dstMac } ));
        ...
        String flowId = dpId.toString() + NwConstants.ELAN_DMAC_TABLE + "lldp" + dstMac;
        FlowEntity lldpFlow = MDSALUtil.buildFlowEntity(dpId, NwConstants.ELAN_DMAC_TABLE,
                flowId, 15, "LLDP dMac Table Flow", 0, 0,
                ElanConstants.COOKIE_ELAN_KNOWN_DMAC, mkMatches, mkInstructions);
