...
public class ElanNodeListener extends AsyncDataTreeChangeListenerBase<Node, ElanNodeListener> implements AutoCloseable {
    ...
    protected void add(InstanceIdentifier<Node> identifier, Node add) {
        ...
        createArpResponseMatchFlowForGwMacTable(dpId);
    }
    ...
    private void createArpResponseMatchFlowForGwMacTable(BigInteger dpId) {
        List<MatchInfo> matches = Arrays.asList(MatchEthernetType.ARP, MatchArpOp.REPLY);
        List<ActionInfo> actionInfos = Arrays.asList(new ActionPuntToController(),
                new ActionNxResubmit(NwConstants.LPORT_DISPATCHER_TABLE));
        List<InstructionInfo> gwInstructions = Collections.singletonList(new InstructionApplyActions(actionInfos));
