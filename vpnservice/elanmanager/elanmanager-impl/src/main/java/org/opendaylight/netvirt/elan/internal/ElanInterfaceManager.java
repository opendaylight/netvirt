...
public class ElanInterfaceManager extends AbstractDataChangeListener<ElanInterface> implements AutoCloseable {
    ...
    protected void remove(InstanceIdentifier<ElanInterface> identifier, ElanInterface del) {
        ...
        ElanInterfaceRemoveWorker configWorker = new ElanInterfaceRemoveWorker(elanInstanceName, elanInfo,
    ...
    void addElanInterface(ElanInterface elanInterface, InterfaceInfo interfaceInfo, ElanInstance elanInstance) {
        ...
        if (dpId != null && !dpId.equals(ElanConstants.INVALID_DPN)) {
            ...
            Optional<DpnInterfaces> existingElanDpnInterfaces = ElanUtils.read(broker, LogicalDatastoreType.OPERATIONAL,
                    elanDpnInterfaces);
        ...
        if (elanDpns != null) {
            ...
            for (DpnInterfaces dpnInterface : dpnInterfaceses) {
                if (ElanUtils.isDpnPresent(dpnInterface.getDpId()) && dpnInterface.getDpId() != dpnId
                    ...
                    try {
                        List<Action> listAction = ElanUtils.getInternalItmEgressAction(dpnId, dpnInterface.getDpId(),
                                elanTag);
        ...
        if (elanDpns != null) {
            ...
            for (DpnInterfaces dpnInterface : dpnInterfaceses) {
                if (ElanUtils.isDpnPresent(dpnInterface.getDpId()) && dpnInterface.getDpId() != dpnId
                        && dpnInterface.getInterfaces() != null && !dpnInterface.getInterfaces().isEmpty()) {
                    try {
                        List<Action> listActionInfo = ElanUtils.getInternalItmEgressAction(dpnId,
        ...
        if (elanDpns != null) {
            ...
            for (DpnInterfaces dpnInterface : dpnInterfaceses) {
                if (ElanUtils.isDpnPresent(dpnInterface.getDpId()) && dpnInterface.getDpId() != dpnId
                    ...
                    List<String> actualInterfaces = dpnInterface.getInterfaces();
                    bucketId = addDPNsWithRootInterfaces(dpnId, bucketId, elanTag, listBucketInfo, dpnInterface,
                            actualInterfaces);
                }
            }
    ...
    private int addDPNsWithRootInterfaces(BigInteger dpnId, int bucketId, int elanTag, List<Bucket> listBucketInfo,
            DpnInterfaces dpnInterface, List<String> actualInterfaces) {
        for (String actualInterface : actualInterfaces) {
            if (ElanUtils.isEtreeRootInterfaceByInterfaceName(actualInterface)) {
                try {
                    List<Action> listActionInfo = ElanUtils.getInternalItmEgressAction(dpnId, dpnInterface.getDpId(),
                            elanTag);
    ...
    private void setElanBCGrouponOtherDpns(ElanInstance elanInfo, InterfaceInfo interfaceInfo) {
        BigInteger dpnId = interfaceInfo.getDpId();
        int elanTag = elanInfo.getElanTag().intValue();
    ...
    public void removeMacAddressTables(ElanInstance elanInfo, InterfaceInfo interfaceInfo) {
        ElanInterfaceMac elanInterfaceMac = ElanUtils
                .getElanInterfaceMacByInterfaceName(interfaceInfo.getInterfaceName());
        if (elanInterfaceMac != null && elanInterfaceMac.getMacEntry() != null) {
            List<MacEntry> macEntries = elanInterfaceMac.getMacEntry();
            for (MacEntry macEntry : macEntries) {
    ...
    public void setupLeavesEtreeBroadcastGroups(ElanInstance elanInfo, BigInteger dpnId) {
        ...
        if (etreeInstance != null) {
            ...
            DpnInterfaces dpnInterfaces = ElanUtils.getElanInterfaceInfoByElanDpn(elanInfo.getElanInstanceName(),
                    dpnId);
            bucketId = addAllRootInterfaces(listBucket, bucketId, dpnInterfaces);
    ...
    private void createDropBucket(List<Bucket> listBucket) {
        ...
        Bucket dropBucket = MDSALUtil.buildBucket(actionsInfos, MDSALUtil.GROUP_WEIGHT, 0,
                MDSALUtil.WATCH_PORT, MDSALUtil.WATCH_GROUP);
        listBucket.add(dropBucket);
    }

    private int addAllRootInterfaces(List<Bucket> listBucket, int bucketId, DpnInterfaces dpnInterfaces) {
        for (String ifName : dpnInterfaces.getInterfaces()) {
            InterfaceType interfaceType = InterfaceInfo.InterfaceType.VLAN_INTERFACE;
            InterfaceInfo ifInfo = interfaceManager.getInterfaceInfoFromOperationalDataStore(ifName, interfaceType);
            if (!isOperational(ifInfo)) {
    ...
    private void removeUnknownDmacFlow(BigInteger dpId, ElanInstance elanInfo) {
        ...
        Flow flow = new FlowBuilder().setId(new FlowId(getUnknownDmacFlowRef(ElanConstants.ELAN_UNKNOWN_DMAC_TABLE,
                elanInfo.getElanTag(), /* SH flag */ false))).setTableId(ElanConstants.ELAN_UNKNOWN_DMAC_TABLE).build();
        mdsalManager.removeFlow(dpId, flow);
    ...
    protected void updatedIfPrimaryAttributeChanged(ElanInterface elanInterface, boolean isUpdated) {
        ...
        Optional<DpnInterfaces> dpnInterfaces = ElanUtils.read(broker, LogicalDatastoreType.OPERATIONAL,
                dpnInterfaceId);
