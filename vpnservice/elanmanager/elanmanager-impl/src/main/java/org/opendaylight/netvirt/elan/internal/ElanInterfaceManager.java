...
import org.opendaylight.yang.gen.v1.urn.opendaylight.netvirt.elan.etree.rev160614.EtreeLogicalSwitches;
import org.opendaylight.yang.gen.v1.urn.opendaylight.netvirt.elan.etree.rev160614.EtreeLogicalSwitches.SwitchInterfacesType;
...
public class ElanInterfaceManager extends AbstractDataChangeListener<ElanInterface> implements AutoCloseable {
        ...
        if(elanDpns != null) {
            ...
            for(DpnInterfaces dpnInterface : dpnInterfaceses) {
                if(ElanUtils.isDpnPresent(dpnInterface.getDpId()) && dpnInterface.getDpId() != dpnId && dpnInterface.getInterfaces() != null && !dpnInterface.getInterfaces().isEmpty()) {
                    ...
                    bucketId = addDPNsWithRootInterfaces(dpnId, bucketId, elanTag, listBucketInfo, dpnInterface, actualInterfaces);
                }
            }

            List<Bucket> elanL2GwDevicesBuckets = getEtreeLeavesRemoteBCGroupBucketsOfElanL2GwDevices(elanInfo, dpnId, bucketId);
            listBucketInfo.addAll(elanL2GwDevicesBuckets);
        }
        return listBucketInfo;
    }
    ...
    public void setupElanBroadcastGroups(ElanInstance elanInfo, BigInteger dpnId) {
        ...
        setupEtreeLeavesBroadcastGroups(elanInfo, dpnId);
    }
    ...
    public void setupEtreeLeavesBroadcastGroups(ElanInstance elanInfo, BigInteger dpnId) { 
    ...
    private int addInterfaceIfRootInterface(int bucketId, String ifName, List<Bucket> listBucket, InterfaceInfo ifInfo) {
    ...
    public void setupLocalBroadcastGroups(ElanInstance elanInfo, InterfaceInfo interfaceInfo) {
        ...
        setupEtreeLeavesLocalBroadcastGroups(elanInfo, interfaceInfo);
    }
    ...
    private void setupEtreeLeavesLocalBroadcastGroups(ElanInstance elanInfo, InterfaceInfo interfaceInfo) {
        EtreeInstance etreeInstance = elanInfo.getAugmentation(EtreeInstance.class);
        if (etreeInstance != null) {
            long etreeLeafTag = etreeInstance.getEtreeLeafTagVal().getValue();
            List<Bucket> listBucket = new ArrayList<>();
...
//        mdsalManager.removeFlow(dpId, flow);
    ...
    private void bindEtreeService(ElanInstance elanInfo, ElanInterface elanInterface) {
        if (elanInterface.getAugmentation(EtreeInterface.class).getEtreeInterfaceType() == EtreeInterfaceType.Root) { 
            ...
            EtreeInstance etreeInstance = elanInfo.getAugmentation(EtreeInstance.class);
            if (etreeInstance == null) {
                logger.error("EtreeInterface " + elanInterface.getName() + " is associated with a non EtreeInstance: " + elanInfo.getElanInstanceName());
            } else {
                bindElanService(etreeInstance.getEtreeLeafTagVal().getValue(), elanInfo.getElanInstanceName(), elanInterface.getName());
            }
        }
    }
    ...
    private void bindElanService(long elanTag, String elanInstanceName, String interfaceName) {
        ...
        BoundServices
                serviceInfo =
                ElanUtils.getBoundServices(String.format("%s.%s.%s", "vpn",elanInstanceName, interfaceName),
    ...
    public List<Bucket> getEtreeLeavesRemoteBCGroupBucketsOfElanL2GwDevices(ElanInstance elanInfo, BigInteger dpnId,
            int bucketId) {
        List<Bucket> listBucketInfo = new ArrayList<>();
        ConcurrentMap<String, L2GatewayDevice> map = ElanL2GwCacheUtils
                .getInvolvedL2GwDevices(elanInfo.getElanInstanceName());
        for (L2GatewayDevice device : map.values()) {
            ...
            NodeId hwvtepNodeId = new NodeId(device.getHwvtepNodeId());
            LogicalSwitches logicalSwitch = HwvtepUtils.getLogicalSwitch(broker, LogicalDatastoreType.OPERATIONAL,
                    hwvtepNodeId, elanInfo.getElanInstanceName());
            EtreeLogicalSwitches etreeLogicalSwitch = logicalSwitch.getAugmentation(EtreeLogicalSwitches.class);
            if (etreeLogicalSwitch == null) {
                logger.error("Logical Switch: " + logicalSwitch.getHwvtepNodeName().getValue() + " is not an Etree Logical Switch but it's connected to an Etree Network: " + elanInfo.getElanInstanceName());
            } else {
                if (addIfRootLogicalSwitch(elanInfo, bucketId, listBucketInfo, interfaceName, etreeLogicalSwitch)) {
                    ++bucketId;
                }
            }
        }
    ...
    private boolean addIfRootLogicalSwitch(ElanInstance elanInfo, int bucketId, List<Bucket> listBucketInfo, String interfaceName,
            EtreeLogicalSwitches etreeLogicalSwitch) {
        if (etreeLogicalSwitch.getSwitchInterfacesType() == SwitchInterfacesType.Root) {
            List<Action> listActionInfo = ElanUtils.buildItmEgressActions(interfaceName, elanInfo.getVni());
            listBucketInfo.add(MDSALUtil.buildBucket(listActionInfo, MDSALUtil.GROUP_WEIGHT, bucketId,
            MDSALUtil.WATCH_PORT, MDSALUtil.WATCH_GROUP));
            return true;
