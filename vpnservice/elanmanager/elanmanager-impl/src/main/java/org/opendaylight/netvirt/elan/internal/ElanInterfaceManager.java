...
public class ElanInterfaceManager extends AsyncDataTreeChangeListenerBase<ElanInterface, ElanInterfaceManager>
    ...
    protected void remove(InstanceIdentifier<ElanInterface> identifier, ElanInterface del) {
        ...
        /*
         * Handling in case the elan instance is deleted.If the Elan instance is deleted, there is no need to explicitly
         * delete the elan interfaces
         */
        if (elanInfo == null) {
            return;
        }
        ...
        InterfaceRemoveWorkerOnElan configWorker =
                new InterfaceRemoveWorkerOnElan(elanInstanceName, elanInfo, interfaceName, interfaceInfo, false, this);
    ...
    public void removeElanInterface(ElanInstance elanInfo, String interfaceName, InterfaceInfo interfaceInfo,
        ...
        if (interfaceInfo != null) {
            ...
            DpnInterfaces dpnInterfaces =
                    removeElanDpnInterfaceFromOperationalDataStore(elanName, dpId, interfaceName, elanTag, tx);
            /*
             * If there are not elan ports, remove the unknown dmac, terminating service table flows, remote/local bc
             * group
             */
    ...
    void removeEntriesForElanInterface(ElanInstance elanInfo, InterfaceInfo interfaceInfo, String interfaceName,
        ...
        if (interfaceInfo != null) {
            Optional<ElanInterfaceMac> existingElanInterfaceMac =
                    ElanUtils.read(elanServiceProvider.getBroker(), LogicalDatastoreType.OPERATIONAL, elanInterfaceId);
            ...
            if (updatedPhysAddress.size() > 0) {
                for (PhysAddress physAddress : updatedPhysAddress) {
                    ...
                    Optional<MacEntry> existingMacEntry =
                            ElanUtils.read(elanServiceProvider.getBroker(), LogicalDatastoreType.OPERATIONAL, macId);
                    WriteTransaction tx = elanServiceProvider.getBroker().newWriteOnlyTransaction();
    ...
    protected void add(InstanceIdentifier<ElanInterface> identifier, ElanInterface elanInterfaceAdded) {
        ...
        DataStoreJobCoordinator coordinator = DataStoreJobCoordinator.getInstance();
        InterfaceAddWorkerOnElan addWorker =
                new InterfaceAddWorkerOnElan(elanInstanceName, elanInterfaceAdded, interfaceInfo, elanInstance, this);
    ...
    private void installEntriesForElanInterface(ElanInstance elanInstance, InterfaceInfo interfaceInfo,
        ...
        if (isFirstInterfaceInDpn) {
            ...
            /*
             * Install remote DMAC flow. This is required since this DPN is added later to the elan instance and remote
             * DMACs of other interfaces in this elan instance are not present in the current dpn.
             */
    ...
    private void createElanStateList(String elanInstanceName, String interfaceName, WriteTransaction tx) {
        ...
        Optional<Elan> elanInterfaceLists =
                ElanUtils.read(elanServiceProvider.getBroker(), LogicalDatastoreType.OPERATIONAL, elanInstance);
        // Adding new Elan Interface Port to the operational DataStore without Static-Mac Entries..
