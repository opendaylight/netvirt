...
public class ElanInterfaceManager extends AbstractDataChangeListener<ElanInterface> implements AutoCloseable {
    ...
    protected void remove(InstanceIdentifier<ElanInterface> identifier, ElanInterface del) {
        ...
        ElanInterfaceRemoveWorker configWorker =
                new ElanInterfaceRemoveWorker(elanInstanceName, elanInfo, interfaceName, interfaceInfo, this);
    ...
    public void removeElanInterface(ElanInstance elanInfo, String interfaceName, InterfaceInfo interfaceInfo) {
        ...
        if (interfaceInfo == null) {
            ...
            ElanUtils.delete(broker, LogicalDatastoreType.OPERATIONAL,
                    ElanUtils.getElanInterfaceMacEntriesOperationalDataPath(interfaceName));
    ...
    private void removeElanInterface(ElanInstance elanInfo, InterfaceInfo interfaceInfo) {
        ...
        InstanceIdentifier<ElanInterfaceMac> elanInterfaceId =
                ElanUtils.getElanInterfaceMacEntriesOperationalDataPath(interfaceName);
        Optional<ElanInterfaceMac> existingElanInterface =
                ElanUtils.read(broker, LogicalDatastoreType.OPERATIONAL, elanInterfaceId);
        ...
        if (dpId.equals(ElanConstants.INVALID_DPN)) {
            ...
            if (elanDpnInterfacesList != null && !elanDpnInterfacesList.getDpnInterfaces().isEmpty()) {
                ...
                for (DpnInterfaces dpnInterface : dpnIfList) {
                    DpnInterfaces dpnIfLists =
                            ElanUtils.getElanInterfaceInfoByElanDpn(elanName, dpnInterface.getDpId());
        ...
        if (elanInterfaces.isEmpty()) {
            ...
            // ElanUtils.getElanInstanceConfigurationDataPath(elanName));
        } else {
            Elan updateElanState = new ElanBuilder().setElanInterfaces(elanInterfaces).setName(elanName)
                    .setKey(new ElanKey(elanName)).build();
            MDSALUtil.syncWrite(broker, LogicalDatastoreType.OPERATIONAL,
        ...
        mdsalManager
                .removeFlow(dpId,
                        MDSALUtil.buildFlow(ElanConstants.ELAN_DMAC_TABLE,
                                ElanUtils.getKnownDynamicmacFlowRef(ElanConstants.ELAN_DMAC_TABLE, dpId, currentDpId,
        ...
        if (updatedPhysAddress != null && !updatedPhysAddress.isEmpty()) {
            ...
            if (updatedPhysAddress.size() > 0) {
                for (PhysAddress physAddress : updatedPhysAddress) {
                    ...
                    Optional<MacEntry> existingMacEntry =
                            ElanUtils.read(broker, LogicalDatastoreType.OPERATIONAL, macId);
                    if (existingMacEntry.isPresent()) {
                        elanForwardingEntriesHandler.updateElanInterfaceForwardingTablesList(elanName, interfaceName,
                                existingMacEntry.get().getInterface(), existingMacEntry.get());
                    } else {
                        elanForwardingEntriesHandler.addElanInterfaceForwardingTableList(
                                ElanUtils.getElanInstanceByName(elanName), interfaceName, physAddress);
                    }
                }
            }
    ...
    protected void add(InstanceIdentifier<ElanInterface> identifier, ElanInterface elanInterfaceAdded) {
        ...
        DataStoreJobCoordinator coordinator = DataStoreJobCoordinator.getInstance();
        ElanInterfaceAddWorker addWorker =
                new ElanInterfaceAddWorker(elanInstanceName, elanInterfaceAdded, interfaceInfo, elanInstance, this);
    ...
    void programRemoteDmacFlow(ElanInstance elanInstance, InterfaceInfo interfaceInfo) {
        ElanDpnInterfacesList elanDpnInterfacesList =
                ElanUtils.getElanDpnInterfacesList(elanInstance.getElanInstanceName());
    ...
    void addElanInterface(ElanInterface elanInterface, InterfaceInfo interfaceInfo, ElanInstance elanInstance) {
        ...
        if (dpId != null && !dpId.equals(ElanConstants.INVALID_DPN)) {
            InstanceIdentifier<DpnInterfaces> elanDpnInterfaces =
                    ElanUtils.getElanDpnInterfaceOperationalDataPath(elanInstanceName, dpId);
            Optional<DpnInterfaces> existingElanDpnInterfaces =
                    ElanUtils.read(broker, LogicalDatastoreType.OPERATIONAL, elanDpnInterfaces);
            if (!existingElanDpnInterfaces.isPresent()) {
                // ELAN's 1st ElanInterface added to this DPN
                createElanInterfacesList(elanInstanceName, interfaceName, dpId);
                /*
                 * Install remote DMAC flow. This is required since this DPN is added later to the elan instance and
                 * remote DMACs of other interfaces in this elan instance are not present in the current dpn.
                 */
            ...
            for (PhysAddress physAddress : staticMacAddresses) {
                ...
                if (existingMacEntry.isPresent()) {
                    elanForwardingEntriesHandler.updateElanInterfaceForwardingTablesList(elanInstanceName,
                            interfaceName, existingMacEntry.get().getInterface(), existingMacEntry.get());
                } else {
    ...
    protected void removeInterfaceStaticMacEntires(String elanInstanceName, String interfaceName,
        ...
        elanForwardingEntriesHandler.deleteElanInterfaceForwardingEntries(
                ElanUtils.getElanInstanceByName(elanInstanceName), interfaceInfo, macEntry);
    ...
    public void installFlowsAndGroups(final ElanInstance elanInfo, final InterfaceInfo interfaceInfo) {
        if (isOperational(interfaceInfo)) {
            ...
            bindService(elanInfo, ElanUtils.getElanInterfaceByElanInterfaceName(interfaceInfo.getInterfaceName()));

            // update the remote-DPNs remoteBC group entry with Tunnels
            setBCGroupOnOtherDpns(elanInfo, interfaceInfo);
        }
    }
    ...
    public void setupFilterEqualsTable(ElanInstance elanInfo, InterfaceInfo interfaceInfo) {
        ...
        Flow flow = MDSALUtil.buildFlowNew(ElanConstants.ELAN_FILTER_EQUALS_TABLE,
                getFlowRef(ElanConstants.ELAN_FILTER_EQUALS_TABLE, ifTag), 9, elanInfo.getElanInstanceName(), 0, 0,
        ...
        Flow flowEntry = MDSALUtil.buildFlowNew(ElanConstants.ELAN_FILTER_EQUALS_TABLE,
                getFlowRef(ElanConstants.ELAN_FILTER_EQUALS_TABLE, 1000 + ifTag), 10, elanInfo.getElanInstanceName(), 0,
    ...
    public void removeFilterEqualsTable(ElanInstance elanInfo, InterfaceInfo interfaceInfo) {
        ...
        Flow flowEntity = MDSALUtil.buildFlowNew(ElanConstants.ELAN_FILTER_EQUALS_TABLE,
                getFlowRef(ElanConstants.ELAN_FILTER_EQUALS_TABLE, 1000 + ifTag), 10, elanInfo.getElanInstanceName(), 0,
                0, ElanConstants.COOKIE_ELAN_FILTER_EQUALS.add(BigInteger.valueOf(ifTag)),
                getMatchesForFilterEqualsLPortTag(ifTag), MDSALUtil.buildInstructionsDrop());
                ...
                if (ElanUtils.isDpnPresent(dpnInterface.getDpId()) && dpnInterface.getDpId() != dpnId
                    ...
                    try {
                        List<Action> listAction =
                                ElanUtils.getInternalItmEgressAction(dpnId, dpnInterface.getDpId(), elanTag);
    ...
    private List<Bucket> getRemoteBCGroupBuckets(ElanInstance elanInfo, BigInteger dpnId, int bucketId, long elanTag) {
        ...
        if (elanDpns != null) {
            ...
            for (DpnInterfaces dpnInterface : dpnInterfaceses) {
                if (ElanUtils.isDpnPresent(dpnInterface.getDpId()) && dpnInterface.getDpId() != dpnId
                        && dpnInterface.getInterfaces() != null && !dpnInterface.getInterfaces().isEmpty()) {
                    try {
                        List<Action> listActionInfo =
                                ElanUtils.getInternalItmEgressAction(dpnId, dpnInterface.getDpId(), elanTag);
                        listBucketInfo
                                .add(MDSALUtil.buildBucket(listActionInfo, 0, bucketId, 0xffffffffL, 0xffffffffL));
                        bucketId++;
                    } catch (Exception ex) {
                        logger.error("Logical Group Interface not found between source Dpn - {}, destination Dpn - {} ",
                                dpnId, dpnInterface.getDpId());
    ...
    private void setBCGroupOnOtherDpns(ElanInstance elanInfo, InterfaceInfo interfaceInfo) {
        setElanBCGrouponOtherDpns(elanInfo, interfaceInfo, elanInfo.getElanTag());
        setEtreeBCGroupOnOtherDpnsIfNeeded(elanInfo, interfaceInfo);
    }

    private void setEtreeBCGroupOnOtherDpnsIfNeeded(ElanInstance elanInfo, InterfaceInfo interfaceInfo) {
        EtreeInterface etreeInterface =
                ElanUtils.getEtreeInterfaceByElanInterfaceName(interfaceInfo.getInterfaceName());
        if (etreeInterface != null) {
            setEtreeLeavesBCGroupOnOtherDpns(elanInfo, interfaceInfo,
                    elanInfo.getAugmentation(EtreeInstance.class).getEtreeLeafTagVal().getValue());
        }
    }

    private void setElanBCGrouponOtherDpns(ElanInstance elanInfo, InterfaceInfo interfaceInfo, long elanTag) {
        BigInteger dpnId = interfaceInfo.getDpId();
        ...
        if (elanDpns != null) {
            ...
            for (DpnInterfaces dpnInterface : dpnInterfaceses) {
                ...
                if (ElanUtils.isDpnPresent(dpnInterface.getDpId()) && !dpnInterface.getDpId().equals(dpnId)
                        && dpnInterface.getInterfaces() != null && !dpnInterface.getInterfaces().isEmpty()) {
                    ...
                    for (DpnInterfaces otherFes : dpnInterfaceses) {
                        if (isOtherValidDPN(dpnInterface, otherFes)) {
                            try {
                                List<Action> remoteListActionInfo = ElanUtils.getInternalItmEgressAction(
                                        dpnInterface.getDpId(), otherFes.getDpId(), elanTag);
                                remoteListBucketInfo.add(MDSALUtil.buildBucket(remoteListActionInfo,
                                        MDSALUtil.GROUP_WEIGHT, bucketId, MDSALUtil.WATCH_PORT, MDSALUtil.WATCH_GROUP));
                                bucketId++;
                            } catch (Exception ex) {
                                logger.error(
                    ...
                    List<Bucket> elanL2GwDevicesBuckets =
                            getRemoteBCGroupBucketsOfElanL2GwDevices(elanInfo, dpnId, bucketId);
                    ...
                    buildAndInstallGroup(elanInfo, groupId, dpnInterface, remoteListBucketInfo);
                }
            }
        }
    }

    private void buildAndInstallGroup(ElanInstance elanInfo, long groupId, DpnInterfaces dpnInterface,
            List<Bucket> remoteListBucketInfo) {
    ...
    private void setEtreeLeavesBCGroupOnOtherDpns(ElanInstance elanInfo, InterfaceInfo interfaceInfo, long etreeTag) {
        BigInteger dpnId = interfaceInfo.getDpId();
        long groupId = ElanUtils.getElanRemoteBCGID(etreeTag);
        List<Bucket> listBucket = new ArrayList<>();
        int bucketId = 0;
        ElanDpnInterfacesList elanDpns = ElanUtils.getElanDpnInterfacesList(elanInfo.getElanInstanceName());
        if (elanDpns != null) {
            ...
            for (DpnInterfaces dpnInterface : dpnInterfaceses) {
                ...
                if (ElanUtils.isDpnPresent(dpnInterface.getDpId()) && !dpnInterface.getDpId().equals(dpnId)
                    ...
                    for (String ifName : dpnInterface.getInterfaces()) {
                        ...
                        if (ElanUtils.isEtreeRootInterfaceByInterfaceName(ifName)) {
                            listBucket.add(MDSALUtil.buildBucket(getInterfacePortActions(ifInfo),
                                    MDSALUtil.GROUP_WEIGHT, bucketId, MDSALUtil.WATCH_PORT, MDSALUtil.WATCH_GROUP));
                            bucketId++;
                        }
                    }
                    ...
                    for (DpnInterfaces otherFes : dpnInterfaceses) {
                        if (isOtherValidDPN(dpnInterface, otherFes)) {
                            try {
                                List<Action> remoteListActionInfo = ElanUtils.getInternalItmEgressAction(
                                        dpnInterface.getDpId(), otherFes.getDpId(), etreeTag);
                                remoteListBucketInfo.add(MDSALUtil.buildBucket(remoteListActionInfo,
                                        MDSALUtil.GROUP_WEIGHT, bucketId, MDSALUtil.WATCH_PORT, MDSALUtil.WATCH_GROUP));
                                bucketId++;
                            } catch (Exception ex) {
    ...
    private boolean isOtherValidDPN(DpnInterfaces dpnInterface, DpnInterfaces otherFes) {
        return ElanUtils.isDpnPresent(otherFes.getDpId()) && otherFes.getDpId() != dpnInterface.getDpId()
    ...
    /**
     * Builds the list of instructions to be installed in the External Tunnel table (38), which so far consists in
     * writing the elanTag in metadata and send packet to the new DHCP table
     *
     * @param elanTag
    ...
    private List<InstructionInfo> getInstructionsExtTunnelTable(Long elanTag) {
        ...
        // mac learning
        mkInstructions
                .add(new InstructionInfo(InstructionType.goto_table, new long[] { ElanConstants.ELAN_DMAC_TABLE }));
    ...
    public void installMacAddressTables(ElanInstance elanInfo, InterfaceInfo interfaceInfo) {
        ...
        if (elanInterfaceMac != null && elanInterfaceMac.getMacEntry() != null) {
            ...
            for (MacEntry macEntry : macEntries) {
                PhysAddress physAddress = macEntry.getMacAddress();
                ElanUtils.setupMacFlows(elanInfo, interfaceInfo,
                        macEntry.isIsStaticAddress() ? ElanConstants.STATIC_MAC_TIMEOUT : elanInfo.getMacTimeout(),
                        physAddress.getValue());
            }
            // Programming the remoteDMACFlows
            ElanDpnInterfacesList elanDpnInterfacesList =
                    ElanUtils.getElanDpnInterfacesList(elanInfo.getElanInstanceName());
            List<DpnInterfaces> dpnInterfaceLists = elanDpnInterfacesList.getDpnInterfaces();
            for (DpnInterfaces dpnInterfaces : dpnInterfaceLists) {
                if (dpnInterfaces.getDpId().equals(interfaceInfo.getDpId())) {
                    continue;
                    ...
                    if (remoteMacEntries != null) {
                        for (MacEntry macEntry : remoteMacEntries) {
                            ...
                            ElanUtils.installDmacFlowsToInternalRemoteMac(currentDpn, remoteInterface.getDpId(),
                                    remoteInterface.getInterfaceTag(), elanInfo.getElanTag(), physAddress.getValue(),
    ...
    public void removeMacAddressTables(ElanInstance elanInfo, InterfaceInfo interfaceInfo) {
        ElanInterfaceMac elanInterfaceMac =
                ElanUtils.getElanInterfaceMacByInterfaceName(interfaceInfo.getInterfaceName());
        if (elanInterfaceMac != null && elanInterfaceMac.getMacEntry() != null) {
            List<MacEntry> macEntries = elanInterfaceMac.getMacEntry();
            for (MacEntry macEntry : macEntries) {
    ...
    public void setupElanBroadcastGroups(ElanInstance elanInfo, BigInteger dpnId) {
        setupStandardElanBroadcastGroups(elanInfo, dpnId);
        setupLeavesEtreeBroadcastGroups(elanInfo, dpnId);
    }

    private void setupStandardElanBroadcastGroups(ElanInstance elanInfo, BigInteger dpnId) {
        List<Bucket> listBucket = new ArrayList<>();
        int bucketId = 0;
        ...
        List<Bucket> listBucketInfoRemote = getRemoteBCGroupBuckets(elanInfo, dpnId, bucketId, elanInfo.getElanTag());
    ...
    public void setupLeavesEtreeBroadcastGroups(ElanInstance elanInfo, BigInteger dpnId) {
        ...
        if (etreeInstance != null) {
            ...
            DpnInterfaces dpnInterfaces =
                    ElanUtils.getElanInterfaceInfoByElanDpn(elanInfo.getElanInstanceName(), dpnId);
            bucketId = addOnlyRootInterfaces(listBucket, bucketId, dpnInterfaces);
            List<Bucket> listBucketInfoRemote =
                    getRemoteBCGroupBuckets(elanInfo, dpnId, bucketId, etreeInstance.getEtreeLeafTagVal().getValue());
            listBucket.addAll(listBucketInfoRemote);
            ...
            Group group = MDSALUtil.buildGroup(groupId, elanInfo.getElanInstanceName(), GroupTypes.GroupAll,
                    MDSALUtil.buildBucketLists(listBucket));
            logger.trace("installing etree leaves remote BroadCast Group:{}", group);
            mdsalManager.syncInstallGroup(dpnId, group, ElanConstants.DELAY_TIME_IN_MILLISECOND);
        }
    }
    ...
    private int addOnlyRootInterfaces(List<Bucket> listBucket, int bucketId, DpnInterfaces dpnInterfaces) {
        for (String ifName : dpnInterfaces.getInterfaces()) {
            InterfaceType interfaceType = InterfaceInfo.InterfaceType.VLAN_INTERFACE;
            InterfaceInfo ifInfo = interfaceManager.getInterfaceInfoFromOperationalDataStore(ifName, interfaceType);
            if (!isOperational(ifInfo)) {
    ...
    public void setupLocalBroadcastGroups(ElanInstance elanInfo, InterfaceInfo interfaceInfo) {
        setupStandardElanLocalBroadcastGroups(elanInfo, interfaceInfo);
        setupLeavesLocalBroadcastGroups(elanInfo, interfaceInfo);
    }

    private void setupStandardElanLocalBroadcastGroups(ElanInstance elanInfo, InterfaceInfo interfaceInfo) {
        List<Bucket> listBucket = new ArrayList<>();
        int bucketId = 0;
        BigInteger dpnId = interfaceInfo.getDpId();
        ...
        for (String ifName : dpnInterfaces.getInterfaces()) {
            ...
            // operational state, skip processing it
            InterfaceInfo ifInfo =
                    interfaceManager.getInterfaceInfoFromOperationalDataStore(ifName, interfaceInfo.getInterfaceType());
            if (!isOperational(ifInfo)) {
                continue;
            }
    ...
    private void setupLeavesLocalBroadcastGroups(ElanInstance elanInfo, InterfaceInfo interfaceInfo) {
        EtreeInstance etreeInstance = elanInfo.getAugmentation(EtreeInstance.class);
        if (etreeInstance != null) {
            long etreeLeafTag = etreeInstance.getEtreeLeafTagVal().getValue();
            List<Bucket> listBucket = new ArrayList<>();
    ...
    public void removeLeavesLocalBroadcastGroup(ElanInstance elanInfo, InterfaceInfo interfaceInfo) {
        EtreeInstance etreeInstance = elanInfo.getAugmentation(EtreeInstance.class);
        if (etreeInstance != null) {
            BigInteger dpnId = interfaceInfo.getDpId();
            long groupId = ElanUtils.getElanLocalBCGID(etreeInstance.getEtreeLeafTagVal().getValue());
            List<Bucket> listBuckets = new ArrayList<>();
            Group group = MDSALUtil.buildGroup(groupId, elanInfo.getElanInstanceName(), GroupTypes.GroupAll,
                    MDSALUtil.buildBucketLists(listBuckets));
            logger.trace("deleted the etree localBroadCast Group:{}", group);
    ...
    public void removeLeavesEtreeBroadcastGroup(ElanInstance elanInfo, InterfaceInfo interfaceInfo) {
        EtreeInstance etreeInstance = elanInfo.getAugmentation(EtreeInstance.class);
        if (etreeInstance != null) {
            List<Bucket> listBuckets = new ArrayList<>();
            BigInteger dpnId = interfaceInfo.getDpId();
            long groupId = ElanUtils.getEtreeLeafRemoteBCGID(etreeInstance.getEtreeLeafTagVal().getValue());
            Group group = MDSALUtil.buildGroup(groupId, elanInfo.getElanInstanceName(), GroupTypes.GroupAll,
                    MDSALUtil.buildBucketLists(listBuckets));
            logger.trace("deleting the etree remoteBroadCast group:{}", group);
            mdsalManager.syncRemoveGroup(dpnId, group);
        }
    }
    ...
    /**
     * Installs a flow in the External Tunnel table consisting in translating the VNI retrieved from the packet that
     * came over a tunnel with a TOR into elanTag that will be used later in the ELANs pipeline.
     *
     * @param dpnId
     *            the dpn id
     * @param elanInfo
    ...
    public void setExternalTunnelTable(BigInteger dpnId, ElanInstance elanInfo) {
        ...
        FlowEntity flowEntity = MDSALUtil.buildFlowEntity(dpnId, NwConstants.EXTERNAL_TUNNEL_TABLE,
                ...
                buildMatchesForVni(elanInfo.getVni()), getInstructionsExtTunnelTable(elanTag));

        mdsalManager.installFlow(flowEntity);
    }

    /**
     * Removes, from External Tunnel table, the flow that translates from VNI to elanTag. Important: ensure this method
     * is only called whenever there is no other ElanInterface in the specified DPN
     *
     * @param dpnId
    ...
    public void setupTerminateServiceTable(ElanInstance elanInfo, InterfaceInfo interfaceInfo) {
        setupTerminateServiceTable(elanInfo, interfaceInfo, elanInfo.getElanTag());
        setupEtreeTerminateServiceTable(elanInfo, interfaceInfo);
    }

    private void setupEtreeTerminateServiceTable(ElanInstance elanInfo, InterfaceInfo interfaceInfo) {
        EtreeInstance etreeInstance = elanInfo.getAugmentation(EtreeInstance.class);
        if (etreeInstance != null) {
            setupTerminateServiceTable(elanInfo, interfaceInfo, etreeInstance.getEtreeLeafTagVal().getValue());
        }
    }

    public void setupTerminateServiceTable(ElanInstance elanInfo, InterfaceInfo interfaceInfo, long elanTag) {
    ...
    private void installLocalBCFlow(ElanInstance elanInfo, InterfaceInfo interfaceInfo, long elanTag) {
        if (elanInfo.getVni() != null && elanInfo.getVni() != 0) {
            Flow flowEntity2 = MDSALUtil.buildFlowNew(ElanConstants.ELAN_UNKNOWN_DMAC_TABLE,
                    getUnknownDmacFlowRef(ElanConstants.ELAN_UNKNOWN_DMAC_TABLE, elanTag, /* SH flag */true), 5,
    ...
    private void installRemoteBCFlow(ElanInstance elanInfo, InterfaceInfo interfaceInfo, long elanTag) {
        Flow flowEntity = MDSALUtil.buildFlowNew(ElanConstants.ELAN_UNKNOWN_DMAC_TABLE,
                getUnknownDmacFlowRef(ElanConstants.ELAN_UNKNOWN_DMAC_TABLE, elanTag, /* SH flag */false), 5,
                elanInfo.getElanInstanceName(), 0, 0,
                ElanConstants.COOKIE_ELAN_UNKNOWN_DMAC.add(BigInteger.valueOf(elanTag)),
    ...
    private void removeStaticELanFlows(final ElanInstance elanInfo, final InterfaceInfo interfaceInfo) {
        BigInteger dpId = interfaceInfo.getDpId();
        /*
         * If there are not elan ports, remove the unknown smac and default dmac flows
         */
        ...
        if (dpnInterfaces == null || dpnInterfaces.getInterfaces() == null || dpnInterfaces.getInterfaces().isEmpty()) {
            ...
            removeEtreeBroadcastGroups(elanInfo, interfaceInfo);
    ...
    private void removeEtreeBroadcastGroups(final ElanInstance elanInfo, final InterfaceInfo interfaceInfo) {
        removeLeavesEtreeBroadcastGroup(elanInfo, interfaceInfo);
        removeLeavesLocalBroadcastGroup(elanInfo, interfaceInfo);
    }
    ...
    private void removeUnknownDmacFlow(BigInteger dpId, ElanInstance elanInfo) {
        ...
        Flow flow =
                new FlowBuilder()
                        .setId(new FlowId(getUnknownDmacFlowRef(ElanConstants.ELAN_UNKNOWN_DMAC_TABLE,
                                elanInfo.getElanTag(), /* SH flag */ false)))
                        .setTableId(ElanConstants.ELAN_UNKNOWN_DMAC_TABLE).build();
        mdsalManager.removeFlow(dpId, flow);
    ...
    private void bindService(ElanInstance elanInfo, ElanInterface elanInterface) {
        if (isStandardElanService(elanInterface)) {
            ...
            bindEtreeService(elanInfo, elanInterface);
        }
    }

    private void bindEtreeService(ElanInstance elanInfo, ElanInterface elanInterface) {
    ...
    private void bindElanService(long elanTag, String elanInstanceName, String interfaceName) {
        ...
        BoundServices serviceInfo = ElanUtils.getBoundServices(
                String.format("%s.%s.%s", "vpn", elanInstanceName, interfaceName), ElanConstants.ELAN_SERVICE_INDEX,
    ...
    private void unbindService(ElanInstance elanInfo, String interfaceName) {
        ElanUtils.delete(broker, LogicalDatastoreType.CONFIGURATION,
                ElanUtils.buildServiceId(interfaceName, ElanConstants.ELAN_SERVICE_INDEX), ElanUtils.DEFAULT_CALLBACK);
    }
    ...
    private void createElanInterfaceTablesList(String interfaceName) {
        InstanceIdentifier<ElanInterfaceMac> elanInterfaceMacTables =
                ElanUtils.getElanInterfaceMacEntriesOperationalDataPath(interfaceName);
        Optional<ElanInterfaceMac> interfaceMacTables =
                ElanUtils.read(broker, LogicalDatastoreType.OPERATIONAL, elanInterfaceMacTables);
    ...
    private void createElanStateList(String elanInstanceName, String interfaceName) {
        ...
        if (elanInterfaceLists.isPresent()) {
            ...
            MDSALUtil.syncWrite(broker, LogicalDatastoreType.OPERATIONAL,
                    ElanUtils.getElanInstanceOperationalDataPath(elanInstanceName), elanState);
        }
    }
    ...
    protected void updatedIfPrimaryAttributeChanged(ElanInterface elanInterface, boolean isUpdated) {
        ...
        if (macEntries != null && !macEntries.isEmpty()) {
            for (MacEntry macEntry : macEntries) {
                if (isUpdated) {
                    ElanUtils.setupMacFlows(elanInfo, interfaceInfo, ElanConstants.STATIC_MAC_TIMEOUT,
                            macEntry.getMacAddress().getValue());
                } else {
                    ElanUtils.deleteMacFlows(elanInfo, interfaceInfo, macEntry);
                }
            }
        ...
        InstanceIdentifier<DpnInterfaces> dpnInterfaceId = ElanUtils
                .getElanDpnInterfaceOperationalDataPath(elanInterface.getElanInstanceName(), interfaceInfo.getDpId());
        Optional<DpnInterfaces> dpnInterfaces =
                ElanUtils.read(broker, LogicalDatastoreType.OPERATIONAL, dpnInterfaceId);
        ...
        DpnInterfaces updateDpnInterfaces = new DpnInterfacesBuilder().setInterfaces(interfaceLists).setDpId(dpId)
                .setKey(new DpnInterfacesKey(dpId)).build();
    ...
    private boolean validateExternalTunnelStateEvent(ExternalTunnel externalTunnel, Interface intrf) {
        if (intrf.getOperStatus() == Interface.OperStatus.Up) {
            ...
            ExternalTunnel otherEndPointExtTunnel =
                    ElanUtils.getExternalTunnel(srcDevice, destDevice, LogicalDatastoreType.CONFIGURATION);
            ...
            if (otherEndPointExtTunnel != null) {
                boolean otherEndPointInterfaceOperational =
                        ElanUtils.isInterfaceOperational(otherEndPointExtTunnel.getTunnelInterfaceName(), broker);
    ...
    public void handleInterfaceUpdated(InterfaceInfo interfaceInfo, ElanInstance elanInstance, boolean isStateUp) {
        ...
        if (isStateUp) {
            ...
            if (dpnInterfaces == null) {
                ...
                DpnInterfaces dpnInterface = new DpnInterfacesBuilder().setDpId(dpId).setInterfaces(dpnElanInterfaces)
                        .setKey(new DpnInterfacesKey(dpId)).build();
                MDSALUtil.syncWrite(broker, LogicalDatastoreType.OPERATIONAL,
                        ElanUtils.getElanDpnInterfaceOperationalDataPath(elanName, interfaceInfo.getDpId()),
                        dpnInterface);
            }
            ...
            if (elanInstance.getVni() != null && elanInstance.getVni() != 0) {
                List<PhysAddress> macAddresses =
                        ElanUtils.getElanInterfaceMacAddresses(interfaceInfo.getInterfaceName());
    ...
    public static List<Bucket> getRemoteBCGroupBucketsOfElanL2GwDevices(ElanInstance elanInfo, BigInteger dpnId,
            int bucketId) {
        List<Bucket> listBucketInfo = new ArrayList<>();
        ConcurrentMap<String, L2GatewayDevice> map =
                ElanL2GwCacheUtils.getInvolvedL2GwDevices(elanInfo.getElanInstanceName());
        for (L2GatewayDevice device : map.values()) {
            String interfaceName =
                    ElanL2GatewayUtils.getExternalTunnelInterfaceName(String.valueOf(dpnId), device.getHwvtepNodeId());
