...
public class EvpnUtils {
    ...
    private void programEvpnL2vniFlow(ElanInstance elanInfo, BiConsumer<BigInteger, FlowEntity> flow) {
        ...
        NWUtil.getOperativeDPNs(broker).forEach(dpnId -> {
            ...
            FlowEntity flowEntity = MDSALUtil.buildFlowEntity(
                    ...
                    mkMatches,
                    instructions);
            flow.accept(dpnId, flowEntity);
        });
    }

    public void programEvpnL2vniDemuxTable(String elanName, BiConsumer<String, String> bindService,
                                           BiConsumer<BigInteger, FlowEntity> flow) {
        ElanInstance elanInfo = ElanUtils.getElanInstanceByName(broker, elanName);
        ...
        tunnelInterfaceNameList.forEach(tunnelInterfaceName -> {
            bindService.accept(elanName, tunnelInterfaceName);
        });
        programEvpnL2vniFlow(elanInfo, flow);
    }
}
