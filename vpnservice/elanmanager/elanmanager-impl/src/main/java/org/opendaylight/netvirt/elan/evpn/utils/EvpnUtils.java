...
public class EvpnUtils {
    ...
    public boolean isWithdrawEvpnRT2Routes(ElanInstance original, ElanInstance update) {
        return isNetDetach.test(getEvpnNameFromElan(original).get(), getEvpnNameFromElan(update).get());
    }

    public boolean isAdvertiseEvpnRT2Routes(ElanInstance original, ElanInstance update) {
        return isNetAttach.test(getEvpnNameFromElan(original).get(), getEvpnNameFromElan(update).get());
    }
    ...
    public com.google.common.base.Optional<String> getGatewayMacAddressForInterface(String vpnName,
        ...
        return com.google.common.base.Optional.of((gwPort != null && gwPort.isSubnetIp())
                ? gwPort.getMacAddress()
                : interfaceManager.getInterfaceInfoFromOperationalDataStore(ifName).getMacAddress());
    }

    public Optional<String> getL3vpnNameFromElan(ElanInstance elanInfo) {
        if (elanInfo == null) {
        ...
        return Optional.of(evpnAugmentation != null ? evpnAugmentation.getL3vpnName() : null);
    }

    public static Optional<String> getEvpnNameFromElan(ElanInstance elanInfo) {
        if (elanInfo == null) {
            LOG.debug("getEvpnNameFromElan :elanInfo is NULL");
            return null;
        }
        EvpnAugmentation evpnAugmentation = elanInfo.getAugmentation(EvpnAugmentation.class);
        return Optional.of(evpnAugmentation != null ? evpnAugmentation.getEvpnName() : null);
    }

    public String getEvpnRd(ElanInstance elanInfo) {
        Optional<String> evpnName = getEvpnNameFromElan(elanInfo);
    ...
    private void advertisePrefix(ElanInstance elanInfo, String rd,
        ...
        Optional<String> l3VpName = getL3vpnNameFromElan(elanInfo);
        if (l3VpName.isPresent()) {
            VpnInstance l3VpnInstance = vpnManager.getVpnInstance(broker, l3VpName.get());
            l3vni = l3VpnInstance.getL3vni();
            com.google.common.base.Optional<String> gatewayMac = getGatewayMacAddressForInterface(l3VpName.get(),
