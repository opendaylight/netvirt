...
public class EvpnUtils {
    ...
    public String getEvpnNameFromElan(ElanInstance elanInfo) {
        EvpnAugmentation evpnAugmentation = elanInfo.getAugmentation(EvpnAugmentation.class);
    ...
    private void advertisePrefix(ElanInstance elanInfo, String rd,
        ...
        if (rd == null) {
            LOG.error("advertisePrefix rd is null for elanName {}, interfaceName {}",
                    elanInfo.getElanInstanceName(), interfaceName);
            return;
        }
    ...
    public void withdrawEvpnRT2Routes(EvpnAugmentation evpnAugmentation, String elanName) {
        ...
        if (rd == null) {
            if (rd == null) {
                LOG.error("withdrawEvpnRT2Routes rd is null for elanName {}, macEntries {}", elanName, macEntries);
                return;
            }
        }
    ...
    public void withdrawPrefix(ElanInstance elanInfo, String prefix) {
        String rd = getEvpnRd(elanInfo);
        if (rd == null) {
            LOG.error("withdrawPrefix rd is null for elanName {}, prefix {}", elanInfo.getElanInstanceName(), prefix);
            return;
        }
        bgpManager.withdrawPrefix(rd, prefix);
    }
