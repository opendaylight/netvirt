...
public class ElanOvsdbNodeListener extends AbstractDataChangeListener<Node> {
    ...
    protected void add(InstanceIdentifier<Node> identifier, Node node) {
        ...
        Optional<Map<String, String>> providerMappings = bridgeUtils.getOpenvswitchOtherConfigMap(node,
                BridgeUtils.PROVIDER_MAPPINGS_KEY);

        String datapathId = bridgeUtils.getBridgeDatapathId(node);
        if (datapathId != null) {
            addPhysnetPortsToCache(datapathId, providerMappings.or(Collections.emptyMap()));
        }
    }

    private void addPhysnetPortsToCache(String datapathId, Map<String, String> providerMappings) {
        for (Entry<String, String> providerMapping : providerMappings.entrySet()) {
            ElanPhysicalPortCacheUtils.addPhysnetPort(providerMapping.getKey(), datapathId, providerMapping.getValue());
        }
    }
