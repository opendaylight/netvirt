...
public class ElanPacketInHandler implements PacketProcessingListener {
    ...
    public void onPacketReceived(PacketReceived notification) {
        ...
        if (pktInReason == NoMatch.class && tableId == NwConstants.ELAN_SMAC_TABLE) {
            ...
            try {
                ...
                enqueueJobForFlowsManipulation(macAddress, elanTag, interfaceName, physAddress, elanInstance,
                        interfaceInfo, macEntry, portDataStoreCoordinator);

                enqueueJobForMacEntriesManipulation(macAddress, elanTag, interfaceName, elanName, physAddress, macEntry,
    ...
    private void enqueueJobForMacEntriesManipulation(final String macAddress, final long elanTag, String interfaceName,
            String elanName, PhysAddress physAddress, MacEntry macEntry,
        ...
        portDataStoreCoordinator.enqueueJob(ElanUtils.getElanMacKey(elanTag, macAddress), () -> {
            ...
            BindingTransactionChain txChain = broker.createTransactionChain(ElanUtils.DEFAULT_CHAIN_CALLBACK);
            WriteTransaction writeTx = txChain.newWriteOnlyTransaction();
            if (macEntry != null && macEntry.getInterface().equals(interfaceName)) {
                /*
                 * Protection time expired. Even though the MAC has been learnt (it is in the cache) the packets are
                 * punted to controller. Which means, the the flows were not successfully created in the DPN, but the
                 * MAC entry has been added successfully in the cache.
                 *
                 ...
                 */
                InstanceIdentifier<MacEntry> macEntryId1 =
                        ElanUtils.getInterfaceMacEntriesIdentifierOperationalDataPath(interfaceName, physAddress);
                writeTx.delete(LogicalDatastoreType.OPERATIONAL, macEntryId1);
                ...
                // MAC flows
                InstanceIdentifier<MacEntry> macEntryId2 =
                        ElanUtils.getInterfaceMacEntriesIdentifierOperationalDataPath(interfaceName, physAddress);
                writeTx.delete(LogicalDatastoreType.OPERATIONAL, macEntryId2);
                futures.add(writeTx.submit());
            ...
            InstanceIdentifier<MacEntry> macEntryId3 =
                    ElanUtils.getInterfaceMacEntriesIdentifierOperationalDataPath(interfaceName, physAddress);
            writeTx.put(LogicalDatastoreType.OPERATIONAL, macEntryId3, newMacEntry);
    ...
    private void enqueueJobForFlowsManipulation(final String macAddress, final long elanTag, String interfaceName,
