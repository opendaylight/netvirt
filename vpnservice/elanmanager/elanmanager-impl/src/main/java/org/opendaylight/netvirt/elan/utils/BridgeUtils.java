...
public class BridgeUtils {
    ...
    public boolean isOvsdbNode(Node node) {
        return ovsdbSouthboundUtils.extractNodeAugmentation(node) != null;
    }
    ...
    public boolean isIntegrationBridge(Node node) {
        ...
        String bridgeName = ovsdbSouthboundUtils.extractBridgeName(node);
        if (bridgeName == null) {
            return false;
        }
    ...
    public boolean isBridgeNode(Node node) {
        return ovsdbSouthboundUtils.extractBridgeAugmentation(node) != null;
    }

    public void prepareNode(Node node, boolean generateIntBridgeMac) {
        ...
        Node ovsdbNode = ovsdbSouthboundUtils.readOvsdbNode(node);
        if (ovsdbNode == null) {
    ...
    public void patchExternalPhysnetBridgeToBrInt(Node ovsdbNode, Node physnetBridge) {
        String physnetBridgeName = ovsdbSouthboundUtils.extractBridgeName(physnetBridge);
        String portName = physnetBridgeName.substring(EXT_BR_PFX.length());

        Node intBridgeNode = ovsdbSouthboundUtils.readBridgeNode(ovsdbNode, INTEGRATION_BRIDGE);
    ...
    public void createExternalBridgesForPhysnets(Node ovsdbNode) {
    ...
    public void createBridges(Node ovsdbNode, boolean generateIntBridgeMac) {
    ...
    /*
    public void preparePhysicalInterfaces(Node intBridgeNode) {
        LOG.warn("preparePhysicalInterfaces {}", intBridgeNode);
        Node ovsdbNode = ovsdbSouthboundUtils.readOvsdbNode(intBridgeNode);
        if (ovsdbNode == null) {
            LOG.error("Could not extract ovsdb node from int-br node {}", intBridgeNode);
            return;
        }
        LOG.warn("preparePhysicalInterfaces ovsdb {}", ovsdbNode);

        Optional<Map<String, String>> providerMappings = getOpenvswitchOtherConfigMap(ovsdbNode, PROVIDER_MAPPINGS_KEY);

        for (String portName : providerMappings.or(Collections.emptyMap()).values()) {
            addPhysicalInterface(ovsdbNode, intBridgeNode, portName);
        }
    }
    */
    ...
    public String getBridgeSidePatchName(String physicalInterfaceName) {
        return "patch-br-" + physicalInterfaceName;
    }

    /*
    public boolean addPhysicalInterface(Node ovsdbNode, Node intBridgeNode, String externalInterfaceName) {
        ...
        //waitForBridge(ovsdbNode, bridgeName);
        Node extBridge = ovsdbSouthboundUtils.readBridgeNode(ovsdbNode, bridgeName);
        if (extBridge == null) {
            LOG.error("Timed out waiting for bridge {} in operational", bridgeName);
        }

        if (!addPortToBridge(extBridge, bridgeName, externalInterfaceName)) {
            return false;
        }

        String portNameInt = getPatchPortName(externalInterfaceName);
        String portNameExt = getBridgeSidePatchName(externalInterfaceName);
        if (!addPatchPort(intBridgeNode, INTEGRATION_BRIDGE, portNameInt, portNameExt)) {
            return false;
        }

        if (!addPatchPort(extBridge, bridgeName, portNameExt, portNameInt)) {
            return false;
        }
