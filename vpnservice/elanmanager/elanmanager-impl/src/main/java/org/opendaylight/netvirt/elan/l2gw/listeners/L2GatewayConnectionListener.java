...
public class L2GatewayConnectionListener extends AsyncClusteredDataTreeChangeListenerBase<L2gatewayConnection,
    ...
    private static Function<Node, InstanceIdentifier<Node>> TO_GLOBAL_PATH = (psNode) -> {
        return HwvtepHAUtil.getGlobalNodePathFromPSNode(psNode);
    };

    private static Function<Node, InstanceIdentifier<Node>> TO_NODE_PATH = (node) -> {
        return HwvtepSouthboundUtils.createInstanceIdentifier(node.getNodeId());
    };

    private static Function<InstanceIdentifier<Node>, String> GET_DEVICE_NAME = (psIid) -> {
        return HwvtepHAUtil.getPsName(psIid);
    };

    private static Predicate<InstanceIdentifier<Node>> IS_PS_NODE = (psIid) -> {
        return HwvtepHAUtil.getPsName(psIid) != null;
    };

    private static Predicate<Node> IS_HA_PARENT_NODE = (node) -> {
    ...
    private static BiPredicate<InstanceIdentifier<Node>, Node> PS_NODE_OF_PARENT_NODE = (psIid, node) -> {
    ...
    private void loadL2GwDeviceCache(final int trialNo) {
        scheduler.getScheduledExecutorService().schedule(() -> {
            ...
            CheckedFuture<Optional<Topology>, ReadFailedException> ft = tx.read(CONFIGURATION, topoIid);
            Futures.addCallback(ft, new FutureCallback<Optional<Topology>>() {
                @Override
