...
public class TransportZoneNotificationUtil {
    ...
    private boolean shouldCreateVtep(VpnToDpnList vpnToDpn) {
        // A vtep should be created only if an internal interface exists
        return !vpnToDpn.getVpnInterfaces().stream()
                .allMatch(vi -> interfaceManager.isExternalInterface(vi.getInterfaceName()));
    }
