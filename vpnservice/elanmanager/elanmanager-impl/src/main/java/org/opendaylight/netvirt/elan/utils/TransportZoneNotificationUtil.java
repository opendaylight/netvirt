...
public class TransportZoneNotificationUtil {
    ...
    public void updateTransportZone(List<String> elanInterfaces) {
        ...
        InstanceIdentifier<ElanDpnInterfaces> elanDpnInterfacesInst = InstanceIdentifier.create(ElanDpnInterfaces.class);
        ...
        {
            LOG.warn("No elan dpn interfaces found.");
            return;
        }

        for (String elanInterface : elanInterfaces) {

            Optional<ElanDpnInterfacesList> elanDpnInterfacesList = elanDpnInterfaces.getElanDpnInterfacesList().stream().filter(
                    e -> e.getDpnInterfaces().stream().anyMatch(di -> di.getInterfaces().contains(elanInterface))).findFirst();

            if (!elanDpnInterfacesList.isPresent())
            {
                LOG.warn("elan interface {} not found in elan dpn interfaces.", elanInterface);
                continue;
            }

            Optional<DpnInterfaces> dpnInterfaces = elanDpnInterfacesList.get().getDpnInterfaces().stream().filter(
                    di -> di.getInterfaces().contains(elanInterface)).findFirst();
            ...
            InstanceIdentifier<TransportZone> inst = InstanceIdentifier.create(TransportZones.class)
                    .child(TransportZone.class, new TransportZoneKey(elanDpnInterfacesList.get().getElanInstanceName()));
            ...
            try {
                ...
                LOG.warn("failed to add tunnels for elan interface {}. ", elanInterface, e);
            }
        }
