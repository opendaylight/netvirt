...
public class TransportZoneNotificationUtil {
    ...
    public void updateTransportZone(List<String> elanInterfaces) {
        ...
        for (String elanInterface : elanInterfaces) {
            ...
            Optional<DpnInterfaces> dpnInterfaces = elanDpnInterfacesList.get().getDpnInterfaces()
                    .stream().filter(di -> di.getInterfaces().contains(elanInterface)).findFirst();
            ...
            InstanceIdentifier<TransportZone> inst = InstanceIdentifier.create(TransportZones.class)
                    .child(TransportZone.class, new TransportZoneKey(
                            elanDpnInterfacesList.get().getElanInstanceName()));
    ...
    /*
     * takes all elan interface ports that are related to the given interface state
     *
    ...
    private List<ElanInterface> getElanInterfacesFromInterface(InterfaceInfo interfaceStateFromOperDS) {
        ...
        for (Interface inter : inters) {
            ...
            if (elanInt == null) {
                LOG.debug("got Interface State of non elan interface {}", interfaceStateFromOperDS.getPortName());
                continue;
            }
