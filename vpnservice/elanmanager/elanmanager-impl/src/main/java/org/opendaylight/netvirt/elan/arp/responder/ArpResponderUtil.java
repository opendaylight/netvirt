...
public class ArpResponderUtil {
    ...
    public static List<MatchInfo> getMatchCriteria(final java.util.Optional<Integer> lportTag,
        ...
        final BigInteger metadata = lportTag.map(v->ElanUtils.getElanMetadataLabel(elanInstance.getElanTag(), v)).orElse(ElanUtils.getElanMetadataLabel(elanInstance.getElanTag()));
        final BigInteger metadataMask = lportTag.map(v->MetaDataUtil.METADATA_MASK_SERVICE.or(MetaDataUtil.METADATA_MASK_LPORT_TAG)).orElse(MetaDataUtil.METADATA_MASK_SERVICE);
...
//        actions.addAll(getEgressActionsForInterface(ifaceMgrRpcService, vpnInterface, actionCounter));
    ...
    public static String getFlowID(final java.util.Optional<Integer> lportTag, final String ipAdress) {
        return lportTag.map(v->MessageFormat.format(ArpResponderConstant.FLOW_ID_FORMAT_WITH_LPORT.value(),
            NwConstants.ARP_RESPONDER_TABLE, v, ipAdress)).orElse(MessageFormat.format(ArpResponderConstant.FLOW_ID_FORMAT_WITHOUT_LPORT.value(),
    ...
    public static BigInteger generateCookie(final java.util.Optional<Integer> lportTag,
        ...
        final BigInteger cookie = NwConstants.COOKIE_ARP_RESPONDER.add(BigInteger.ONE).add(BigInteger.valueOf(ipTolong(ipAddress)));
