...
public class NodeConnectedHandler {
    ...
    private void copyHANodeConfigToChild(Node srcNode,
            ...
            throws ReadFailedException, ExecutionException, InterruptedException {
        if (srcNode == null) {
            LOG.info("copyHANodeConfigToChild no parent node found");
            return;
        }
        HwvtepGlobalAugmentation src = srcNode.getAugmentation(HwvtepGlobalAugmentation.class);
        if (src == null) {
            LOG.info("copyHANodeConfigToChild no augmentation in parent node {}",
                    srcNode.getNodeId().getValue());
            return;
        }
        LOG.info("copyHANodeConfigToChild from parent {}",
                srcNode.getNodeId().getValue());
    ...
    private void copyChildOpToHA(Node childNode,
                                 ...
                                 ReadWriteTransaction tx)
            throws ReadFailedException, ExecutionException, InterruptedException {
        LOG.info("HA child reconnected handleNodeReConnected {} copyChildOpToHA",
                childNode.getNodeId().getValue());
        if (childNode == null) {
            return;
        }
        ...
        if (childData == null) {
            LOG.info("HA child reconnected handleNodeReConnected {} copyChildOpToHA nothing to copy",
                    childNode.getNodeId().getValue());
            return;
        }
