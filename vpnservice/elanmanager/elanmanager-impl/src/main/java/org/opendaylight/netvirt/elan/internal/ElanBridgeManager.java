...
public class ElanBridgeManager {
    ...
    public void processNodePrep(Node node, boolean generateIntBridgeMac) {
        ...
        if (isIntegrationBridge(node)) {
            addPhysnetInterfaces(ovsdbNode, node);
        }

    }

   private void addPhysnetInterfaces(Node ovsdbNode, Node intBridgeNode) {
        ...
        for (String portName : providerMappings.or(Collections.emptyMap()).values()) {
            if (!addPortToBridge(intBridgeNode, INTEGRATION_BRIDGE, portName)) {
                LOG.error("Failed to add {} port to {}", portName, intBridgeNode);
                return;
            }
        }
    }
