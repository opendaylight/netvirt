...
public class ElanBridgeManager implements IElanBridgeManager {
    ...
    public void handleNewProviderNetBridges(Node originalNode, Node updatedNode) {
        ...
        for (ManagedNodeEntry nodeEntry : updatedManagedNodes) {
            String nodeId = nodeEntry.getBridgeRef().getValue().firstKeyOf(Node.class).getNodeId().getValue();
            nodeId = nodeId.substring(nodeId.lastIndexOf('/') + 1);
            if (nodeId.equals(INTEGRATION_BRIDGE)) {
                continue;
            }
            if (providerVals.contains(nodeId)) {
                patchBridgeToBrInt(brInt, southboundUtils.readBridgeNode(updatedNode, nodeId), nodeId);
            }
        }
