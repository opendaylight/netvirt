...
public class ElanBridgeManager implements IElanBridgeManager {
    ...
    private void prepareIntegrationBridge(Node ovsdbNode, Node brIntNode) {
        if (southboundUtils.getBridgeFromConfig(ovsdbNode, INTEGRATION_BRIDGE) == null) {
            ...
            List<OvsdbTerminationPointAugmentation> tps = southboundUtils.extractTerminationPointAugmentations(brIntNode);
            if (null != null) {
                for(OvsdbTerminationPointAugmentation aug : tps) {
                    OvsdbTerminationPointAugmentationBuilder augBuilder = new OvsdbTerminationPointAugmentationBuilder(aug);
                    augBuilder.setIfindex(null);
                    bridgeNodeBuilder.addAugmentation(OvsdbTerminationPointAugmentation.class, augBuilder.build());
                }
            }
...
//                    new OvsdbBridgeName(INTEGRATION_BRIDGE));
            LOG.warn("JOSH {}", bridgeNodeBuilder);
