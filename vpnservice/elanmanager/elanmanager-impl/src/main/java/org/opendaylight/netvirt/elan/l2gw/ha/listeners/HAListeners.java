...
public class HAListeners implements AutoCloseable {
    ...
    public HAListeners(DataBroker broker) {
        this.broker = broker;
        addListener(LocalMcastMacs.class, new LocalMcastCmd());
        addListener(RemoteMcastMacs.class, new RemoteMcastCmd());
        addListener(LocalUcastMacs.class, new LocalUcastCmd());
        addListener(RemoteUcastMacs.class, new RemoteUcastCmd());
        addListener(LogicalSwitches.class, new LogicalSwitchesCmd());
        ...
        LOG.error("check registered listeners");
    }
    ...
    private <T extends ChildOf<HwvtepGlobalAttributes>> void addListener(Class<T> clazz, MergeCommand mergeCommand) {
    ...
    class PhysicalLocatorListener extends HwvtepNodeDataListener<TerminationPoint> {
        ...
        boolean isGlobalNode(InstanceIdentifier<TerminationPoint> identifier) {
            return !identifier.firstKeyOf(Node.class).getNodeId().getValue().contains("physical");
        }
    }
}
