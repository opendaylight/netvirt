...
public class ElanUtilsTest {
    ...
    @Mock RpcProviderRegistry rpcRegistry;
    @Mock OdlInterfaceRpcService ifaceMgrRpcService;
    @Mock ItmRpcService itmRpcService;
    ...
    @Mock ElanInstanceManager elanInstanceManager;
    @Mock ElanBridgeManager bridgeMgr;
    @Mock ElanInterfaceManager elanInterfaceManager;
    @Mock ElanStatusMonitor elanStatusMonitor;
    //    @Mock ElanUtils elanUtils;
    @Mock EntityOwnershipService entityOwnershipService;
    ElanServiceProvider elanServiceProvider;
    ElanUtils elanUtils;
    private static final Logger logger = LoggerFactory.getLogger(ElanUtilsTest.class);

    @Before
    public void setUp() throws Exception {
        elanServiceProvider = new ElanServiceProvider(idManager, interfaceManager,
                elanInstanceManager, bridgeMgr,
                dataBroker, elanInterfaceManager,
                elanStatusMonitor, elanUtils,
                entityOwnershipService);

        when(dataBroker.newReadOnlyTransaction()).thenReturn(readTx);
        when(dataBroker.newWriteOnlyTransaction()).thenReturn(writeTx);
        elanUtils = new ElanUtils(dataBroker, mdsalMgr, elanInstanceManager, ifaceMgrRpcService, itmRpcService, elanInterfaceManager, entityOwnershipService, bridgeMgr);
        ...
        GetTunnelInterfaceNameOutput output2 =
                new GetTunnelInterfaceNameOutputBuilder().setInterfaceName(ElanTestTopo.dpn2ToDpn1TrunkIfaceName)
                .build();
        Future future4 = mock(Future.class);
        when(future4.get()).thenReturn(RpcResultBuilder.success(output2).build());
        Class<? extends TunnelTypeBase> tunType = TunnelTypeVxlan.class;

        GetTunnelInterfaceNameInput input2 =
                new GetTunnelInterfaceNameInputBuilder().setDestinationDpid(ElanTestTopo.dpn1Id)
                .setSourceDpid(ElanTestTopo.dpn2Id)
        ...
        GetTunnelInterfaceNameOutput output3 = new GetTunnelInterfaceNameOutputBuilder()
                .setInterfaceName(ElanTestTopo.dpn1ToDpn2TrunkIfaceName)
                .build();
        ...
        GetTunnelInterfaceNameInput input3 = new GetTunnelInterfaceNameInputBuilder()
                .setDestinationDpid(ElanTestTopo.dpn2Id)
                .setSourceDpid(ElanTestTopo.dpn1Id)
                .setTunnelType(tunType)
                .build();
    ...
    public void stubSuccessfulGetEgressActions(String localIfName) throws InterruptedException, ExecutionException {
        List<Action> localIfEgressActions = ElanTestTopo.localIfActionsMap.get(localIfName);
        GetEgressActionsForInterfaceOutput ifEgressActionsOutput =
                new GetEgressActionsForInterfaceOutputBuilder().setAction(localIfEgressActions).build();
        ...
        GetEgressActionsForInterfaceInput input =
                new GetEgressActionsForInterfaceInputBuilder().setIntfName(ElanTestTopo.dpn11IfaceName).build();
    ...
    private void stubGetInternalTunnelEgressActions(String trunkIfName, String localIfName, boolean success)
        ...
        GetEgressActionsForInterfaceOutput output =
                new GetEgressActionsForInterfaceOutputBuilder().setAction(egressActions).build();
        Future future = mock(Future.class);
        if ( success ) {
        ...
        GetEgressActionsForInterfaceInput input =
                new GetEgressActionsForInterfaceInputBuilder().setIntfName(trunkIfName)
                .setTunnelKey(Long.valueOf(lportTag))
                .build();
    ...
    public void testSetupMacFlows() throws Exception {
        ...
        when(readTx.read(eq(LogicalDatastoreType.OPERATIONAL),
                argThat(isIIdType(ElanDpnInterfacesList.class)))).thenReturn(chkdFuture2);


        when(readTx.read(eq(LogicalDatastoreType.CONFIGURATION),
                eq(ElanUtils.getElanInterfaceConfigurationDataPathId(ElanTestTopo.dpn11Info.getInterfaceName())))).thenReturn(chkdFuture3);
        when(chkdFuture3.get()).thenReturn(Optional.of(new ElanInterfaceBuilder().build()));
        // Whenever ifaceMgr is asked about the egressActions for Tunnel between DPN2 and DPN1 ("INT_TUNNEL21") with
        // lportTag of dpn11 as tunnelKey
        stubGetInternalTunnelEgressActions(ElanTestTopo.dpn2ToDpn1TrunkIfaceName,
                ElanTestTopo.dpn11IfaceName,  // to retrieve the lportTag
                /*success*/true);
        ...
        // Method under test /////////
        //////////////////////////////
        // Testing that a new ElanInterface has been created on the interface "dpn11"
        elanUtils.setupMacFlows(elan222.getElanInstance(),ElanTestTopo.dpn11Info,
                elan222.getMacTimeout(),  elan222Vm1Mac.getValue(), writeTx);
        ...
        verify(mdsalMgr).addFlowToTx(argThat(new FlowEntityMatcher(expectedSmacFlowEntity)), any(WriteTransaction.class));
        ...
        verify(mdsalMgr, times(1)).addFlowToTx(any(BigInteger.class), argumentCaptor.capture(), any(WriteTransaction.class));

        // argumentCaptor should have exactly 1 values because mdsalMgr.installFlow is called 1 times for the same DpId,
        //  for DMAC
    ...
    public void testSetupMacFlowsDpnNotPresent() throws Exception {
        ...
        // Whenever broker is asked about if DPN2 is present, return Optional.absent()
        ...
        // Checking that the Tunnel Termination flow and DMAC flows (local and remote) has been installed
        ArgumentCaptor<Flow> argumentCaptor = ArgumentCaptor.forClass(Flow.class);
        verify(mdsalMgr, times(1)).addFlowToTx(any(BigInteger.class), argumentCaptor.capture(), any(WriteTransaction.class));

        // argumentCaptor should have exactly 2 values because mdsalMgr.installFlow is called 1 times for the same DpId,
        //  for DMAC
        List<Flow> installedFlowsCaptured = argumentCaptor.getAllValues();
        assert( installedFlowsCaptured.size() == 1 );
    ...
    public void testGetElanInstanceByName() throws Exception {
        ...
        when(readTx.read(eq(LogicalDatastoreType.CONFIGURATION),
                eq(elan222.getElanInstanceConfigurationDataPath()))).thenReturn(chkdFuture);
        ...
        assert(returnedElanInstance != null
                && returnedElanInstance.getElanInstanceName() != null
    ...
    public void testUpdateElanOperationalDS() throws Exception {
        ...
        // Method under test /////////
        //////////////////////////////
        ElanUtils.updateOperationalDataStore(dataBroker, idManager, elan222.getElanInstance(), new ArrayList<String>(), writeTx);

        //////////////////////////////
        // Verification //////////////
        //////////////////////////////

        // Verifies that there have been 3 creations in the Elan's Operational DS:a
        ...
        verify(writeTx, times(2)).put(any(LogicalDatastoreType.class), argumentCaptor.capture(), any(DataObject.class), any(Boolean.class));

        // Verifies that ElanInstance in Configuration DS is updated with the elan
        verify(writeTx, times(1)).merge(eq(LogicalDatastoreType.CONFIGURATION),
                eq(elan222.getElanInstanceConfigurationDataPath()),
                any(ElanInstance.class), any(Boolean.class)); // TODO: verify the elanTag too
