...
public class ElanServiceTestBase {
    ...
    BiPredicate<LogicalDatastoreType, InstanceIdentifier<? extends DataObject>> dataAvailable = (dsType, iid) -> {
        return MDSALUtil.read(dsType, iid, db).isPresent();
    };

    BiPredicate<LogicalDatastoreType, InstanceIdentifier<? extends DataObject>> dataNotAvailable
            = dataAvailable.negate();

    void awaitForData(LogicalDatastoreType dsType, InstanceIdentifier<? extends DataObject> iid) {
        getAwaiter().until(() -> dataAvailable.test(dsType, iid));
    }

    void awaitForDataDelete(LogicalDatastoreType dsType, InstanceIdentifier<? extends DataObject> iid) {
        getAwaiter().until(() -> dataNotAvailable.test(dsType, iid));
    }
    ...
    protected void addInterfaceStates() {

    }

    protected InstanceIdentifier<Flow> getFlowIid(short tableId, BigInteger dpnId) {
        ...
        return InstanceIdentifier.builder(Nodes.class)
                ...
                .child(Table.class, new TableKey(tableId)).child(Flow.class).build();
    }
    ...
    protected void createElanInstance(String elan1, Long elan1SegmentId) {
        ...
        MDSALUtil.syncWrite(db, LogicalDatastoreType.CONFIGURATION,
