...
public class PodListener implements DataTreeChangeListener<Pods> {
    ...
    private void update(Pods podsAfter, Pods podsBefore, Interface podInterfaceBefore, Interface podInterfaceAfter) {
        ...
        if (!Objects.equals(podsAfter.getNetworkNS(), podsBefore.getNetworkNS())
            ...
            if (podsBefore.getNetworkNS() != null || podsBefore.getHostIpAddress() != null) {
                ...
                jobCoordinator.enqueueJob(podsAfter.getName(), new RendererConfigRemoveWorker(podsBefore));
            }
            jobCoordinator.enqueueJob(podsAfter.getName(), new RendererConfigAddWorker(podsAfter, podInterfaceAfter));
        }
        // TODO use infrautils caching mechanism to add this info to cache.
    }
    ...
    private void remove(Pods pods, Interface podInterface) {
        ...
        jobCoordinator.enqueueJob(pods.getName(), new RendererConfigRemoveWorker(pods));
    }
    ...
    private class RendererConfigRemoveWorker implements Callable<List<ListenableFuture<Void>>> {
        ...
        RendererConfigRemoveWorker(Pods pods) {
            this.pods = pods;
        }
