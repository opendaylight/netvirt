...
public class Ipv6ServiceInterfaceEventListener extends AsyncDataTreeChangeListenerBase<Interface,
    ...
    protected void remove(InstanceIdentifier<Interface> key, Interface dataObjectModification) {
        ...
        if (null != port) {
            String nodeIdentifier = dataObjectModification.getLowerLayerIf().get(0);
            nodeIdentifier = nodeIdentifier.substring(0, nodeIdentifier.lastIndexOf(':'));
            ipv6ServiceUtils.programRouterSolicitationEntry(nodeIdentifier, broker, Ipv6ServiceUtils.DEL_FLOW);
        }
    }
    ...
    protected void add(InstanceIdentifier<Interface> key, Interface dataObjectModification) {
        ...
        if (null != port) {
            String nodeIdentifier = dataObjectModification.getLowerLayerIf().get(0);
            nodeIdentifier = nodeIdentifier.substring(0, nodeIdentifier.lastIndexOf(':'));
            ipv6ServiceUtils.programRouterSolicitationEntry(nodeIdentifier, broker, Ipv6ServiceUtils.ADD_FLOW);
        }
    }
