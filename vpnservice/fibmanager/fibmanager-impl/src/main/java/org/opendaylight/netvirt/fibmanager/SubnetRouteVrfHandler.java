...
public class SubnetRouteVrfHandler implements IVrfHandler {
    ...
    public SubnetRouteVrfHandler(DataBroker dataBroker) {
    ...
    public void doProcessing() {
        ...
        if (vrfInput.getAction() == VrfInput.ADD_SUBNET_ROUTE) {
        ...
        if (vrfInput.getAction() == VrfInput.DELETE_SUBNET_ROUTE) {
            if (vpnToDpnList != null) {
                DataStoreJobCoordinator dataStoreCoordinator = DataStoreJobCoordinator.getInstance();
                dataStoreCoordinator.enqueueJob("FIB-" + rd.toString() + "-" + vrfEntry.getDestPrefix(),
                        new Callable<List<ListenableFuture<Void>>>() {
        ...
        if (vrfInput.getAction() == VrfInput.POPULATE_SUBNETROUTE_ON_DPN) {
            if (vpnToDpnList != null) {
                DataStoreJobCoordinator dataStoreCoordinator = DataStoreJobCoordinator.getInstance();
                dataStoreCoordinator.enqueueJob("FIB-" + rd.toString() + "-" + vrfEntry.getDestPrefix(),
                        new Callable<List<ListenableFuture<Void>>>() {
                            ...
                            public List<ListenableFuture<Void>> call() throws Exception {
                                WriteTransaction tx = dataBroker.newWriteOnlyTransaction();
                                for (final VpnToDpnList curDpn : vpnToDpnList) {
                                    installSubnetRouteFlows(curDpn.getDpnId(), elanTag, rd, vpnId, vrfEntry, tx, dataBroker);
                                }
                                List<ListenableFuture<Void>> futures = new ArrayList<>();
                                futures.add(tx.submit());
                                return futures;
