...
public class FibUtil {
    ...
    public static Long getLabelFromRoutePaths(final VrfEntry vrfEntry) {
        List<RoutePaths> routePaths = vrfEntry.getRoutePaths();
        if (routePaths == null || routePaths.isEmpty()) {
            throw new NoSuchElementException("RoutePath does not exists for the vrfEntry " + vrfEntry);
        }
        return vrfEntry.getRoutePaths().get(0).getLabel();
    }

    public static String getFirstNextHopAddress(final VrfEntry vrfEntry) {
        List<RoutePaths> routePaths = vrfEntry.getRoutePaths();
        if (routePaths == null || routePaths.isEmpty()) {
            throw new NoSuchElementException("RoutePath does not exists for the vrfEntry " + vrfEntry);
        }
    ...
    public static Optional<Routes> getLastRoutePathExtraRoute(DataBroker dataBroker, Long vpnId, String rd, String prefix) {
