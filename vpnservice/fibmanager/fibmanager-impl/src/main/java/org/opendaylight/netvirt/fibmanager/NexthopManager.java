...
public class NexthopManager implements AutoCloseable {
    ...
    private static final long WAIT_TIME_TO_ACQUIRE_LOCK = 7000L;
            ...
            try {
                FibUtil.lockCluster(lockManager, nextHopLockStr, WAIT_TIME_TO_ACQUIRE_LOCK);
                {
                    ...
                    if (nexthop == null) {
                        ...
                        GroupEntity groupEntity = MDSALUtil.buildGroupEntity(dpnId, groupId, primaryIpAddress, GroupTypes.GroupAll,
                                listBucketInfo);
                        ...
                        installGroupOnDpn(groupId, dpnId, primaryIpAddress, listBucketInfo, getNextHopKey(vpnId, primaryIpAddress),
                                GroupTypes.GroupAll);
                        // install Group
                        ...
                        if (prefixesList != null && prefixesList.contains(prefix)) {
                            ...
                            IpAdjacenciesBuilder ipPrefixesBuilder = new IpAdjacenciesBuilder().setKey(new IpAdjacenciesKey(currDestIpPrefix));
                ...
                LOG.error("createLocalNextHop: Create/Update of l3nexthop failed for vpnId {} dpn {} primaryIpAddress {} curIpPrefix {}",
                        vpnId, dpnId, primaryIpAddress, currDestIpPrefix, ex);
            } finally {
    ...
    public void removeLocalNextHop(BigInteger dpnId, Long vpnId, String primaryIpAddress, String currDestIpPrefix) {
        ...
        try {
                ...
                if (nh != null) {
                    ...
                    if (prefixesList.isEmpty()) { //remove the group only if there are no more flows using this group
                        GroupEntity groupEntity = MDSALUtil.buildGroupEntity(
                                dpnId, nh.getEgressPointer(), primaryIpAddress, GroupTypes.GroupAll, null);
                        // remove Group ...
            ...
            LOG.error("removeLocalNextHop: Removing group failed for vpnId {} curIpPrefix {} dpn {} primaryIpAddress {}",
                    vpnId, currDestIpPrefix, dpnId, primaryIpAddress, ex);
        } finally {
            FibUtil.unlockCluster(lockManager, nextHopLockStr);
        }
    }
