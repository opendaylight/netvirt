...
public class NexthopManager implements AutoCloseable {
    ...
    static class AdjacencyResult {
        ...
        public AdjacencyResult(String interfaceName, Class<? extends InterfaceType> interfaceType, String nextHopIp) {
    ...
    long createNextHopGroups(Long vpnId, String rd, BigInteger dpnId, VrfEntry vrfEntry,
        ...
        List<BucketInfo> listBucketInfo = new ArrayList<BucketInfo>();
        List<Routes> clonedVpnExtraRoutes  = ((List<Routes>) ((ArrayList<Routes>) vpnExtraRoutes).clone());
    ...
    private List<BucketInfo> getBucketsForRemoteNexthop(Long vpnId, BigInteger dpnId, VrfEntry vrfEntry, String rd,
            List<Routes> clonedVpnExtraRoutes) {
        List<BucketInfo> listBucketInfo = new ArrayList<BucketInfo>();
        ...
        clonedVpnExtraRoutes.stream().forEach(vpnExtraRoute -> vpnExtraRoute.getNexthopIpList().stream().forEach(remoteNextHopIp -> {
            ...
            if(adjacencyResult != null) {
                ...
                if (Tunnel.class.equals(adjacencyResult.getInterfaceType())) {
                    Class<? extends TunnelTypeBase> tunnelType = VpnHelper.getTunnelType(interfaceManager, egressInterface);
                    if(tunnelType.equals(TunnelTypeVxlan.class)) {
                        Long label = VpnHelper.getNonBgpRoutePath(vrfEntry).getLabel();
                        ...
                        if(egressActionMap.containsKey(egressInterface)) {
                            ...
                            egressActions = getEgressActionsForInterface(egressInterface);
    ...
    public List<ListenableFuture<Void>> programDcGwLoadBalancingGroup(List<String> availableDcGws,
            BigInteger dpnId, int addOrRemove) {
        Preconditions.checkArgument(availableDcGws != null && !availableDcGws.isEmpty());
        WriteTransaction tx = dataBroker.newWriteOnlyTransaction();
        ...
        for (int index = 0; index < availableDcGws.size(); index++) {
            ...
            String tunnelInterfaceName = getTunnelInterfaceName(dpnId, new IpAddress(availableDcGws.get(index).toCharArray()));
        ...
        if (NwConstants.DEL_FLOW == addOrRemove) {
            mdsalApiManager.removeGroupToTx(dpnId, group, tx);
        } else {
