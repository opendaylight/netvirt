...
public class NexthopManager implements AutoCloseable {
    ...
    private List<BucketInfo> getBucketsForLocalNexthop(Long vpnId, BigInteger dpnId,
        ...
        routes.getNexthopIpList().parallelStream().forEach(nextHopIp -> {
            String localNextHopIP = nextHopIp + FibConstants.HOST_MASK;
            Prefixes localNextHopInfo = FibUtil.getPrefixToInterface(dataBroker, vpnId,
                    nextHopIp + FibConstants.HOST_MASK);
            if (localNextHopInfo != null) {
                long groupId = createLocalNextHop(vpnId, dpnId,
                        ...
                        vrfEntry.getGatewayMacAddress(), FibUtil.getJobKey(vpnId, dpnId, vrfEntry.getDestPrefix()));
                if (groupId == FibConstants.INVALID_GROUP_ID) {
    ...
    private List<BucketInfo> getBucketsForRemoteNexthop(Long vpnId, BigInteger dpnId, VrfEntry vrfEntry, String rd,
        ...
        vpnExtraRoutes.stream().forEach(vpnExtraRoute -> vpnExtraRoute.getNexthopIpList()
                .stream().forEach(nexthopPrefixIp -> {
                    String prefixIp = nexthopPrefixIp + FibConstants.HOST_MASK;
                    List<String> tepIpAddresses = FibUtil.getNextHopAddresses(dataBroker, rd, prefixIp);
                    ...
                    Class<? extends TunnelTypeBase> tunnelType = VpnExtraRouteHelper
                            .getTunnelType(interfaceManager,
                                    egressInterface);
                    if (adjacencyResult == null || !FibUtil.isTunnelInterface(adjacencyResult)
                            || !tunnelType.equals(TunnelTypeVxlan.class)) {
                        return;
                    }
                    ...
                    String macAddress = FibUtil.getMacAddressFromPrefix(dataBroker, ifName, prefixIp);
