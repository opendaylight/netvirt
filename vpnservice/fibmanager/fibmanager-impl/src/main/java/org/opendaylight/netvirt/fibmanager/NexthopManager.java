...
public class NexthopManager implements AutoCloseable {
    ...
    public long createLocalNextHop(long vpnId, BigInteger dpnId,
        ...
        synchronized (nextHopLockStr.intern()) {
            ...
            if (nexthop == null) {
                ...
                // MAC re-write
                if (macAddress != null) {
                    final Optional<String> nextHopSrcMac = getNextHopSourceMac(vpnId, ipAddress);
                    if (nextHopSrcMac.isPresent()) {
                        LOG.trace("The Local NextHop Group Source Mac {} for VpnInrerface {} on VPN {}",
                                nextHopSrcMac.get(), ifName, vpnId);
                        listActionInfo.add(
                                new ActionSetFieldEthernetSource(actionKey++, new MacAddress(nextHopSrcMac.get())));
                    }
    ...
    private Optional<String> getNextHopSourceMac(final long vpnId,  final String ipPrefixAddress) {

        Optional<String> srcMac = Optional.absent();
        String vpnRd = FibUtil.getVpnRd(dataBroker, FibUtil.getVpnNameFromId(dataBroker,vpnId));
        InstanceIdentifier<VrfEntry> vrfEntryId =
                InstanceIdentifier.builder(FibEntries.class)
                        .child(VrfTables.class, new VrfTablesKey(vpnRd))
                        .child(VrfEntry.class, new VrfEntryKey(ipPrefixAddress)).build();
        Optional<VrfEntry> entry = MDSALUtil.read(dataBroker, LogicalDatastoreType.CONFIGURATION, vrfEntryId);
        if (entry.isPresent()) {
            srcMac = Optional.of(entry.get().getGatewayMacAddress());
        }
        return srcMac;
    }
