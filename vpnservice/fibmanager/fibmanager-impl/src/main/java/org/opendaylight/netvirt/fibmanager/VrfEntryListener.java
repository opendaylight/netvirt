    ...
    protected void add(final InstanceIdentifier<VrfEntry> identifier, final VrfEntry vrfEntry) {
        ...
        if (RouteOrigin.value(vrfEntry.getOrigin()) != RouteOrigin.BGP) {
            ...
            for (ConcurrentHashMap.Entry<Short, VrfEntryHandler> entry : vrfHandlerMap.entrySet()) {
                ...
                switch(entry.getKey()) {
                    case VrfEntryHandler.BGP_ROUTE_HANDLER:
                        ...
                        if (process) {
                            continueVrfEntryProcessing = bgpRouteVrfEntryHandler.enqueue(identifier, vrfEntry, rd, ActionableResource.CREATE);
            ...
            for (ConcurrentHashMap.Entry<Short, VrfEntryHandler> entry : vrfHandlerMap.entrySet()) {
                ...
                switch(entry.getKey()) {
                    case VrfEntryHandler.BGP_ROUTE_HANDLER:
                        ...
                        boolean process =  bgpRouteVrfEntryHandler.wantToProcessVrfEntry(vrfEntry);
                        if (process) {
                            continueVrfEntryProcessing = bgpRouteVrfEntryHandler.enqueue(identifier, vrfEntry, rd, ActionableResource.DELETE);
                            if (continueVrfEntryProcessing == VrfEntryHandler.VRF_PROCESSING_CONTINUE) {
                                continue;
                            }
                            // TODO: Below function eventually will be moved to IntervpnLinkHandler
            ...
            for (ConcurrentHashMap.Entry<Short, VrfEntryHandler> entry : vrfHandlerMap.entrySet()) {
                ...
                switch(entry.getKey()) {
                    case VrfEntryHandler.BGP_ROUTE_HANDLER:
                        ...
                        if (process) {
                            continueVrfEntryProcessing = bgpRouteVrfEntryHandler.enqueueForUpdate(identifier, original, update, rd);
                            if (continueVrfEntryProcessing == VrfEntryHandler.VRF_PROCESSING_CONTINUE) {
                                continue;
                            }
                        } else {
                        ...
                        public List<ListenableFuture<Void>> call() throws Exception {
                            ...
                            if (localDpnIdList.size() <= 0) {
                                for (VpnToDpnList curDpn : vpnToDpnList) {
                                    //TODO: Check whether below piece of code for BGP is reqd anymore.
                                    // This function is invoked only for non-bgp routes
                                    if (RouteOrigin.value(vrfEntry.getOrigin()) == RouteOrigin.BGP) {
                                        if (curDpn.getDpnState() == VpnToDpnList.DpnState.Active) {
