...
public class FibTunnelInterfaceStateListener extends AsyncDataTreeChangeListenerBase<StateTunnelList,
        FibTunnelInterfaceStateListener> {
    ...
    protected void update(InstanceIdentifier<StateTunnelList> key, StateTunnelList original, StateTunnelList update) {
        ...
        if (original.getOperState() == TunnelOperStatus.Down && update.getOperState() == TunnelOperStatus.Up) {
            handleTunnelEvent(update);
        }
    }

    @Override
    protected void add(InstanceIdentifier<StateTunnelList> key, StateTunnelList stateTunnelList) {
        LOG.trace("FibTunnelInterfaceStateListener: add tunnel event {}", stateTunnelList);
        if (stateTunnelList.getOperState() != TunnelOperStatus.Up) {
            LOG.trace("Tunnel {} is not yet UP.", stateTunnelList.getTunnelInterfaceName());
            return;
        }
        handleTunnelEvent(stateTunnelList);
    }

    private void handleTunnelEvent(StateTunnelList stateTunnelList) {
        ...
        LOG.trace("PENDING_FIB_ENTRY: delete the pending tunnel entry {} from OperDS", tunnelEntry.get());
