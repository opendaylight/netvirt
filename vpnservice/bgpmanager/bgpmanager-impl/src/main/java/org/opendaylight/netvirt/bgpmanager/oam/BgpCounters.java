...
public class BgpCounters extends TimerTask {
    ...
    private static final String BGP_IP_SUMMARY_FILE = "cmd_ip_bgp_summary.txt";
    private static final String BGP_IPV4_UNICAST_FILE = "cmd_bgp_ipv4_unicast_statistics.txt";
    private static final String ERROR_FILE_PROCESSING = "Could not process the file {}";
    ...
    public void run() {
        try {
            ...
            resetCounters();
            fetchCmdOutputs(BGP_IP_SUMMARY_FILE, "show ip bgp summary");
            fetchCmdOutputs(BGP_IPV4_UNICAST_FILE, "show bgp ipv4 unicast statistics");
    ...
    private void parseIpBgpSummary() {
        File file = new File(BGP_IP_SUMMARY_FILE);
        ...
        try (Scanner scanner = new Scanner(file)) {
            ...
            LOG.error(ERROR_FILE_PROCESSING, file.getAbsolutePath());
        }
    }
    /*
    ...
    private void parseBgpIpv4UnicastStatistics() {
        File file = new File(BGP_IPV4_UNICAST_FILE);
        String totPfx = "";
        try (Scanner scanner = new Scanner(file)) {
            while (scanner.hasNextLine()) {
    ...
    private void resetCounters() {
        countersMap.clear();
        resetFile(BGP_IP_SUMMARY_FILE);
        resetFile(BGP_IPV4_UNICAST_FILE);
    ...
    static Map<String, String> parseIpBgpVpnAllSummary(Map<String, String> countMap,
        ...
        try (Scanner scanner = new Scanner(file)) {
            ...
            LOG.trace(ERROR_FILE_PROCESSING, file.getAbsolutePath());
            return null;
        }
