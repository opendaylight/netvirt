...
public class BgpConfigurationManager {
    ...
    public static void onUpdatePushRoute(String rd, String prefix, int plen, String nextHop, int label) {
        ...
        if (addroute) {
            ...
            Optional<String> vpnName = BgpUtil.getVpnNameFromRd(dataBroker, rd);
            if ( vpnName.isPresent() ) {
                Optional<InterVpnLinkDataComposite> optIVpnLink =
                        InterVpnLinkCache.getInterVpnLinkByVpnId(vpnName.get());
                if ( optIVpnLink.isPresent() ) {
                    optIVpnLink.get().leakRouteIfNeeded(dataBroker, vpnName.get(), prefix, nextHopList, label,
                                                        RouteOrigin.BGP);
                }
            }
