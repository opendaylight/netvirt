    ...
    protected void bindService(String interfaceName) {
        ...
        BoundServices serviceInfo =
                AclServiceUtils.getBoundServices(String.format("%s.%s.%s", "vpn", "egressacl", interfaceName),
                        NwConstants.ACL_SERVICE_INDEX, flowPriority, AclConstants.COOKIE_ACL_BASE, instructions);
        InstanceIdentifier<BoundServices> path =
                AclServiceUtils.buildServiceId(interfaceName, NwConstants.ACL_SERVICE_INDEX, ServiceModeIngress.class);
    ...
    protected void programAceRule(BigInteger dpId, int lportTag, int addOrRemove, Ace ace, String portId,
        ...
        if (aceType instanceof AceIp) {
            ...
            if (syncAllowedAddresses != null) {
                flowMap = AclServiceOFFlowBuilder.getFlowForAllowedAddresses(syncAllowedAddresses, flowMap, false);
            } else if (aceAttr.getRemoteGroupId() != null) {
                flowMap = AclServiceOFFlowBuilder.getFlowForRemoteAcl(aceAttr.getRemoteGroupId(), portId, flowMap,
                        false);
