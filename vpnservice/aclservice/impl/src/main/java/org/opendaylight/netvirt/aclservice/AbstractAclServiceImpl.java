...
public abstract class AbstractAclServiceImpl implements AclServiceListener {
     ...
     */
    protected Integer getIdleTimoutForApplyStatefulChangeOnExistingTraffic(Ace ace) {
    ...
    /**
     * Write or remove the flow of apply the ACL changes on existing traffic.
     *
     * @param dpId the dpId
     * @param priority the priority
     * @param matches the matches
     * @param filterTable the filterTable
     * @param applyChangeOnExistingTrafficFlowName flow name
     * @param ace the ace
     * @param tableId the tableId
     * @param elanId the Elan id
     * @param addOrRemove flag value
     */
    protected void syncFlowForApplyChangeForExistingTraffic(BigInteger dpId, int priority, List<MatchInfoBase> matches,
            Short filterTable, String applyChangeOnExistingTrafficFlowName, Ace ace, Short tableId,
            Long elanId, int addOrRemove) {

        final List<MatchInfoBase> flowMatches = matches.stream()
                .filter(obj -> !(obj instanceof NxMatchCtState))
                .collect(Collectors.toList());
        flowMatches.add(new NxMatchCtState(AclConstants.TRACKED_RPL_CT_STATE, AclConstants.TRACKED_RPL_CT_STATE_MASK));
        List<InstructionInfo> instructions = new ArrayList<>();
        ...
        syncFlow(dpId, tableId, applyChangeOnExistingTrafficFlowName, priority, "ACL",
            getIdleTimoutForApplyStatefulChangeOnExistingTraffic(ace), 0, AclConstants.COOKIE_ACL_BASE,
                flowMatches, instructions, (addOrRemove == NwConstants.DEL_FLOW) ? 0 : 1);
    }
}
