...
public class AclElanInterfaceListener extends AsyncDataTreeChangeListenerBase<ElanInterface, AclElanInterfaceListener>
    ...
    @Override
    protected void update(InstanceIdentifier<ElanInterface> key, ElanInterface dataObjectModificationBefore, ElanInterface dataObjectModificationAfter) {
    ...
    protected void add(InstanceIdentifier<ElanInterface> key, ElanInterface elanInterface) {
        ...
        if(aclInterface != null && aclInterface.getElanId() == null) {
            Long elanId = AclServiceUtils.getElanIdFromInterface(interfaceId, dataBroker);
            synchronized (aclInterface.getInterfaceId().intern()) {
                //update ElanId
            ...
            if (aclClusterUtil.isEntityOwner()) {
                ...
                //Notify ADD flows, if InterfaceStateListener has processed before ELanID getting populated
