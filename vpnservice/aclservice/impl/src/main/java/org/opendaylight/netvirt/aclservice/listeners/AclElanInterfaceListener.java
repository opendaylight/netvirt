...
public class AclElanInterfaceListener
        extends
            AsyncDataTreeChangeListenerBase<ElanInterface, AclElanInterfaceListener>
        implements
            ClusteredDataTreeChangeListener<ElanInterface> {
    private static final Logger LOG = LoggerFactory
            .getLogger(AclElanInterfaceListener.class);
    ...
    public AclElanInterfaceListener(AclServiceManager aclServiceManager,
            AclClusterUtil aclClusterUtil, DataBroker dataBroker) {
    ...
    @Override
    protected void remove(InstanceIdentifier<ElanInterface> key,
            ElanInterface dataObjectModification) {
        // do nothing
    }

    @Override
    protected void update(InstanceIdentifier<ElanInterface> key,
            ElanInterface dataObjectModificationBefore,
            ElanInterface dataObjectModificationAfter) {
        // do nothing
    }

    @Override
    protected void add(InstanceIdentifier<ElanInterface> key,
            ElanInterface elanInterface) {
        String interfaceId = elanInterface.getName();
        AclInterface aclInterface = AclInterfaceCacheUtil
                .getAclInterfaceFromCache(interfaceId);
        if (aclInterface == null) {
            LOG.debug(
                    "On Add event, ignore if AclInterface is not found in cache");
            return;
        }
        if (aclInterface != null && aclInterface.getElanId() == null) {
            ElanInstance elanInfo = AclServiceUtils.getElanInstanceByName(
                    elanInterface.getElanInstanceName(), dataBroker);
            Long elanId = elanInfo.getElanTag();
            // update ElanId
            aclInterface.setElanId(elanId);
            if (aclClusterUtil.isEntityOwner()) {
                LOG.debug(
                        "On add event, notify ACL service manager to BIND ACL for interface: {}",
                        aclInterface);
                ...
                if (aclInterface.getDpId() != null) {
                    LOG.debug(
                            "On add event, notify ACL service manager to ADD ACL for interface: {}",
                            aclInterface);
