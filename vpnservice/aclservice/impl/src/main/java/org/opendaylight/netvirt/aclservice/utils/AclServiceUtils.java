...
public final class AclServiceUtils {
     ...
     */
    public void createIdPool(String poolName, PacketHandling packetHandling) {
        ...
        if ((config.getDefaultBehavior() == DefaultBehavior.Deny && packetHandling instanceof Permit)
                || (config.getDefaultBehavior() == DefaultBehavior.Allow && packetHandling instanceof Deny)) {
        ...
        } else if ((config.getDefaultBehavior() == DefaultBehavior.Deny && packetHandling instanceof Deny)
                || (config.getDefaultBehavior() == DefaultBehavior.Allow && packetHandling instanceof Permit)) {
            createPool = new CreateIdPoolInputBuilder()
            ...
            LOG.error("Got unexpected PacketHandling {} combined with default behavior {}, skipping creation"
                    + "of pool {}", packetHandling, config.getDefaultBehavior(), poolName);
            return;
        }
        try {
     ...
     * @param poolName the pool name
     */
    public void deleteIdPool(String poolName, PacketHandling packetHandling) {
    ...
    public static String getAclPoolName(BigInteger dpId, short tableId, PacketHandling packetHandling) {
        return AclConstants.ACL_FLOW_PRIORITY_POOL_NAME + "." + dpId + "." + tableId + "." + packetHandling;
    }
    ...
    public void createAclIdPools(BigInteger dpId) {
        createIdPool(getAclPoolName(dpId, NwConstants.INGRESS_ACL_FILTER_TABLE, Permit), Permit);
        createIdPool(getAclPoolName(dpId, NwConstants.INGRESS_ACL_FILTER_TABLE, Deny), Deny);
        createIdPool(getAclPoolName(dpId, NwConstants.EGRESS_ACL_FILTER_TABLE, Permit), Permit);
        createIdPool(getAclPoolName(dpId, NwConstants.EGRESS_ACL_FILTER_TABLE, Deny), Deny);
    }
    ...
    public void deleteAclIdPools(BigInteger dpId) {
        deleteIdPool(getAclPoolName(dpId, NwConstants.INGRESS_ACL_FILTER_TABLE, Permit), Permit);
        deleteIdPool(getAclPoolName(dpId, NwConstants.INGRESS_ACL_FILTER_TABLE, Deny), Deny);
        deleteIdPool(getAclPoolName(dpId, NwConstants.EGRESS_ACL_FILTER_TABLE, Permit), Permit);
        deleteIdPool(getAclPoolName(dpId, NwConstants.EGRESS_ACL_FILTER_TABLE, Deny), Deny);
    }
}
