...
public class AclInterfaceListener extends AsyncDataTreeChangeListenerBase<Interface, AclInterfaceListener>
    ...
    protected void update(InstanceIdentifier<Interface> key, Interface portBefore, Interface portAfter) {
        ...
        if ((aclInPortAfter != null && aclInPortAfter.isPortSecurityEnabled())
            ...
            AclInterface aclInterface = updateAclInterfaceCache(interfaceId, aclInPortBefore, aclInPortAfter);
            if (aclInterface.isPortSecurityEnabled()) {
                processAclUpdate(aclInterface, aclInterface.getOrigSecurityGroups(), aclInterface.getSecurityGroups());
            }
            aclServiceManager.notify(aclInterface, AclServiceManager.Action.UPDATE);
        }
    }

    private AclInterface updateAclInterfaceCache(String interfaceId, InterfaceAcl aclInPortBefore,
            InterfaceAcl aclInPortAfter) {
        if (aclInPortBefore == null) {
            return addAclInterfaceToCache(interfaceId, aclInPortAfter);
        }
        ...
        if (aclInterface != null) {
            if (aclInPortAfter != null && !aclInPortAfter.isPortSecurityEnabled()) {
                ...
                aclInterface.setOrigPortSecurityEnabled(aclInPortBefore.isPortSecurityEnabled());
            } else {
                aclInterface.setPortSecurityEnabled(aclInPortAfter.isPortSecurityEnabled());
                ...
                aclInterface.setOrigPortSecurityEnabled(aclInPortBefore.isPortSecurityEnabled());
                aclInterface.setOrigSecurityGroups(aclInPortBefore.getSecurityGroups());
                aclInterface.setOrigAllowedAddressPairs(aclInPortBefore.getAllowedAddressPairs());
            }
        } else {
