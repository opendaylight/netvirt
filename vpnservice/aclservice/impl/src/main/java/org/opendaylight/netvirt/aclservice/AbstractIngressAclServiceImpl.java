...
public abstract class AbstractIngressAclServiceImpl extends AbstractAclServiceImpl {
    ...
    protected void updateRemoteAclTableForPort(AclInterface port, Uuid acl, int addOrRemove,
        ...
        List<Long> vpnList = port.getVpnId() == null ? new ArrayList<Long>() : port.getVpnId();
        ...
        if (flowMatchesPerVpnId.size() == 0) {
            List<? extends MatchInfoBase> mib = AclServiceUtils
                    .buildIpAndSrcServiceMatch(elanTag, ip, dataBroker, null);
            if (mib != null) {
        ...
        for (Entry<List<? extends MatchInfoBase>, Long> entryMibVpnId : flowMatchesPerVpnId.entrySet()) {
            ...
            String flowNameAdded = "Acl_Filter_Ingress_" + new String(ip.getIpAddress().getValue()) + "_"
