...
public interface ICloudServiceChain {
    ...
    /**
     * Creates/removes the flows that send the packets from a VPN to the SCF domain. These flows are programmed in
     * all DPNs where the VPN has footprint
     *
     * @param vpnName Vpn instance name, typically the UUID
     * @param tableId Id of the SCF pipeline to where LPortDispatcher must send the packet. Typically it should by
     *                Uplink or Downlink Dynamic Subs Filter
    ...
    /**
     * Creates/removes the flows that handover packets from SCF pipeline to a specific VPN. This happens when there
     * is a ScHop with an egressPort that is the VpnPseudoLPort
     *
     * @param vpnName Vpn instance name, typically the UUID
     * @param scfTag ServiceChainForwarding Tag
     ...
     * @param isLastServiceChain Only considered in removal operations. States if there are no more
