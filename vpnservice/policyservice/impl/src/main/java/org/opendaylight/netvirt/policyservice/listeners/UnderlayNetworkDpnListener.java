...
public class UnderlayNetworkDpnListener
    ...
    protected void remove(InstanceIdentifier<DpnToInterface> key, DpnToInterface dpnToInterface) {
        // Ace flows will be removed only when the last referenced underlay network is deleted
    }
    ...
    private void populatePolicyRulesToDpn(String underlayNetwork, BigInteger dpId) {
        List<PolicyAclRule> aclRules = policyServiceUtil.getUnderlayNetworkAclRules(underlayNetwork);
        if (aclRules == null || aclRules.isEmpty()) {
            LOG.debug("No policy ACE rules found for underlay network {}", underlayNetwork);
            return;
        }
        ...
        aclRules.forEach(aclRule -> {
            ...
            if (aceRules != null) {
                aceRules.forEach(aceRule -> {
                    ...
                    aceFlowProgrammer.programAceFlows(policyAce, underlayNetwork, dpId, NwConstants.ADD_FLOW);
                });
            }
        });
