...
public class TunnelStateChangeListener
    ...
    @Inject
    public TunnelStateChangeListener(DataBroker dataBroker, final IInterfaceManager interfaceManager,
    ...
    private void handleTunnelUpdate(StateTunnelList tunnelState, boolean isAdded) {
        ...
        policyServiceUtil.updateUnderlayNetworkTunnelInterface(underlayNetwork, dpnId, tunnelInterfaceName, isAdded);
    }
    ...
    private static BigInteger getTunnelDpnId(StateTunnelList tunnelState) {
        if (tunnelState.getSrcInfo() != null && tunnelState.getSrcInfo().getTepDeviceId() != null) {
            new BigInteger(tunnelState.getSrcInfo().getTepDeviceId());
        }
