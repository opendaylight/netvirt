...
public class PolicyProfileChangeListener
    ...
    protected void remove(InstanceIdentifier<PolicyProfile> key, PolicyProfile policyProfile) {
        LOG.info("Policy profile {} removed", policyProfile.getPolicyClassifier());
        List<String> underlayNetworks = PolicyServiceUtil
                .getUnderlayNetworksFromPolicyRoutes(policyProfile.getPolicyRoute());
        String policyClassifierName = policyProfile.getPolicyClassifier();
        handlePolicyProfileUpdate(policyClassifierName, underlayNetworks, false);
        policyServiceUtil.removeOperationalPolicyProfile(policyClassifierName);
    }
