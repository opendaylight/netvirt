...
public class PolicyRouteGroupProgrammer {
    ...
    public void programPolicyClassifierGroups(String policyClassifier, List<BigInteger> dpIds, int addOrRemove) {
        coordinator.enqueueJob(policyClassifier, () -> {
            WriteTransaction tx = dataBroker.newWriteOnlyTransaction();
            dpIds.forEach(dstDpId -> {
                long groupId = policyIdManager.getPolicyClassifierGroupId(policyClassifier, dstDpId);
                String groupName = getClassifierGroupName(policyClassifier, dstDpId);
                programPolicyClassifierGroups(groupName, groupId, dpIds, dstDpId, tx, addOrRemove);
            });
    ...
    public void programPolicyClassifierGroups(String policyClassifier, BigInteger dpId,
        ...
        coordinator.enqueueJob(policyClassifier, () -> {
            ...
            tunnelInterfaces.forEach(tunnelInterface -> {
                ...
                String groupName = getClassifierGroupName(policyClassifier, remoteDpId);
                programPolicyClassifierGroups(groupName, groupId, Collections.singletonList(dpId), remoteDpId, tx,
                        addOrRemove);
            });
            return Collections.singletonList(tx.submit());
        });
    ...
    private void programPolicyClassifierGroups(String groupName, long groupId, List<BigInteger> srcDpIds,
            BigInteger dstDpId, WriteTransaction tx, int addOrRemove) {
        srcDpIds.forEach(srcDpId -> {
    ...
    private String getClassifierGroupName(String policyClassifier, BigInteger dpId) {
        return policyClassifier + '-' + dpId;
    }
}
