...
public class PolicyRouteGroupProgrammer {
    ...
    public void updatePolicyClassifierGroups(String policyClassifier, List<String> underlayNetworks, int addOrRemove) {
    ...
    public void updatePolicyClassifierGroupBuckets(String policyClassifier, List<String> underlayNetworks,
            int addOrRemove) {
            ...
            for (int idx = 0; idx < underlayNetworks.size(); idx++) {
                ...
                if (dpnToInterfaceList != null) {
                    dpnToInterfaceList.forEach(dpnToInterfaces -> {
                        ...
                        updatePolicyClassifierBuckets(policyClassifier, tunnelInterfaces, dpId, bucketId, addOrRemove,
                                tx);
                    });
                } else {
    ...
    public void updatePolicyClassifierBuckets(String policyClassifier, List<TunnelInterface> tunnelInterfaces,
            BigInteger dpId, int bucketId, int addOrRemove) {
        coordinator.enqueueJob(policyClassifier, () -> {
            WriteTransaction tx = dataBroker.newWriteOnlyTransaction();
            updatePolicyClassifierBuckets(policyClassifier, tunnelInterfaces, dpId, bucketId, addOrRemove, tx);
            return Collections.singletonList(tx.submit());
        });
    }

    private void updatePolicyClassifierBuckets(String policyClassifier, List<TunnelInterface> tunnelInterfaces,
