...
public class TunnelUnderlayNetworkChangeListener
    ...
    protected InstanceIdentifier<OpenvswitchOtherConfigs> getWildCardPath() {
        return InstanceIdentifier.create(NetworkTopology.class)
                ...
                .child(OpenvswitchOtherConfigs.class, new OpenvswitchOtherConfigsKey(PolicyServiceUtil.LOCAL_IPS));
    }
    ...
    @Override
    protected void remove(InstanceIdentifier<OpenvswitchOtherConfigs> key, OpenvswitchOtherConfigs localIps) {
        LOG.debug("local_ips {} removed for node {}", localIps.getOtherConfigValue(),
                key.firstKeyOf(Node.class).getNodeId().getValue());
    }

    @Override
    protected void update(InstanceIdentifier<OpenvswitchOtherConfigs> key, OpenvswitchOtherConfigs origLocalIps,
            OpenvswitchOtherConfigs updatedLocalIps) {
        String nodeId = key.firstKeyOf(Node.class).getNodeId().getValue();
        LOG.debug("local_ips {} updated for node {}", updatedLocalIps.getOtherConfigValue(), nodeId);
        handleTepIpsUpdateEvent(origLocalIps, updatedLocalIps, nodeId);
    }

    @Override
    protected void add(InstanceIdentifier<OpenvswitchOtherConfigs> key, OpenvswitchOtherConfigs localIps) {
        LOG.debug("local_ips {} added for node {}", localIps.getOtherConfigValue(),
                key.firstKeyOf(Node.class).getNodeId().getValue());
    }
    ...
    private void handleTepIpsUpdateEvent(OpenvswitchOtherConfigs origLocalIps,
        ...
        if (entriesDiffering == null || entriesDiffering.isEmpty()) {
            LOG.trace("No underlay network for existing TEP ips detected for node {}", nodeId);
            return;
        }

        Optional<BigInteger> dpIdOpt = policyServiceUtil.getNodeDpId(nodeId);
