...
//                (routerId), MetaDataUtil.METADATA_MASK_VRFID));
    ...
    private long getVpnId(Uuid extNwId, Uuid floatingIpExternalId) {
        ...
        if (floatingIpIdMapOp.isPresent()) {
            ...
            if (vpnId != NatConstants.INVALID_ID) {
                ...
                LOG.info("YAIR we got floatingIpIdMap for floatingIpExternalId {} - subnetId {}",
                        floatingIpExternalId, floatingIpIdMapOp.get().getFloatingIpPortSubnetId());
                return vpnId;
            }
        }
    ...
    void createNATFlowEntries(String interfaceName, final InternalToExternalPortMap mapping,
        ...
        LOG.info("YAIR inside createNATFlowEntries with interfaceName {}, mapping {} and routerName {}",
                interfaceName, mapping, routerName);
