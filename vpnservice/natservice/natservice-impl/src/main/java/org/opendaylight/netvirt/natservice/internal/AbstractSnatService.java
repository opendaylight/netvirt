...
public abstract class AbstractSnatService implements SnatServiceListener {
    ...
    @Override
    public boolean addExternalRouter(Routers routers) {
        LOG.info("AbstractSnatService: Add  router event for {}", routers.getRouterName());
        if (!routers.isEnableSnat()) {
            LOG.info("AbstractSnatService : SNAT is disabled for external router {} ", routers.getRouterName());
            return true;
        }
        ...
        if (primarySwitchId == null || primarySwitchId.equals(BigInteger.ZERO)) {
            LOG.error("AbstractSnatService: Failed to get or allocated NAPT switch");
            return false;
        }
        handleSnatAllSwitch(routers, primarySwitchId, NwConstants.ADD_FLOW);
        return true;
    }

    @Override
    public boolean updateExternalRouter(Routers original, Routers update) {
        LOG.info("AbstractSnatService: Update  router event for {}", original.getRouterName());
        String routerName = original.getRouterName();
        Long routerId = NatUtil.getVpnId(dataBroker, routerName);
        if (routerId == NatConstants.INVALID_ID) {
            LOG.error("AbstractSnatService : Update external router event - Invalid routerId for routerName {}",
                    routerName);
            return false;
        }
        ...
        LOG.debug("AbstractSnatService : update of externalRoutersListener called with originalFlag and updatedFlag"
        ...
        if (originalSnatEnabled != updatedSnatEnabled) {
            if (originalSnatEnabled) {
                ...
                handleSnatAllSwitch(original, primarySwitchId, NwConstants.DEL_FLOW);
            } else {
                if (primarySwitchId == null || primarySwitchId.equals(BigInteger.ZERO)) {
                    LOG.error("AbstractSnatService: Failed to get or allocated NAPT switch in "
                            + "ExternalRouterListener.Update()");
                    return false;
                }
                LOG.info("AbstractSnatService : SNAT enabled for Router {}", original.getRouterName());
                handleSnatAllSwitch(original, primarySwitchId, NwConstants.ADD_FLOW);
            }
        }
        return true;
    }
    ...
    @Override
    public boolean removeExternalRouter(Routers router) {
        LOG.info("AbstractSnatService: Remove  router event for {}", router.getRouterName());
        boolean isSnatEnabled = router.isEnableSnat();
        String routerName = router.getRouterName();
        Long routerId = NatUtil.getVpnId(dataBroker, routerName);
        BigInteger primarySwitchId = getPrimaryNaptSwitch(routerName, routerId);
        if (isSnatEnabled) {
            handleSnatAllSwitch(router, primarySwitchId, NwConstants.DEL_FLOW);
        }
        return true;
    }
