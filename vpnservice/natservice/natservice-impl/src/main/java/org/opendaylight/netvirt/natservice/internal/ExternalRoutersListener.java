...
public class ExternalRoutersListener extends AsyncDataTreeChangeListenerBase<Routers, ExternalRoutersListener> {
    ...
    protected void add(InstanceIdentifier<Routers> identifier, Routers routers) {
        ...
        if (routerId == NatConstants.INVALID_ID) {
            LOG.error("NAT Service : Add external router event - Invalid routerId for routerName {}", routerName);
            return;
        }
        LOG.info("NAT Service : Add external router event for {} routerId {}", routerName, routerId);

        RouterIds rtrs = new RouterIdsBuilder().setKey(new RouterIdsKey(routerId)).setRouterId(routerId)
