...
public class ExternalRoutersListener extends AsyncDataTreeChangeListenerBase<Routers, ExternalRoutersListener> {
    ...
    void installRouterGwMacFlow(Routers router, BigInteger primarySwitchId) {
        handleRouterGwMacFlow(router, primarySwitchId, NwConstants.ADD_FLOW);
    }

    void removeRouterGwMacFlow(Routers router, BigInteger primarySwitchId) {
        handleRouterGwMacFlow(router, primarySwitchId, NwConstants.DEL_FLOW);
    }

    private void handleRouterGwMacFlow(Routers router, BigInteger primarySwitchId, int addOrRemove) {
        String routerGwMac = router.getExtGwMacAddress();
        if (routerGwMac == null) {
        ...
        Uuid vpnId = NatUtil.getVpnIdfromNetworkId(dataBroker, extNetworkId);
        if (vpnId == null) {
            LOG.debug("Network {} is not associated to VPN", extNetworkId.getValue());
            return;
        }

        LOG.debug("{} router Gw flows for router-id {} on switch {}",
                addOrRemove == NwConstants.ADD_FLOW ? "Installing" : "Removing", router.getRouterName(),
                primarySwitchId);
        WriteTransaction writeTx = dataBroker.newWriteOnlyTransaction();
        vpnManager.setupSubnetMacIntoVpnInstance(vpnId.getValue(), routerGwMac, primarySwitchId, writeTx, addOrRemove);
        writeTx.submit();
    }
