...
public class NatTunnelInterfaceStateListener extends AsyncDataTreeChangeListenerBase<StateTunnelList, NatTunnelInterfaceStateListener> implements
    ...
    private void handleTunnelEventForDPN(StateTunnelList stateTunnelList, TunnelAction tunnelAction) {
        ...
        try {
            ...
            switch(tunnelAction){
                case TUNNEL_EP_ADD:
                    if(!handleAddTunnelEventForDPN(srcDpnId, tunnelType, tunnelName, srcTepIp, destTepIp)){
                        LOG.debug("NAT Service : Unable to process TEP ADD");
                    }
                    break;
                case TUNNEL_EP_DELETE:
                    if(!handleDelTunnelEventForDPN(srcDpnId, tunnelType, tunnelName, srcTepIp, destTepIp)){
    ...
    private boolean handleAddTunnelEventForDPN(BigInteger srcDpnId, String tunnelType, String tunnelName, String srcTepIp,
        ...
        for (RoutersList router : routersList) {
            ...
            advtFloatingIpRtsToBgp(router, nextHopIp, srcDpnId);

            LOG.debug("NAT Service : TEP ADD : SNAT -> Advertising routes for router {} ", router.getRouter());
            advtExtFixedIpRtsToBgp(router, srcDpnId, tunnelType, srcTepIp, destTepIp,
                    tunnelName, nextHopIp);
        }
        return true;
    }

    private boolean handleDelTunnelEventForDPN(BigInteger srcDpnId, String tunnelType, String tunnelName, String srcTepIp,
                                               String destTepIp) throws Exception{
        ...
        for (RoutersList router : routersList) {
            ...
            clrFloatingIpRtsFromBgp(router, srcDpnId);
            LOG.debug("NAT Service :  TEP DEL : SNAT -> Withdrawing and Advertising routes for router {} ",
                    router.getRouter());
            clrExternalFixedIpRtsFromBgp(router, srcDpnId, tunnelType, srcTepIp, destTepIp, tunnelName);
        }
        return true;
    }

    private void advtExtFixedIpRtsToBgp(RoutersList router, final BigInteger srcDpnId, String tunnelType,
        ...
        if( !naptId.equals(srcDpnId)){
            ...
            */
            if (!handleTepAddOnNonNaptSwitch(srcDpnId, tunnelType, srcTepIp, destTepIp, tunnelName, routerName, routerId)){
                LOG.debug("NAT Service : Unable to process the TEP add event on NON-NAPT switch {}", srcDpnId);
                return;
            }
            return;
        }
        if (!handleTepAddOnNaptSwitch(srcDpnId, tunnelType, srcTepIp, destTepIp, tunnelName, routerId, routerData, nextHopIp)){
    ...
    private boolean handleTepAddOnNonNaptSwitch(BigInteger srcDpnId, String tunnelType, String srcTepIp, String destTepIp,
    ...
    private boolean handleTepAddOnNaptSwitch(BigInteger srcDpnId, String tunnelType, String srcTepIp,
