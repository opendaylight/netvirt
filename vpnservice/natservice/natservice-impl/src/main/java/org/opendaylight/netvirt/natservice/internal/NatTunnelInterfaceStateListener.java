            ...
            for (final String externalIp : externalIps) {
                ...
                if (externalSubnetVpnId != NatConstants.INVALID_ID) {
                    LOG.debug("NAT Service : Will install custom FIB router with external subnet VPN ID {}",
                            externalSubnetVpnId);
                    BigInteger subnetIdMetaData = BigInteger.valueOf(externalSubnetVpnId).shiftLeft(1);
                    customInstructions.add(new InstructionWriteMetadata(subnetIdMetaData,
                            MetaDataUtil.METADATA_MASK_VRFID).buildInstruction(0));
                }
                customInstructions.add(new InstructionGotoTable(NwConstants.INBOUND_NAPT_TABLE).buildInstruction(1));
