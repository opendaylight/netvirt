...
public class NatTunnelInterfaceStateListener
        ...
        try {
            ...
            if (!naptStatus) {
                ...
                try {
                    ...
                    if (flowEntity == null) {
                        LOG.debug("NAT Service : SNAT -> Failed to populate flowentity for "
                    ...
                    LOG.debug("NAT Service : SNAT -> Failed to remove default SNAT miss entry flow entity {} : {}",
                        flowEntity, ex);
                    return;
                }
                LOG.debug("NAT Service : SNAT -> Removed default SNAT miss entry flow for dpnID {} with routername {}",
                    dpnId, routerName);
                ...
                try {
                    ...
                    if (flowEntity == null) {
                        LOG.debug("NAT Service : SNAT -> Failed to populate flowentity for router {} with dpnId {}",
                            routerName, dpnId);
                        return;
                    }
                    LOG.debug("NAT Service : SNAT -> Removing default SNAT miss entry flow entity for "
            ...
            LOG.debug("NAT Service : SNAT -> Exception while handling naptSwitch down for router {} : {}",
                routerName, ex);
        }
    }
        ...
        try {
            ...
            switch (tunnelAction) {
                ...
                case TUNNEL_EP_DELETE:
                    ...
                    break;
                default:
                    LOG.warn("NAT Service : hndlTepEvntsForDpn: unknown tunnelAction: {}", tunnelAction);
                    break;
            }
        } catch (Exception e) {
    ...
    private boolean handleTepDelForAllRtrs(BigInteger srcDpnId, String tunnelType, String tunnelName, String srcTepIp,
        ...
        if (endpointIpForDPN != null) {
            LOG.trace("NAT Service : Ignore TEP DELETE event received for DPN {} VTEP IP {} since its "
    ...
    private boolean hndlTepAddOnNonNaptSwitch(BigInteger srcDpnId, BigInteger primaryDpnId, String tunnelType,
        ...
        */
        ...
        if (vpnName == null) {
            ...
            LOG.debug("NAT Service : SNAT -> Install the group which forward packet to the tunnel port "
    ...
    private boolean hndlTepAddOnNaptSwitch(BigInteger srcDpnId, String tunnelType, String srcTepIp,
        ...
        if (!routerData.isPresent()) {
            LOG.warn("NAT Service : hndlTepAddOnNaptSwitch: routerData is not present");
            return false;
        }
        ...
        if (externalIps != null) {
            for (final String externalIp : externalIps) {
                if (extNwProviderType.getName().equals("VXLAN")) {
                    ...
                    NatUtil.addPrefixToBGP(dataBroker, bgpManager, fibManager, externalVpnName, rd, null /*macAddress*/,
                            externalIp, nextHopIp, extNwProviderTypeEncap, NatConstants.DEFAULT_LABEL_VALUE, l3Vni, LOG,
                            gwMacAddress, writeTx, RouteOrigin.STATIC, srcDpnId);
                    ...
                    LOG.debug("NAT Service : SNAT -> Advertise the route to the externalIp {} with provider type {} "
                            + "having nextHopIp {}", externalIp,extNwProviderType.getName(), nextHopIp);
                    NatUtil.addPrefixToBGP(dataBroker, bgpManager, fibManager, externalVpnName, rd, null /*macAddress*/,
                            externalIp, nextHopIp, extNwProviderTypeEncap, label, NatConstants.DEFAULT_L3VNI_VALUE, LOG,
                            gwMacAddress, writeTx, RouteOrigin.STATIC, srcDpnId);
    ...
    private void hndlTepAddForDnatInEachRtr(RoutersList router, String nextHopIp, BigInteger tepAddedDpnId) {
        ...
        if (rd == null) {
            LOG.error("NAT Service : Unable to retrieve the RD from VpnName {} "
                    + "in NatTunnelInterfaceStateListener.hndlTepAddForDnatInEachRtr()", vpnName);
            return;
        }
        VrfEntry.EncapType extNwProviderTypeEncap = NatUtil.getExtNwProviderType(dataBroker, rd);
        long routerId = NatUtil.getVpnId(dataBroker, vpnName);
        if (routerId == NatConstants.INVALID_ID) {
            LOG.error("NAT Service : Unable to retrieve routerId from VpnName {} "
                    + "in NatTunnelInterfaceStateListener.hndlTepAddForDnatInEachRtr()", vpnName);
            return;
        }
        String gwMacAddress = NatUtil.getExtGwMacAddFromRouterId(dataBroker, routerId);
        ...
        for (Ports port : interfaces) {
            ...
            if (!fipCfgdDpnId.equals(tepAddedDpnId)) {
                LOG.debug("NAT Service : DNAT -> TEP added DPN {} is not the DPN {} which has the "
            ...
            for (InternalToExternalPortMap intExtPortMap : intExtPortMapList) {
                ...
                if (extNwProviderType.getName().equals("VXLAN")) {
                    ...
                    LOG.debug("NAT Service : DNAT -> Advertise the route to the externalIp {} with provider type {} "
                            + "having nextHopIp {}", externalIp,extNwProviderType.getName(), nextHopIp);
                    NatUtil.addPrefixToBGP(dataBroker, bgpManager, fibManager, vpnName, rd, null /*macAddress*/,
                            externalIp + "/32", nextHopIp, extNwProviderTypeEncap, NatConstants.DEFAULT_LABEL_VALUE,
                            l3Vni, LOG, gwMacAddress, writeTx, RouteOrigin.STATIC, fipCfgdDpnId);
                    //If external provider type is VXLAN then service id would be l3vni
                    serviceId = l3Vni;
                    ...
                    LOG.debug("NAT Service : DNAT -> Advertise the route to the externalIp {} with provider type {} "
                            + "having nextHopIp {}", externalIp,extNwProviderType.getName(), nextHopIp);
                    NatUtil.addPrefixToBGP(dataBroker, bgpManager, fibManager, vpnName, rd, null /*macAddress*/,
                            externalIp + "/32", nextHopIp, extNwProviderTypeEncap, label,
                            NatConstants.DEFAULT_L3VNI_VALUE, LOG, gwMacAddress, writeTx, RouteOrigin.STATIC,
                            fipCfgdDpnId);
                    //If external provider type is other than VXLAN then service id would be label
                    serviceId = label;
    ...
    private void hndlTepDelForDnatInEachRtr(RoutersList router, BigInteger tepDeletedDpnId) {
        ...
        ProviderTypes extNwProviderType = NatUtil.getProviderTypefromNetworkId(dataBroker, extNwId);
        if (extNwProviderType == null) {
            LOG.error("NAT Service : Unable to retrieve the Provider Type for external network ID {} "
                    + "in NatTunnelInterfaceStateListener.hndlTepDelForDnatInEachRtr()", extNwId);
            return;
        }
        final String vpnName = NatUtil.getAssociatedVPN(dataBroker, extNwId, LOG);
        if (vpnName == null) {
        ...
        if (rd == null) {
            LOG.error("NAT Service : Unable to retrieve the RD from VpnName {} "
                    + "in NatTunnelInterfaceStateListener.hndlTepDelForDnatInEachRtr()", vpnName);
            return;
        }
        List<Ports> interfaces = routerPorts.getPorts();
        for (Ports port : interfaces) {
            ...
            if (!fipCfgdDpnId.equals(tepDeletedDpnId)) {
                LOG.info("NAT Service : DNAT -> TEP deleted DPN {} is not the DPN {} which has the "
                    + "floating IP configured for the port: {}",
            ...
            for (InternalToExternalPortMap intExtPortMap : intExtPortMapList) {
                ...
                if (extNwProviderType.getName().equals("VXLAN")) {
                    ...
                    if (l3Vni == NatConstants.INVALID_ID) {
                        LOG.error("NAT Service : Unable to retrieve L3VNI value for External IP {} with Provider Type "
                                        + "{} in NatTunnelInterfaceStateListener.hndlTepDelForDnatInEachRtr() "
                                        + "DNAT -> Unable to remove the table 21 entry pushing l3vni value to the "
                                + "tunnel since l3vni is invalid", externalIp,extNwProviderType.getName());
                        return;
                    }
                    serviceId = l3Vni;
                } else {
                ...
                Futures.addCallback(listenableFuture, new FutureCallback<RpcResult<Void>>() {
                    ...
                    public void onFailure(Throwable error) {
                        if (extNwProviderType.getName().equals("VXLAN")) {
                            LOG.error("NAT Service : DNAT -> Error in removing the table 21 entry pushing "
                                    + "the l3vni value to the tunnel since l3vni is invalid ", error);
                        } else {
                            LOG.error("NAT Service : DNAT -> Error in removing the table 21 entry pushing "
                    ...
                    public void onSuccess(RpcResult<Void> result) {
                        if (result.isSuccessful()) {
                            ...
                            if (extNwProviderType.getName().equals("VXLAN")) {
                                LOG.error("NAT Service : DNAT -> Error in fib rpc call to remove the table 21 "
                                        + "entry pushing the l3vni value to the tunnnel due to {}", result.getErrors());
                            } else {
                                LOG.error("NAT Service : DNAT -> Error in fib rpc call to remove the table 21 "
                                        + "entry pushing the MPLS label to the tunnnel due to {}", result.getErrors());
                            }
                        }
                    }
                });
