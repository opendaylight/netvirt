...
public class NaptSwitchHA {
    ...
    protected void removeSnatFlowsInOldNaptSwitch(String routerName, BigInteger naptSwitch,
        ...
        if (extNwProviderType.getName().equals("VXLAN")) {
            ...
            // out per router for External VXLAN Provider Type only (36->46 on NAPT Switch)
            ...
            tsFlowRef = externalRouterListener.getFlowRefTs(naptSwitch, NwConstants.INTERNAL_TUNNEL_TABLE,
                    routerLportTag);
        } else {
            //Other than VXLAN Provider type will be continuing to use existing router-id as tunnel-id
            tsFlowRef = externalRouterListener.getFlowRefTs(naptSwitch, NwConstants.INTERNAL_TUNNEL_TABLE, routerId);
        }
        //Remove the Terminating Service table entry which forwards the packet to Outbound NAPT Table (table36->46)
        ...
        if (newExternalIps != null && externalIpsCache != null) {
            ...
            for (String externalIp : removedExternalIps) {
                if (externalIpLabel.containsKey(externalIp)) {
                    ...
                    LOG.debug("NAT Service: Label {} for ExternalIp {} for router {}", label, externalIp, routerName);
                } else {
                ...
                externalRouterListener.clearBgpRoutes(externalIp, vpnName);
                externalRouterListener.delFibTsAndReverseTraffic(naptSwitch, routerId, externalIp, vpnName, label);
                LOG.debug("NAT Service: Successfully removed fib entries in switch {} for router {} and externalIps {}",
