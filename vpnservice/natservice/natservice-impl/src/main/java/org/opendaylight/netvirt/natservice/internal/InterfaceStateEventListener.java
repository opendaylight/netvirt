...
public class InterfaceStateEventListener extends AbstractDataChangeListener<Interface> implements AutoCloseable {
    ...
    private void updateNaptTunnelsGroups(Interface intrf) {
        // get tunnel's DPN
        BigInteger tunnelDpn = BigInteger.ZERO;
        try {
            tunnelDpn = NatUtil.getDpIdFromInterface(intrf);
            LOG.debug("updateNaptTunnelsGroups for dpnId {}", tunnelDpn);
        } catch (Exception e) {
            LOG.error("Unable to obtain dpnId for tunnel interface {},",
                    " update NAPT tunnel failed with exception {}",
                    intrf.getName(), e);
            return;
        }
