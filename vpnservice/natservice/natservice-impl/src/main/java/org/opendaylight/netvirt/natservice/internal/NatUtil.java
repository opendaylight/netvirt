...
public class NatUtil {
    ...
    */
    ...
    public static Port getNeutronPortForIp(DataBroker broker, IpAddress targetIP, String deviceType) {
        for (Port port : getAllNeutronPortsList(broker)) {
    ...
    public static String getMacOfRouterByDeviceOwner(DataBroker broker, String routerName, String deviceOwner) {
        for (Port port : getAllNeutronPortsList(broker)) {
            if (routerName.equals(port.getDeviceId()) && deviceOwner.equals(port.getDeviceOwner())) {
    ...
    private static List<Port> getAllNeutronPortsList(
            DataBroker broker) {
