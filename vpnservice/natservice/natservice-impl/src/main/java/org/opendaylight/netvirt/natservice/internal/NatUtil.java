...
public class NatUtil {
    ...
    */
    ...
    public static String getExternalVpnInterface(DataBroker dataBroker, long vpnId, BigInteger dpnId) {
        String vpnName = getVpnInstanceFromVpnIdentifier(dataBroker, vpnId);
        if (vpnName == null) {
            LOG.warn("No VPN instance found for vpn id {}", vpnId);
            return null;
        }

        String routerId = getRouterIdfromVpnInstance(dataBroker, vpnName);
        if (routerId == null) {
            LOG.warn("No router associated with vpn {}", vpnName);
            return null;
        }

        String extNetwork = getAssociatedExternalNetwork(dataBroker, routerId);
        if (extNetwork == null) {
            LOG.warn("No external network associated with router id {} vpn {}", routerId, vpnName);
            return null;
        }

        return NatServiceAccessor.getElanProvider().getExternalElanInterface(extNetwork, dpnId);
    }
