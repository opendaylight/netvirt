...
public class EvpnSnatFlowProgrammer {
    ...
    */
        ...
        LOG.debug("NAT Service : Install custom FIB routes for external network prefix {} ", externalIp);
    ...
    public void evpnDelFibTsAndReverseTraffic(final BigInteger dpnId, final long routerId, final String externalIp,
        ...
        LOG.info("NAT Service : Successfully Removed custom FIB entries for external prefix {} ", externalIp);
        long vpnId = NatUtil.getVpnId(dataBroker, vpnName);
        if (vpnId == NatConstants.INVALID_ID) {
            LOG.warn("NAT Service : Invalid Vpn Id is found for Vpn Name {}", vpnName);
        }
    ...
    public void makeTunnelTableEntry(BigInteger dpnId, long l3Vni, List<Instruction> customInstructions, short tableId)
    {
