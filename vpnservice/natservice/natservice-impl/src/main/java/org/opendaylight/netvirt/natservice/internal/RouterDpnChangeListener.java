...
public class RouterDpnChangeListener
    ...
    protected void add(final InstanceIdentifier<DpnVpninterfacesList> identifier, final DpnVpninterfacesList dpnInfo) {
        ...
        LOG.info("YAIR in add router {}", routerId);
        ...
        if (routerData.isPresent()) {
            LOG.info("YAIR router {} is present", routerId);
            ...
            if (networkId != null) {
                ...
                if (vpnName == null) {
                    ...
                    if (router.getExternalIps() != null) {
                        LOG.info("YAIR Installing default route in FIB on dpn {} for router {} with vpn {} - {}",
                                dpnId, routerId, vpnId, router.getExternalIps().get(0).getSubnetId());
                    }
            ...
            LOG.info("YAIR Router {} is not associated with External network", routerId);
    ...
    private void installDefaultNatRouteForRouterExternalSubnets(BigInteger dpnId, List<Uuid> externalSubnetIds) {
        if (externalSubnetIds == null) {
            LOG.info("YAIR NAT Service : No external subnets for router");
        ...
        for (Uuid subnetId : externalSubnetIds) {
            ...
            if (vpnIdForSubnet != NatConstants.INVALID_ID) {
                ...
                LOG.info("YAIR no vpnID for subnet {}", subnetId);
