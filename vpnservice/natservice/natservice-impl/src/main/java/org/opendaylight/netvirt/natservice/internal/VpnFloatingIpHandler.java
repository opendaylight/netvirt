        ...
        ListenableFuture<RpcResult<Void>> future = Futures.transform(JdkFutureAdapters.listenInPoolThread(labelFuture),
            ...
            public ListenableFuture<RpcResult<Void>> apply(RpcResult<GenerateVpnLabelOutput> result) throws Exception {
                if(result.isSuccessful()) {
                    ...
                    String extInterfaceName = elanService.getExternalElanInterface(networkId.getValue(), dpnId);
                    if (extInterfaceName == null) {
                        String errMsg = String.format("Failed to install FloatingIp ARP responder flow for {}. No external interface found for DPN id {}",
                    ...
                    if (extInterfaceState == null) {
                        String errMsg = String.format("No interface state found for interface {}. Delaying FloatingIp ARP responder flow for {}",
                                extInterfaceName, externalIp, result.getErrors());
                        LOG.debug(errMsg);
                        return Futures.immediateFailedFuture(new RuntimeException(errMsg));
                    }
                    ...
                    if (lportTag == null) {
                        String errMsg = String.format("No Lport tag found for interface {}. Delaying FloatingIp ARP responder flow for {}",
                                extInterfaceName, externalIp, result.getErrors());
                        LOG.debug(errMsg);
                        return Futures.immediateFailedFuture(new RuntimeException(errMsg));
                    }
                    ...
                    if (floatingIpPortMacAddress != null) {
                        ...
                        vpnManager.setupArpResponderFlowsToExternalNetworkIps(routerId, Arrays.asList(externalIp),
                                floatingIpPortMacAddress, dpnId, vpnId, extInterfaceName, lportTag, writeTx,
                                NwConstants.ADD_FLOW);
                        writeTx.submit();
                    }
