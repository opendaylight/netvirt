...
public class DhcpServiceUtils {
    ...
    public static java.util.Optional<String> getIpV4Address(Port port) {
        ...
        return port.getFixedIps().stream().filter(v -> isIPv4AddressAvailable(v.getIpAddress()))
                .map(v -> v.getIpAddress().getIpv4Address().getValue()).findFirst();
    }

    private static boolean isIPv4AddressAvailable(IpAddress ipAddress) {
        return  ipAddress.getIpv4Address() != null && ipAddress.getIpv4Address().getValue() != null;
    }
