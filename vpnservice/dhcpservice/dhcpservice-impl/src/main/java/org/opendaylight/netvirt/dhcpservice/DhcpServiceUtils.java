...
public class DhcpServiceUtils {
    ...
    public static boolean isDpnAlreadyPresentInElanInstance(DataBroker broker, BigInteger dpId, String elanInstanceName) {
    ...
    public static boolean isLastDpnInterfaceInElanInstance(DataBroker broker, BigInteger dpId,
                                                           String interfaceName, String elanInstanceName) {
        InstanceIdentifier<ElanDpnInterfacesList> elanDpnInterfaceId = getElanDpnOperationDataPath(elanInstanceName);
        Optional<ElanDpnInterfacesList> existingElanDpnInterfaces = MDSALUtil.read(broker,
                LogicalDatastoreType.OPERATIONAL, elanDpnInterfaceId);
        if (!existingElanDpnInterfaces.isPresent()) {
            LOG.trace("No ElanDpn interfaces found for the ELAN instance {}.",elanInstanceName);
            return true;
        }
        List<DpnInterfaces> dpnInterfaces = existingElanDpnInterfaces.get().getDpnInterfaces();
        for (DpnInterfaces dpnInterface : dpnInterfaces) {
            if (dpnInterface.getDpId().equals(dpId)) {
                List<String> intf = dpnInterface.getInterfaces();
                if (intf.size() > 1) {
                    LOG.trace("{} ELAN Dpn interfaces are already existing for DPN {}.",intf.size(), dpId);
                    return false;
                } else if (intf.size() == 1 && intf.equals(interfaceName)) {
                    LOG.trace("Dpn interface {} for the DPN {} is the last.",interfaceName,dpId);
                    return true;
                }
                return true;
            }
        }
        //No matching dpnInterfaces
        return true;
    }
    ...
    public static  Optional<NetworkToDhcpport>  getNetworkDhcpportData(DataBroker broker, String networkId) {
        ...
        Optional<NetworkToDhcpport> optional = Optional.absent();
        optional = MDSALUtil.read(broker,LogicalDatastoreType.CONFIGURATION,id);
    ...
    public static BigInteger getDpnFromInterface(String interfaceName, OdlInterfaceRpcService odlInterfaceRpcService) {
        BigInteger dpId = null;
        Future<RpcResult<GetDpidFromInterfaceOutput>> output = odlInterfaceRpcService
    ...
    public static Optional<String> getAvailableDpnInterface(DataBroker broker, BigInteger dpId, String elanInstanceName) {
        Optional<String> dpnInterface = Optional.absent();
        InstanceIdentifier<ElanDpnInterfacesList> elanDpnInterfaceId = getElanDpnOperationDataPath(elanInstanceName);
        Optional<ElanDpnInterfacesList> existingElanDpnInterfaces = MDSALUtil.read(broker,
                LogicalDatastoreType.OPERATIONAL, elanDpnInterfaceId);
        if (!existingElanDpnInterfaces.isPresent()) {
        ...
        List<DpnInterfaces> dpnInterfaces = existingElanDpnInterfaces.get().getDpnInterfaces();
        for (DpnInterfaces interfaces : dpnInterfaces) {
            if (interfaces.getDpId().equals(dpId)) {
                dpnInterface = Optional.of(interfaces.getInterfaces().get(0));
                return  dpnInterface;
            }
        }
