...
public class DhcpServiceUtils {
    ...
    public static boolean anyOtherInterfaceOnDpn(DataBroker broker, BigInteger dpId, String elanInstanceName,
    ...
    static BigInteger getDpIdFromInterface(org.opendaylight.yang.gen.v1.urn.ietf.params.xml.ns.yang.ietf.interfaces
    ...
    private static String getDpnFromNodeConnectorId(NodeConnectorId portId) {
         ...
         */
        String[] split = portId.getValue().split(DhcpMConstants.OF_URI_SEPARATOR);
        return split[1];
    }

    public static final Predicate<Port> IS_DHCP_PORT = port -> port != null
            && port.getDeviceOwner().equals(NeutronConstants.DEVICE_OWNER_DHCP);

    static final Predicate<Port> IS_ODL_DHCP_PORT = port -> port != null
            && port.getDeviceOwner().equals(NeutronConstants.DEVICE_OWNER_DHCP) && port.getDeviceId() != null
