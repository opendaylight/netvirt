...
public class DhcpInterfaceAddJob implements Callable<List<ListenableFuture<Void>>> {
    ...
    public DhcpInterfaceAddJob(DhcpManager dhcpManager, DhcpExternalTunnelManager dhcpExternalTunnelManager,
                               DataBroker dataBroker, final Interface intf, BigInteger dpnId,
    ...
    public List<ListenableFuture<Void>> call() throws Exception {
        ...
        LOG.trace("Received add DCN for interface {}, dpid {}", intf.getName(), dpnId);
        org.opendaylight.yang.gen.v1.urn.ietf.params.xml.ns.yang.ietf.interfaces.rev140508.interfaces.Interface iface =
                interfaceManager.getInterfaceInfoFromConfigDataStore(intf.getName());
        if (iface != null) {
        ...
        if (!dpnId.equals(DhcpMConstants.INVALID_DPID)) {
            Port port = dhcpManager.getNeutronPort(intf.getName());
            Subnet subnet = dhcpManager.getNeutronSubnet(port);
            if (null != subnet && subnet.isEnableDhcp()) {
                LOG.info("DhcpInterfaceEventListener add isEnableDhcp" + subnet.isEnableDhcp());
                installDhcpEntries(intf.getName(), dpnId, futures);
                LOG.trace("Checking ElanDpnInterface {} for dpn {} ", intf.getName(), dpnId);
                String networkId = port.getNetworkId().getValue();
                //if (!DhcpServiceUtils.anyOtherInterfacceOnDpn(dataBroker, dpnId, networkId, intf.getName())) {
                    DhcpServiceUtils.getNetworkDhcpPortData(dataBroker, networkId).ifPresent(networkToDhcp -> {
                        LOG.trace("Installing the Arp responder for interface {} with DHCP MAC {} & IP {}.",
                                intf.getName(), networkToDhcp.getMacAddress(), networkToDhcp.getPortFixedip());
                        elanService.addArpResponderFlow(dpnId, intf.getName(), networkToDhcp.getPortFixedip(),
                                networkToDhcp.getMacAddress(), java.util.Optional.of(intf.getIfIndex()), false);
                    });
