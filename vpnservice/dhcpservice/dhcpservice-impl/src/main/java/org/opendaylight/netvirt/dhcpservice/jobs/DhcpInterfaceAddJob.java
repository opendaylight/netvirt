...
public class DhcpInterfaceAddJob implements Callable<List<ListenableFuture<Void>>> {
    ...
    private void getAndUpdateNeworkDhcpPortData(String interfaceName,Port port) {
        String networkId = port.getNetworkId().getValue();
        InstanceIdentifier<NetworkToDhcpport> instanceIdentifier =
                InstanceIdentifier.builder(NetworkDhcpportData.class)
                        .child(NetworkToDhcpport.class, new NetworkToDhcpportKey(networkId)).build();
        Optional<NetworkToDhcpport> existingEntry =
                MDSALUtil.read(dataBroker, LogicalDatastoreType.CONFIGURATION, instanceIdentifier);
        if (!existingEntry.isPresent()) {
            LOG.trace("Updating NetworkDhcpPort map with {}, {}", interfaceName,port);
            NetworkToDhcpport networkToDhcpport =
                    new NetworkToDhcpportBuilder()
                            .setKey(new NetworkToDhcpportKey(networkId))
                            .setPortName(interfaceName).setMacAddress(port.getMacAddress().getValue()).build();
            MDSALDataStoreUtils.asyncUpdate(dataBroker, LogicalDatastoreType.OPERATIONAL, instanceIdentifier,
                    networkToDhcpport, DEFAULT_CALLBACK);
            LOG.trace("Achuth NetworkDhcpPort map created with {}, {}", interfaceName,port);
        }
    }
