...
public class DhcpPktHandler implements PacketProcessingListener {
    ...
    private DhcpserviceConfig.ControllerDhcpMode neutronMode = DhcpserviceConfig.ControllerDhcpMode.Neutronport;
    ...
    private DhcpInfo getDhcpInfo(Port port, Subnet subnet) {
        ...
        if (port != null && subnet != null) {
             ...
             */
            if (config.getControllerDhcpMode() == neutronMode) {
                ...
                if (dhcpPortData.isPresent()) {
                    ...
                    LOG.error("Neutron DHCP port is not available for the VM port {}.",port.getUuid());
                }
            //When neutronport-dhcp flag is disabled continue legacy handling of DHCP service with Subnet Gateway IP
