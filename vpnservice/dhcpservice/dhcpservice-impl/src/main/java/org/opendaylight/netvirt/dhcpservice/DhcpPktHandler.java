...
public class DhcpPktHandler implements PacketProcessingListener {
    ...
    public void onPacketReceived(PacketReceived packet) {
        ...
        if ((tableId == NwConstants.DHCP_TABLE || tableId == NwConstants.DHCP_TABLE_EXTERNAL_TUNNEL)
            ...
            if (pktIn != null) {
                ...
                if (subnet != null && config.getControllerDhcpMode() ==
                        DhcpserviceConfig.ControllerDhcpMode.UseOdlDhcpNeutronPort) {
    ...
    private DHCP handleDhcpPacket(DHCP dhcpPkt, String interfaceName, String macAddress, BigInteger tunnelId,
                                  Port port, Subnet subnet, String serverIp) {
        LOG.trace("DHCP pkt rcvd {}", dhcpPkt);
        byte msgType = dhcpPkt.getMsgType();
        Port dhcpPort;
        if (tunnelId != null) {
            dhcpPort = dhcpExternalTunnelManager.readVniMacToPortCache(tunnelId, macAddress);
        } else {
            dhcpPort = port;
        }
        DhcpInfo dhcpInfo = null;
        if (dhcpPort != null) {
            dhcpInfo = handleDhcpNeutronPacket(msgType, dhcpPort, subnet, serverIp);
