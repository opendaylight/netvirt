...
public class StatisticsPluginImpl implements StatisticsPluginService {
    ...
    private boolean getNodeConnectorResult(List<CounterResult> counterResults, BigInteger dpId, String portNumber)
        ...
        CounterResultDataStructure counterResult = counterRetriever.getNodeConnectorCounters(dpId, portNumber);
        ...
        Map<String, Map<String, BigInteger>> counterResultGroups = counterResult.getGrups(resultId);
        if(counterResultGroups == null || counterResultGroups.isEmpty()){
            return false;
        }
        for (String groupName : counterResultGroups.keySet()) {
            groups.add(createGroupsResult(groupName, counterResultGroups.get(groupName)));
        }
