...
import org.opendaylight.yang.gen.v1.urn.opendaylight.params.xml.ns.yang.statistics.plugin.rev150105.GetNodeAggregatedCountersOutputBuilder;
...
import org.opendaylight.yang.gen.v1.urn.opendaylight.params.xml.ns.yang.statistics.plugin.rev150105.GetNodeCountersOutputBuilder;
...
public class StatisticsPluginImpl implements StatisticsPluginService {
    ...
    public Future<RpcResult<GetNodeCountersOutput>> getNodeCounters(GetNodeCountersInput input) {
        ...
        LOG.debug("getting node counters for node {}", dpId);
        ...
        try {
            if (!getNodeResult(counterResults, dpId)) {
                return RpcResultBuilder.<GetNodeCountersOutput>failed()
                        .withError(ErrorType.APPLICATION, "failed to get node counters").buildFuture();
            }
        } catch (Exception e) {
    ...
    public Future<RpcResult<GetNodeAggregatedCountersOutput>> getNodeAggregatedCounters(
        ...
        BigInteger dpId = input.getNodeId();
        LOG.debug("getting aggregated node counters for node {}", dpId);
        ...
        try {
            if (!getNodeAggregatedResult(aggregatedCounterResults, dpId)) {
                return RpcResultBuilder.<GetNodeAggregatedCountersOutput>failed()
                        .withError(ErrorType.APPLICATION, "failed to get node aggregated counters").buildFuture();
            }
        } catch (Exception e) {
            LOG.warn("failed to get counter result for node " + dpId, e);
        }
    ...
    public Future<RpcResult<GetNodeConnectorCountersOutput>> getNodeConnectorCounters(
        ...
        LOG.debug("getting port counters of port {}", portId);
