...
public class CounterRetriever implements OpendaylightFlowStatisticsListener, OpendaylightPortStatisticsListener{
    ...
    private <T extends TransactionAware> TransactionImpl transactifyFuture(NodeId nodeId, Future<RpcResult<T>> future, CounterRequestType type) {
        if (future == null) {
    ...
    public Map<String, BigInteger> getSwitchFlowCounters(BigInteger dpId, Flow flow) {
        ...
        TransactionImpl transactifyFuture = transactifyFuture(new NodeId(IfmConstants.OF_URI_PREFIX + dpId),
    ...
    public Map<String, Map<String, BigInteger>> getNodeConnectorCounters(BigInteger dpId, String portNumber) {
        NodeRef nodeRef = new NodeRef(InstanceIdentifier.builder(Nodes.class)
                .child(Node.class, new NodeKey(new NodeId(IfmConstants.OF_URI_PREFIX + dpId))).toInstance());
        GetNodeConnectorStatisticsInputBuilder nodeConnectorBuilder =
                new GetNodeConnectorStatisticsInputBuilder().setNode(nodeRef).setNodeConnectorId(new NodeConnectorId(
                        IfmConstants.OF_URI_PREFIX + dpId + OF_DELIMITER + portNumber));
        GetNodeConnectorStatisticsInput gncsi = nodeConnectorBuilder.build();
        TransactionImpl transactifyFuture = transactifyFuture(
                new NodeId(IfmConstants.OF_URI_PREFIX + dpId),
    ...
    private NodeConnectorCounterResult getNodeConnectorStatTransactionResult(NodeId nodeId, TransactionAware notification) {
        TransactionId mdsalTransactionId = notification.getTransactionId();
            ...
            if (flowAndStatisticsMapList != null) {
                for (FlowAndStatisticsMapList fasml : flowAndStatisticsMapList) {
                    if (fasml.getByteCount() != null) {
                        resultMap.put(BYTE_COUNTER_NAME, fasml.getByteCount().getValue());
                    }
                    if (fasml.getPacketCount() != null) {
                        resultMap.put(PACKET_COUNTER_NAME, fasml.getPacketCount().getValue());
                    }
                    if (fasml.getDuration() != null && fasml.getDuration().getSecond() != null) {
                        resultMap.put(DURATION_SECOND_COUNTER_NAME, big(fasml.getDuration().getSecond().getValue()));
                    }
                    if (fasml.getDuration() != null && fasml.getDuration().getNanosecond() != null) {
                        resultMap.put(DURATION_NANO_SECOND_COUNTER_NAME,
    ...
    private Map<String, Map<String, BigInteger>> createNodeConnectorResultMap(TransactionImpl transactifyFuture) {
        ...
        for(NodeConnectorStatisticsUpdate nodeConnectorUpdate : nodeConnectorUpdates){
            List<NodeConnectorStatisticsAndPortNumberMap> statList = nodeConnectorUpdate.getNodeConnectorStatisticsAndPortNumberMap();
            for(NodeConnectorStatisticsAndPortNumberMap stat : statList){
                if (stat.getBytes() != null) {
                    ...
                    bytesGroup.put(BYTES_RECEIVED_COUNTER_NAME, stat.getBytes().getReceived());
                    bytesGroup.put(BYTES_TRANSMITTED_COUNTER_NAME, stat.getBytes().getTransmitted());
                    resultMap.put(BYTES_GROUP_NAME, bytesGroup);
                }
                if (stat.getPackets() != null) {
                    Map<String, BigInteger> packetsGroup= new HashMap<>();
                    packetsGroup.put(PACKETS_RECEIVED_COUNTER_NAME, stat.getPackets().getReceived());
                    packetsGroup.put(PACKETS_TRANSMITTED_COUNTER_NAME, stat.getPackets().getReceived());
                    resultMap.put(PACKETS_GROUP_NAME, packetsGroup);
                }
                if (stat.getDuration() != null) {
                    Map<String, BigInteger> durationGroup= new HashMap<>();
                    durationGroup.put(DURATION_SECOND_COUNTER_NAME, big(stat.getDuration().getSecond().getValue()));
                    durationGroup.put(DURATION_NANO_SECOND_COUNTER_NAME, big(stat.getDuration().getNanosecond().getValue()));
                    resultMap.put(DURATION_GROUP_NAME, durationGroup);
                }
            }
        }
