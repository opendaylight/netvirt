...
Steps in both data centers :
  ...
  - Create an external network with IPv6 subnet where the DCGW is connected.
    That subnet is using the same tenant as an IPv4 subnet used for MPLS over
    GRE tunnels endpoints. DCGW IPv6 address in that subnet should be the nexthop
    for all prefixes received from outside Data Center and advertised to ODL.
  - Create a Neutron Router and connect its ports to all internal subnets that
    belongs to the same L3 BGPVPN identified by a Route Distinguisher.
  - Start BGP stack managed by ODL, possibly on same host as ODL.
  - Create L3VPN instance.
  - Associate the Router with the internal and external networks and the L3VPN
    instance, default gateway for reaching external networks must be the Data
    center gateway.
...
| Classifier Table (0) =>
| Internal Tunnel (36) ``match: GRE header, pop GRE`` =>
...
| L3 FIB Table (21) ``match: vpn-id=l3vpn-id, nw-dst=ext-ip-address set eth-dst-mac=dst-mac-address, tun-id=mpls_label, output to ext-MPLSoGRE-tun-port``


Traffic local to DPN
~~~~~~~~~~~~~~~~~~~~
...
| Lport Dispatcher Table (17) =>
| DMAC Service Filter (19) ``match: dst-mac=vpn-subnet-gateway-mac-address l3vpn service: set vpn-id=l3vpn-id`` =>
| L3 FIB Table (21) ``match: vpn-id=l3vpn-id, nw-dst=int-ip-address set eth-dst-mac=dst-mac-address, tun-id=l3vni, output to vxlan-dest-net-tun-port``
...
performances. So, Globally unique address are used and there is no NAT involved
in the datapath, but since prefixes advertised are /128, meaning that every VM
spawned on a compute node will have its entry in BGP RIB of ODL controller and
DCGW.
This may imply BGP table with very high number of entries. This also
implies a high number of entries in ODL routing table and equivalent number of
flows inserted in OVS.
...
* Configure BGP speaker in charge of retrieving prefixes for/from data center
  ...
  Start BGP stack using karaf CLI

::

 odl:configure-bgp -op start-bgp-server --as-num <as> --router-id <ip-bgp-stack>
...
* Create an external FLAT network with an IPv6 (or dual-stack) subnet. This
  subnet has a default gateway which is the IPv6 address of the Data Center
  Gateway
