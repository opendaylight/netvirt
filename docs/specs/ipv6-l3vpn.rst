...
| Classifier Table (0) =>
| LFIB Table (20) ``match: tun-id=mpls_label set vpn-id=l3vpn-id`` =>
| L3 Gateway MAC Table (19) ``match: vpn-id=l3vpn-id, dst-mac=vpn-subnet-gateway-mac-address`` =>
| L3 FIB Table (21) ``match: vpn-id=l3vpn-id, nw-dst=dst-vm-ipv6-address set output to nexthopgroup-dst-vm`` =>
...
the neutron port of the subnet gateway router.
...
| L3 FIB Table (21) ``match: vpn-id=l3vpn-id, nw-dst=dst-vm-ip-address set tun-id=mpls_label output to nexthopgroup`` =>
...
++++++++++++++++++++
One new container ipv6Table will be added to container ``fibEntries``

.. code-block:: none
   :caption: odl-fib.yang

    grouping ipv6Entries{
        list ipv6Entry{
            key  "destPrefix";
