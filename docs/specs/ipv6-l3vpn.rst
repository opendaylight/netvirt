...
----------------------------
Impact on scaling inside datacenter essentially rely on IPv6 North-South
implementation performances. Since Globally Unique Address are used and there is
no NAT involved in the datapath, it implies prefixes advertised are /128 and 
nexthop value is the DPN ip address within the network of the DCGW. At the end,
it means that every VM spawned on subnets associated to L3VPN will have its entry
in BGP RIB of all ODL controller and DCGW involved in L3VPN (ie all bgp aware
