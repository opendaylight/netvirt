...
is processed by the egress SFC classifier, it will be resubmit to the
SFF for SFC processing.
...
packets from these Neutron and Vxgep ports, and also those that have
...
hop is an SF on this same SFF. If the received NSH packet has the NSH
C1 and C2 fields both set to 0 (zero) then the packet was just resubmit
from the classifier, in which case the packet will be sent back to the
Ingress Dispatch so it can be sent down the rest of the Netvirt pipeline
so the original packet destination information can be calculated and will
be used by the Egress SFC classifier service. If the received NSH packet
has the NSH C1 and C2 fields set (they are not 0) then the packet will
be sent directly to the Egress Dispatcher.
...
.. code-block:: none
  ...
  cookie=0xf005ball00000101 table=11, n_packets=11, n_bytes=918,
      priority=550,nsp=42 actions=resubmit(,GENIUS_DISPATCHER_TABLE)
  ...
  cookie=0xf005ball00000103, table=12, n_packets=11, n_bytes=918,
      ...
      actions=push_nsh,
          ...
          resubmit(,GENIUS_DISPATCHER_TABLE)
...
.. code-block:: none
  ...
  cookie=0x14 table=252, n_packets=0, n_bytes=0,
      ...
      actions=resubmit(SFC_SFF_PORT, GENIUS_DISPATCHER_TABLE)
...
.. code-block:: none
  ...
  cookie=0x14 table=150, n_packets=0, n_bytes=0,
      priority=5
      actions=resubmit(,GENIUS_DISPATCHER_TABLE)
  ...
  cookie=0x14 table=153, n_packets=8, n_bytes=836,
    ...
    // Match on specific NSH NSI/NSP and C1/C2 not set
    // prepare pkt for egress, send back to Ingress Dispatcher
  cookie=0xba5eba1100000101 table=158, n_packets=11, n_bytes=918,
          priority=660,nsi=255,nsp=42,nshc1=0,nshc2=0
  ...
  cookie=0xba5eba1100000102 table=158, n_packets=11, n_bytes=918,
