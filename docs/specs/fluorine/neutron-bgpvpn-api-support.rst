...
Openstack Neutron (and so in OpenDaylight), BGP Engine needs to know which of the vpn-instances out of the
list to choose to import a route from the external neighbor (case a). 
...
The same principle applies when an external bgp route is to be removed (case b).

Advertising (or) Withdrawing an ODL managed route will be driven by the VRFEngine towards the BGPEngine.
The BGPEngine will have no role to play here.

Within the NeutronVPN Engine:
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
Usage
=====
To excite the feature, HOT Templates need to be used.
The types of HOT Templates referred by this spec are given below.  These
same templates will be used in CSIT suites.
