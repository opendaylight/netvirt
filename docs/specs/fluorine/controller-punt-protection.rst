...
The timeout for the learnt flows will be configurable.
...
2. ARP pipeline changes
   ...
   Two new tables(95, 96) will be introduced for ARP processing:

   **Table 43** for resubmitting to tables 95, 96 and 48 (similar to [0])

   **Table 95** for punting the packet as well as installing learnt flows in table 95 
   and 96. Table 95 will have flow with higher priority than punt flow and will match
   on packet's actual ARP SPA and TPA and set a register value.

   **Table 96** will do a reverse match on packet's SPA and TPA. This match coupled with match
   from table 95 will be used to identify GARP packets.

   **Table 48** will be modified, with a new flow, which will use the match results from table
   95 and 96. A match in table 95 will indicate that this arp packet is already punted to
   controller, and only rest of the pipeline processing is required. A match both in table 95 and
   96 will identify garp packet and this will be processed as done currently.
   ...
   **Example of flows after change:**
   ...
   .. code-block:: bash
       ...
       cookie=0x8220000, duration=5040.725s, table=43, n_packets=64, n_bytes=2688, priority=20,arp actions=resubmit(,95),resubmit(,96),resubmit(,48)
       cookie=0x8220000, duration=9740.878s, table=43, n_packets=4, n_bytes=288, priority=0 actions=goto_table:48

       cookie=0x0, duration=40.001s, table=95, n_packets=9, n_bytes=378, hard_timeout=10,
                   priority=20,arp,arp_spa=10.1.1.3,arp_tpa=10.1.1.4, actions=load:0x1->NXM_NX_REG4[0..7]
       cookie=0x0, duration=23.806s, table=95, n_packets=9, n_bytes=378, hard_timeout=10,
                   priority=20,arp,arp_spa=10.1.1.3,arp_tpa=10.1.1.3, actions=load:0x1->NXM_NX_REG4[0..7]

       cookie=0x0, duration=454.032s, table=95, n_packets=4, n_bytes=168, priority=10,arp,arp_op=1
                   actions=learn(table=95,hard_timeout=5,priority=20,eth_type=0x806,NXM_OF_ARP_SPA[],NXM_OF_ARP_TPA[],load:0x1->NXM_NX_REG4[0..7]),
                   learn(table=96,hard_timeout=5,priority=10,eth_type=0x806,NXM_OF_ARP_SPA[]=NXM_OF_ARP_TPA[],NXM_OF_ARP_TPA[]=NXM_OF_ARP_SPA[],
                   load:0x1->NXM_NX_REG4[8..15]), CONTROLLER:65535, group:5000

       cookie=0x0, duration=423.689s, table=95, n_packets=0, n_bytes=0, priority=10,arp,arp_op=2
                   actions=learn(table=95,hard_timeout=5,priority=20,eth_type=0x806,NXM_OF_ARP_SPA[],NXM_OF_ARP_TPA[],load:0x1->NXM_NX_REG4[0..7]),
                   learn(table=96,hard_timeout=5,priority=10,eth_type=0x806,NXM_OF_ARP_SPA[]=NXM_OF_ARP_TPA[],NXM_OF_ARP_TPA[]=NXM_OF_ARP_SPA[],
                   load:0x1->NXM_NX_REG4[8..15]), CONTROLLER:65535
       cookie=0x0, duration=42.444s, table=96, n_packets=0, n_bytes=0, hard_timeout=5,
                   priority=10, arp,arp_spa=10.1.1.4,arp_tpa=10.1.1.3 actions=load:0x1->NXM_NX_REG4[8..15]
       cookie=0x0, duration=26.249s, table=96, n_packets=9, n_bytes=378, hard_timeout=5,
                   priority=10, arp,arp_spa=10.1.1.3,arp_tpa=10.1.1.3, actions=load:0x1->NXM_NX_REG4[8..15]

       cookie=0x0, duration=4980.439s, table=48, n_packets=18, n_bytes=756,
                   priority=200,arp,reg4=0x101/0xffff, actions=load:0->NXM_NX_REG4[],CONTROLLER:65535,resubmit(,49),resubmit(,50)
       cookie=0x0, duration=4542.191s, table=48, n_packets=38, n_bytes=1596,
                   priority=100,arp,reg4=0x1/0xffff, actions=load:0->NXM_NX_REG4[],resubmit(,49),resubmit(,50)
       cookie=0x8500000, duration=9745.872s, table=48, n_packets=89, n_bytes=3858, priority=0 actions=resubmit(,49),resubmit(,50)
