...
an VPN associated to Network External with IPv4 subnet configured with default gateway IP set to DC-GW.
The DC-GW IP must be known as default gateway IP. This is for facilitating host resolution when forging MPLSoGRE
packet from router.
Later on the changes proposed, we will refer to config 1, config 2, and config 3.
...
- Create an external network, and IPv4 subnetwork, as illustrated in example below.
  The IPv4 subnetwork is because private traffic is encapsulatesd into IPv4 MPLSoGRE tunnel.

::

      neutron net-create --router:external=true gateway_net
      neutron subnet-create --name ipv6-public-subnet --gateway <Tunnel Endpoint (aka DCGW)> <gateway-net-uuid>
...
only consider IPv6 information. IPv4 should follow default rules applied to the router. That is to say that because
an external network with IPv4 configured should permit IPV4 traffic to go out, with NAT application on the traffic.
...
As there is a defautl IPv4 gateway, IPv4 traffic from private network, eligible to external network will be sent to NAT tables.
IPv4 pipeline is not described below.
...
As there is a defautl IPv4 gateway, IPv4 traffic from private VPN will be sent to NAT tables.
IPv4 pipeline is not described below.
