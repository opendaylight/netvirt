...
This document relies on VPN concepts to provide the same behaviour.
...
- if access method is using external VPN, then the external VPN will be associated to external network.
  internal IPv6 flows and IPv6 flows, will be imported in that external VPN.
  For the dual stack case, internal IPv4 flows will not be imported, because IPv4 addresses are not public addresses.
  However, the public IPv4 address of the gateway of the router will be imported.

  On controlplane point of view, IPv6 GUA will be imported to that VPN. Also, only IPv4 public IPs will be
  imported. Note that in the case there is IPv4 external network, it is needed to configure external IPv4
  subnetwork, so that it is imported to that VPN.
  In the dual stack case, there is no change in behaviour, since private IPv4 addresses will not be
  imported.

  On dataplane point of view, IPv6 will be encapsulated in the MPLSoGRE tunnel for being sent to the DC-GW.
  IPv4 flow, after being translated by NAT, will be sent to the MPLSoGRE tunnel too. Note that it is not necessary
...
- providing a mechanism in the data path, so that the router recognizes there is an external VPN to use,
  ...
  If above conditions are not met, IPv6 packets not matching flows in its VPN will be dropped.
