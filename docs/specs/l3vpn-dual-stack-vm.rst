...
1.1 This will be possible to configure one single VPN with two associated routers:
...
router, i.e. if we try to make this router dualstack, exception won't be raised.
But this router configuration will become invalid and will not be supported.
The same restriction is applicable for IPv6 singlestack routers.

This specification authorizes only the case, when a subnet port is added to a
singlestack router, which already has only one subnet's port and which is
...
2.1 One network can't be associate with 2 VPN instances simuntaneously.

2.2 If we try to associate a network with VPN, and this network has more than two subnetworks
with different ethertypes, exception won't be raised. But this configuration isn't considered as valid.

2.3 When we associate a network with VPN, we do not care if subnetworks from
this network are the Neutron Ports in some routers, which can be associated with
