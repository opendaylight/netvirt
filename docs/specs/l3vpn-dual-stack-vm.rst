...
values for ipv4On, ipv6On flags we need parse subnets list.  Then in dependency
...
- Currently, list vpn-interface holds a leaf vpn-instance-name, which is a
  ...
  vpn-instance-name by a leaf-list of VPN router IDs, because VPN instance can
...
1.1 Maximum number of associated with a one VPN singlestack routers, is limited
    to two.  Maximum number of associated with a one VPN dualstack routers is
    limited to one.

1.2 If a VPN has already associated with a one singlestack router and we try to
    associate this VPN again with a dualstack router, exception will not be raised.
    But this configuration will not be still valid.

1.3 If a singlestack router is already associated to a VPN, and it has more than
    one port and we try to add a port to this router with another ethertype, i.e.
    we try to make this router dualstack, exception will not be raised.  But this
    configuration will not be still valid and supported.

1.4 When a differ ethertype port is added to a singlestack router, which already
    has only one port and which is already associated to a VPN, singlestack router
    in this case becomes dualstack router with only two ports. This router
    configuration is allowed by current specification.

2. Limitations for subnetworks configurations

2.1 Maximum numbers of networks associated to a one VPN is limited to one.

2.2 Maximum numbers of different ethertype subnetworks associated to a one VPN
    is limited to two. If in a network contains more than two different ethertype
    subnetworks, exception won't be raised, but this configuration isn't supported.

2.3 When we associate a network with a VPN, we do not care if subnetworks from
    this network are ports in some routers andn these routers were associated with
    other VPNs.  This configuration is not considered as supported as well.
...
L3 BGPVPN and IPv6 L3VPN (OK).  The dual stack VM connectivity should not be
...
* create the DC-GW VPN settings
  ...
  - Create a VPN context. This context will have the same settings as in [7].
    Note that for the [6] case, the VPN should be slightly modified.  In dualstack
    case both IPv4 and IPv6 prefixes will be injected in the same VPN.
  ...
  - Create a BGP context. This step permits to start QBGP module depicted in [8]
    ...
    The BGP creation context handles the following:
     ...
     - declaration of remote BGP neighbor with the AFI/SAFI affinities. In our
       case, VPNv4 and VPNv6 addresses families will be used.

  - create a L3VPN, this L3VPN will have a name and will contain VRF settings.

* associate created VPN to router

    - "two-router" solution: associate routers A and B with a created L3VPN;
    - "dualstack-router" solution: associate router A with a created L3VPN.
