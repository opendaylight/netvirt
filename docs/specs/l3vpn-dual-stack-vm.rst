...
All changes we can devide in two main parts.

1. Distinguish IPv4 and IPv6 vrf tables with the same RD/iRT/eRT

1.1 Changes in neutronvpn.

To support a pair of IPv4 and IPv6 prefixes for each launched dualstack VM
we need to obtain information about subnets, where dualstack VM was launched and
information about extraroutes enabled for these subnets.
Obtained information will be stored in vmAdj and erAdjList objects respectively.
These objects are attributes of created for new dualstack VM VPN port instance.
Created VPN port instance will be stored as part of already existed VPN instance
node in MDSAL DataStore.

When we update VPN instance node (associate/disassociated router or network), we
need to provide information about ethertype of new attached/detached subnets,
hence, neutron ports. New argument flags ipv4On and ipv6On will be supported in
NeutronvpnManager function signature calling for update current VPN instance
(updateVpnInstanceNode() method).
UpdateVpnInstanceNode() method is also called, when we create new VPN instance.
So to provide appropriate flag values we need to parce attached to new VPN subnets list.
In VPN instance in dependency of provided flags values we will set either
Ipv4Family attribute or Ipv6Family attribute, or both.
These attributes provide ipv4 or ipv6 VRF context for underlayed APIs of
vpnmanager and bgpmanager.

1.2. Changes in vpnmanager.

When VPN instance is created or updated, VRF tables have to be created for QBGP
as well. What we want, is to introduce separate VRF tables, created in dependency
of set VPN IpFamily, i.e. we want to distinguish IPv4 and IPv6 vrf tables, which
will have the same RD/iRT/eRT lists, because this will bring flexibility in QBGP.
For example, if QBGP receives an entry IPv6 MPLSVPN on a router, that is expecting
to receive only IPv4 entries, then this entry will be ignored. The same for IPv4
MPLSVPN entries respectively.

For creating VRF table we need RouteDistinguisher list, which will be obtained
either for IPv4 or for IPv6 attached to VPN subnetworks or for both. In case when
we have both ethertype subnetworks, IPv4 and IPv6, attached to the same VPN,
RD/iRT/eRT will be the same.

1.3  Changes in bgpmanager

1.4 Changes in yang model.

1.5 Changes in thrift interface.
...
thrift API method signatures addvrf and delvrf must be changed to include the
...
Appropriate changes are needed in signatures of thrift BgpConfigurator and
BgpRouter classes:
...
Along with changes in thrift API we need to support AFI and SAFI attributes in
ebgp.yang (see chapter Yang changes for more details) and, hence, this
attributes must be added in Vrf.java class. This changes trigger consequent
signature modifications of addVrf() and deleteVrf() methods in Vrf class, in
IBgpManager interface and in BgpManager class, which implements it.
BgpConfigurationManager class uses addVrf() and delVrf() methods, which should be also
changed in appropriate way.

Netvirt VPNManager Changes
--------------------------
 vpnInstance.getIpv4Family()

When router or network is associated with VPN or disassociated from it, these
events is processed in VpnInstanceListener class. VpnInstanceListener triggers
VRF context creation or deletion in dependency of operation type and VPN ports ethertypes.
VpnInstanceListener.PostAddVpnInstanceWorker() method  will be called for VRF context creation, when associatedRouter or associatedNetworks command are processing.
VpnInstanceListener.DeleteVpnInstanceWorker() method will be called for VRF context deletion, when disassociatedRouter or disassociatedNetworks command are processing, respectively.
In PostAddVpnInstanceWorker() method


PostAddVpnInstanceWorker() and

VpnInstanceListener class checks VPN ports ethertypes. In dependency of VPN ports ethertypes, when router or network will be associated,PostAddVpnInstanceWorker() will be called.
in
calls PostAddVpnInstanceWorker() and DeleteVpnInstanceWorker() methods
appropriately.
...
ECMP feature is currently provided for Neutron BGPVPN networks and described in the specification [10].
3 cases have been cornered to use ECMP feature for BGPVPN usability.
...
ECMP feature for Neutron BGPVPN networks was successfully (OK) tested with IPv4 L3VPN and IPv6 L3VPN (OK).
The dual stack VM connectivity should not be affected by supporting ECMP.
