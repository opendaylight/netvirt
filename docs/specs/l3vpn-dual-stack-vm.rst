...
to provide L3-forwarding support for DualStack VMs. Intra-DC L2-switching for DualStack VMs will be
part of an other spec. Dual stack VMs are virtual machines that will have 2 L3-identities: one
with IPv4 and one with IPv6. More specifically, DualStack VMs will have a single network interface
in them (i.e., single VNIC) but that interface will actively hold both an IPv4 and IPv6 address.
...
The test result associated for setups ranging from (1) to (4) is out of scope of this spec. 
...
The routers provide the DHCP allocation for the VMs that will connect VMs with appropriate
subnets by neutron ports.
