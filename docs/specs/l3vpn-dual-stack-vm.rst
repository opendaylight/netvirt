...
to provide Intra-DC L2-switching and L3-forwarding support for DualStack VMs.  DualStack VMs
are virtual machines that will have 2 L3-identities: one with IPv4 and one with IPv6.
More specifically, DualStack VMs will have a single network interface in them (i.e., single VNIC)
but that interface will actively hold both an IPv4 and IPv6 address.

In addition to providing initial support for DualStack VMs, this Spec ensures that such DualStack
VMs work on BGPVPNs too.
...
That means that VM can get both IP4 or IPv6 addresses.
...
Dual stack VM refers to a VM which has one neutron port, that inherits from two IPs, one
...
their location is.
As we will see in this document, we can distinguish some use cases that are described below:

- (1) intra DC, intra subnet, local DC, VM on same compute node

- (2) intra DC, intra subnet, local DC, VM on different compute node

- (3) intra DC, inter subnet, local DC, VM on same compute node

- (4) intra DC, inter subnet, local DC, VM on different compute node

- (5) inter DC, inter subnet ( or intra subnet) using BGPVPN

- (6) IPv4/IPv6 access to internet using BGPVPN

For setups (5) and (6), we already know that there are some issues that prevent from doing the testing.
This is the central problematic of the spec. Currently dual stack is not supported across BGPVPN.

Some complementary testing will be done for setups ranging from (1) to (4). By default, we suppose it will
work. However, if we encounter an issue, we will try to address the problematic by using BGPVPN for this
case too.

About (5) and (6), here is described the problem with BGPVPN allocation scheme.
...
This makes possible to create neutron ports whose attributes will inherit from
information from the two subnetworks.

Two routers are created. Each router is connected to separate subnetworks.
The routers provide the DHCP allocation for the VMs that will connect to the neutron
ports.
...
By extension, it should theorically possible to carry the same information in a single VPN, but belonging
from a list of more than 2 routers, independently of the subnet type attached to the router.
That is why, the neutron proposal change permits to associate as many routers as we want to the same VPN.
However, the IPs retrieved from a neutron port would be limited to 2.
