...
for that in NeutronvpnManager function signature, called to update current VPN
...
values for ipv4On, ipv6On flags we need to parce subnets list, attached to a new
VPN.  Then in dependency of ipv4On, ipv6On flags values we will set either
...
well. What we want, is to introduce separate VRF tables, created in dependency
of set Ipv4Family/Ipv6Family VPN attributes, i.e. we want to distinguish IPv4
...
2. Support two routers, attached to the same VPN
...
appropriate changes need to be done in nvpnManager method signatures.
...
Attached to VPN routers list must contain maximum 2 router IDs. Routers, which
...
Subnetmaps will be passed as argument, when createVpnInterface method is called.
