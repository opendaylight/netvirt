...
network and L3 forwarding across VxLAN-type networks using routers/bgpvpns will use tenant supplied
VNI values for such VXLAN networks.
...
* Complete removal of use of ``MPLS labels`` everywhere in ODL: Use of ``MPLS labels`` for
  realizing an inter-dc communication over L3 BGPVPN continue to use ``MPLS labels``.
...
#. Trunk/Sub-Port support with Network associated BGPVPN and Router with or without associated BGPVPN.
#. Multi-segment L2 configuration.
...
The Ingress OVS traffic flows will remain same as above for Router Associated and Network Associated VPNs.
...
CSIT
----
New testcases will be added to validate the functionality for L2 Switching, L3 Forwarding and NAT with Openstack Semantics Set.
...
* `Dual Stack <http://docs.opendaylight.org/en/stable-nitrogen/submodules/netvirt/docs/specs/l3vpn-dual-stack-vms.html>`
* `Multi-segment L2 configuration <https://docs.openstack.org/newton/networking-guide/config-routed-networks.html>`
* `Trunk/Sub-Port Support <https://docs.openstack.org/neutron/pike/admin/config-trunking.html>`