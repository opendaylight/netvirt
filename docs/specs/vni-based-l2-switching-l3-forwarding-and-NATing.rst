...
* While constructing Remote FIB flows, it is required to know the network-type and network's
  segmentation-ID. Currently, these values are back-pulled from neutronVPN's subnet-map
  which is incorrect.

  +  ``vpn-interface`` will be augmented and ``prefix-to-interface`` container in ``odl-l3vpn.yang``
     will be enhanced to hold additional network attributes - ``network-id, network-type and segmentation-id``.
     This will be referred to obtain the required network attributes during Remote FIB flows construction.
  ...
  .. code-block:: none
     ...
     augment "/l3vpn:vpn-interfaces/l3vpn:vpn-interface" {
        ext:augment-identifier "networkParameters";
        uses nvpn:network-attributes;
     }
