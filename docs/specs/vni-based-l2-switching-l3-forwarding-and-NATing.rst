...
* While constructing Remote FIB flows, it is required to know the network-type and network's
  ...
  +  ``prefix-to-interface`` container in ``odl-l3vpn.yang`` will be enhanced to hold additonal
     network attibutes - ``network-id, network-type and segmentation-id``. This will be referred
     to obtain the required network attributes during Remote FIB flows construction.
