...
NAT related FIB Changes
~~~~~~~~~~~~~~~~~~~~~~~

For External Network of type "GRE", it is required to use ``Internet-VPN-VNI``
for intra-DC communication, but still require ``MPLS Label`` for reaching SNAT/DNAT VMs from
external entities via MPLSOverGRE.

We will make use of ``l3vni`` attribute which will be added to fibEntries container
(as part of EVPN_RT5 spec).

NAT will populate both ``label`` and ``l3vni`` value for fibEntries created for floating-ips
and external-fixed-ips with external-network of type "GRE". FIB will use this l3vni value
while programming Remote-Fib Entries flows(on all dpns which are part of same VRF). But, still
label will be used for advertising Prefix and in ``L3_LFIB_TABLE``(table=20) taking to
INBOUND_NAPT_TABLE(table=44) and ``PDNAT_TABLE``(table=25).
