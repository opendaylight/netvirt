...
public final class QosAlertManager implements Runnable {
    ...
    public void setQosalertConfig(QosalertConfig config) {
        ...
        LOG.trace("New QoS alert config threshold:{} polling alertEnabled:{} interval:{}",
    ...
    public void restoreDefaultConfig() {
        LOG.trace("Restoring default configuration");
        getDefaultConfig();
        if (thread != null) {
            thread.interrupt();
        } else {
    ...
    public void addToQosAlertCache(Port port) {
        ...
        if (dpnId.equals(BigInteger.ZERO)) {
            LOG.trace("DPN ID for port {} not found", port.getUuid());
            return;
        }
    ...
    private void pollDirectStatisticsForAllNodes() {
        ...
        for (Entry<BigInteger, ConcurrentMap<String, QosAlertPortData>> entry : qosAlertDpnPortNumberMap.entrySet()) {
            ...
            try {
                ...
                LOG.warn("Could not get Direct-Statistics for node {}", dpn);
                LOG.debug("Exception {} occurred with node {} Direct-Statistics get", e, dpn);
            }
            if (rpcResult != null && rpcResult.isSuccessful() && rpcResult.getResult() != null) {
                ...
                LOG.warn("Direct-Statistics not available for node {}", dpn);
            }
