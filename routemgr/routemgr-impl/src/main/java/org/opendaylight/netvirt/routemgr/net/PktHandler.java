...
public class PktHandler implements PacketProcessingListener {
    ...
    private class PacketHandler implements Runnable {
        ...
        public void run() {
            if (type == RoutemgrUtil.ICMPv6_NS_CODE) {
                ...
                if (type == RoutemgrUtil.ICMPv6_RS_CODE) {
                    ...
                    updateRAResponse(rsPdu, raPacket, rsPdu.getSourceMac(), port);
                    // Serialize the response packet
        ...
        private void updateRAResponse(RouterSolicitationPacket pdu, RouterAdvertisementPacketBuilder raPacket,
                                      MacAddress vmMac, VirtualPort port) {
            RoutemgrUtil instance = RoutemgrUtil.getInstance();
            ...
            for (VirtualSubnet subnet : port.getSubnets()) {
                ...
                if (gatewayMac == null) {
                    gatewayMac = ifMgr.getInterfaceForAddress(new Ipv6Address(gatewayIp.getIpv6Address().getValue()))
                            .getMacAddress();
                }
                ...
                if (subnet.getIpv6RAMode().equalsIgnoreCase(ifMgr.IPV6_DHCPV6_STATELESS)) {
                    ...
                    icmpv6RaFlags = (short) (icmpv6RaFlags | (1 << 7)); // Managed Address Configuration.
                }
            }
