...
public class PktHandler implements PacketProcessingListener {
    ...
    private class PacketHandler implements Runnable {
                ...
                if (type == RoutemgrUtil.ICMPv6_RS_CODE) {
                    ...
                    for (VirtualSubnet subnet : port.getSubnets()) {
                        ...
                        if (ifMgr.IPV6_AUTO_ADDRESS_SUBNETS.contains(subnet.getIpv6AddressMode())
                            ...
                            pktProccessedCounter++;
                            LOG.debug("Subnet is not an auto-address subnet. Returning.");
                            return;
                        }
                    }

                    RouterAdvertisementPacketBuilder raPacket = new RouterAdvertisementPacketBuilder();
                    updateRAResponse(rsPdu, raPacket, gatewayMac, rsPdu.getSourceMac(), prefixList);
                    // Serialize the response packet
        ...
        private void updateRAResponse(RouterSolicitationPacket pdu, RouterAdvertisementPacketBuilder raPacket,
                                      String gatewayMac, MacAddress vmMac, List<String> ipV6Prefix) {
            RoutemgrUtil instance = RoutemgrUtil.getInstance();
            ...
            raPacket.setIpv6Length(ICMPV6_RA_LENGTH_WO_OPTIONS + ICMPV6_OPTION_SOURCE_LLA_LENGTH
                    + ipV6Prefix.size() * ICMPV6_OPTION_PREFIX_LENGTH);
            ...
            short flag = 0;
            // Todo: Flags need to be tweaked for DHCPv6 subnets.
            flag = (short) (flag | (1 << 7));
            flag = (short) (flag | (1 << 6));
            ...
            for (String v6Prefix : ipV6Prefix) {
