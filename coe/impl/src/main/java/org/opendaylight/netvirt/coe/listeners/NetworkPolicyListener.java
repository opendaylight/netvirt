...
import static org.opendaylight.netvirt.coe.utils.AclUtils.buildName2;
import static org.opendaylight.netvirt.coe.utils.AclUtils.buildName3;
...
public class NetworkPolicyListener extends AbstractSyncDataTreeChangeListener<NetworkPolicy> {
    ...
    private String getAclNameFromPolicy(NetworkPolicy policy) {
        String aclName = "";
    ...
    private Acl buildAcl(NetworkPolicy policy) {
        String aclName = getAclNameFromPolicy(policy);
                    ...
                    if (rule.getIngressPorts() != null) {
                        for (IngressPorts port : rule.getIngressPorts()) {
                            if (port.getNetworkPolicyPort() != null) {
                                Ace ace = buildPortAce(false, aclName,
                    ...
                    if (rule.getFrom() != null) {
                        for (From from: rule.getFrom()) {
                            if (from.getNetworkPolicyPeer() != null) {
                                Ace ace = buildPolicyAce(false, aclName,
    ...
    private AceBuilder getAceBuilder(boolean isDeleted, String ruleName,
                                     @Nonnull Class<? extends DirectionBase> direction,
                                     AceIpBuilder aceIpBuilder) {
    ...
    private Ace buildPortAce(boolean isDeleted, @Nonnull String aclName,
                             @Nonnull Class<? extends DirectionBase> direction,
                             @Nonnull NetworkPolicyPort port) {
        AceIpBuilder aceIpBuilder = new AceIpBuilder();
        String ruleName = buildName3(aclName, DIRECTION_MAP.get(direction), "port");
        if (port.getProtocol() != null) {
            aceIpBuilder.setProtocol(PROTOCOL_MAP.get(port.getProtocol()));
            ruleName = buildName2(ruleName, port.getProtocol().toString());
        }
        ...
        if (port.getPort() != null) {
            ...
            ruleName = buildName2(ruleName, portNumber.getValue().toString());
        }
    ...
    private Ace buildPolicyAce(boolean isDeleted, @Nonnull String aclName,
                               @Nonnull Class<? extends DirectionBase> direction,
                               @Nonnull NetworkPolicyPeer peer) {
        AceIpBuilder aceIpBuilder = new AceIpBuilder();
        String ruleName = buildName3(aclName, DIRECTION_MAP.get(direction), "peer");

        if (peer.getIpBlock() != null) {
            // TODO handle except
            String  cidr = peer.getIpBlock().getCidr();
            ruleName = buildName3(ruleName, "cidr", cidr);
