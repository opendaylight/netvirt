...
@Singleton
public class K8sListener extends AbstractSyncDataTreeChangeListener<K8s> {
    private static final Logger LOG = LoggerFactory.getLogger(K8sListener.class);

    @Inject
    public K8sListener(DataBroker dataBroker) {
        super(dataBroker, LogicalDatastoreType.CONFIGURATION,
            //InstanceIdentifier.create(K8s.class).child(NetworkPolicies.class).child(NetworkPolicy.class));
            InstanceIdentifier.create(K8s.class));
        LOG.info("constructor");
    }

    @Override
    public void add(@Nonnull InstanceIdentifier<K8s> instanceIdentifier, @Nonnull K8s policy) {
