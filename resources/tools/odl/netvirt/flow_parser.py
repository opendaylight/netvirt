...
OPTIONALS = ['ifname', 'lport','elan-tag', 'reason']
MAC_LEN = 17
...
def stale_acl_flow(ifaces, ifindexes, einsts, eifaces, flow, flow_info):
    ...
    if lport:
        if not iface:
            ...
            iface_eltag = get_eltag_for_iface(eifaces, einsts, iface)
            if eltag != iface_eltag:
...
def get_flow_info_from_ifm_table(flow_info, flow):
    ...
    if m_reg6 and m_reg6.get('value'):
        lport = ('%x' % (m_reg6.get('value') & LPORT_REG6_MASK))[:-LPORT_REG6_MASK_ZLEN]
        flow_info['lport'] = int(lport,16)
    return flow_info
