...
public class Ipv6ServiceUtils {
    ...
    private final ConcurrentMap<Uuid, List<ActionInfo>> egressActionForInterfaceMap = new ConcurrentHashMap();
    ...
    public void installIcmpv6NaResponderFlow(OdlInterfaceRpcService interfaceManagerRpc, short tableId, BigInteger dpId,
        ...
        if (addOrRemove == Ipv6Constants.DEL_FLOW) {
            ...
            mdsalUtil.removeFlowToTx(rsFlowEntity, removeFlowInvTx);
            egressActionForInterfaceMap.remove(intf.getIntfUUID());
        } else {
            List<ActionInfo> actionsInfos = new ArrayList<>();
            ...
            if (egressActionForInterfaceMap.isEmpty() || egressActionForInterfaceMap.get(intf.getIntfUUID()) == null) {
                ...
                egressActionForInterfaceMap.put(intf.getIntfUUID(), egressActions);
            }
            actionsInfos.addAll(egressActionForInterfaceMap.get(intf.getIntfUUID()));
            List<InstructionInfo> instructions = new ArrayList<>();
            instructions.add(new InstructionApplyActions(actionsInfos));
