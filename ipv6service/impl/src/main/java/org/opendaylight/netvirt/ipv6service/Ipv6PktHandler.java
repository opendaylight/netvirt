...
public class Ipv6PktHandler implements AutoCloseable, PacketProcessingListener {
    ...
    private class PacketHandler implements Runnable {
        ...
        private void processRouterSolicitationRequest() {
            ...
            ConcurrentMap<Uuid, List<Action>> portToEgressActionMap = new ConcurrentHashMap<>();
            List<Action> portEgressAction = ifMgr.getEgressAction(portId.getValue());
            if (portEgressAction == null) {
                LOG.warn("Empty egress action for the port {}", portId.getValue());
                return;
            }
            portToEgressActionMap.put(portId, portEgressAction);
            ipv6RouterAdvert.transmitRtrAdvertisement(Ipv6RtrAdvertType.SOLICITED_ADVERTISEMENT,
                routerPort, portToEgressActionMap, rsPdu, port.getDpId());
            pktProccessedCounter.incrementAndGet();
        }
