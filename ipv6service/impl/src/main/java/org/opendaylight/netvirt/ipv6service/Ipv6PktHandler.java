...
public class Ipv6PktHandler implements AutoCloseable, PacketProcessingListener {
    ...
    private class PacketHandler implements Runnable {
        ...
        private void processNeighborAdvertisementPacket() {
            byte[] data = packet.getPayload();
            NeighborAdvertisePacket naPdu = deserializeNAPacket(data);
        ...
        private NeighborAdvertisePacket deserializeNAPacket(byte[] data) {
            NeighborAdvertisePacketBuilder naPdu = new NeighborAdvertisePacketBuilder();
            int bitOffset = 0;

            try {
                naPdu.setDestinationMac(new MacAddress(
                        Ipv6ServiceUtils.bytesToHexString(BitBufferHelper.getBits(data, bitOffset, 48))));
                bitOffset = bitOffset + 48;
                naPdu.setSourceMac(new MacAddress(
                        Ipv6ServiceUtils.bytesToHexString(BitBufferHelper.getBits(data, bitOffset, 48))));
                bitOffset = bitOffset + 48;
                ...
                LOG.warn("Exception obtained when deserializing NA packet", e);
            }
            return naPdu.build();
        }
    }
    ...
    private void fireNotification(Notification notification) {
        ...
        if (offerNotification != null && offerNotification.equals(NotificationPublishService.REJECTED)) {
            LOG.trace("Offered Notification was rejected: {}", notification);
        }
    }
