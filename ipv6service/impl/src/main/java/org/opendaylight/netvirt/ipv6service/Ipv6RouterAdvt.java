...
public class Ipv6RouterAdvt {
    ...
    private final ConcurrentMap<Uuid, List<Action>> egressActionForInterfaceMap = new ConcurrentHashMap();
    ...
    public boolean transmitRtrAdvertisement(Ipv6RtrAdvertType raType, VirtualPort routerPort,
        ...
        for (Uuid outport: outportList) {
            if (egressActionForInterfaceMap.isEmpty() || egressActionForInterfaceMap.get(outport) == null) {
                List<Action> actions = ifMgr.getEgressAction(outport.getValue());
                if (actions == null) {
                    LOG.warn("Egress action is null for interface {}. Continue on next port iteration",
                            outport.getValue());
                    continue;
                }
                egressActionForInterfaceMap.put(outport, actions);
            }
            TransmitPacketInput input = MDSALUtil.getPacketOut(egressActionForInterfaceMap.get(outport), txPayload,
                    dpnId);
            ...
            if (raType == Ipv6RtrAdvertType.CEASE_ADVERTISEMENT) {
                egressActionForInterfaceMap.remove(outport);
            }
        }
        return true;
    }
