...
public class Ipv6RouterAdvt {
    ...
    public boolean transmitRtrAdvertisement(Ipv6RtrAdvertType raType, VirtualPort routerPort,
                                            long elanTag, RouterSolicitationPacket rsPdu, BigInteger dpnId,
                                            Uuid port) {
        ...
        if (raType == Ipv6RtrAdvertType.SOLICITED_ADVERTISEMENT) {
            ...
            if (actions == null || actions.isEmpty()) {
                LOG.error("Unable to send solicited router advertisement packet out. Since Egress action is null for "
                        + "interface {}. ", port.getValue());
                return false;
            }
        ...
        TransmitPacketInput input = MDSALUtil.getPacketOutDefault(lstActionInfo, txPayload, dpnId);
