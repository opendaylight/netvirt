        ...
        try {
            synchronized (dpnId.toString().intern()) {
                ...
                if (dpnListOptional.isPresent()) {
                    List<VpnInstances> vpnInstanceListOriginal = dpnListOptional.get().getVpnInstances();
                    List<VpnInstances> vpnInstancesListUpdate = new ArrayList<>(vpnInstanceListOriginal);
                    try {
                        if (vpnInstancesListUpdate.remove(vpnInstance)) {
                            if (vpnInstancesListUpdate.isEmpty()) {
                                txRunner.callWithNewWriteOnlyTransactionAndSubmit(OPERATIONAL,
                                    tx -> tx.delete(id)).get();
                            } else {
                                txRunner.callWithNewWriteOnlyTransactionAndSubmit(OPERATIONAL,
                                    tx -> tx.delete(id.child(VpnInstances.class, new VpnInstancesKey(vpnName)))).get();
                            }
                        }
                    } catch (InterruptedException | ExecutionException e) {
