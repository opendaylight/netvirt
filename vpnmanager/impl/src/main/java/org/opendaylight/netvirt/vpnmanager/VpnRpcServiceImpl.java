        ...
        if (optIVpnLink.isPresent()) {
            ...
            try {
                txRunner.callWithNewWriteOnlyTransactionAndSubmit(CONFIGURATION,
                    confTx -> vpnManager.addExtraRoute(vpnInstanceName, destination, nexthop, vpnRd, null /* routerId */,
        ...
        try {
            txRunner.callWithNewWriteOnlyTransactionAndSubmit(CONFIGURATION, confTx -> {
                if (optVpnLink.isPresent()) {
                    fibManager.removeOrUpdateFibEntry(vpnRd, destination, nexthop, confTx);
                    bgpManager.withdrawPrefix(vpnRd, destination);
                } else {
                    vpnManager.delExtraRoute(vpnInstanceName, destination,
                        nexthop, vpnRd, null /* routerId */, null /* intfName */, confTx);
                }
            }).get();
            result.set(RpcResultBuilder.success(new RemoveStaticRouteOutputBuilder().build()).build());
        } catch (InterruptedException | ExecutionException e) {
            LOG.error("Error removing static route {}", input, e);
            result.set(RpcResultBuilder.<RemoveStaticRouteOutput>failed().withError(ErrorType.APPLICATION,
                "Error removing static route " + input, e).build());
        }
