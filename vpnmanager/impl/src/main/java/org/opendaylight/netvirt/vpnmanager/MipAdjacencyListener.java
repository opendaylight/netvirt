...
public class MipAdjacencyListener
    ...
    protected InstanceIdentifier<MipIp> getWildCardPath() {
        return InstanceIdentifier.create(MipAdjacency.class).child(MipDpn.class).child(MipVpn.class)
                .child(MipIp.class);
    }
    ...
    protected void add(InstanceIdentifier<MipIp> identifier, MipIp value) {
        ...
        String srcInterface = value.getPortName();
        String vpnName = identifier.firstKeyOf(MipVpn.class).getVpnName();
        String macAddress = value.getMacAddress();
        handleMipAdjacencyEvent(srcIpToQuery, srcInterface, vpnName, macAddress, LearntVpnVipToPortEventAction.Add);
    }
    ...
    protected void remove(InstanceIdentifier<MipIp> identifier, MipIp value) {
        ...
        String vpnName = identifier.firstKeyOf(MipVpn.class).getVpnName();
        String macAddress = value.getMacAddress();
        handleMipAdjacencyEvent(srcIpToQuery, srcInterface, vpnName, macAddress, LearntVpnVipToPortEventAction.Delete);
    }

    private List<ListenableFuture<Void>> handleMipAdjacencyEvent(String srcIp, String srcInterface, String vpnName,
                                                                 String macAddress,
                                                                 LearntVpnVipToPortEventAction action) {
        return Collections.singletonList(txRunner.callWithNewWriteOnlyTransactionAndSubmit(writeOperTxn -> {
            createLearntVpnVipToPortEvent(vpnName, srcIp, srcInterface, macAddress, action, writeOperTxn);
        }));
    }

    private void createLearntVpnVipToPortEvent(String vpnName, String srcIp, String portName, String macAddress,
                                               LearntVpnVipToPortEventAction action, WriteTransaction writeOperTxn) {
        ...
        writeOperTxn.put(LogicalDatastoreType.OPERATIONAL, id, builder.build());
        LOG.info("createLearntVpnVipToPortEvent: ARP learned for fixedIp: {}, vpn {}, interface {}, mac {} action {}"
                + "eventId {}", srcIp, vpnName, portName, macAddress, action, eventId);
    }
