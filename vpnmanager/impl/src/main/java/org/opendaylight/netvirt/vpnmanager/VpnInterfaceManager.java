...
public class VpnInterfaceManager extends AsyncDataTreeChangeListenerBase<VpnInterface, VpnInterfaceManager> {
    ...
    protected void processVpnInterfaceAdjacencies(BigInteger dpnId, final int lportTag, String vpnName,
        ...
        Optional<VpnInterface> vpnInteface = VpnUtil.read(dataBroker, LogicalDatastoreType.CONFIGURATION, identifier);
            ...
            if (nextHop.getAdjacencyType() == AdjacencyType.PrimaryAdjacency) {
                ...
                Prefixes prefixes = (intfnetworkUuid != null)
                    ? VpnUtil.getPrefixToInterface(dpnId, interfaceName, prefix, nextHop.getSubnetId(),
                            intfnetworkUuid ,networkType, segmentationId, prefixCue) :
                    VpnUtil.getPrefixToInterface(dpnId, interfaceName, prefix, nextHop.getSubnetId(),
                        prefixCue);
                writeOperTxn.merge(
    ...
    protected void addNewAdjToVpnInterface(InstanceIdentifier<VpnInterfaceOpDataEntry> identifier, String primaryRd,
        ...
        if (optVpnInterface.isPresent()) {
            ...
            if (adj.getNextHopIpList() != null && !adj.getNextHopIpList().isEmpty()
                ...
                Optional<VpnInterface> vpnIntefaceConfig = VpnUtil.read(dataBroker, LogicalDatastoreType.CONFIGURATION,
                        vpnIfaceConfigidentifier);
                ...
                if (vpnIntefaceConfig.isPresent()) {
                    pnfPrefix = VpnUtil.getPrefixToInterface(BigInteger.ZERO, currVpnIntf.getName(), prefix,
                            adj.getSubnetId(), vpnIntefaceConfig.get().getNetworkId(),
                            vpnIntefaceConfig.get().getNetworkType(), vpnIntefaceConfig.get().getSegmentationId(),
