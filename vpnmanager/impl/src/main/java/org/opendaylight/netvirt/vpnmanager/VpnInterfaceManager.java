...
public class VpnInterfaceManager extends AsyncDataTreeChangeListenerBase<VpnInterface, VpnInterfaceManager> {
    ...
    protected void processVpnInterfaceAdjacencies(BigInteger dpnId, final int lportTag, String vpnName,
        ...
        for (Adjacency nextHop : nextHops) {
            ...
            Subnetmap adjSubnetMap = vpnUtil.getSubnetmapFromItsUuid(nextHop.getSubnetId());
            if (!vpnUtil.isBgpVpnInternet(vpnName) && adjSubnetMap != null
                    && !vpnName.equals(adjSubnetMap.getVpnId().getValue())) {
                /* Dual stack network with two router case : V4 subnet is part of one router and V6 subnet
                 * is part of diff router
                 */
