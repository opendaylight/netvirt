...
public final class VpnUtil {
    ...
    String getVpnInterfaceName(OdlInterfaceRpcService odlInterfaceRpcService, BigInteger metadata)
     ...
     */
    String getVpnRd(String vpnName) {
     ...
     */
    void removeVrfEntriesByOrigin(String rd, RouteOrigin origin) {
    ...
    List<VrfEntry> findVrfEntriesByNexthop(String rd, String nexthop) {
        InstanceIdentifier<VrfTables> vpnVrfTableIid =
            InstanceIdentifier.builder(FibEntries.class).child(VrfTables.class, new VrfTablesKey(rd)).build();
        Optional<VrfTables> vrfTablesOpc = read(LogicalDatastoreType.CONFIGURATION, vpnVrfTableIid);
    ...
    void removeVrfEntries(String rd, List<VrfEntry> vrfEntries) {
        InstanceIdentifier<VrfTables> vpnVrfTableIid =
            InstanceIdentifier.builder(FibEntries.class).child(VrfTables.class, new VrfTablesKey(rd)).build();
        ListenableFutures.addErrorLogging(
                new ManagedNewTransactionRunnerImpl(dataBroker).callWithNewWriteOnlyTransactionAndSubmit(tx -> {
    ...
    void withdrawRoutes(String rd, List<VrfEntry> vrfEntries) {
    ...
    VpnInstanceOpDataEntry getVpnInstanceOpData(String rd) {
    ...
    @Deprecated
    <T extends DataObject> void syncWrite(LogicalDatastoreType datastoreType, InstanceIdentifier<T> path, T data) {
        try {
    ...
    @Deprecated
    <T extends DataObject> void syncUpdate(LogicalDatastoreType datastoreType, InstanceIdentifier<T> path, T data) {
        try {
    ...
    Optional<String> getVpnSubnetGatewayIp(final Uuid subnetUuid) {
     ...
     */
    String getPrimaryRd(String vpnName) {
    ...
    org.opendaylight.yang.gen.v1.urn.opendaylight.netvirt.natservice.rev160111.external.subnets
        .Subnets getExternalSubnet(Uuid subnetId) {
    ...
    static boolean isEligibleForBgp(String rd, String vpnName, BigInteger dpnId, String networkName) {
        if (rd != null) {
    ...
    boolean isVpnPendingDelete(String rd) {
        VpnInstanceOpDataEntry vpnInstanceOpData = getVpnInstanceOpData(rd);
    ...
    List<VpnInstanceOpDataEntry> getVpnsImportingMyRoute(final String vpnName) {
    ...
    void addRouterPortToElanDpn(String elanInstanceName, String routerInterfacePortId, BigInteger dpnId) {
        ...
        Optional<DpnInterfaces> dpnInElanInterfaces = read(LogicalDatastoreType.OPERATIONAL,
                elanDpnInterfaceId);
