...
public final class VpnUtil {
    ...
    public static boolean removeOrUpdateDSForExtraRoute(DataBroker dataBroker, IFibManager fibManager, String vpnName,
            String primaryRd, String extraRouteRd, String vpnInterfaceName, String prefix, String nextHop,
    ...
    private Subnet readNeutronSubnet(DataBroker dataBroker, String uuid) {
    ...
    public static String getAssociatedExternalRouter(DataBroker dataBroker,
        ...
        for (Routers routerData : extRouterData.get().getRouters()) {
            List<ExternalIps> externalIps = routerData.getExternalIps();
            for (ExternalIps externalIp : externalIps) {
                Subnet neutronSubnet = new SubnetBuilder().build(); //neutronVpnService.getNeutronSubnet(externalIp.getSubnetId());
    ...
    public static boolean isAdjacencyEligibleToVpnInternet(DataBroker dataBroker, Adjacency adjacency) {
        ...
        if (ipVerChoice.isIpVersionChosen(IpVersionChoice.IPV6)) {
            Subnetmap sn = getSubnetmapFromItsUuid(dataBroker, adjacency.getSubnetId());
    ...
    static Set<org.opendaylight.yang.gen.v1.urn.huawei.params.xml.ns.yang.l3vpn.rev140815.vpn.af.config.vpntargets
        ...
        InstanceIdentifier<VpnInstance> vpnInstanceId = InstanceIdentifier.builder(VpnInstances.class)
                .child(VpnInstance.class, new VpnInstanceKey(vpnName)).build();
        Optional<VpnInstance> vpnInstanceOptional = read(dataBroker, LogicalDatastoreType.CONFIGURATION,
                vpnInstanceId);
    ...
    static Set<RouteTarget> getRouteTargetSet(DataBroker dataBroker, Set<org.opendaylight.yang.gen.v1.urn.huawei
        .params.xml.ns.yang.l3vpn.rev140815.vpn.af.config.vpntargets.VpnTarget> vpnTargets) {
        ...
        for (org.opendaylight.yang.gen.v1.urn.huawei.params.xml.ns.yang.l3vpn.rev140815.vpn.af.config.vpntargets
            ...
            switch (rt.getVrfRTType()) {
                case ImportExtcommunity: {
                    Optional<RouteTarget> exportRouteTargetOptional = read(dataBroker,
                            LogicalDatastoreType.OPERATIONAL, getRouteTargetsIdentifier(rtValue,
                                    RouteTarget.RtType.ERT));
                ...
                case ExportExtcommunity: {
                    Optional<RouteTarget> importRouteTargetOptional = read(dataBroker,
                            LogicalDatastoreType.OPERATIONAL, getRouteTargetsIdentifier(rtValue,
                                    RouteTarget.RtType.IRT));
