...
public final class AlivenessMonitorUtils {
    ...
    void startIpMonitoring(MacEntry macEntry, Long ipMonitorProfileId) {
        ...
        if (ipMonitorProfileId == null || ipMonitorProfileId.equals(0L)) {
            Long profileId = allocateIpMonitorProfile(targetIp);
            if (profileId == null) {
    ...
    public Long allocateIpMonitorProfile(IpAddress targetIp) {
        Long profileId = null;
        if (targetIp.getIpv4Address() != null) {
            profileId = allocateArpMonitorProfile();
        } else if (targetIp.getIpv6Address() != null) {
            profileId = allocateIpv6NaMonitorProfile();
        }
        return profileId;
    }

    public Long allocateArpMonitorProfile() {
    ...
    public Long allocateIpv6NaMonitorProfile() {
        Long monitorInterval = vpnConfig.getIpv6NdMonitorInterval() * 1000; // converting to milliseconds
        Optional<Long> profileIdOptional = allocateProfile(vpnConfig.getIpv6NdMonitorFailureThreshold(),
                monitorInterval, vpnConfig.getIpv6NdMonitorWindow(), EtherTypes.Ipv6Nd);
