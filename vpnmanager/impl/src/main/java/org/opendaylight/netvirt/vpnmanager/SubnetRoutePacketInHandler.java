...
public class SubnetRoutePacketInHandler implements PacketProcessingListener {
    ...
    private SubnetOpDataEntry getTargetSubnetForPacketOut(long elanTag, String ipAddress) {
        ...
        try {
            ...
            LOG.error("{} getTargetSubnetForPacketOut: Failed to read data store for elan {}", LOGGING_PREFIX,
                    elanInfo.getName());
        }
        return null;
    }
    ...
    private static BigInteger getTargetDpnForPacketOut(DataBroker broker, long elanTag, int ipAddress) {
        ...
        Optional<NetworkMap> optionalNetworkMap = TransactionUtil.read(broker,
        ...
        if (optionalNetworkMap.isPresent()) {
            ...
            for (Uuid subnetId : subnetList) {
                ...
                Optional<SubnetOpDataEntry> optionalSubs = TransactionUtil.read(broker,
                ...
                if (subOpEntry.getNhDpnId() != null) {
                    ...
                    LOG.trace("{} getTargetDpnForPacketOut: Viewing Subnet {} matching {}", LOGGING_PREFIX,
                            subnetId.getValue(), match);
                    if (match) {
                        dpnid = subOpEntry.getNhDpnId().longValue();
                        return dpnid;
                    }
                }
            }
        } else {
            LOG.warn("getTargetDpnForPacketOut: NetworkMap not present for {} elanTag {}", elanInfo.getName(), elanTag);
        }
        return dpnid;
    }

    public static long getElanTagFromSubnetRouteMetadata(Uint64 metadata) {
        return ((metadata.longValue().and(MetaDataUtil.METADATA_MASK_ELAN_SUBNET_ROUTE)).shiftRight(32)).longValue();
    }
