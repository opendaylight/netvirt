...
public class LearntVpnVipToPortEventProcessor extends AsyncClusteredDataTreeChangeListenerBase<LearntVpnVipToPortEvent, LearntVpnVipToPortEventProcessor> {
    ...
    public void start() {
        ...
        try {
            ...
            LOG.error("LearntVpnVipToPortEventProcessor: failed to register the entity " + VpnConstants.ARP_MONITORING_ENTITY);
        }
    }
    ...
    protected void add(InstanceIdentifier<LearntVpnVipToPortEvent> identifier, LearntVpnVipToPortEvent value) {
        ...
        VpnUtil.runOnlyInLeaderNode(entityOwnershipService, () -> {
            try {
                ...
                String ipAddress = value.getPortFixedip();
                ...
                LOG.error("LearntVpnVipToPortEventProcessor: Error in deserializing packet {} with exception", value, e);
            } finally {
    ...
    private class AddMipAdjacencyWorker implements Callable<List<ListenableFuture<Void>>> {
        ...
        AddMipAdjacencyWorker(LearntVpnVipToPortEvent event) {
            ...
            this.ipAddress = event.getPortFixedip();
        ...
        public List<ListenableFuture<Void>> call() throws Exception {
            ...
            try {
                addMipAdjacency(vpnName, interfaceName,
                        ipAddress, macAddress, writeConfigTxn);
                VpnUtil.createVpnPortFixedIpToPort(dataBroker, vpnName, ipAddress,
                        interfaceName, Boolean.TRUE, macAddress, writeConfigTxn);
                VpnUtil.createLearntVpnVipToPort(dataBroker, vpnName, ipAddress,
                        interfaceName, macAddress, writeOperTxn);
            } catch (Exception e) {
                LOG.error("AddMipAdjacencyWorker: Error in adding Mip adjacency exception", e);
            }
        ...
        private void addMipAdjacency(String vpnName, String vpnInterface, String ipAddress, String mipMacAddress,
                                     WriteTransaction writeConfigTxn) {
            LOG.trace("addMipAdjacency: Adding adjacency {} with mac {} to VPN Interface {}", ipAddress, mipMacAddress,
                    vpnInterface);
            ...
            if (adjacencies.isPresent()) {
                ...
                if (nextHopIpAddr != null) {
                    ...
                    if (label == 0) {
                        LOG.error("addMipAdjacency: Unable to fetch label from Id Manager. " +
                                "Bailing out of adding MIP adjacency {} " +
                    ...
                    writeConfigTxn.merge(LogicalDatastoreType.CONFIGURATION, vpnIfId, newVpnIntf, true);
                    LOG.error("addMipAdjacency: Successfully stored subnetroute Adjacency {} with nextHop {} into Interface {}"
                            + " vpn {}", ip, nextHopIp, vpnInterface, vpnName);
                }
            }
        }
    }
