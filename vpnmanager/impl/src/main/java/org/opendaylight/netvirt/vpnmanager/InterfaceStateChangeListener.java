                                ...
                                = txRunner.callWithNewWriteOnlyTransactionAndSubmit(writeConfigTxn -> {
                                    ListenableFuture<Void> operFuture
                                            = txRunner.callWithNewWriteOnlyTransactionAndSubmit(writeOperTxn -> {
                                                ...
                                                LOG.info("Detected interface add event for interface {}",
                                                        interfaceName);
                                                final VpnInterface vpnIf =
                                                       vpnUtil.getConfiguredVpnInterface(interfaceName);
                                                if (vpnIf != null) {
                                                    for (VpnInstanceNames vpnInterfaceVpnInstance :
                                                        ...
                                                        if (!vpnInterfaceManager.isVpnInstanceReady(vpnName)) {
                                                            LOG.info("VPN Interface add event - intfName {} onto"
                                                                    + " vpnName {} running oper-driven, VpnInstance not"
                                                                    + " ready holding on", vpnIf.getName(), vpnName);
                                                        } else if (vpnUtil.isVpnPendingDelete(primaryRd)) {
                                                            LOG.error("add: Ignoring addition of vpnInterface {}, as"
                                                                            + " vpnInstance {} with primaryRd {} is"
                                                            ...
                                                            try {
                                                                ...
                                                                LOG.error("Unable to retrieve dpnId for interface {}. "
                                                                                + "Process vpn interface add failed",
                                                                        intrf.getName(), e);
                                                                return;
                                                            }
                                                            ...
                                                            LOG.info("VPN Interface add event - intfName {} onto "
                                                                    + "vpnName {} running oper-driven", vpnIf.getName(),
                                                                    vpnName);
                                                            Set<String> prefixes = new HashSet<>();
                                                            vpnInterfaceManager.processVpnInterfaceUp(dpnId, vpnIf,
                                                                    primaryRd, ifIndex, false, writeConfigTxn,
                                                                    writeOperTxn, writeInvTxn, intrf, vpnName,
                                                                    prefixes);
            ...
            if (L2vlan.class.equals(update.getType())) {
                ...
                jobCoordinator.enqueueJob("VPNINTERFACE-" + ifName, () -> {
                    ...
                    futures.add(txRunner.callWithNewWriteOnlyTransactionAndSubmit(writeOperTxn -> {
                        ...
                        // in bucket actions on bgp-vpn delete
                        Map<String, Set<String>> mapOfRdAndPrefixesForRefreshFib = new HashMap<>();
                        ListenableFuture<Void> configTxFuture = null;
                        configTxFuture =
                               txRunner.callWithNewWriteOnlyTransactionAndSubmit(writeConfigTxn -> {
                                   futures.add(txRunner.callWithNewWriteOnlyTransactionAndSubmit(writeInvTxn -> {
                                       final VpnInterface vpnIf =
                                       ...
                                       if (vpnIf != null) {
                                           ...
                                           IntfTransitionState state =
                                                   getTransitionState(original.getOperStatus(),
                                                           update.getOperStatus());
                                           if (state.equals(IntfTransitionState.STATE_IGNORE)) {
                                               LOG.info("InterfaceStateChangeListener: Interface {}"
                                                               + " state original {} updated {} not handled",
                                                       ifName, original.getOperStatus(),
                                                       update.getOperStatus());
                                               return;
                                           }
                                           if (state.equals(IntfTransitionState.STATE_UP)) {
                                               for (VpnInstanceNames
                                                       vpnInterfaceVpnInstance : vpnIf.getVpnInstanceNames()) {
                                                   String vpnName = vpnInterfaceVpnInstance.getVpnName();
                                                   String primaryRd = vpnUtil.getPrimaryRd(vpnName);
                                                   Set<String> prefixes = new HashSet<>();
                                                   if (!vpnInterfaceManager.isVpnInstanceReady(vpnName)) {
                                                       ...
                                                       LOG.error("update: Ignoring UP event for vpnInterface "
                                                                       + "{}, as vpnInstance {} with "
                                                                       + "primaryRd {} is already marked for "
                                                                       + "deletion", vpnIf.getName(),
                                                               vpnName, primaryRd);
                                                   } else {
                                                       vpnInterfaceManager.processVpnInterfaceUp(dpnId, vpnIf,
                                                               primaryRd, ifIndex, true, writeConfigTxn,
                                                               writeOperTxn, writeInvTxn, update, vpnName,
                                                               prefixes);
                                                       mapOfRdAndPrefixesForRefreshFib.put(primaryRd, prefixes);
                                                   }
                                               }
                                               ...
                                               for (VpnInstanceNames
                                                       vpnInterfaceVpnInstance : vpnIf.getVpnInstanceNames()) {
                                                   String vpnName = vpnInterfaceVpnInstance.getVpnName();
                                                   LOG.info("VPN Interface update event - intfName {} onto"
                                                                   + " vpnName {} running oper-driven DOWN",
                                                           vpnIf.getName(), vpnName);
                                                   Optional<VpnInterfaceOpDataEntry> optVpnInterface =
                                                   ...
                                                   if (optVpnInterface.isPresent()) {
                                                       ...
                                                       vpnInterfaceManager.processVpnInterfaceDown(dpnId,
                                                               vpnIf.getName(), ifIndex,
                                                               update.getPhysAddress().getValue(),
                                                               vpnOpInterface, true,
