...
public class VpnFootprintService implements IVpnFootprintService {
    ...
    private void createOrUpdateVpnToDpnListForInterfaceName(long vpnId, String primaryRd, BigInteger dpnId,
        ...
        final AtomicBoolean newDpnOnVpn = new AtomicBoolean(false);
        try {
            synchronized (vpnName.intern()) {
                txRunner.callWithNewWriteOnlyTransactionAndSubmit(writeTxn -> {
        ...
        if (newDpnOnVpn.get()) {
            ...
            VpnUtil.checkAndUpdateV6InternetDefFlowIfNeeded(dataBroker, nvManager, dpnId, vpnName, vpnId);
        }
    }
    ...
    private void removeOrUpdateVpnToDpnListForInterfaceName(long vpnId, String rd, BigInteger dpnId, String intfName,
            String vpnName) {
        final AtomicBoolean lastDpnOnVpn = new AtomicBoolean(false);
        try {
            synchronized (vpnName.intern()) {
                    ...
                    txRunner.callWithNewWriteOnlyTransactionAndSubmit(writeTxn -> {
                        if (ipAddresses.isEmpty()) {
                            ...
                            if (vpnInterfaces == null || vpnInterfaces.isEmpty()) {
                                ...
                                LOG.warn(
                                    "ip addresses are empty but vpn interfaces are present for the vpn {} in dpn {}",
                                    vpnName, dpnId);
                            }
