    ...
    private void createOrUpdateVpnToDpnListForInterfaceName(Uint32 vpnId, String primaryRd, Uint64 dpnId,
        ...
        try {
            synchronized (vpnName.intern()) {
                FluentFuture<?> future = txRunner.callWithNewWriteOnlyTransactionAndSubmit(OPERATIONAL, writeTxn -> {
        ...
        try {
            synchronized (vpnName.intern()) {
                ...
                if (vpnInterfaces.remove(currVpnInterface)) {
                    FluentFuture<?> future = txRunner.callWithNewWriteOnlyTransactionAndSubmit(OPERATIONAL,
                        writeTxn -> {
                            if (vpnInterfaces.isEmpty()) {
