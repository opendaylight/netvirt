...
package org.opendaylight.vpnmanager.api.intervpnlink;
...
import org.opendaylight.yang.gen.v1.urn.opendaylight.params.xml.ns.yang.vpnservice.inter.vpn.link.rev160311.InterVpnLinkStates;
import org.opendaylight.yang.gen.v1.urn.opendaylight.params.xml.ns.yang.vpnservice.inter.vpn.link.rev160311.InterVpnLinks;
import org.opendaylight.yang.gen.v1.urn.opendaylight.params.xml.ns.yang.vpnservice.inter.vpn.link.rev160311.inter.vpn.link.states.InterVpnLinkState;
import org.opendaylight.yang.gen.v1.urn.opendaylight.params.xml.ns.yang.vpnservice.inter.vpn.link.rev160311.inter.vpn.links.InterVpnLink;
...
public class InterVpnLinkCache {
    ...
    private static final Logger logger = LoggerFactory.getLogger(InterVpnLinkCache.class);

    ///////////////////////////////////
    //  Initialization / Destuction  //
    ///////////////////////////////////
    ...
    public static void addInterVpnLinkToCaches(InterVpnLink iVpnLink) {
        ...
        logger.debug("Adding InterVpnLink {} with vpn1=[id={} endpoint={}] and vpn2=[id={}  endpoint={}] ]",
        ...
        InterVpnLinkDataComposite iVpnLinkComposite = null;
    ...
    public static void addInterVpnLinkStateToCaches(InterVpnLinkState iVpnLinkState) {
        ...
        logger.debug("Adding InterVpnLinkState {} with vpn1=[lportTag={}  dpn={}]  and vpn2=[lportTag={} and dpn={}]",
                     iVpnLinkState.getInterVpnLinkName(), iVpnLinkState.getFirstEndpointState().getLportTag(),
                     iVpnLinkState.getFirstEndpointState().getDpId(),
                     iVpnLinkState.getSecondEndpointState().getLportTag(),
                     iVpnLinkState.getSecondEndpointState().getDpId());
        ...
        if ( optIVpnLinkComposite.isPresent() ) {
            iVpnLinkComposite = optIVpnLinkComposite.get();
            iVpnLinkComposite.setInterVpnLinkState(iVpnLinkState);
        } else {
            iVpnLinkComposite = new InterVpnLinkDataComposite(iVpnLinkState);
            addToIVpnLinkNameCache(iVpnLinkComposite);
        }
    ...
    private static void addToIVpnLinkNameCache(InterVpnLinkDataComposite iVpnLink) {
        ...
        if ( iVpnLink.getInterVpnLinkName().isPresent() ) {
            cache.put(iVpnLink.getInterVpnLinkName().get(), iVpnLink);
        }
        if ( iVpnLink.getSecondEndpointIpAddr().isPresent() ) {
            cache.put(iVpnLink.getSecondEndpointIpAddr().get(), iVpnLink);
        }
    ...
    private static void removeFromInterVpnLinkNameCache(InterVpnLinkDataComposite iVpnLinkComposite) {
        ConcurrentHashMap<String, InterVpnLinkDataComposite> cache =
            (ConcurrentHashMap<String, InterVpnLinkDataComposite>) CacheUtil.getCache(IVPNLINK_NAME_2_IVPNLINK_CACHE_NAME);
        Optional<String> interVpnLinkName = iVpnLinkComposite.getInterVpnLinkName();
        if ( interVpnLinkName.isPresent() )
            cache.remove(interVpnLinkName.get());
    }
    ...
    public static Optional<InterVpnLinkDataComposite> getInterVpnLinkByName(String iVpnLinkName) {
        ...
        return iVpnLink == null ? Optional.<InterVpnLinkDataComposite>absent()
                                : Optional.of(iVpnLink);
    }

    public static Optional<InterVpnLinkDataComposite> getInterVpnLinkByEndpoint(String endpointIp) {
        logger.trace("Checking if {} is configured as an InterVpnLink endpoint", endpointIp);
        ConcurrentHashMap<String, InterVpnLinkDataComposite> cache =
            (ConcurrentHashMap<String, InterVpnLinkDataComposite>) CacheUtil.getCache(ENDPOINT_2_IVPNLINK_CACHE_NAME);
        InterVpnLinkDataComposite iVpnLink = cache.get(endpointIp);
        logger.trace("_XX IP {} is configured for InterVpnLink {}", endpointIp, iVpnLink);
